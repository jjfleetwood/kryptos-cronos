...
The good news: ~80% already exists. The backend (Next.js API routes + Upstash Redis + Supabase Auth + Anthropic + Stripe) is effectively a REST backend a mobile client can consume, and the 801 stages are structured TypeScript data that's reusable as-is.
