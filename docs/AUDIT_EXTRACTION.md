...
- [ ] **Remaining (non-exposure, non-breaking) follow-ups:**
  - **Deep per-module rendering in apps/audit** — currently only a landing `page.tsx`; needs `[domain]` + `[domain]/[module]` routes. Blocked on the StageContainer decision (copy into apps/audit vs. extract a shared `@kryptos/ui` pkg).
