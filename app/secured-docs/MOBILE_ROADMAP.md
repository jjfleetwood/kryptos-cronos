...
- [ ] **Env var reconciliation** — `src/lib/supabase.ts` reads `SUPABASE_URL` / `SUPABASE_ANON_KEY` / `SUPABASE_SERVICE_ROLE_KEY`, but `CLAUDE.md` lists `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY`. Confirm which are actually set in Vercel, align the docs and `.env.example`.
