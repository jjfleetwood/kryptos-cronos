...
Fragment-3: 1NPUT}`,
  ...
  // ─── atlas-06: Defense Evasion ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2022,
      overview: [
        "AML.TA0006 Defense Evasion bypasses ML-based security controls:\n- As security tools lean on ML — malware classifiers, anomaly detectors, spam and NSFW filters — they become targets.\n- Attackers craft inputs tuned to slip past those specific models.",
        "Evading ML malware detectors (AML.T0015.002) keeps the payload, edits the features:\n- Detectors score binaries on features like byte n-grams, entropy, and API-call sequences.\n- Adversarial ML produces variants that keep malicious functionality while shifting features below the classifier's threshold.",
        "It's an arms race with a sobering lesson:\n- Defenders train detectors, attackers evade, defenders add adversarial training, attackers find new evasions.\n- ATLAS's point: ML-based defenses aren't inherently more robust than signatures — they add a new attack surface.",
      ],
      technical: {
...
Fragment-3: 3V4S10N}`,
  ...
  // ─── atlas-07: ML Discovery ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2023,
      overview: [
        "AML.TA0007 Discovery maps the target's whole ML pipeline, not just the model:\n- Training-data sources, preprocessing, the model registry, deployment infra, monitoring, and feedback loops.\n- Each component is its own potential attack vector.",
        "Attackers target specific pipeline stages:\n- Data ingestion (AML.T0001 — acquire/poison training data) and feature preprocessing (inject adversarial features).\n- The model registry (swap in a backdoored model), the serving layer (intercept/modify responses), and feedback loops (corrupt logged data to poison retraining).",
        "Discovery reads the pipeline however access allows:\n- Pipeline docs and config files, model-metadata endpoints, and CI/CD configs (GitHub Actions workflows that reveal data sources and training steps).\n- Where no access exists, components are inferred from model behavior.",
      ],
      technical: {
...
Fragment-3: D1SC}`,
  ...
  // ─── atlas-08: Collection / Membership Inference ──────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "AML.TA0008 Collection extracts information out of ML models — including their training data:\n- Traditional collection targets files and databases.\n- ML collection targets what the model has learned and memorized.",
        "Membership Inference (AML.T0024) asks whether a specific sample was in training:\n- With only a data sample and black-box access, the attacker decides member vs non-member.\n- It's a serious privacy breach — against a model trained on medical records, it reveals whether a given patient's record was used.",
        "Model inversion (AML.T0026) goes further — reconstructing the training data itself:\n- From black-box access, it rebuilds approximate representations of training samples.\n- Against face recognition, inversion has reconstructed recognizable faces of training subjects from class embeddings alone.",
      ],
      technical: {
...
Fragment-3: INF}`,
  ...
  // ─── atlas-09: ML Attack Staging ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2022,
      overview: [
        "AML.TA0009 ML Attack Staging prepares attacks before execution — the AI analog of traditional staging:\n- AML.T0043 (Craft Adversarial Data via Proxy Model) — use a substitute to craft transferable examples.\n- AML.T0022 (Backdoor ML Model) — plant hidden triggers in the model before deployment.",
        "Adversarial transferability is what makes black-box attacks work:\n- Examples crafted on one model often fool others trained on similar data, even with different architectures.\n- So an attacker needs only a substitute, never the target itself.",
        "Universal adversarial perturbations (UAP) generalize the attack across inputs:\n- One image-agnostic perturbation fools a classifier on most inputs at once — a single patch applied to any image.\n- UAPs also transfer across architectures — a ResNet UAP often fools VGG, EfficientNet, and ViT.",
      ],
      technical: {
...
Fragment-3: TR1G}`,
  ...
  // ─── atlas-10: Exfiltration / Model Stealing ─────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2023,
      overview: [
        "AML.TA0010 Exfiltration steals value out of ML systems, not stored files:\n- Model stealing (AML.T0056 — Model Replication) and gradient leakage (AML.T0024 — Gradient Inversion).\n- The target is the learned intelligence encoded in model weights.",
        "Model stealing replicates a proprietary model from its API:\n- Enough queries produce a substitute that matches the target's held-out accuracy at a fraction of the training cost.\n- For commercial AI products, that's direct IP theft.",
        "Gradient inversion attacks federated learning's core promise:\n- Federated clients share gradients, not raw data, with a central aggregator.\n- Inversion reconstructs the original training data from those gradients — leaking exactly the 'private' data federated learning was meant to protect.",
      ],
      technical: {
...
Fragment-3: STLN}`,
  ...
  // ─── atlas-11: Impact — Data Poisoning ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2016,
      overview: [
        "AML.TA0011 Impact is the final stage — inflicting the intended harm on the ML system:\n- AML.T0020 (Backdoor ML Model) — a hidden trigger forces misclassification.\n- AML.T0031 (Poison Training Data) — corrupt the data to degrade performance or inject bias; AML.T0048 (Erode ML Model Integrity) — slow corruption via feedback loops.",
        "Data poisoning (AML.T0031) attacks the training pipeline, not the model:\n- Injecting corrupted samples shapes what the model learns.\n- It can degrade overall accuracy (availability), force targeted misclassification (integrity), or introduce demographic bias (fairness).",
        "The most dangerous variant is slow corruption through a feedback loop:\n- The model's outputs feed future training data, and the attacker nudges those outputs to steer the model toward malicious behavior over time.\n- It's hard to catch because metrics decay gradually rather than failing all at once.",
      ],
      technical: {
...
Fragment-3: P01S0N}`,
  ...
  // ─── atlas-12: Full AI Kill Chain Synthesis ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "MITRE ATLAS maps a complete adversarial lifecycle for AI, and this stage stitches all 11 tactics into one narrative:\n- Recon of the ML system → resource development → initial access → model access → execution.\n- Then defense evasion → discovery → collection → attack staging → exfiltration → impact.",
        "Real attacks don't run that path linearly — they pivot on what they find:\n- Clearview AI = recon at scale (3B faces) fused with exfiltration (the scraped faces are the training data).\n- Tay = discovery (feedback loop) into immediate impact (poisoning); GPT-4 extraction = model access into exfiltration.",
        "NIST AI RMF is the defensive counterpart to ATLAS:\n- MAP (understand context and risks) → MEASURE (assess) → MANAGE (mitigate) → GOVERN (accountability).\n- ATLAS describes what attackers do; AI RMF describes how defenders respond.",
      ],
      technical: {
