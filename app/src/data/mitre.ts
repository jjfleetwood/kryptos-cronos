...
Fragment-3: APT29}`,
  ...
  // ─── mitre-11: Exfiltration ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2017,
      overview: [
        "Exfiltration (TA0010) is the theft of collected data off the target network:\n- The hard part isn't taking the data — it's moving large volumes out unnoticed.\n- Attackers must dodge network DLP, bandwidth monitoring, and anomaly detection.",
        "The core techniques hide the outbound flow in ordinary-looking traffic:\n- T1048 (Exfil Over Alternative Protocol) — DNS or ICMP tunneling, or HTTP/S to attacker domains.\n- T1041 (Exfil Over C2 Channel) — riding the same encrypted channel as command-and-control.\n- T1567 (Exfil Over Web Service) — uploading to Dropbox, Google Drive, or Mega to blend with normal cloud use.",
        "Timing and channel choice keep it under the radar:\n- Sophisticated actors trickle data out over days or weeks to stay below anomaly thresholds.\n- DNS tunneling is slow but almost never blocked by enterprise firewalls — the preferred covert long-term channel.",
      ],
      technical: {
...
Fragment-3: 76D4YS}`,
  ...
  // ─── mitre-12: Impact ─────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "Impact (TA0040) is the final tactic — where the attacker's objective lands:\n- T1486 (Data Encrypted for Impact) ransomware and T1485 (Data Destruction) wiper malware.\n- T1491 Defacement, T1498 Network Denial of Service, and T1529 System Shutdown/Reboot to trigger damage at scale.",
        "Ransomware (T1486) dominates because it monetizes access directly:\n- Modern operations are multi-stage: recon and data exfiltration first, for double extortion.\n- Then the encryptor deploys across every reachable system at once, followed by the ransom note.",
        "Supply-chain ransomware (T1195.002) multiplies the blast radius:\n- Compromising one managed service provider (MSP) can push ransomware to hundreds of its customers at once.\n- Kaseya VSA proved it: one compromised RMM platform, ~1,500 downstream victims.",
      ],
      technical: {
