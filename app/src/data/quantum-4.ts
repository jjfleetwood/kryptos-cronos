...
#  'hndl_protected': True}`,
  ...
  // ─── quantum-d09: Quantum-Safe Architecture ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Quantum-safe architecture is more than algorithm substitution — it makes cryptographic agility a first-class property:\n- Agility means updating the algorithms across systems without redesigning the systems themselves.\n- That requires abstract cryptographic interfaces (not hardcoded implementations), centralized key management supporting multiple algorithm types at once, and certificate pipelines that mint certs in any supported algorithm on demand.",
        "NIST SP 800-218 and SP 800-57 Part 2 both make crypto agility a design requirement, enabled by three components:\n- A cryptographic service abstraction layer (CSAL) that turns algorithm-agnostic requests into specific algorithm calls.\n- A key management system (KMS) that stores keys tagged with algorithm metadata and supports rotation, plus a CA that issues certificates in multiple formats (RSA, ML-DSA, composite).",
        "Architecture must also handle long-lived keys, which are the urgent ones:\n- TLS session keys are ephemeral — generated per session and discarded — so migrating to PQC protects future sessions immediately.\n- But CA root keys, HSM master keys, SSH host keys, and code-signing keys may stay valid for years or decades; these migrate first, because a CA root compromise forges every certificate it has ever signed, past and future.",
      ],
      technical: {
...
signature = provider.sign(document_bytes, private_key)`,
  ...
  // ─── quantum-d10: Supply Chain Quantum Risk ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Third-party and supply-chain quantum risk is one of the least-addressed parts of PQC migration:\n- An organization can migrate its own systems, but if it exchanges sensitive data with suppliers over RSA-encrypted channels, those exchanges stay HNDL-vulnerable.\n- B2B integrations (SOAP, REST, EDI, SFTP) often run RSA-2048 TLS with no PQC roadmap because external vendors were never asked for one — CISA's supply-chain guidance explicitly includes cryptographic dependency assessment in third-party risk management.",
        "CISA's ICT Supply Chain Risk Management Task Force recommends extending CBOM programs to third-party connections:\n- Inventory every API integration, SFTP feed, B2B VPN tunnel, and EDI exchange, documenting the algorithms each uses.\n- Vendor PQC questionnaires should ask: completed a crypto inventory? have a NIST PQC roadmap? target migration date? support hybrid TLS? and what's the SLA for cryptographic algorithm updates?",
        "Hardware supply-chain risk is the hardest because algorithms are baked into silicon:\n- HSMs, smart cards, TPM chips, and TLS accelerators carry fixed algorithm assumptions — a TPM 2.0 chip made before 2024 likely supports RSA and ECC but not ML-KEM/ML-DSA, and can't be firmware-upgraded to add them.\n- So bake PQC algorithm support into hardware procurement specs, and plan for hardware replacement (not firmware update) for silicon-level quantum migration.",
      ],
      technical: {
