...
function quizFor(id, dn, sub, f, opinion) {
  ...
  // Distractors are written to be plausible-but-wrong AND comparable in length to
  // the correct answer — so neither "pick the longest" nor "pick the only serious
  // one" works. (The play UI also shuffles option order each attempt, so position
  // is never a tell; length is what we control here.)
  return { questions: [
    build(1, "Objective", "Control objective", `What is the primary audit objective for the "${sub}" sub-process of ${dn}?`,
      ...
      [`Deploy and operate the ${sub.toLowerCase()} control on the team's behalf so the gap is closed during fieldwork`,
       `Confirm management is comfortable that the ${sub.toLowerCase()} control works, based on their verbal assurance`,
       `Benchmark how many tools the team uses for ${sub.toLowerCase()} against comparable organisations in the sector`],
      "An audit tests control design and operating effectiveness and reports the gap — it does not run, own, or take assurance on faith for the control."),
    build(2, "Why it matters", "Materiality", `Why does a weakness in "${sub}" matter to the broader ${dn} posture?`,
      ...
      [`It is a stand-alone control, so a gap stays contained and does not affect the other ${dn} controls`,
       `It mainly affects how the annual compliance report reads, rather than the actual risk to ${dn}`,
       `It stops mattering once a firewall and endpoint agent are deployed across the ${dn} estate`],
      "Foundational controls are load-bearing; their failure quietly undermines every control layered on top of them."),
    build(3, "Artifacts", "Evidence", `Which artifact best evidences the "${sub}" control?`,
      ...
      [`A signed management attestation that the ${sub.toLowerCase()} control is in place, with no underlying data attached`,
       `A vendor datasheet describing the product's ${sub.toLowerCase()} capabilities and its recommended configuration`,
       `A point-in-time screenshot of one system's ${sub.toLowerCase()} settings, captured during the walkthrough`],
      "Evidence must be objective and reproducible — a reconciled export judged against policy, not an assertion, a datasheet, or a single screenshot."),
    build(4, "System", "Source of truth", `Where should an auditor pull the evidence for "${sub}"?`,
      `From ${sys} and the other systems of record for this domain, accessed read-only`,
      [`From a spreadsheet the control owner maintains by hand and emails to the audit team on request`,
       `From the auditor's notes on last year's engagement, carried forward without re-testing this period`,
       `From an informal summary the team posted to the internal wiki describing how ${sub.toLowerCase()} works`],
      `Evidence must come from the authoritative systems (e.g. ${sys}) read-only — not hand-maintained spreadsheets, stale notes, or wiki summaries.`),
    build(5, "Data owner", "Accountability", `Who is most likely accountable for the data behind "${sub}"?`,
      `${owner}, with the related functions attesting to the part each of them owns`,
      [`The external audit firm, since it is the party examining the ${sub.toLowerCase()} control this period`,
       `Whoever most recently changed the configuration, regardless of their role or formal accountability`,
       `No single function — the ${sub.toLowerCase()} data is shared, so the accountability sits with no one in particular`],
      `${owner} owns the control data; the auditor independently verifies it but never owns it, and accountability is never ownerless.`),
    build(6, "Agentic", "Human vs agent", `In the agentic workflow for "${sub}", which part stays with the human auditor?`,
      ...
      "Agents gather and correlate evidence at machine speed; the human owns the policy, the judgement, and the signed opinion."),
    build(7, "Tooling", "Read-only", "Why must the MCP server for this module be strictly read-only?",
      ...
      "Non-interference is a hard requirement for audit evidence-gathering tools — not a permissions accident, a speed choice, or a protocol limitation."),
    build(8, "Findings", "Typical finding", `For "${sub}", which of these is a realistic reportable finding?`,
      ...
      `A finding is a concrete, named gap against the standard — e.g. ${f.finding} A clean result, a good tool choice, or an on-time project is not a finding.`),
    build(9, "Deliverable", "The opinion", "How does the coverage report decide its overall opinion?",
      ...
      "The opinion is a deterministic function of how many in-scope items fail the test and how severely."),
    build(10, "Privacy/Risk", "The data angle", `Why does auditing "${sub}" also serve privacy and regulatory goals?`,
      ...
      ["Technical controls and privacy obligations are governed entirely separately, so this control sits outside privacy scope",
       `Regulators review only written policy documents, never the technical controls behind ${sub.toLowerCase()}, so there is no overlap`,
       "The control applies only to public, non-sensitive data, so any gap in it carries no real regulatory exposure"],
      "Security and privacy ride on the same technical controls — a gap here is frequently a compliance and privacy gap as well."),
  ] };
}
...
const epochsMeta = []; // client-safe epoch list (id/name/emoji) for the deck dropdown etc.
for (const [name, subs] of groups) {
  const meta = D[name];
  const dn = meta.displayName ?? name; // display/content name (e.g. expand "ICS")
  epochsMeta.push({ id: meta.slug, name: dn, emoji: meta.emoji });
  ...
  subs.forEach((sub, i) => {
    ...
    const stage = {
      ...
      xp: value * 20, // reward scales with the module's audit value (120–200), not a flat number
...
// ── audit-epochs.generated.ts (client-safe — no stage content) ────────────────
fs.writeFileSync(
  path.join(SRC, "audit-epochs.generated.ts"),
  ...
  `export const auditEpochsMeta: { id: string; name: string; emoji: string }[] = ${JSON.stringify(epochsMeta, null, 2)};\n`,
  "utf8",
);
