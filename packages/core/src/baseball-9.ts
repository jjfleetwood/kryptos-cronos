...
export const baseball9Stages: StageConfig[] = [
  // ─── baseball-9-01: The Anchor of the Infield ─────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Beyond receiving, the first baseman fields their own area (the '3-hole' between first and second), holds runners on base, charges bunts, starts and finishes double plays, and acts as a cutoff man on throws from right field. Because first base sits at the end of so many plays, the position rewards soft hands, nimble footwork around the bag, and constant awareness of the situation. The stereotype of the slow slugger who 'hides' at first is outdated — elite first-base defense saves runs on a nightly basis.",
  ...
  // ─── baseball-9-02: Footwork at the Bag ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1985,
      overview: [
        "Receiving a throw at first base starts with footwork, not the glove. As the ball is hit, the first baseman moves to the bag and squares up to the throwing fielder, finding the base with the feet without staring down at it. The standard setup: stand with the heels at the front edge of the bag, present a target with the glove, read the throw's direction, and only then stretch toward the ball. The mistake young players make is anchoring a foot on the bag too early and reaching, which locks them out of adjusting to an off-line throw.",
        "Choosing which foot to put on the bag matters. The first baseman touches the bag with the foot on the same side as the throwing hand (a right-handed first baseman uses the right foot), then strides toward the throw with the glove-side foot. This maximizes reach and keeps the body balanced. The catcher-like rule applies: catch the ball first, keep a foot on the bag — but if the throw pulls you off the base to catch it, prioritize catching the ball and tagging the runner or the bag, because an uncaught throw is far worse than a missed touch.",
  ...
  // ─── baseball-9-03: Scooping and Picking Throws ───────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The technique for a short hop: stay down and let the glove work low, presenting the pocket to the ball and 'giving' slightly as it arrives so the ball settles into the glove rather than bouncing off. The hardest throw to handle is the in-between hop — not a long, readable hop and not right at the glove — so the first baseman tries to either catch the ball on the long hop (reading the bounce early) or smother the short hop right out of the dirt. Picking the short hop is usually safer than reaching for an awkward in-between bounce.",
        "Footwork supports the pick: a first baseman may need to come off the bag to field a wild throw, then tag the runner or dive back to the base. On throws to the side, the first baseman shifts to get the body in front when possible, using the chest as a backstop so a missed pick still stays close. Picking is a skill built entirely through repetition — thousands of short hops in practice until the hands are soft and the read is instinctive. It is the single most valuable defensive skill a first baseman can own.",
      ],
      technical: {
  ...
  // ─── baseball-9-04: Holding Runners ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1990,
      overview: [
        "When a runner is on first base, the first baseman 'holds' him on — standing with a foot near the bag to give the pitcher a target for a pickoff throw and to keep the runner's lead short. The job has three parts: present a good target and take the pickoff throw cleanly to apply a quick tag, keep the runner honest so he can't get a huge jump to steal, and then get off the bag quickly into fielding position once the pitch is delivered, because a held first baseman has a smaller range to field a batted ball.",
        "Taking the pickoff throw: the first baseman sets up with the throwing-hand-side foot against the inside corner of the bag, presents a low target, catches the throw, and sweeps the tag down to the inside corner where the diving runner's hand returns. The tag must be quick and low — the runner is diving back, so the glove has to beat the hand to the bag. A clean catch comes first; a great tag means nothing if the ball isn't secured. Pickoff plays are timing plays practiced between pitcher and first baseman.",
  ...
  // ─── baseball-9-05: Building the First Baseman's Body ─────────────────────────
  {
    ...
    info: {
      ...
      year: 2008,
      overview: [
        "The image of the first baseman as a big, slow slugger misses what the position actually demands physically. The most valuable first-base traits are flexibility (to stretch low and wide for throws and to pick short hops), agility (quick feet around the bag and lateral range on ground balls), and reach (a longer wingspan genuinely helps receive off-line throws — one reason taller players gravitate to the position). Strength and size help with the bat and with reaching, but they are not the defensive foundation.",
        "Flexibility is the first baseman's most underrated physical asset. The deep, wide stretch that shortens a throw's flight requires hip, hamstring, and groin flexibility; the low pick out of the dirt requires the ability to get down and stay down comfortably. Mobility work — hip openers, hamstring and groin flexibility, ankle mobility — directly translates to more outs. A stiff first baseman can't stretch far or pick low, no matter how strong they are.",
  ...
  // ─── baseball-9-06: Fielding the 3-Hole and Double Plays ──────────────────────
  {
    ...
    info: {
      ...
      year: 1982,
      overview: [
        "First basemen field their own ground balls too. The area to the first baseman's right — the '3-hole' between first and second base — is their primary range responsibility, along with anything hit hard at or near the bag. Fielding these balls uses the same fundamentals as any infielder: get in front when possible, field the ball out front with the glove down, and make a controlled play. But the first baseman has unique decisions: with a runner on, do they field and flip to the pitcher covering first, or take it themselves to the bag?",
        "On a ground ball to the first baseman's right with no force, the play is usually simple: field it and step on first, or tag the runner. But on a ground ball with a runner on first and a double-play opportunity, the first baseman can start a 3-6-3 (field, throw to the shortstop covering second, take the return throw at first) or a 3-6-1 (field, throw to second, then the pitcher covers first for the return). These are among the most athletic plays a first baseman makes, requiring a strong, accurate throw to second and quick footwork to get back to the bag.",
  ...
  // ─── baseball-9-07: Charging Bunts ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Charging the bunt requires the same footwork principles as a catcher fielding a bunt: attack the ball, circle it so the body is aligned toward the target before fielding, and field with two hands or a bare-hand scoop on a dead ball. Because the first baseman is charging away from first base, someone else must cover the bag — usually the second baseman rotates over to first. This coordination, where fielders rotate to cover the bases vacated by charging infielders, is part of the defense's bunt coverage.",
        "The 'wheel play' is an aggressive bunt defense used with a runner on second (or first and second) when the defense wants to get the lead runner at third. The corners (first and third basemen) charge hard, the shortstop covers third, and the second baseman covers first — the infield 'wheels' around. It's a high-risk, high-reward play: if executed well, it cuts down the lead runner; if the bunt is good or the timing is off, it can leave bases open. The first baseman's aggressive charge is central to making it work.",
      ],
      technical: {
  ...
  // ─── baseball-9-08: Cutoffs, Relays, and Backups ──────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Being a good cutoff man means lining up correctly, giving the outfielder a clear target (often with raised hands), and listening for the catcher's direction. The catcher, who sees the whole play, calls 'cut' and a base or lets the throw go through. The first baseman must catch the throw and execute the call instantly — relay home, cut and throw to another base to catch a trailing runner, or let it continue. Positioning is key: stand in line with the outfielder and the target so the throw stays on a straight, fast path.",
        "The first baseman also has backup duties. On throws and plays where they're not directly involved, they back up bases to prevent overthrows from costing extra bases — for example, backing up second base on a throw from the catcher or right fielder when no one else is covering, or backing up a rundown. Like every position, the first baseman must know, before each pitch, whether they'll be a cutoff man, a base coverer, or a backup on a given batted ball. The position is woven into the whole defense's throwing game.",
      ],
      technical: {
  ...
  // ─── baseball-9-09: Situational IQ ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2000,
      overview: [
        "A complete first baseman plays the game in their head before every pitch. Positioning is the first decision: how deep and how far off the line to stand depends on the situation. Late in a close game with the tying or go-ahead run, the first baseman guards the line to prevent a double down the right-field line. With a runner on first, they hold the runner. With a slow hitter or a likely pull, they adjust depth and angle. Standing in the right spot before the pitch turns hard plays into routine ones.",
        "Decision-making on the play is the next layer. On a ground ball, the first baseman must instantly know: is this a force out (just touch the bag) or a tag play (the runner isn't forced)? With runners on, where's the lead runner, and is the double play on? Should they take the sure out or try for the lead runner? With the infield in to cut down a run at the plate, the first baseman charges and may throw home. These reads happen in a fraction of a second, which is only possible if the thinking was done before the pitch.",
