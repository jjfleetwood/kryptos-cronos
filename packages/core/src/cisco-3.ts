  ...
  // ─── Stage m26: CVE-2022-20828 — Cisco FTD CLI Command Injection ──────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "Cisco FTD runs a restricted CLI (clish) that limits you to firewall operations and blocks direct OS shell access — but certain diagnostic commands handled their arguments unsafely:\n- Instead of `execv()` with a separated argument list, the CLI built an OS command string by concatenation — effectively `system(\"show interface \" + user_arg)`.\n- When `user_arg` contained a semicolon, the shell ran everything after it as a second, independent command.\n- That broke out of the restricted clish context into the underlying Linux root shell.",
          "The exposed surface was any CLI command that took free-form string arguments and passed them to OS utilities without filtering:\n- Even a read-only FTD account used only for `show` commands could inject commands that ran as root.\n- From there an attacker could read the FTD's VPN keys and SSL certificates, edit iptables rules directly at the OS level (bypassing FTD policy), and plant persistence in Linux startup scripts that survived FTD restarts.\n- The patch replaced string concatenation with allowlist validation that rejected semicolons, pipes, backticks, dollar signs, and other shell metacharacters before the OS call.",
        ],
        codeExample: {
...
# Audit: every FTD CLI account should have individual credentials`,
      ...
      incident: {
        ...
        body: [
          "Cisco disclosed CVE-2022-20828 in May 2022, affecting FTD versions before 7.0.2, and the bar for exploitation was deliberately low:\n- A valid CLI account at any privilege level was enough.\n- Operations teams routinely create shared read-only monitoring accounts for SIEM integrations and network-management tools.\n- Those credentials regularly turn up in configuration backups, ticket attachments, monitoring-tool config files, and CI/CD environment variables — anywhere a breach of a secondary system would hand over FTD access.",
          "Root on an FTD appliance breaks the integrity of the entire security stack it enforces:\n- An attacker can rewrite iptables rules at the OS layer, invisible in the FMC policy view.\n- They can read decrypted VPN traffic crossing the device and extract the TLS certificates and private keys used for SSL inspection.\n- They can insert persistent backdoors in Linux init scripts.\nNone of it shows up as an FTD policy change in the console, because it all happens below the FTD software layer.",
  ...
  // ─── Stage m27: CVE-2021-1224 — Cisco Firepower Snort Bypass — Quiz ────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "TCP is a stream protocol where segments can overlap in sequence-number space — from retransmission or deliberate crafting — and both the IPS and the host must resolve overlaps the same way:\n- The two dominant behaviors are BSD policy (favor the first-received segment) and Linux/Windows policy (favor the later-received segment).\n- When they differ, an attacker puts malicious data in the second segment: the IPS on BSD policy ignores it, the Linux host on Linux policy accepts it, and the signature never fires.\n- CVE-2021-1224 was a specific out-of-order reassembly case where Cisco's Snort engine — whatever policy was configured — failed to resolve the overlap and produced an incorrect byte stream for inspection.",
          "In practice, an attacker who knew the target sat behind Snort and ran Linux could craft TCP sessions the host reassembles as malicious while Snort reassembles as benign:\n- No signatures fire and no alerts are generated; the only fallback detection is post-hoc capture and manual reassembly.\n- Snort 2's stream5 preprocessor approximated correct reassembly with per-host policies (`policy linux`, `policy windows`, `policy bsd`), but CVE-2021-1224 exposed edge cases even correctly configured policies missed.\n- The fix updated Snort's internal reassembly logic, not the policy configuration.",
        ],
        codeExample: {
...
# Defense-in-depth: never rely on IPS as sole detection layer`,
      ...
      incident: {
        ...
        body: [
          "Cisco disclosed CVE-2021-1224 in January 2021 at CVSS 8.6, and it was practical for real intrusions:\n- It required no authentication — any attacker with network access to a Firepower-protected segment could craft evasive TCP streams.\n- The only prerequisite was knowing the target host's OS, trivially obtained from TCP fingerprinting or banner grabbing.\n- That made it viable for any targeted intrusion where the attacker knew their target was Linux-based.",
          "The historical context made it notable: Ptacek and Newsham documented TCP IPS evasion in their seminal 1998 paper \"Insertion, Evasion, and Denial of Service: Eluding Network Intrusion Detection,\" and the Snort project had worked on reassembly correctness for over two decades — yet CVE-2021-1224 showed production engines could still diverge from host behavior in exploitable ways. Cisco shipped Snort engine update 3.0.3 to correct the overlap handling, and FMC pushed it to managed FTD devices automatically.",
          "The Snort 3 rewrite — shipped with FTD 7.0 — redesigned the packet-processing pipeline specifically to fix the structural reassembly weaknesses behind CVE-2021-1224, using formal-verification techniques to make configured OS policies match real kernel behavior across documented edge cases. But the broader lesson, echoing the whole history of IPS evasion since 1998, is that an IPS can't be the sole detection layer:\n- EDR catches malicious code executing on the endpoint regardless of how it was delivered.\n- NetFlow analysis flags unusual traffic volumes and connection patterns.\n- DNS monitoring catches C2 beaconing.\nA bypassed IPS means a payload reached the host — not that the attacker is undetectable.",
        ],
      },
      diagram: {
        nodes: [
  ...
  // ─── Stage m28: CVE-2022-20773 — Cisco Umbrella SAML Auth Bypass ──────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "SAML is an XML standard for exchanging authentication data between an Identity Provider (IdP) and a Service Provider (SP):\n- When a user logs in, the IdP generates an XML assertion with the user's identity, signs it, and sends it to the SP.\n- The SP must validate that XML signature before trusting the assertion's contents.",
          "XML Signature Wrapping (XSW) attacks exploit the gap between how an XML document is parsed and how it's signed: by restructuring the document, an attacker keeps a valid signature over one portion while substituting malicious content into the portion the application actually reads — effectively forging a signed assertion.",
          "CVE-2022-20773 happened because Cisco's Umbrella SP never re-validated the assertion's canonical form after parsing, letting XSW restructuring bypass signature verification entirely.",
        ],
        codeExample: {
...
<!-- SP uses admin@corp.com; signature validates on real — bypass! -->`,
      ...
      incident: {
        ...
        body: [
          "Cisco disclosed CVE-2022-20773 in April 2022 at CVSS 9.6 — near-maximum severity:\n- No authentication was required: anyone who could reach the internet-accessible Umbrella SSO endpoint could forge an admin assertion and log in as any user in any tenant.\n- Cisco patched the server-side SAML validation within days and notified affected customers.\n- The server-side fix needed no customer action — but any tenant accessed in the preceding window had to audit its DNS-policy history for unauthorized changes.",
          "The flaw underscores a persistent SAML failure mode: rolling custom XML parsing instead of using well-audited SAML libraries. SAML's XML Signature spec has subtle canonicalization requirements — the XML must be put in a specific canonical form before the signature is computed and verified. When the code that reads the assertion (after the parser may have restructured it) differs from the code that verified the signature (over the original bytes), the XSW window opens. It's the same architectural error that broke Shibboleth in 2011, AWS in 2012, and SimpleSAMLphp in 2017.",
          "The defensive pattern for XSW has been known since the early 2010s: the SP must locate the signed element by its ID, verify the signature over exactly that element, and then read every identity claim only from the verified element — never from another element appearing earlier in the document. Well-audited libraries (OneLogin's python3-saml, Microsoft.Identity.Client, JSAML) do this correctly. That XSW bugs keep surfacing despite 15 years of public documentation reflects how often organizations build custom SAML consumers instead of using battle-tested libraries, and how rarely SAML flows make it into security-testing scope.",
        ],
      },
      diagram: {
        nodes: [
  ...
  // ─── Stage m29: Cisco SecureX / XDR Platform Security — Quiz ─────────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "Cisco XDR integrates over REST APIs and OAuth 2.0:\n- Each connected product publishes an API module.\n- The XDR orchestration engine calls those APIs to query telemetry and execute response actions.\n- Tokens scoped to XDR carry broad read permissions across all integrated products.",
          "A leaked XDR API key can expose network DNS queries, endpoint process telemetry, email threat data, and VPN session logs all at once — a far larger blast radius than a single-product credential leak.",
          "Automated playbooks can take high-privilege actions, which cut both ways:\n- Blocking IPs at Umbrella, quarantining endpoints via Secure Endpoint, resetting sessions in Duo.\n- An attacker who reaches the XDR orchestration layer can fire those same playbooks to disrupt the defender's own infrastructure.",
        ],
        codeExample: {
...
# Audit all API calls in Cisco SecureX Activity Log`,
      ...
      incident: {
        ...
        body: [
          "Multiple red-team reports and investigations found Cisco SecureX and XDR API tokens hardcoded in automation scripts, CI/CD config files, and IaC repos that were accidentally made public:\n- Because XDR tokens span the whole security stack, one leaked credential exposed far more than a typical API key — DNS query logs, endpoint process telemetry, email threat verdicts, VPN session records, and firewall events, all from a single token.\n- GitGuardian and similar scanners began alerting on Cisco XDR token patterns in 2022.\n- But teams often failed to rotate flagged tokens, treating the alert as informational rather than urgent.",
          "In one documented case, an attacker who phished XDR admin credentials fired multiple automated response playbooks before defenders could revoke access:\n- They quarantined 40 endpoints and blocked a range of corporate IP addresses.\n- The attack surfaced only when quarantined users started calling the help desk.\n- The playbooks built to protect the organization became denial-of-service weapons — legitimate workstations isolated, legitimate IP ranges blocked, legitimate analyst accounts locked by attacker-triggered response actions.",
  ...
  // ─── Stage m30: CVE-2019-1896 — Cisco IMC REST API Command Injection ───────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "The IMC exposes a REST API on a dedicated out-of-band management network, separate from the server's production interfaces, and the virtual-media endpoint was the weak point:\n- `/api/v2/virtual-media/mount` took a JSON body with a `remoteShare` field naming a network path to an ISO — meant for remote CD/DVD mounting during OS installs.\n- The handler built the mount command by concatenation — `mount <remoteShare>` — and passed it to `system()`.\n- Setting `remoteShare` to `192.168.1.1/iso/x.iso; id` made the shell run both the mount attempt and the injected `id` as root.",
          "The IMC firmware runs on its own ARM or x86 microcontroller with a separate OS, so root there persists no matter what happens to the main server:\n- SSH private keys in the IMC configuration are readable.\n- Persistent startup scripts in IMC flash survive server reboots and OS reinstalls.\n- The IMC's virtual-media and KVM features can mount malicious ISOs for OS-level code execution on the next boot.\n- Its network interface pivots to the UCS Manager management network that controls every blade in a chassis — so one exploited IMC credential can cascade to the whole chassis.",
        ],
        codeExample: {
...
# Use dedicated, per-server IMC credentials — never shared monitoring accounts`,
      ...
      incident: {
        ...
        body: [
          "Cisco disclosed CVE-2019-1896 on August 21, 2019, as part of a batch of six UCS IMC vulnerabilities in one advisory:\n- Any authenticated IMC user could exploit the injection — including the default read-only monitoring account shipped in factory configuration on many UCS deployments.\n- Organizations that stood up centralized monitoring via those default IMC accounts had effectively granted them root on the IMC firmware without realizing it.",
          "Because IMC interfaces sit on dedicated management networks with limited traffic monitoring and rarely make it into SIEM scope, the attack could go undetected for a long time:\n- Server-level tools (EDR, host IDS, file-integrity monitoring) run on the main OS and have no visibility into the IMC's separate environment.\n- The IMC's own audit log, if enabled, would record the API endpoint and user — but not the malicious content of the request body, so it wouldn't flag the injection.",
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Common token-security failures stack up fast:\n- Hardcoded in automation scripts.\n- Stored in plaintext in Git repositories.\n- Granted excessive scope (read-write when read-only would do).\n- Long or non-expiring lifetimes.\n- No rotation policy.",
          "YANG (Yet Another Next Generation) is the data-modeling language behind NETCONF and RESTCONF; its models define the schema for network-device configuration, so understanding YANG is essential for auditing what a given API call will actually change on a device.",
        ],
        codeExample: {
...
# Audit all DNAC API calls in Splunk/SecureX`,
      ...
      incident: {
        ...
        body: [
          "Multiple penetration tests and red-team exercises found Cisco DNA Center and Meraki API tokens hardcoded in IaC repos, CI/CD configs, and monitoring scripts that were accidentally exposed publicly:\n- These tokens carry network-admin authority — the same as a senior network engineer with console access — so the blast radius dwarfs a typical application API key.\n- DNA Center tokens let a caller reconfigure VLANs, modify ACLs, push routing-policy changes, provision new devices, and delete existing device configurations across the whole managed campus.",
          "In one documented red-team exercise, testers used a DNA Center read-write token found in a public GitHub repo to reconfigure 200+ switches in 15 minutes:\n- They modified VLAN configurations and ACL rules across the campus with no other credentials needed.\n- The organization had no DNA Center API monitoring in its SIEM, so the changes looked like legitimate network operations.\n- They weren't noticed until the network team spotted unexpected VLAN assignments in a routine audit two days later.",
  ...
  // ─── Stage m32: CyberOps — SOC Analyst SIEM Triage — Quiz ────────────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "SIEM alerts come from correlation rules matching patterns in log data, and the common rule types are:\n- Threshold rules — e.g. >100 failed logins in 5 minutes.\n- Sequence rules — e.g. login → mass file access → external data transfer.\n- Behavioral baselines — statistical anomaly detection against normal activity profiles.",
          "The NIST incident-response lifecycle has four phases — Preparation → Detection & Analysis → Containment/Eradication/Recovery → Post-Incident Activity — and SOC Tier 1 analysts operate in Detection & Analysis, receiving, triaging, and escalating.",
          "MITRE ATT&CK provides a structured vocabulary for attacker TTPs (Tactics, Techniques, Procedures), and mapping alerts to ATT&CK techniques enables faster triage and more accurate severity classification.",
        ],
        codeExample: {
...
// Correlate with ATT&CK T1021.001 (Remote Services: RDP)`,
      ...
      incident: {
        ...
        body: [
          "In a widely-cited 2023 investigation, a SOC with a 95%+ false-positive rate on its brute-force detection rule had effectively trained analysts to dismiss nearly every failed-login alert:\n- The rule fired so often — from authorized pentest tools, password-manager retries, and misconfigured service accounts — that dismissal became the default workflow.\n- When a real credential-stuffing attack succeeded and the attacker began lateral movement, the same rule type fired and was dismissed.\n- The attacker dwelled 47 days before a Tier 2 analyst happened to notice an anomalous data-staging pattern during an unrelated review.",
          "The root cause was an under-ticketed rule-tuning task that had sat in the backlog for nine months:\n- The pentest tool causing the false positives had never been added to an exclusion list after deployment.\n- The rule itself was technically correct — it fired on real brute-force patterns — but the authorized scanner's traffic looked identical.\n- Treating alert quality as a security outcome rather than a dev task would have meant a four-hour fix: one engineer adding an exclusion filter to restore the rule's signal.",
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Command-and-Control (C2) beaconing is a high-value hunt target — malware checks in with a C2 server at regular intervals over HTTP/HTTPS or DNS, leaving distinctive patterns:\n- Regular connection intervals (e.g. every 60 seconds).\n- Connections to recently registered domains.\n- DNS queries with high-entropy names from DGA (Domain Generation Algorithm) malware.",
          "Lateral movement leaves authentication artifacts:\n- Pass-the-hash shows as NTLM authentication to multiple hosts in a short window.\n- Pass-the-ticket appears as Kerberos TGS requests for service accounts.\n- RDP lateral movement shows as Event ID 4624 (logon type 10) on destination hosts.",
        ],
        codeExample: {
...
| eval hunt_tag="regular_c2_beacon"`,
      ...
      incident: {
        ...
        body: [
          "A threat-hunting team investigating anomalous DNS query volume found a workstation querying high-entropy domain names at 58-second intervals — consistent with DGA-based C2 beaconing:\n- The SIEM had never fired in 187 days: the beacon volume sat below every configured rule's threshold.\n- None of the generated domains appeared in any threat-intel feed, because DGA produces fresh domains daily — making blocklist detection structurally ineffective against it.",
          "Investigation showed the host had been compromised 187 days earlier via a spear-phishing attachment, with persistence through a C2 framework that used a date-seeded Domain Generation Algorithm:\n- Every day it generated 100 new domains, contacted them at 58-second intervals, and only the attacker's infrastructure answered any of them.\n- The generated names ran consistently above 3.5 bits-per-character entropy — distinguishably higher than legitimate CDN or cloud domains.\n- Both the entropy and the regular beacon interval were detectable only through active analysis, not passive signature matching.",
