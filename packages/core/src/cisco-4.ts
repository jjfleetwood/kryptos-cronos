  ...
  // ─── Stage m34: CVE-2023-20198 — Cisco IOS XE HTTP Privilege Escalation ─────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "The IOS XE Web UI is an embedded HTTP/HTTPS server that, when enabled, exposes a management interface at the device's management IP:\n- The flaw lived in a specific URL path used during the registration workflow.\n- A crafted POST to that path with the right parameters made the HTTP server create a privilege-15 local user account.\n- Crucially, that happened before authentication was ever checked.",
          "Privilege level 15 in Cisco IOS is the equivalent of root — it unlocks every privileged EXEC command, including `write memory`, `reload`, `configure terminal`, and full routing configuration — so a privilege-15 account can rewrite every aspect of the device's configuration.",
          "Chained with CVE-2023-20273 (a separate Web UI command injection), attackers used the level-15 account to run OS-level commands and install a malicious Lua plugin as a persistent implant — one that disguised itself as part of the normal IOS XE configuration and survived device reloads.",
        ],
        codeExample: {
...
# ip http access-class <mgmt-only-acl> out`,
      ...
      incident: {
        ...
        body: [
          "VulnCheck published its findings on October 16, 2023 — the same day Cisco released its advisory — by which point over 50,000 Cisco IOS XE devices had already been compromised with an HTTP implant awaiting commands. The scale was extraordinary: attackers automated the attack and scanned the entire internet for vulnerable devices within days of finding the zero-day.",
          "CISA issued Emergency Directive 24-01, requiring U.S. federal agencies to identify and remediate all affected IOS XE devices within 48 hours. The episode laid bare the danger of exposing network-device management interfaces to the internet — a common misconfiguration that turns critical infrastructure into an easily exploitable target.",
          "The threat actor (tracked by Cisco Talos) ran the campaign in two malware stages:\n- Stage one used CVE-2023-20198 to create the admin account.\n- Stage two used CVE-2023-20273 to execute commands and install a Lua-based implant in the IOS XE filesystem.\n- The implant provided persistent access that survived reloads and stayed invisible to administrators checking authorized users.",
        ],
      },
      diagram: {
        nodes: [
  ...
  // ─── Stage m35: CVE-2023-20273 — IOS XE Web UI Command Injection (Chained) ──
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "CVE-2023-20273 exploits insufficient input validation in a different IOS XE Web UI endpoint:\n- The attacker first authenticates with the level-15 account created via CVE-2023-20198.\n- They then send a crafted request containing shell metacharacters (`;`, `|`, `&&`) to a Web UI management endpoint.\n- The Web UI passes those characters to an underlying shell command unsanitized, yielding arbitrary OS command execution.",
          "Unlike CVE-2023-20198 (which targets the IOS XE user database), CVE-2023-20273 breaks into the underlying Linux-based OS that IOS XE runs on — commands injected here execute as root at the OS level, not as an IOS EXEC privilege-15 session, a categorically deeper access level.",
          "With OS command access, the threat actor wrote a malicious Lua script to disk and registered it as an IOS XE plugin — Lua is natively supported in IOS XE for network automation:\n- Malicious plugins can intercept CLI commands.\n- They can modify routing decisions.\n- They can communicate over the management interface — all while appearing as legitimate IOS XE functionality.",
        ],
        codeExample: {
...
# After cleanup: reload device to clear in-memory implant state`,
      ...
      incident: {
        ...
        body: [
          "After creating the admin account via CVE-2023-20198, the threat actor automated CVE-2023-20273 to deploy the Lua implant on thousands of devices, engineering it to be hard to spot:\n- It appeared in the IOS XE plugin system as a loaded module using Cisco's own legitimate plugin API.\n- It used the same HTTPS management port 443 as normal administrative traffic.\n- It required a specific session password to activate, so scanners probing the management interface wouldn't accidentally trip its C2 functionality.",
          "Cisco Talos reverse-engineered the implant and confirmed a deep capability set:\n- It could intercept all CLI commands before execution.\n- It could modify or suppress specific log entries to prevent detection.\n- It could exfiltrate the running configuration and routing tables on demand, and proxy attacker traffic through the router's management interface.\nFor an attacker targeting financial or government networks, a persistent foothold on a BGP border router is extraordinarily valuable — every transiting packet is visible, routing policy can be bent to redirect specific flows, and the router's management adjacencies open the management plane of all connected infrastructure.",
          "Cisco Talos attributed the October 2023 IOS XE campaign to a single threat actor based on shared C2 infrastructure across both the mass-exploitation phase and the implant deployment. The Lua-based design was the notable technique: rather than running a custom process that would stand out in `show processes`, the attacker abused Cisco's own native scripting engine — a legitimate, platform-trusted extension mechanism. It's analogous to UEFI implants in Windows or firmware rootkits in server BMCs: hiding malicious functionality inside vendor-provided extension mechanisms that slip past conventional detection tooling. CISA's post-incident guidance included specific IOS XE audit steps — `dir flash:*.lua`, comparing `show running-config` against backups, and reviewing IOS XE EEM (Embedded Event Manager) applets for unexpected external script references.",
        ],
      },
      diagram: {
        nodes: [
  ...
  // ─── Stage m36: ArcaneDoor — CVE-2024-20359 / CVE-2024-20353 (ASA) ──────────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "Line Dancer is a sophisticated in-memory shellcode implant for Cisco ASA that hooks into the SSL VPN processing pipeline:\n- It intercepts specific HTTPS packets destined for the management interface.\n- Packets carrying a specific secret header skip normal processing and instead deliver shellcode payloads.\n- That creates a covert C2 channel blending into legitimate HTTPS management traffic.",
          "Line Runner exploits CVE-2024-20359, abusing the ASA's ASDM (Adaptive Security Device Manager) plugin-loading mechanism:\n- ASDM plugins are ZIP archives that extend the management interface.\n- Line Runner installs itself as a malicious plugin loaded at boot.\n- That lets it survive reloads, software upgrades, and even factory resets that preserve the plugin directory.",
          "The combination is devastating: Line Runner guarantees persistence across all standard remediation steps while Line Dancer provides a covert C2 channel invisible in standard network logs — and only Line Dancer needs active connectivity, with Line Runner lying dormant until Line Dancer reactivates it.",
        ],
        codeExample: {
...
# 3. Enable logging to external syslog — Line Dancer suppresses local logs`,
      ...
      incident: {
        ...
        body: [
          "Cisco Talos identified ArcaneDoor in early 2024 during incident response at government customers, and it bore the marks of a nation-state actor with advanced operational security:\n- The attacker had deep knowledge of ASA internals.\n- They used zero-days that required substantial research to discover.\n- They deployed a two-stage implant designed to persist through every standard IR step.",
          "Targeting perimeter security devices was deliberate: ASA firewalls sit at the boundary of internal networks, see all north-south traffic, process VPN sessions for remote access, and connect to management networks that reach all internal infrastructure — so a persistent implant on an ASA is a persistent implant on the entire network.",
  ...
  // ─── Stage m37: CVE-2018-0171 — Cisco Smart Install RCE (GRU Campaign) ───────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "Smart Install uses a proprietary Cisco protocol over TCP port 4786, designed for zero-touch provisioning: a new switch powers on, broadcasts for a Smart Install Director, and automatically receives its configuration — with no authentication, because Smart Install assumes every device on the local network is trusted.",
          "CVE-2018-0171 is a stack-based buffer overflow in the Smart Install message parser:\n- A malformed Smart Install message sent to TCP port 4786 overflows a stack buffer on the target.\n- An unauthenticated attacker thereby achieves arbitrary code execution.\n- On a Cisco router that means full administrative control — equivalent to privilege 15 with OS access.",
          "The attack is completely silent — the malformed packet triggers the overflow before any authentication is checked, leaves no standard log entry, and can change the running configuration in memory without writing to flash — making forensic detection extremely difficult.",
        ],
        codeExample: {
...
# Verify: show vstack config | include Mode → should show 'Disabled'`,
      ...
      incident: {
        ...
        body: [
          ...
          "APT28 used compromised routers as both targets and pivot platforms:\n- Router compromise gave access to all traffic traversing the device and the ability to manipulate routing to redirect it.\n- It established a persistent presence on networks that usually have weaker endpoint monitoring than servers or workstations.\n- Several compromised devices were ISP infrastructure, so a single compromise yielded visibility into traffic for hundreds of downstream organizations.",
          "At the same time, Iranian threat actors independently discovered and exploited CVE-2018-0171 in targeted attacks on Middle Eastern networks — a coincidence that highlighted how a single unpatched network-device feature could attract multiple nation-state adversaries at once.",
        ],
      },
      diagram: {
        nodes: [
  ...
  // ─── Stage m38: Cisco ISE — Identity Services Engine NAC Security — Quiz ─────
  {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          "802.1X uses three roles, and the port stays locked until they complete the exchange:\n- The Supplicant — the device seeking access.\n- The Authenticator — the network switch or wireless AP.\n- The Authentication Server — Cisco ISE.\nWhen a device connects, the switch starts the 802.1X exchange and holds the port in an unauthorized state, passing only EAP (Extensible Authentication Protocol) traffic until authentication succeeds.",
          "ISE communicates with switches via RADIUS over UDP, and RADIUS uses MD5 for message authentication — a known-weak algorithm — so modern deployments use RADIUS over TLS (RadSec) to remove the MD5 weakness and prevent replay; without RadSec, an attacker who can intercept RADIUS traffic can run offline dictionary attacks against captured handshakes.",
          "ISE posture assessment uses the Cisco Secure Client (formerly AnyConnect) agent on endpoints, which reports patch compliance, antivirus-definition age, disk-encryption status, and firewall state — and based on the posture policy, ISE grants full access, redirects to remediation, or blocks entirely.",
        ],
        codeExample: {
...
! Expected: Port-Control FORCE-AUTH = NO, Auth SM State = AUTHENTICATED`,
      ...
      incident: {
        ...
        body: [
          "In a documented 2022 healthcare incident, an attacker gained physical access to a hospital wiring closet and plugged a laptop into a port configured for MAC Authentication Bypass (MAB) — a fallback that lets devices unable to run an 802.1X supplicant authenticate by MAC address:\n- They spoofed the MAC of an authorized infusion pump, obtained from a discarded network diagram found in a public recycling bin.\n- That got them onto the clinical VLAN holding the hospital's Electronic Medical Records system.",
          "The root cause was an authentication policy applied indiscriminately to all switch ports — falling back to MAB if no 802.1X authentication started within 10 seconds:\n- That fallback makes sense for dedicated ports serving medical devices that genuinely can't run 802.1X supplicants.\n- Applied to general-access ports in a wiring closet, it was a trivial bypass: any device that didn't start 802.1X within 10 seconds got clinical-VLAN access if its MAC was on the MAB whitelist.\n- And the whitelist was maintained loosely — medical-device MACs were added but rarely reviewed or removed when devices were decommissioned.",
          "Because the attacker reached Electronic Medical Records — a covered data category — the hospital had to issue a HIPAA breach notification: reporting to the HHS Office for Civil Rights, notifying affected patients, and completing a full HIPAA Risk Assessment. The corrective action plan required 802.1X with machine-certificate authentication for every supplicant-capable endpoint, restricting MAB to a documented inventory of known IoT devices (infusion pumps, imaging equipment, vitals monitors) each assigned to a device-category-specific VLAN, and deploying Cisco ISE posture assessment for all managed endpoints. Total remediation — breach notification, legal fees, security assessment, and ISE reconfiguration — topped $2 million; the port-configuration change that would have prevented the breach outright would have taken a network engineer about four hours.",
        ],
      },
      diagram: {
        nodes: [
