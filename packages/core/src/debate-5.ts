...
export const debate5Stages: StageConfig[] = [
  // ─── debate-5-01: The Flow as Battlefield ─────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Flowing (epoch 1) is the foundation; in the clash, the flow becomes a strategic instrument — a live map of the battlefield showing the state of every argument. At any moment in a round, each argument on the flow is in one of a few states: winning (you've made a response the opponent hasn't answered), losing (they've made a response you haven't answered), contested (both sides have traded responses), or dropped (one side never responded at all). Reading these states across your flow tells you, at a glance, where you're ahead, where you're behind, and where the decisive battles are — which is the information you need to make every strategic decision in the round.",
  ...
  // ─── debate-5-02: Advanced Refutation ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Basic refutation (epoch 1's four-step) answers an argument with one response. Advanced refutation layers multiple independent responses to the same argument, so the argument is beaten even if some of your answers fail. The classic structure is a cascade of 'even if' layers: first deny the argument outright, but even if it's true, deny that it links to the impact, but even if it links, deny the impact matters, but even if it matters, show you outweigh. Each layer is an independent reason you beat the argument, so the opponent must win every layer to win the argument — while you need only win one.",
        ...
        "Layered refutation must be disciplined, not scattershot. The layers should be genuinely independent (each a separate reason you win) and consistent (not contradicting each other — avoid the double-turn from epoch 1). They should be ordered logically, usually from the most decisive ('this is just false') down through the 'even if' fallbacks ('even if true, you still lose'). And they should be prioritized by strength — lead with your best layer, since the opponent and judge weight the first response most. Done well, layered refutation makes your answers robust: a single response can be beaten by a single good rebuild, but three independent layers force the opponent to win three separate battles, and usually they can't.",
      ],
      technical: {
  ...
  // ─── debate-5-03: Turns ───────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "A turn (introduced in epoch 1) is the most powerful refutation because it doesn't merely neutralize the opponent's argument — it converts the argument into offense for your side. There are two kinds. A link turn flips the causal direction: the opponent says your plan causes a bad thing; you show it actually causes the opposite (a good thing), or prevents the bad thing. An impact turn concedes the link but flips the value: the opponent says your plan causes X and X is bad; you show X is actually good. Either way, an argument the opponent intended to hurt you now helps you — and if they drop the turn, it becomes conceded offense that can win you the round.",
        ...
        "But turns carry the risk the earlier epochs flagged: the double-turn. If you both link-turn and impact-turn the same argument ('the plan doesn't cause X' AND 'X is good'), you contradict yourself — implying you'd want the plan to cause the good thing you just denied it causes — and a sharp opponent makes your contradiction prove their point. So turns require discipline: pick one type per argument, ensure the turn is consistent with the rest of your case, and have the evidence to defend the flipped link or revalued impact. Used carefully, turns are the judo of debate — using the opponent's own force against them — and the single highest-leverage refutation move available.",
      ],
      technical: {
  ...
  // ─── debate-5-04: Cross-Examination Strategy ──────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Strategic cross-ex has specific goals you plan in advance: extract a concession you'll use in your speech (a missing funding source, an admitted contradiction, an overstated piece of evidence), expose that a key term was never defined or that two arguments conflict, or pin down the opponent's position so they can't shift it later. You design the questions backward from the concession you want: what must they admit, and what series of unavoidable small questions leads there? The questions are short and leading ('Your evidence is from 2009, correct?' 'And internet usage has changed enormously since then, yes?') so the opponent has little room to maneuver and you keep control.",
        "Control is the through-line. You control the exchange by asking, not arguing (don't deliver speeches as questions); by keeping questions short and closed (yes/no or narrow), denying the opponent room to filibuster; by cutting off non-responsive rambling politely but firmly ('I appreciate that, but my question was...'); and by never asking a question whose answer you can't predict or handle. When answering cross-ex, you resist the funnel — give honest but minimal answers, decline to concede framings you reject, and don't volunteer material. Cross-examination won't usually win the round on its own, but the concessions it harvests, deployed in your next speech, often decide it — and the composure and control it displays shape the judge's impression throughout.",
      ],
      technical: {
  ...
  // ─── debate-5-05: Extending Arguments ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "A proper extension does three things: it references the original argument (so the judge connects it on the flow), it answers any responses the opponent made (you can't just repeat the argument as if uncontested if they attacked it — you must extend through their response), and it re-impacts (reminds the judge why the argument matters and how it's weighed). 'Extend our second contention on economic growth — they responded that growth hurts the environment, but [answer], so the contention stands, and it outweighs because [weighing].' A bare repetition isn't an extension; a real extension carries the argument over the opponent's responses and forward in the round.",
        "Extension is selective, not universal — you can't extend everything in limited time, so you extend what you're winning and what matters (the battlefield read from epoch 5-01 tells you which). You extend your strongest contentions, your dropped turns (conceded offense), and the arguments the round will be decided on; you let go of arguments that are losing or irrelevant (the collapse from epoch 5-07). The skill is a relay: each speech receives the argument from the last and carries it further, answering new responses and re-weighing, so that by the final speech the argument arrives intact and weighed. An argument that completes this relay is one the judge can vote on; one dropped along the way fades from the round, however good it was when first made.",
      ],
      technical: {
  ...
  // ─── debate-5-06: Frontlining ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Frontlining (sometimes called 'answering the answers' or 'extending through responses') is defending your own arguments against the opponent's attacks. When you make a contention and the opponent responds to it, frontlining is the work of answering those responses so your contention survives and is extended. It's the defensive half of the clash, complementing the offensive refutation of the opponent's case. A debater who builds a strong case but can't frontline watches their own arguments get dismantled — the case was only as good as its ability to withstand the attacks the opponent was always going to make.",
        ...
        "Frontlining must answer the actual response, not a strawman of it, and must be layered for the responses that matter (epoch 5-02). The common failures are: ignoring the opponent's response and just repeating your contention (which loses, because the response stands unanswered); answering a weaker version of their response than they actually made; or frontlining everything shallowly instead of concentrating on the attacks that threaten your winning arguments. Strong frontlining is selective and deep — it pours defensive effort into protecting the contentions you're winning the round on, answers the opponent's actual responses with specific evidence, and carries the contention through intact. Offense wins rounds, but a debater who can't frontline has no case left to win with.",
      ],
      technical: {
  ...
  // ─── debate-5-07: Collapsing ──────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Collapsing is the strategic narrowing of a round: as it progresses, you stop trying to win every argument and instead concentrate on the few you're winning and that decide the round. Early speeches are broad (you make multiple contentions, multiple responses); later speeches must narrow, because there isn't time to extend and weigh everything, and trying to do so means doing all of it shallowly. The skilled debater collapses — choosing their strongest ground and pouring their remaining time into developing, extending, and weighing it deeply, while letting weaker or less relevant arguments go.",
        ...
        "Collapse intelligently, using your flow read (epoch 5-01). Choose the arguments to collapse to based on: which you're winning (your responses stand, or the opponent dropped them), which are hardest for the opponent to recover, and which weigh best. Often you collapse to a dropped turn (conceded offense) plus a winning contention, or to the single cleanest path to the ballot. Then you spend the final speeches extending those few fully, frontlining the attacks on them, and weighing them against everything the opponent has — telling the judge 'here are the two reasons we win, and here's why they outweigh.' Collapsing is the strategic heart of the late round: the discipline to choose your ground and commit, rather than dissipating your force across a front too wide to hold.",
      ],
      technical: {
  ...
  // ─── debate-5-08: Strategic Concession ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "There are several reasons to concede. First, irrelevance: if an argument doesn't bear on the arguments that will decide the round, contesting it wastes time you need elsewhere ('we'll grant their point about X — it doesn't affect our winning argument about Y'). Second, weighing: you can concede an opponent's impact entirely and still win by outweighing it ('even granting their economic harm, our impact is larger and irreversible, so we win'). Third, strategic use: sometimes conceding an argument sets up your own — granting a premise that, combined with your argument, leads to your conclusion. Conceding the right things is a scalpel that removes distractions and focuses the round on your winning ground.",
        "The discipline is knowing what's safe to concede. You concede arguments that don't threaten your path to the ballot; you never concede an argument that, if granted, loses you the round (an argument that turns your case, or that you need to win to access your impacts). This requires the flow read (epoch 5-01) and weighing analysis (epoch 4-07) — you concede an impact only when you're confident you outweigh it; you concede a contention only when it's not load-bearing for your win. Done well, strategic concession is liberating: it frees your time and the judge's attention from arguments that don't matter, lets you grant the opponent's true points gracefully (which builds credibility, epoch 1's ethics), and concentrates the entire round on the few arguments where you win. Conceding what doesn't matter is how you win what does.",
      ],
      technical: {
  ...
  // ─── debate-5-09: Crystallization ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "A crystallized final speech has a recognizable shape. It identifies the key voting issues (the few arguments that decide the round), explains why your side wins each one (extending through the opponent's responses, frontlining), and — most importantly — weighs them against the opponent's best arguments, telling the judge why your reasons to vote outweigh theirs ('even if they win their argument, we win because ours is larger, more probable, and we've conceded nothing essential'). It ends by explicitly asking for the ballot and stating the clearest one-sentence reason: 'Vote for us because our plan solves a real, present harm and they've given you no impact that outweighs it.'",
        "Crystallization is persuasion at its most distilled. The judge is about to decide, and a great final speech makes that decision easy and obvious — it pre-writes the judge's reason for decision. The failures are: giving a final speech that's just more line-by-line (overwhelming the judge with details instead of synthesizing), introducing new arguments (disregarded), or failing to weigh (leaving the comparison to the judge). The skill is to rise above the trees and show the forest — to take everything that happened in the round and crystallize it into a clear, weighted story that points unmistakably to your ballot. Like a lawyer's closing argument, it doesn't present new evidence; it tells the jury what the evidence means and why it compels a verdict in your favor. Master crystallization and you win the close rounds, because you, not the judge, decide what the round was about.",
      ],
      technical: {
  ...
  // ─── debate-5-10: The Dropped Argument ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Not every drop wins the round, though — the skill is leveraging the drop that matters. A dropped argument is only as valuable as its impact. A conceded but trivial point doesn't win; a conceded argument that's decisive does. So the move is: identify the drops on your flow, select the ones that matter (a dropped turn — conceded offense — is the gold standard, but also a dropped contention or a dropped impact you can weigh), and then extend and weigh them explicitly. 'They dropped our second contention entirely — it's conceded, and it independently wins the round because [weighing].' You make the judge see that the concession decides the ballot.",
        "Leveraging a drop has a specific technique. First, flag it clearly so the judge registers it ('they completely dropped our economic argument'). Second, explain the implication of the concession ('which means it's conceded as true — they've given us this'). Third, weigh it ('and it independently wins because it outweighs everything they're going for'). And do this before it's too late for the opponent to claim they'd answered it elsewhere — once an argument is dropped, the opponent generally can't answer it in a later speech (new responses to dropped arguments are disregarded, like new arguments). The dropped argument is the unguarded gate: the opponent has left an opening, and the debater who spots it and drives through wins the round through the gap the opponent failed to defend. Clean flowing finds the gate; clear extension and weighing walk through it.",
      ],
      technical: {
