...
# higher autonomy x bigger blast radius = deeper, more skeptical testing`,
      ...
      technical: {
        ...
        body: [
          "The core control loop is: input → model plans → `tool_use` request → your code executes the tool → `tool_result` returned → model reasons again → repeat until `end_turn`. Every arrow in that loop is an audit surface:\n- the instruction that shaped the plan\n- the tool that was allowed to run\n- the arguments passed\n- the data returned (which can carry injected instructions)\n- the decision to continue or stop",
          ...
          "Map the workflow before you test it. The first deliverable of any agentic audit is a system map:\n- every model, prompt, and tool\n- every data source and memory store\n- every external integration\n- every human checkpoint\n- the trust boundary drawn around what the agent can touch\nYou cannot scope risk you have not drawn.",
...
=> the layer tags prove no risk category was skipped`,
      ...
      technical: {
        ...
        body: [
          "Generation layer:\n- Representative controls — grounding/RAG with citation, output validation and schema enforcement, content filters, factuality/groundedness evals with thresholds, red-teaming for jailbreaks and bias.\n- Evidence artifacts — eval datasets + reports, red-team report, model card, content-filter config.",
          "Integration layer:\n- Representative controls — least-privilege agent identity, tool allow-lists and scoped permissions, input sanitization and prompt-injection defenses, human-in-the-loop on high-impact actions, egress/network controls, third-party model and tool vetting.\n- Evidence — tool registry, IAM/identity records, AIBOM with provenance, integration test results, HITL approval logs, guardrail policy.",
          "Amplification layer:\n- Representative controls — rate limits and spend/step budgets, circuit breakers and kill switches, blast-radius limits (scoped permissions cap damage), multi-agent loop detection, drift monitoring, synthetic-data governance.\n- Evidence — observability traces, drift dashboards, incident records, kill-switch test results, capacity/limit configs.",
...
# this is exactly the control Air Canada's chatbot lacked`,
      ...
      year: 2025,
      overview: [
        "Auditing the Generation layer means giving assurance that what the agent produces is accurate, safe, and within policy — measured with evidence rather than a few hand-picked good outputs. The controls that address every one of these threats leave the same kind of artifact: an evaluation. The threats:\n- Hallucination and factual inaccuracy.\n- Bias, toxicity, and unsafe content.\n- IP/copyright infringement.\n- Leakage of sensitive data into outputs.",
        ...
        "The model card ties the generation evidence together:\n- intended use\n- training-data summary\n- evaluation results\n- known limitations and failure modes\n- ownership\nFor a third-party model the auditor can't run training-time tests, so the vendor's model card plus the organization's own evals on its use case are the evidence. No model card and no use-case evals over a third-party model is a generation-layer blind spot worth flagging.",
      ],
      technical: {
...
=> a reusable template for the next agent`,
      ...
      year: 2025,
      overview: [
        "This capstone assembles the baseline epoch into a single repeatable engagement. The agentic audit follows the classic audit arc — scope, evidence request, walkthrough, testing, findings, reporting — but each phase is specialized for agents using the tools this epoch built:\n- the system map\n- the G-I-A risk lens\n- the artifact trail\n- the pipeline gates\n- the baseline control matrix\nDone once, it becomes a template you re-run on every agent.",
