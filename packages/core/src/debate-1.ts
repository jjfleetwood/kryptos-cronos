...
export const debate1Stages: StageConfig[] = [
  // ─── debate-1-01: What Debate Actually Is ─────────────────────────────────────
  {
    ...
    info: {
      ...
      year: -508,
      overview: [
        "Debate is a formal, rule-governed contest in which two sides argue opposite positions on a single statement called the resolution, and a neutral judge decides who argued more persuasively. It is fundamentally different from a quarrel: there are time limits, an order of speeches, a shared topic both sides must address, and a standard of evidence. The goal is not to humiliate your opponent or to be the loudest — it is to give the judge better reasons to vote for your side than the other team gives for theirs.",
  ...
  // ─── debate-1-02: The Resolution ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: -80,
      overview: [
        "The resolution is the sentence the entire debate orbits, and its type tells you what kind of proof is required. There are three classic types. A resolution of fact claims that something is or is not true ('Resolved: Social media does more harm than good to teenagers'). A resolution of value claims that something is better, worse, just, or moral ('Resolved: A just society values liberty above equality'). A resolution of policy claims that an actor should take an action ('Resolved: The federal government should provide universal childcare'). Recognizing the type is the first analytical move in any round.",
        "Each type carries a different burden. Fact resolutions require empirical proof — evidence and reasoning about what is true. Value resolutions require a standard of evaluation: you cannot argue 'better' without first establishing the criterion by which better is measured. Policy resolutions require you to show a problem exists, that your plan solves it, and that solving it is worth the cost. A new debater who treats a value topic like a policy topic (or vice versa) will argue past the actual point of clash.",
  ...
  // ─── debate-1-03: Claim, Warrant, Impact ──────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1858,
      overview: [
        "The atom of debate is the argument, and a complete argument has three parts: claim, warrant, and impact. The claim is the statement you want the judge to accept ('A higher minimum wage reduces poverty'). The warrant is the reasoning or evidence that proves the claim ('because workers earning more can afford necessities, and studies X and Y show poverty fell after wage increases'). The impact is why the claim matters to the round ('therefore the Affirmative's plan should be adopted because reducing poverty is the central goal we agreed to weigh'). Drop any one part and the argument is incomplete.",
  ...
  // ─── debate-1-04: Evidence and Sources ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1973,
      overview: [
        "Evidence is the material a debater uses to prove a warrant: statistics, peer-reviewed studies, expert testimony, government reports, historical examples, and credible journalism. Its purpose is to convert 'I say so' into 'a knowledgeable, neutral source says so', borrowing the source's credibility. But not all evidence is equal, and a central skill of debate is evaluating quality — because a confident claim backed by a random website loses to a modest claim backed by a peer-reviewed study.",
        "Three questions test any piece of evidence. Who said it? (A domain expert or institution beats an anonymous or partisan source.) When? (Recent data beats stale data on fast-moving topics; a 2009 internet statistic is nearly worthless today.) How do they know? (A study with a method and a sample beats an assertion; primary research beats someone summarizing someone else.) A debater who can ask these three questions out loud in a round can dismantle weak evidence and protect strong evidence.",
        "Honesty about evidence is non-negotiable. Quoting a source out of context, inventing statistics, clipping a quote so it appears to say the opposite of the author's meaning, or fabricating a citation are forms of evidence ethics violations that can disqualify a debater from a tournament. The competitive culture of debate treats evidence as a shared trust: you may interpret and spin, but you may never misrepresent what a source actually says. Learning to cite cleanly — author, qualification, date, source — both proves your case and protects your integrity.",
      ],
      technical: {
        ...
        body: [
          ...
          "Librarians teach the CRAAP test for source quality: Currency (how recent), Relevance (does it actually address the point), Authority (who wrote it and are they qualified), Accuracy (is it supported and verifiable), and Purpose (is the source trying to inform or to sell/persuade). In a round you rarely have time to recite all five, but internalizing them lets you instantly spot the weakest source on the flow and target it: 'their only evidence is a 2008 op-ed from an advocacy group — prefer our 2023 government dataset.'",
        ],
        codeExample: {
  ...
  // ─── debate-1-05: Flowing ─────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The mechanics: turn the paper sideways (landscape) and divide it into columns, one for each speech in the format (e.g., 1AC, 1NC, 2AC, 2NC…). As each speaker talks, you write their arguments in their column, aligned with the argument they are answering in the previous column. Affirmative arguments are often written on one sheet and Negative arguments on another, so each 'sheet' or 'flow' tracks one line of clash from start to finish. You write fast, in abbreviations and symbols, capturing the structure (claim/warrant/impact) rather than every word.",
        "The payoff of flowing is the line-by-line: in your rebuttal you go down the flow argument by argument, answering each one and pointing out which of your arguments the opponent dropped. Dropped arguments — points the other side failed to answer — are conceded as true and become some of your strongest weapons. You cannot do any of this from memory in a fast round; the flow is your map. Coaches say it bluntly: 'if it's not on your flow, it didn't happen.' Learning to flow is the difference between reacting and actually debating.",
      ],
      technical: {
        title: "Flow Shorthand and the Line-by-Line",
        body: [
          "Debaters develop personal shorthand to keep up: arrows for cause/effect (→), abbreviations (govt, econ, w/, b/c), symbols for common moves (a circled argument = extend it; a struck-through one = it was answered or dropped). You don't transcribe — you capture the skeleton: the tag (a short label for the argument), the source/warrant, and the impact. Speed comes from writing structure, not sentences. Many debaters now flow on laptops in spreadsheet columns, but the column logic is identical to paper.",
  ...
  // ─── debate-1-06: Speaker Roles and Speech Order ──────────────────────────────
  {
    ...
    info: {
      ...
      year: 1789,
      overview: [
        "Debate speeches come in two families: constructives and rebuttals. Constructive speeches are where new arguments are built — the first speeches of the round, in which each side presents its case. Rebuttal speeches are where arguments are answered, extended, and weighed — generally, no brand-new arguments may be introduced in rebuttals, because the other side would have no chance to respond. Understanding this division prevents the classic novice error of saving a great argument for the last speech, where the judge will disregard it as unfair 'new in the rebuttal.'",
        "The speeches alternate between the sides in a fixed order set by the format, and each speech has a defined role. In a typical two-person format there is a first and second speaker per side, and the duties divide: the first speaker presents the case and handles the early clash; the second speaker deepens the rebuttal and begins weighing; the final speeches (often called the 'final focus', 'rebuttal', or 'last speech') stop introducing arguments entirely and instead crystallize — telling the judge the two or three reasons their side has won. Knowing which speech you are giving tells you what the judge expects from you.",
  ...
  // ─── debate-1-07: Cross-Examination Basics ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1730,
      overview: [
        "Cross-examination (cross-ex, CX, or 'crossfire' in Public Forum) is a period in which one debater directly questions another. Its purpose is widely misunderstood by beginners, who treat it as a chance to argue or 'win' the exchange. Experienced debaters know better: cross-ex is for setting up arguments you will make in your next speech, clarifying confusing parts of the opponent's case, and exposing weaknesses (missing evidence, internal contradictions, dropped warrants) on the record — quietly, with controlled questions. You rarely 'win' cross-ex on the spot; you plant seeds you harvest later.",
        "Good questioning is strategic. The strongest cross-ex questions are narrow and lead toward a point: 'Does your plan cost money? Where does the funding come from? You didn't specify a source, correct?' Each answer boxes the opponent in. Open-ended questions ('What do you think about the economy?') hand the opponent free speaking time to repeat their case — the opposite of what you want. You ask short questions and let them give short answers; you are building a record, not having a conversation.",
  ...
  // ─── debate-1-08: Refutation Basics ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1872,
      overview: [
        "Refutation is the act of answering an opponent's argument, and the cleanest way to teach it is the four-step method: They Say, But, Because, Therefore. First, you signpost the argument you're answering ('They say the policy will raise costs'). Second, you state your response ('But costs will actually fall'). Third, you warrant it ('Because economies of scale lower per-unit price, as the 2022 industry report shows'). Fourth, you impact it back to the round ('Therefore their main disadvantage disappears, and our case stands'). This structure forces every refutation to be complete and trackable on the flow.",
        ...
        "There are recognizable categories of response you can reach for. You can deny the link (the argument's cause-effect doesn't hold), deny the impact (even if true, it doesn't matter much), turn the argument (show it actually supports your side), present counter-evidence (a better source says the opposite), or point out it's non-unique (the supposed harm happens anyway, with or without their plan). A debater who internalizes this menu can respond to almost anything quickly — and then dresses each response in the four-step structure so it lands cleanly.",
      ],
      technical: {
  ...
  // ─── debate-1-09: Delivery Basics ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1863,
      overview: [
        "Delivery is how your argument travels from your mind to the judge's flow. A brilliant case delivered as an unintelligible blur persuades no one, because the judge can only credit what they can understand and write down. The foundational delivery skills are vocal: a controlled rate (fast enough to fit your content, slow enough to be understood), variation in pitch and volume to mark importance, and deliberate pauses that let a key point land and give the judge time to write. Monotone speed-reading is the most common novice failure — it sacrifices every argument to the goal of saying more of them.",
        ...
        "Physical delivery matters too, though less than clarity. Eye contact with the judge (not your notes) builds credibility and lets you read whether they're following. A stable stance, purposeful gestures, and visible composure communicate confidence; fidgeting, swaying, and reading face-down communicate the opposite. In lay-judged formats like Public Forum, persuasive, accessible delivery can be decisive, because the judge is an ordinary person who rewards clarity and presence. Across all formats the rule holds: delivery exists to serve comprehension, and the debater whose reasoning is easiest to follow has a standing advantage.",
      ],
      technical: {
  ...
  // ─── debate-1-10: Ethics and Etiquette ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1945,
      overview: [
        "Debate is an adversarial activity built on a foundation of trust and respect, and the norms that protect that foundation are not optional politeness — they are part of the competition. Evidence ethics (never fabricating or distorting a source) is the most serious: violations can lose rounds and tournaments because the entire activity depends on quoted evidence being real. Beyond evidence, debaters are expected to argue the case, not attack the person — disagreeing sharply with an argument while treating the opponent with courtesy. The phrase that captures it: be tough on the issue, easy on the person.",
        "Etiquette governs the conduct of the room. You shake hands or acknowledge your opponents, you don't talk over people or make faces during their speeches, you address your remarks to the judge rather than sneering at the opposition, and you accept the decision graciously whether you win or lose. These behaviors matter competitively: judges form impressions of credibility and maturity that color close decisions, and many ballots include explicit speaker points for demeanor. A debater who wins arguments but is rude often loses speaker awards — and sometimes loses the round.",
