...
export const baseball13Stages: StageConfig[] = [
  ...
  // ─── baseball-13-02: Reading the Ball off the Bat ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 1990,
      overview: [
        "The single most important skill for any outfielder is reading the ball off the bat and getting a good first step — the 'jump.' An outfielder who reads the ball instantly and breaks in the right direction covers far more ground than one who's faster but reacts late or takes a poor route. The read begins before contact: the outfielder studies the pitch, the count, the hitter's tendencies, and the swing, anticipating where the ball is likely to go. Then, at contact, the sound and initial flight tell the outfielder the ball's trajectory.",
        "The first step and route are where the read becomes range. On a ball hit over their head, the outfielder uses a drop step (pivoting and turning to run back, never backpedaling) and takes an efficient angle to the ball's landing spot. On a ball in front, they charge under control. The goal is the most efficient route — often a slightly curved 'banana' route that gets the outfielder to the spot with their momentum moving toward where they'll throw, rather than a route that leaves them backpedaling or lunging. Reading the ball and running a good route turn average speed into above-average range.",
  ...
  // ─── baseball-13-03: Catching Fly Balls and Line Drives ───────────────────────
  {
    ...
    info: {
      ...
      year: 1985,
      overview: [
        "Catching fly balls is the outfielder's most fundamental skill. The technique: get to the ball's landing spot quickly using a good route, get under control as the ball comes down, and catch it with two hands at or above the throwing-shoulder side of the head — positioned so the catch flows directly into a throw. Catching the ball above the throwing shoulder (rather than basket-style at the belt) lets the outfielder transfer and throw immediately, which matters when runners are tagging or advancing. Two hands secure the catch and speed the transfer.",
        "Different balls require different catches. A routine fly ball is caught with two hands above the throwing shoulder while moving toward the target if possible. A line drive is the hardest read — the outfielder must judge whether it's sinking (charge and possibly catch low or on a short hop) or carrying (drift back) and commit. The over-the-shoulder catch is used on balls hit well over the outfielder's head — after a drop step and full sprint to the spot, the outfielder catches the ball over the shoulder on the run, like a receiver in football. Each catch is built through repetition.",
  ...
  // ─── baseball-13-05: Building the Outfielder's Body ───────────────────────────
  {
    ...
    info: {
      ...
      year: 2015,
      overview: [
        "Outfield is fundamentally a running position, and the left fielder's body is built around speed and the ability to cover ground. The key physical priorities are first-step quickness and acceleration (to get a great jump and reach balls in the gaps and down the line), straight-line and change-of-direction speed (to run down fly balls over a large area), endurance (to chase balls and stay sharp through a long game and season), and a strong, accurate throwing arm. While the corner outfield spots can use a bit less raw speed than center field, range still depends on quickness and reads.",
        "Speed and acceleration are the foundation. First-step quickness and acceleration — built through sprint training, acceleration drills, and reaction work — translate directly to the jump and range. Straight-line speed lets the outfielder run down deep balls; change-of-direction agility helps on slicing balls and in the gaps. Strong legs and a powerful core drive the sprint and the throws. The outfielder's lower body and explosiveness are what allow him to cover the large area the position demands.",
  ...
  // ─── baseball-13-06: The Throw and the Crow Hop ───────────────────────────────
  {
    ...
    info: {
      ...
      year: 1972,
      overview: [
        "Throwing is a major part of outfield defense, and the left fielder's throws — especially to third base — help control the running game. A good outfield throw combines arm strength with footwork (the crow-hop), accuracy, and a low trajectory that lets the cutoff man handle it if needed. The left fielder throws to third base to hold or retire runners trying to advance, to second base to keep hitters to a single, and home on plays at the plate — usually through the cutoff man, who can relay the throw or let it through.",
        "The crow-hop generates a strong throw. After fielding the ball or catching a fly, the outfielder takes a crow-hop — a small shuffle-hop that gathers momentum and transfers weight from the back foot through the throw — and throws over the top with a four-seam grip on a low, carrying line. Throwing on a line (not a high rainbow) lets the ball get there faster and lets the cutoff man catch it at shoulder height if a relay or redirect is needed. A strong, accurate, low throw with good footwork is far more valuable than a wild rocket.",
  ...
  // ─── baseball-13-07: Backups and Positioning ──────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1995,
      overview: [
        "Outfielders have a job on every play, even when the ball isn't hit to them — and much of it is backing up. The left fielder backs up third base on throws and pickoff attempts from the catcher or pitcher, backs up the center fielder on balls hit to left-center, and backs up bases on infield throws and rundowns on the left side. The principle is that an overthrow or a ball that gets past a fielder should never go unguarded — the left fielder positions behind the play so a misplay doesn't cost extra bases. Backing up is constant, unglamorous, and essential.",
        "Positioning before the pitch is the other half. The left fielder's depth and lateral position change with the situation: normal depth for most situations, shallow when a weak hitter or a play at the plate is likely, deep in no-doubles situations (late in close games, to prevent extra-base hits over his head), and shaded toward the line or the gap based on the hitter's tendencies. The left fielder reads the hitter, the pitcher, and the situation, and positions accordingly — and as part of the outfield, he adjusts in concert with the center fielder.",
  ...
  // ─── baseball-13-08: Cutoffs, Relays, and Where to Throw ──────────────────────
  {
    ...
    info: {
      ...
      year: 1995,
      overview: [
        "Knowing where to throw the ball — and deciding before fielding it — is a defining outfield skill. As the ball comes to the left fielder, he must already know the situation (outs, runners, score) and where the play is: hold the ball, throw to second to keep a hitter to a single, throw to third to retire or hold an advancing runner, or throw home on a play at the plate. Deciding in advance lets the outfielder field the ball with his momentum already moving toward the target and make a quick, decisive throw, rather than catching it and then thinking.",
  ...
  // ─── baseball-13-09: Situational IQ ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2000,
      overview: [
        "A complete left fielder plays the game mentally before each pitch. He processes the situation — score, inning, outs, runners and their speed, the count, and the hitter's tendencies — and sets his depth and position accordingly (normal, shallow, deep no-doubles, shaded to the line or gap). He also pre-decides what he'll do if the ball is hit to him: where the play is and where he'll throw. This pre-pitch thinking lets him react instantly and correctly when the ball is hit, rather than figuring it out after the fact.",
        "The elements — sun, wind, and the field — are part of an outfielder's situational awareness in a way they aren't for infielders. The left fielder must know where the sun is and use sunglasses or his glove to shield it on fly balls, account for the wind's effect on fly balls (knocking them down or carrying them), and know the field's quirks (the wall, the corners, the warning track). Checking the wind and sun before each at-bat, and knowing the warning track is near by feel, are part of playing the outfield well and safely.",
