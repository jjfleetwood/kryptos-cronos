  ...
  // ─── poh-01: Side Channels 101 ───────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The leaks come in several physical flavors, each its own attack family. Power analysis exploits how the current the chip draws changes with the data it processes; electromagnetic attacks read the radio waves switching transistors emit, which mirror the computation; timing attacks use the fact that operations take different amounts of time depending on secret values; and acoustic and optical channels read the faint sounds and light that chips and components emit tied to what they compute.",
        "Side channels are dangerous, and so often overlooked, for three reasons. They bypass the math entirely, so a provably secure cipher offers no protection if its implementation leaks; they are passive and silent, since the attacker often just observes, leaving no trace in any log; and they are everywhere, because smartcards, phones, servers, IoT devices, and even air-gapped machines all have a physical body that leaks. Defending them requires thinking like a physicist, not just a coder.",
      ],
      technical: {
  ...
  // ─── poh-02: Power Analysis ──────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "There are two levels of the attack, increasing in power. Simple Power Analysis (SPA) reads the secret almost directly from a single or few power traces, when different operations look visibly different — a classic example being an RSA exponentiation where a key bit of 1 triggers an extra multiply, showing as an extra bump. Differential Power Analysis (DPA) is far stronger: it collects thousands of traces and uses statistics to extract a key even when the leak is tiny and buried in noise, with no need to see the operations clearly.",
        "Power analysis is devastating precisely where security matters most. It broke smartcards, pay-TV cards, and SIM cards — devices designed specifically to protect keys — and the attacker needs only physical access and a cheap measurement setup (a resistor and an oscilloscope), not the chip's design. It proved that putting a key in 'secure hardware' is not enough; the hardware must also be engineered so its power draw does not depend on the secret.",
      ],
      technical: {
  ...
  // ─── poh-03: EM / TEMPEST ────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "EM attacks come in two broad styles. Eavesdropping on data reconstructs information directly, like rebuilding a monitor's image from the radio noise its cable and display emit — 'Van Eck phreaking'. Cryptographic EM analysis is like power analysis but with an antenna: capture the EM field near a chip and apply the same statistics to extract keys, often with finer spatial focus than power.",
        "This threat has been taken seriously at the highest levels for decades. 'TEMPEST' is the NSA codename for the study of, and protection against, compromising emanations — a program once highly classified — and it drives real-world rules: 'red/black' separation (keeping secret-carrying wires away from ones that exit a secure area), shielded rooms, and zoning standards for sensitive facilities. Because EM can be picked up remotely and passively, it is a favorite for espionage against air-gapped and high-value systems.",
      ],
      technical: {
  ...
  // ─── poh-04: Timing Attacks ──────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Several common coding patterns secretly leak through timing. Early-exit comparisons — a password or signature check that returns the moment it finds a mismatch — reveal how many leading characters were correct; secret-dependent branches that take a different code path for a key bit of 1 vs 0 change the time; table lookups indexed by secrets leak the index through whether the needed data is already in fast cache or must be fetched from slow memory (a cache-timing attack); and variable-time math, where big-number operations' duration depends on the values, is classic in naive RSA and recently in PQC (the KyberSlash bug).",
        "Timing attacks are uniquely accessible and far-reaching. They can be remote, since measuring a server's response time over the network can extract keys, as real attacks on TLS have shown; they power some of the biggest CPU vulnerabilities, with Spectre and Meltdown ultimately reading secrets through cache-timing differences; and they are the reason serious cryptographic code is written to be 'constant-time' — engineered so its duration is independent of any secret, which the quantum-safe track also stressed.",
      ],
      technical: {
  ...
  // ─── poh-05: Acoustic & Optical ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Acoustic side channels turn microphones into spies. CPU and power-circuit 'coil whine' emits ultrasonic sounds whose pitch changes with the operation, enough to extract RSA keys from a laptop with a nearby microphone or even a phone; keyboard acoustics differ from key to key, so a recording of typing can reconstruct what was typed, including passwords; and printers and other mechanics leak too — even dot-matrix printers have been shown to leak printed text through their sound.",
        "Optical side channels turn cameras and sensors into eyes. Status LEDs can flicker in step with internal data, leaking it to anyone with a line of sight or a light sensor; screen and reflection leakage recovers images from reflections in eyes, windows, or glossy objects, or from a screen's subtle glow on a wall; and vibration-to-sound attacks point a laser or camera at a light bulb or object near a conversation to recover the sound from tiny vibrations ('Lamphone'-style attacks).",
      ],
      technical: {
  ...
  // ─── poh-06: Rowhammer ───────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "What makes Rowhammer so striking is several things at once. It is a fault in the hardware itself, so no software vulnerability is required and it sidesteps the entire software-security stack; a single flipped bit in the right place can be catastrophic, since flipping a bit in a page-table entry or a permission flag can hand an attacker control of memory or escalate privileges; and it is triggered by ordinary memory reads, so even sandboxed or unprivileged code — including, in some cases, JavaScript in a web browser — can attempt it.",
        "The attack has evolved through a cat-and-mouse with defenses. Early Rowhammer hammered rows adjacent to a target, and defenses such as Target Row Refresh tried to detect and refresh victim rows; researchers responded with 'many-sided' hammering, 'half-double', and other patterns that slip past mitigations, plus tricks to bypass error-correcting memory (ECC). It has been demonstrated for privilege escalation on PCs and servers, on mobile (the 'Drammer' attack on Android), and from the browser — making it one of the most important hardware-security stories of the last decade.",
      ],
      technical: {
  ...
  // ─── poh-07: Cold Boot & Remanence ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The attack flips a comfortable assumption on its head. Full-disk encryption (BitLocker, FileVault, LUKS) protects data at rest, but to use the disk the key must sit unlocked in RAM while the computer runs; if an attacker gets brief physical access to a running or just-suspended machine, they can recover that key from memory and decrypt the disk, defeating the encryption without attacking the cipher. 'Just lock your screen' is not enough — the key is still in RAM behind the lock screen.",
        "The classic technique is almost theatrically physical. The attacker sprays the RAM with an upside-down can of compressed air (which comes out very cold), freezing the chips to slow the charge decay dramatically; then quickly reboots into a tiny program that dumps memory, or physically transplants the chilled DRAM into another machine to read it; and finally reconstructs the keys from the dumped memory image, since encryption keys have recognizable structure and built-in redundancy that survives a few flipped bits.",
      ],
      technical: {
  ...
  // ─── poh-08: Fault Injection ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "There are several ways to glitch a chip, all by disturbing something it depends on. Voltage glitching briefly drops or spikes the supply voltage so a transistor doesn't settle correctly, corrupting one operation; clock glitching inserts an extra or shortened clock pulse so an instruction is skipped or executed before its inputs are ready; electromagnetic fault injection uses a sharp EM pulse from a nearby coil to flip internal signals without any wires; and laser or optical fault injection focuses light on a decapsulated chip to flip specific bits with surgical precision, at a bigger budget.",
        "A single well-placed fault is shockingly powerful. It can skip a check — glitch the instant a 'is the PIN correct?' or 'is secure boot signature valid?' comparison runs, and the chip proceeds as if it passed — or corrupt a computation, where a fault during a cryptographic operation can, through 'differential fault analysis', reveal the secret key from the difference between a correct and a faulty output (famously, one faulted RSA signature can leak the private key). Real targets include payment cards, secure bootloaders, hardware wallets, and game consoles — anywhere a chip enforces a security decision an attacker wants to override.",
      ],
      technical: {
  ...
  // ─── poh-09: Hardware Implants & Supply Chain ────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Supply-chain hardware threats take several forms. Hardware trojans are tiny malicious modifications added during chip design or fabrication, dormant until triggered and capable of leaking data or disabling protections; implants and interdiction physically add a component, or intercept a shipment to modify it, to create a backdoor, as documented in leaked intelligence catalogs of spy hardware; counterfeit and recycled parts are fake or salvaged chips that may be substandard or maliciously altered, common in cheap or grey-market components; and malicious firmware subverts the low-level code (BIOS/UEFI, microcontroller firmware) that runs before any operating system.",
        "This is so hard to defend for three reasons. You cannot easily inspect a finished chip, since billions of transistors are sealed in opaque packaging and a malicious circuit can be a vanishingly small fraction of the whole; trust is transitive, so you rely on every vendor in the chain, and on their vendors, often with little visibility; and the blast radius is enormous, because a backdoor baked into a widely-used component can compromise everything built on it, silently and at scale.",
      ],
      technical: {
  ...
  // ─── poh-10: Defending the Physical Layer ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The defenses gather into four families, each countering attacks you have met. The first makes leaks meaningless — constant-time code against timing, masking and hiding against power and EM, and balanced circuits so physical signals no longer depend on secrets. The second makes secrets not linger — memory encryption, register-only keys, and scrubbing on suspend or shutdown to defeat remanence and cold boot. The third makes disturbance fail safely — redundant checks, sensors for voltage, clock, light, and EM, tamper meshes, and error-correcting or refresh-hardened memory against faults and Rowhammer. The fourth contains and verifies the physical device — shielding and zoning (TEMPEST), secure elements, HSMs, and TPMs as hardened vaults, and provenance and attestation for the supply chain.",
        "The unifying idea is the hardware root of trust. A small, hardened, heavily-defended component — a secure element, TPM, or HSM — holds the most critical secrets and enforces the most critical decisions, built specifically to resist side channels and faults; the rest of the system anchors its trust to that root through measured boot, attestation, and sealed keys, so even if outer layers are probed, the crown jewels stay protected. Security becomes a property designed in from the silicon up, not bolted on — which is exactly the mindset these two foundational tracks have been building toward.",
      ],
      technical: {
...
// Flags live in stage-flags.ts; fragments assemble to them. Now using the shared
// 3-step mkDeepCtf factory (deepened from the original 2-step CTFs).

const POH_CTF: Record<string, CtfConfig> = {
  "poh-01": mkDeepCtf(
    "A secure device leaks information through a physical side channel while it computes. Monitor its emissions, model the leak, and extract a secret bit — without ever reading memory.",
    "OP: SIDE CHANNEL 101\nTarget: a chip doing a secret-dependent computation.\nGoal: monitor emissions, model the leak, extract a bit.\nSequence: monitor-emissions -> model-leak -> extract-bit",
    "FLAG{S1D3_",
    "Mission Brief",
    ["monitor-emissions", "CH4NN3L_", "Emissions Monitored", [
      "$ monitor-emissions --power --em --timing",
      "Captured power, EM, and timing traces while the device runs its secret operation.",
      "All three vary with the data — the computation is leaking.",
      "Next: model-leak",
    ]],
    ["model-leak", "L34K_", "Leak Modeled", [
      "$ model-leak --hamming-weight",
      "Built a leakage model: emission amplitude tracks the Hamming weight of processed bits.",
      "Now each guessed key bit predicts a measurable signal.",
      "Next: extract-bit",
    ]],
    ["extract-bit", "F0UND}", "Secret Bit Extracted", [
      "$ extract-bit --correlate",
      "Correlated the model against the traces — secret bit recovered with high confidence.",
      "No memory was ever read; the machine's physics gave it away.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Monitor emissions. Run: monitor-emissions", "Model the leak. Run: model-leak", "Extract the secret bit. Run: extract-bit", "Run 'assemble', then submit the flag"],
    { "traces.txt": "channels: power, EM, timing\ndata-dependent: yes\nmodel: Hamming-weight" },
  ),
  "poh-02": mkDeepCtf(
    "A smartcard's power draw rises and falls with the key bits it processes. Capture power traces, build a Hamming-weight model, and run differential power analysis (DPA) to correlate out the key.",
    "OP: POWER ANALYSIS (SPA/DPA)\nTarget: smartcard performing AES with a secret key.\nGoal: capture traces, build the model, correlate the key.\nSequence: capture-traces -> build-model -> correlate-key",
    "FLAG{DPA_",
    "Mission Brief",
    ["capture-traces", "P0W3R_", "Power Traces Captured", [
      "$ capture-traces --shots 10000 --align sbox",
      "Recorded 10,000 power traces, each aligned on the first-round S-box.",
      "The tiny per-operation current dips encode the data.",
      "Next: build-model",
    ]],
    ["build-model", "TR4C3_", "Model Built", [
      "$ build-model --hypothesis sbox-out --leakage hamming",
      "For each guess of a key byte, predict the S-box output's Hamming weight -> expected power.",
      "256 hypotheses per byte, ready to test.",
      "Next: correlate-key",
    ]],
    ["correlate-key", "K3Y}", "Key Correlated Out", [
      "$ correlate-key",
      "The correct guess shows a sharp correlation peak; wrong guesses stay in the noise.",
      "Repeated per byte -> full AES key recovered. Masking/hiding would have stopped this.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Capture power traces. Run: capture-traces", "Build the model. Run: build-model", "Correlate out the key. Run: correlate-key", "Run 'assemble', then submit the flag"],
    { "dpa.txt": "traces: 10000 (aligned on S-box)\nmodel: Hamming weight of S-box output\nattack: per-byte correlation peak" },
  ),
  "poh-03": mkDeepCtf(
    "A monitor radiates its image as electromagnetic emanations. Tune an antenna to the pixel-clock harmonics, demodulate the signal, and reconstruct the screen from across the wall — Van Eck phreaking.",
    "OP: TEMPEST / VAN ECK\nTarget: an unshielded display leaking EM emanations.\nGoal: tune the antenna, demodulate, reconstruct the screen.\nSequence: tune-antenna -> demodulate -> reconstruct-screen",
    "FLAG{T3MP3ST_",
    "Mission Brief",
    ["tune-antenna", "VAN_", "Antenna Tuned", [
      "$ tune-antenna --sweep 50M-900M",
      "Swept the band for the display's pixel-clock harmonics; locked at the refresh frequency.",
      "The cable and panel act as an unintentional transmitter.",
      "Next: demodulate",
    ]],
    ["demodulate", "3CK_", "Signal Demodulated", [
      "$ demodulate --am --pixel-clock",
      "AM-demodulated the captured RF and recovered the per-pixel intensity timing.",
      "Sync to the line/frame rate to rebuild the raster.",
      "Next: reconstruct-screen",
    ]],
    ["reconstruct-screen", "SCR33N}", "Screen Reconstructed", [
      "$ reconstruct-screen",
      "Rendered the raster: the victim's screen is now readable from outside the room.",
      "Defense: TEMPEST shielding, filtered cabling, and zoning.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Tune the antenna. Run: tune-antenna", "Demodulate. Run: demodulate", "Reconstruct the screen. Run: reconstruct-screen", "Run 'assemble', then submit the flag"],
    { "tempest.txt": "leak: EM from pixel clock\nband: video refresh harmonic\nfix: TEMPEST shielding + zoning" },
  ),
  "poh-04": mkDeepCtf(
    "A comparison routine returns faster when more bytes match. Profile the endpoint, measure the timing to nanosecond precision, and recover the secret byte-by-byte — a classic timing attack on non-constant-time code.",
    "OP: TIMING ATTACK\nTarget: a server using a non-constant-time secret comparison.\nGoal: profile, measure timing, recover the secret.\nSequence: profile-compare -> measure-timing -> recover-secret",
    "FLAG{T1M1NG_",
    "Mission Brief",
    ["profile-compare", "L34K_", "Comparison Profiled", [
      "$ profile-compare",
      "The check uses memcmp, which returns at the first mismatched byte — an early-exit.",
      "Match more leading bytes => the function runs longer. That timing is the leak.",
      "Next: measure-timing",
    ]],
    ["measure-timing", "N0N_", "Timing Measured", [
      "$ measure-timing --samples 5000 --ns",
      "Averaged 5,000 samples per guess to beat the noise; response time rises with each correct byte.",
      "The signal is small but statistically clear.",
      "Next: recover-secret",
    ]],
    ["recover-secret", "C0NST}", "Secret Recovered", [
      "$ recover-secret --byte-by-byte",
      "Walked the secret one byte at a time, keeping whichever guess was slowest. Full secret recovered.",
      "Fix: constant-time comparison — time must not depend on the secret.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Profile the comparison. Run: profile-compare", "Measure the timing. Run: measure-timing", "Recover the secret. Run: recover-secret", "Run 'assemble', then submit the flag"],
    { "timing.txt": "compare: memcmp (early-exit)\nsignal: response time vs leading-byte matches\nfix: constant-time compare" },
  ),
  "poh-05": mkDeepCtf(
    "A laptop's capacitors emit a faint high-pitched whine that depends on the RSA operation underway. Record the acoustics, filter the signal, and extract the private key — an acoustic side channel.",
    "OP: ACOUSTIC EXFIL\nTarget: a laptop performing RSA decryption.\nGoal: record the sound, filter it, extract the RSA key.\nSequence: record-acoustic -> filter-signal -> extract-rsa",
    "FLAG{",
    "Mission Brief",
    ["record-acoustic", "4C0UST1C_", "Acoustics Recorded", [
      "$ record-acoustic --chosen-ciphertext",
      "Recorded coil whine during chosen-ciphertext decryptions from a phone mic nearby.",
      "The faint whine changes with the operation underway.",
      "Next: filter-signal",
    ]],
    ["filter-signal", "RSA_", "Signal Filtered", [
      "$ filter-signal --bandpass --denoise",
      "Band-passed and averaged the recording to isolate the key-dependent acoustic signature.",
      "Distinct patterns now line up with the secret exponent bits.",
      "Next: extract-rsa",
    ]],
    ["extract-rsa", "L1ST3N3D}", "RSA Key Extracted", [
      "$ extract-rsa",
      "Mapped acoustic signatures to key bits — RSA private key extracted by listening alone.",
      "The 'air gap' was defeated by sound (Genkin-Shamir-Tromer). Fix: blinding + dampening.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Record the acoustics. Run: record-acoustic", "Filter the signal. Run: filter-signal", "Extract the RSA key. Run: extract-rsa", "Run 'assemble', then submit the flag"],
    { "acoustic.txt": "source: capacitor coil whine\nvaries with: RSA exponent bits\nfix: RSA blinding + acoustic dampening" },
  ),
  "poh-06": mkDeepCtf(
    "Repeatedly accessing one DRAM row disturbs its neighbors, flipping bits you don't own. Profile the DRAM, hammer aggressor rows until a bit flips, then exploit the flip for privilege escalation — Rowhammer.",
    "OP: ROWHAMMER\nTarget: DRAM with no/weak Rowhammer mitigation.\nGoal: profile, hammer to flip a bit, exploit it.\nSequence: profile-dram -> hammer-rows -> exploit-flip",
    "FLAG{",
    "Mission Brief",
    ["profile-dram", "R0WH4MM3R_", "DRAM Profiled", [
      "$ profile-dram --map-rows",
      "Reverse-engineered the physical row layout to find aggressor/victim pairs.",
      "No target-row-refresh mitigation present.",
      "Next: hammer-rows",
    ]],
    ["hammer-rows", "B1T_", "Bit Flipped", [
      "$ hammer-rows --double-sided",
      "Hammered both neighbors of a victim row ~500k times per refresh interval.",
      "A reproducible bit flip appears in the victim row.",
      "Next: exploit-flip",
    ]],
    ["exploit-flip", "FL1PP3D}", "Flip Exploited", [
      "$ exploit-flip --target pte",
      "Massaged memory so a page-table entry lands on the flippable bit -> flip grants write access.",
      "Privilege escalated from software alone. Fix: ECC + TRR + higher refresh.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Profile the DRAM. Run: profile-dram", "Hammer the rows. Run: hammer-rows", "Exploit the bit flip. Run: exploit-flip", "Run 'assemble', then submit the flag"],
    { "dram.txt": "layout: aggressor/victim rows mapped\nmitigation: none (no TRR)\nexploit: flip a page-table-entry bit" },
  ),
  "poh-07": mkDeepCtf(
    "DRAM keeps its contents for seconds after power-off — longer when cold. Freeze the RAM, dump physical memory, and recover the disk-encryption key still sitting there — a cold-boot attack.",
    "OP: COLD BOOT\nTarget: a locked laptop with full-disk encryption (key in RAM).\nGoal: freeze the RAM, dump it, recover the key.\nSequence: freeze-ram -> dump-memory -> recover-keys",
    "FLAG{C0LD_",
    "Mission Brief",
    ["freeze-ram", "B00T_", "RAM Frozen", [
      "$ freeze-ram --inverted-air",
      "Sprayed the DRAM with inverted canned air to ~-50C, extending remanence to minutes.",
      "Contents now survive a power cycle long enough to image.",
      "Next: dump-memory",
    ]],
    ["dump-memory", "K3Y_", "Memory Dumped", [
      "$ dump-memory --cold-boot-imager",
      "Cold-booted a tiny imager off USB and dumped physical RAM before it decayed.",
      "The decrypted FDE key is somewhere in this image.",
      "Next: recover-keys",
    ]],
    ["recover-keys", "R3C0V3R3D}", "Keys Recovered", [
      "$ recover-keys --scan-aes-schedule",
      "Scanned for AES key schedules (their structure is recognizable) -> disk key recovered from RAM.",
      "Fix: don't leave keys in DRAM at rest; bind to a TPM/secure element.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Freeze the RAM. Run: freeze-ram", "Dump memory. Run: dump-memory", "Recover the keys. Run: recover-keys", "Run 'assemble', then submit the flag"],
    { "coldboot.txt": "remanence: seconds (minutes when cold)\ntarget: FDE key in DRAM\nrecover: AES key-schedule scan" },
  ),
  "poh-08": mkDeepCtf(
    "A precisely-timed voltage glitch makes a CPU skip an instruction. Profile the boot, tune a glitch, and skip the secure-boot signature check so unsigned code runs — fault injection, the trick that has opened consoles and hardware wallets.",
    "OP: FAULT INJECTION\nTarget: a device whose secure boot verifies a firmware signature.\nGoal: profile boot, tune the glitch, bypass the check.\nSequence: profile-boot -> glitch-voltage -> bypass-check",
    "FLAG{GL1TCH_",
    "Mission Brief",
    ["profile-boot", "S3CUR3_", "Boot Profiled", [
      "$ profile-boot --power-trace",
      "Power trace pinpoints exactly when secure boot runs the signature compare.",
      "That instant is the glitch window.",
      "Next: glitch-voltage",
    ]],
    ["glitch-voltage", "B00T_", "Glitch Tuned", [
      "$ glitch-voltage --width 50ns --sweep-offset",
      "Swept a ~50 ns core-voltage droop around the window until the compare corrupts WITHOUT crashing.",
      "Found a reliable glitch timing.",
      "Next: bypass-check",
    ]],
    ["bypass-check", "BYP4SS}", "Secure Boot Bypassed", [
      "$ bypass-check",
      "Fired the glitch exactly at the signature check -> the 'if (valid)' branch is skipped.",
      "Unsigned firmware boots; trust bypassed. Fix: glitch sensors, redundant checks, double-reads.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Profile the boot. Run: profile-boot", "Tune the glitch. Run: glitch-voltage", "Bypass the check. Run: bypass-check", "Run 'assemble', then submit the flag"],
    { "glitch.txt": "window: signature-compare instant (from power trace)\nglitch: ~50ns Vcore droop\nfix: glitch sensors + redundant checks" },
  ),
  "poh-09": mkDeepCtf(
    "A board came back from an untrusted assembler with an extra component. Scan it against the golden reference, probe the rogue part tapping a data line, and prove a supply-chain compromise.",
    "OP: HARDWARE IMPLANT\nTarget: a server board from an untrusted supply chain.\nGoal: scan, probe the implant, prove the compromise.\nSequence: scan-board -> probe-implant -> prove-supply-chain",
    "FLAG{HW_",
    "Mission Brief",
    ["scan-board", "1MPL4NT_", "Board Scanned", [
      "$ scan-board --xray --diff-golden",
      "X-ray + power-rail diff against the golden reference flags an undocumented component.",
      "It sits between the BMC and a data line.",
      "Next: probe-implant",
    ]],
    ["probe-implant", "SUPPLY_", "Implant Probed", [
      "$ probe-implant --logic-analyzer",
      "The part taps the management bus: it can read and inject on the BMC's serial line.",
      "It's an active implant, not a passive part.",
      "Next: prove-supply-chain",
    ]],
    ["prove-supply-chain", "CH41N}", "Supply Chain Proven", [
      "$ prove-supply-chain",
      "Component isn't on the BOM and was added at assembly -> a supply-chain hardware backdoor.",
      "Defenses: trusted assembly, golden-sample diffing, tamper-evident seals, attestation.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Scan the board. Run: scan-board", "Probe the implant. Run: probe-implant", "Prove the supply-chain compromise. Run: prove-supply-chain", "Run 'assemble', then submit the flag"],
    { "board.txt": "diff vs golden: 1 extra component\nlocation: BMC <-> data line\nnot on BOM -> added at assembly" },
  ),
  "poh-10": mkDeepCtf(
    "Flip to defense. Audit a device's physical-layer protections against everything in this epoch, move the secrets into a secure element, and harden the layer so the attacks fail.",
    "OP: HARDEN THE METAL\nTarget: a device that must resist side-channel, fault, and implant attacks.\nGoal: audit, move secrets into a secure element, harden.\nSequence: audit-defenses -> move-to-se -> harden-layer",
    "FLAG{",
    "Mission Brief",
    ["audit-defenses", "S3CUR3_", "Defenses Audited", [
      "$ audit-defenses",
      "Checked: constant-time crypto? shielding? glitch/voltage sensors? tamper mesh? secure element?",
      "Gaps: no glitch detection, and secrets live in general flash — exposed to every attack in this track.",
      "Next: move-to-se",
    ]],
    ["move-to-se", "3L3M3NT_", "Secrets Relocated", [
      "$ move-to-se",
      "Migrated keys into a certified secure element with on-die crypto + masking.",
      "Secrets never leave the SE in the clear now.",
      "Next: harden-layer",
    ]],
    ["harden-layer", "H4RD3N3D}", "Physical Layer Hardened", [
      "$ harden-layer --sensors --shield --mesh",
      "Enabled fault/voltage sensors, EM shielding, and an active tamper mesh.",
      "Side-channel, fault, and implant attacks now fail — defense designed into the metal.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Audit the defenses. Run: audit-defenses", "Move secrets to the secure element. Run: move-to-se", "Harden the physical layer. Run: harden-layer", "Run 'assemble', then submit the flag"],
    { "harden.txt": "gaps: no glitch sensors, keys in flash\nfix: secure element + sensors + shield + tamper mesh" },
  ),
};
