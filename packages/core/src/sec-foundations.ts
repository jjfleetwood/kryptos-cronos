...
export const secFoundationsStages: StageConfig[] = [
  // ─── 01: Security Controls & Frameworks ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2014,
      overview: [
        "A security control is any measure that reduces risk, and exams classify controls two ways. The first is by category — how the control is implemented: technical (a.k.a. logical) controls are enforced by technology such as firewalls, encryption, and access control lists; managerial (administrative) controls are policies, procedures, and risk assessments; operational controls are carried out by people day to day, like security guards, awareness training, and change management; and physical controls protect the tangible world with locks, fences, and cameras.",
        "The second is by type — what the control does relative to an incident. Preventive controls stop an incident before it happens (a firewall rule, a locked door); deterrent controls discourage an attacker (warning signs, visible cameras); detective controls identify an incident in progress or after the fact (IDS, logs, audits); corrective controls fix or limit damage afterward (restoring from backup, patching); compensating controls are alternatives used when the primary control isn't feasible (extra monitoring in place of a missing patch); and directive controls instruct or guide behavior (an acceptable use policy). A single safeguard can be more than one type — a security guard is preventive, deterrent, and detective.",
        "Frameworks organize controls into coherent programs so organizations don't reinvent the wheel. The NIST Cybersecurity Framework (CSF) structures security around core functions — Identify, Protect, Detect, Respond, Recover, and, in CSF 2.0, Govern; ISO/IEC 27001 defines a certifiable Information Security Management System (ISMS) with Annex A controls; the CIS Critical Security Controls are a prioritized, prescriptive checklist; and NIST SP 800-53 catalogs detailed controls for federal systems. Knowing which framework does what — and being able to classify any control by category and type — is foundational governance literacy tested across Security+, ISC² CC, CISA, and CRISC.",
      ],
      technical: {
  ...
  // ─── 02: Physical Security & Deception ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "Physical security protects the tangible assets — buildings, data centers, devices, and people — that digital controls depend on, and it is layered in defense-in-depth. The perimeter deters and delays with fencing, bollards to stop vehicle ramming, lighting, and clear sightlines; the entry layer controls who gets in with access control vestibules (mantraps) that trap a person between two interlocking doors to stop tailgating, plus access badges, PIN pads, and biometric locks; and the monitoring layer detects and records with security guards, motion and door sensors, and video surveillance/CCTV. Each layer maps to control types: a bollard is preventive, a visible camera is deterrent and detective, a guard is all three.",
        "Tailgating (piggybacking) — following an authorized person through a door — is a top physical-access threat, which is why access control vestibules and guards matter: they enforce one-person-at-a-time entry. Other physical concerns round out the picture: protecting cabling and ports against tapping or unauthorized connections, secure media destruction so discarded drives don't leak data, environmental controls such as HVAC and fire suppression, and visitor management to log and escort anyone who isn't staff. Physical and cyber security converge in industrial and OT environments, where a USB drive carried through the door can bridge an air gap — as the Stuxnet worm demonstrated.",
        "Deception technology turns the tables by luring attackers into fake assets so defenders can detect and study them. A honeypot is a decoy system that looks valuable but exists only to attract and observe attackers; a honeynet is a whole network of honeypots for broader observation and threat intelligence; a honeyfile is a bait document (e.g., 'passwords.xlsx') that triggers an alert when opened; and a honeytoken is a fake credential, API key, or database record whose every use is a guaranteed sign of compromise. Because legitimate users have no reason to touch these decoys, any interaction is high-fidelity evidence of an intruder — deception is a detective control that also disrupts and slows attackers.",
      ],
      technical: {
        title: "Layered Physical Defense and Deception Assets",
        body: [
          "Defense-in-depth physically:\n- deter and delay at the perimeter (fence, bollards, lighting)\n- authenticate at entry (vestibule/mantrap, badge, biometric)\n- detect everywhere (guards, sensors, CCTV)\nThe goal is to make unauthorized physical access slow, noisy, and recorded. Prevent tailgating with vestibules and guards; protect the supply chain of who enters with visitor logs and escorts.",
  ...
  // ─── 03: Change Management ─────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Change management is the formal process for making changes to systems in a controlled, documented, reversible way. It exists because the majority of unplanned outages are self-inflicted — a configuration push, patch, or update that wasn't reviewed or tested — and it protects both availability and security: an unreviewed firewall change can open a hole, and an untested update can take down production. A typical workflow runs from a change request raised with a clear owner stating what and why, through review and approval by stakeholders and a change advisory board (CAB), an impact analysis of what could break and who is affected, test results attached from a non-production environment, a backout plan documenting exactly how to revert if it fails, scheduling in an approved maintenance window, and finally documenting and version-controlling the whole thing.",
        "Exams emphasize the supporting pieces that make a change safe: ownership and stakeholder identification plus approval workflows, standard operating procedures (SOPs) for routine pre-approved changes, allow/deny lists and firewall rules that may need updating, restricted activities and downtime planning for the window, and the correct order of service and application restarts honoring dependencies. Legacy applications and undocumented dependencies are classic sources of change failures — restarting one service can cascade into others — so documentation and version control (updating diagrams, configs in Git, and runbooks) ensure the environment's true state is always known and changes are auditable.",
  ...
  // ─── 04: Resilience & Recovery (BCDR) ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "Resilience keeps services running through failures; recovery restores them after a disaster. The metrics that drive every design decision come from a Business Impact Analysis (BIA): the Recovery Time Objective (RTO) is the maximum tolerable time to restore a service, so a 15-minute RTO means you need hot standby, not a tape restore; the Recovery Point Objective (RPO) is the maximum tolerable data loss measured as a point in time, so a 1-hour RPO means backups or replication must capture data at least hourly; and MTTR (mean time to repair) and MTBF (mean time between failures) are the related reliability metrics. Together these numbers set how much you must spend on availability and data protection.",
        "High-availability designs prevent downtime in the first place: load balancing distributes traffic across redundant servers, clustering lets nodes take over for each other on failure, and platform diversity or multi-cloud avoids any single vendor or technology being a single point of failure. Recovery sites give you somewhere to run if a whole location is lost — a hot site is fully equipped and running, ready in minutes (lowest RTO, highest cost); a warm site has hardware and connectivity in place but needs data and setup (hours); and a cold site is just space and power (days, lowest cost). Geographic dispersion ensures a single regional disaster — flood, fire, power loss — doesn't take out both primary and recovery.",
        "Backups are the recovery foundation, and exams test the details. Keep copies onsite for fast restore and offsite for disaster protection, following the 3-2-1 rule of three copies, two media types, and one offsite; match backup frequency to the RPO, encrypt the backups, and keep an immutable or offline copy so ransomware can't reach them; use the right techniques among full, incremental, and differential backups, snapshots, synchronous or asynchronous replication, and journaling; plan power resilience, where a UPS bridges short outages and enables clean shutdown while generators sustain longer ones; and test recovery with tabletop exercises, simulations, parallel processing, and full failover drills. Untested recovery is a myth — the worst time to discover a broken backup is during a real disaster.",
      ],
      technical: {
        ...
        body: [
          ...
          "Backup strategy essentials:\n- follow 3-2-1 (3 copies, 2 media, 1 offsite)\n- encrypt backups\n- keep at least one immutable or air-gapped copy so ransomware can't encrypt your recovery\nMatch frequency to RPO. Replication and snapshots provide fast recovery points; journaling enables point-in-time recovery. Power: UPS for ride-through and clean shutdown, generators for sustained outages. And test everything — a backup you've never restored is a hope, not a plan.",
        ],
        codeExample: {
  ...
  // ─── 05: Cryptographic Solutions ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2011,
      overview: [
        "Cryptography provides three guarantees — confidentiality through encryption, integrity through hashing, and authenticity/non-repudiation through digital signatures. Symmetric encryption (e.g., AES) uses one shared key, which is fast but means the key must be exchanged securely; asymmetric encryption (e.g., RSA, ECC) uses a public/private key pair where data encrypted with the public key only decrypts with the matching private key, solving key distribution; and in practice the two combine, with asymmetric crypto exchanging a symmetric session key (key exchange, e.g., Diffie-Hellman) and symmetric crypto then doing the bulk work, exactly how TLS operates. Encryption can be applied at many levels — full-disk, partition, volume, file, database, and record — plus transport in transit.",
        "Public Key Infrastructure (PKI) is the system of trust that binds a public key to an identity using digital certificates (X.509). A Certificate Authority (CA) signs certificates, and a chain of trust runs from a trusted root CA through intermediates to the end-entity certificate; to get one, you submit a Certificate Signing Request (CSR) to the CA. Certificates may be self-signed (trusted only by you), CA-signed (trusted publicly), wildcard (*.example.com), or carry Subject Alternative Names; to invalidate a certificate early, clients check a Certificate Revocation List (CRL) or query the Online Certificate Status Protocol (OCSP); and key escrow stores keys with a trusted third party for recovery.",
        "Hashing produces a fixed-length, one-way fingerprint of data (e.g., SHA-256) for integrity — change one bit and the hash changes completely. Salting adds random data before hashing passwords so identical passwords hash differently and rainbow tables fail; key stretching (bcrypt, PBKDF2, Argon2) deliberately makes hashing slow to resist brute force; a digital signature hashes a message and encrypts the hash with the signer's private key, and anyone verifies it with the signer's public key, proving integrity, authenticity, and non-repudiation; keys are protected by hardware, with a TPM on a device, an HSM for high-assurance storage, a secure enclave for isolated processing, and a KMS for lifecycle; and obfuscation techniques such as steganography, tokenization, and data masking protect data in other ways, while blockchain uses hashing and signatures to build an open public ledger.",
      ],
      technical: {
        ...
        body: [
          ...
          "Certificate lifecycle:\n- generate a key pair\n- submit a CSR to a CA\n- the CA validates and issues a signed cert\n- clients trust it via the chain to a trusted root\n- revocation (CRL/OCSP) handles compromise\nFor passwords, never store them reversibly — store a salted, key-stretched hash (bcrypt/Argon2). Protect private keys in a TPM/HSM, never in plaintext. These mechanics underpin every secure connection and authentication system.",
        ],
        codeExample: {
  ...
  // ─── 06: Identity & Access Management ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "Identity and Access Management (IAM) controls who can access what, and proves it — built on the AAA framework of Authentication (proving who you are), Authorization (what you're allowed to do), and Accounting (logging what you did). Authentication draws on factors in distinct categories: something you know, like a password or PIN; something you have, like a hardware token, phone, or smart card; something you are, like biometrics such as a fingerprint or face; somewhere you are, via geolocation; and something you do, via behavioral patterns. Multi-Factor Authentication (MFA) combines factors from different categories — a password plus a code from your phone — so a stolen password alone isn't enough, while passwordless approaches (passkeys, security keys) and password managers reduce reliance on memorized secrets.",
        "Single Sign-On (SSO) lets a user authenticate once and reach many systems, and federation extends that trust across organizations using an Identity Provider (IdP) that vouches for users to Service Providers (SPs). The protocols matter: SAML is XML-based and common in enterprise web SSO; OAuth 2.0 is delegated authorization, granting an app limited access without sharing your password; OpenID Connect (OIDC) is an authentication layer on top of OAuth; LDAP handles directory queries; and Kerberos provides ticket-based authentication in Windows domains. Provisioning creates accounts and assigns permissions, while de-provisioning promptly removes access when someone leaves — orphaned accounts are a major risk — and identity proofing verifies a real person while attestation and recertification periodically review that access is still appropriate.",
        "Access control models define how permissions are granted. Discretionary Access Control (DAC) lets data owners grant access at their discretion; Mandatory Access Control (MAC) has the system enforce access by labels and clearances in high-security and military contexts; Role-Based Access Control (RBAC) ties permissions to job role and is the most common in enterprises; and Rule-Based and Attribute-Based Access Control (ABAC) make decisions using conditions and attributes such as time of day, location, and device. Underpinning them all are least privilege (only the access needed) and separation of duties (no single person controls a whole sensitive process), and for privileged accounts, Privileged Access Management (PAM) adds just-in-time access, password vaulting, and ephemeral credentials so powerful accounts aren't standing targets.",
      ],
      technical: {
  ...
  // ─── 07: Data Protection & Classification ─────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2018,
      overview: [
        "Protecting data starts with classifying it along two axes. The first is classification level — from public (no harm if disclosed) through private, sensitive, and confidential, up to restricted and critical (severe harm if disclosed). The second is data type — regulated data (PII, personally identifiable information; PHI, protected health information; PCI, cardholder data), trade secrets, intellectual property, and financial or legal data. Classification drives the controls: the more sensitive the data, the stronger the protection and the tighter the access, and a major cause of breaches is simply not knowing where sensitive data lives, or treating it all the same.",
        "Data exists in three states, each needing its own protection: at rest, stored on disk and protected with encryption and access controls; in transit, moving across a network and protected with TLS/IPsec; and in use, being processed in memory — the hardest — and protected with access controls, secure enclaves, and confidential computing. Data sovereignty and residency add a legal dimension: data is subject to the laws of the country where it's stored, so a company may be required to keep certain data within specific borders (enforced via geolocation/geographic restrictions), and cross-border transfers are heavily regulated under laws like the EU's GDPR.",
        "Protection methods give you a toolbox: encryption for confidentiality at rest and in transit and hashing for integrity; masking, which shows only part of a value such as **** **** **** 1234; tokenization, which replaces sensitive data with a non-sensitive token while the real value stays in a secure vault; obfuscation, segmentation to isolate sensitive data, and permission restrictions; and Data Loss Prevention (DLP), which detects and blocks sensitive data from leaving the organization. Governance also defines the data lifecycle — retention schedules, minimization (collect only what's needed), and disposal/sanitization — and assigns roles: the data owner (accountable), the controller (decides why and how data is processed), the processor (acts on the controller's behalf), and the custodian/steward (day-to-day protection), while privacy laws add rights like the 'right to be forgotten.'",
      ],
      technical: {
  ...
  // ─── 08: Security Awareness & Operational Security ────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "The vast majority of breaches involve a human element — a clicked phishing link, a reused password, a social-engineering phone call — so security awareness programs aim to turn the workforce from the weakest link into a 'human firewall.' A mature program includes ongoing training at onboarding, recurring through the year, and role-based for higher-risk roles like finance and IT; clear policies and handbooks that set expectations; and simulated phishing campaigns that safely test and teach employees to recognize and report suspicious messages. The goal isn't to shame people who click, but to build reflexes: pause on urgency, verify unexpected requests, and report.",
        "Awareness also covers recognizing anomalous behavior in systems and in people: risky behavior, like ignoring policy for convenience; unexpected behavior, like logins at odd hours or locations; and unintentional behavior, like accidentally emailing data to the wrong person. Insider-threat awareness teaches everyone to notice and report concerning signs, recognizing that threats can be malicious (a disgruntled employee), negligent (careless), or compromised (an account taken over). Reporting channels and a blame-light culture are essential — people must feel safe raising a hand quickly when something seems off, because early reporting shrinks the impact of an incident.",
        "Operational security (OpSec) is the day-to-day discipline of not leaking information attackers can use: handling removable media safely with no unknown USB drives, resisting social engineering by verifying identity before acting on requests, securing hybrid and remote work against home-network risks and shoulder-surfing with clean-desk and clean-screen habits, and practicing strong password management and a minimized digital footprint that denies attackers targeting material. Awareness is reinforced by monitoring and metrics — phishing-simulation click rates, reporting rates, and training completion — and it must be developed, executed, and continuously refreshed, because attackers constantly evolve their lures. A workforce that pauses, verifies, and reports is the control no technology can replace.",
      ],
      technical: {
  ...
  // ─── 09: Network Media, Cabling & Topologies ─────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "At the physical layer, data travels over media, and the two families trade off differently. Copper twisted-pair (RJ45 Ethernet) rises in bandwidth by category — Cat5e (1 Gbps), Cat6 (1–10 Gbps on short runs), Cat6a (10 Gbps to 100 m), and Cat7/Cat8 for the data center — and it's cheap and easy but limited to ~100 m and prone to electromagnetic interference (EMI), so shielded (STP) variants exist for noisy environments. Fiber-optic carries data as light, with single-mode (SMF, tiny core) for long distances of kilometers at high speed and multimode (MMF, larger core) cheaper for short data-center runs; fiber is immune to EMI, doesn't emit a tappable signal, and supports the highest bandwidths, but costs more and is more fragile. Choosing between them is an engineering trade-off of distance, bandwidth, cost, and interference resistance.",
        "Transceivers connect a device to the media. Hot-swappable modules — SFP (1 Gbps), SFP+ (10 Gbps), QSFP/QSFP28 (40/100 Gbps) — plug into switch and router ports and accept either copper or fiber, so one device can speak many media types; connectors include RJ45 for copper and LC/SC for fiber; and Power over Ethernet (PoE) carries electrical power alongside data on copper, running APs, cameras, and phones without a separate power cable. This modularity is what lets a single switch serve a building's mix of copper desks and a fiber backbone.",
        "Topology describes how nodes are connected. A star wires every device to a central switch and is the dominant modern LAN topology, where one cable fault affects only one device; a mesh links many nodes directly (full or partial) for redundancy and is common in WANs and wireless backhauls because multiple paths survive a link failure; three-tier (core/distribution/access) structures large campus networks for scalability, while a collapsed core merges layers for smaller sites; spine-leaf connects every leaf switch to every spine switch, giving predictable, low-latency 'east-west' server-to-server traffic favored in modern data centers; and point-to-point links a single pair of nodes. Knowing these shapes — and that physical and logical topology can differ — is core Network+ literacy.",
      ],
      technical: {
        title: "Choosing Media and Topology",
        body: [
          "Media decision factors:\n- distance (fiber for long/backbone, copper for short access)\n- bandwidth (fiber and higher Cat for more)\n- interference (fiber or STP in noisy/industrial settings)\n- security (fiber is harder to tap and doesn't radiate)\n- cost (copper is cheaper for short runs)\nA typical building uses fiber for the backbone between closets and copper to the desk, with PoE powering endpoints.",
          "Topology decision factors:\n- star for LAN access (simple, fault-isolated)\n- spine-leaf for data centers (uniform low-latency east-west)\n- three-tier for large campuses (scalable hierarchy)\n- mesh for redundancy where uptime is critical\nRemember the physical-vs-logical distinction: a network physically wired as a star can behave logically as a different topology depending on switching/VLAN configuration.",
        ],
        codeExample: {
  ...
  // ─── 10: Network Troubleshooting Methodology & Tools ──────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Network+ teaches a structured troubleshooting methodology so engineers fix problems systematically instead of randomly swapping parts, and the seven steps run in order. First, identify the problem — gather information, question users, note symptoms, determine what recently changed, and duplicate the problem if possible. Second, establish a theory of probable cause, questioning the obvious and considering multiple possibilities top-to-bottom or bottom-to-top of the OSI model. Third, test the theory to confirm or deny it — if confirmed, move on; if not, form a new theory or escalate. Fourth, establish a plan of action to resolve the problem, identifying potential effects. Fifth, implement the solution, or escalate as necessary. Sixth, verify full system functionality and, if applicable, add preventive measures. Seventh, document findings, actions, and outcomes. Following the steps in order — especially resisting the urge to skip from symptom to a guessed fix — is what the exam tests.",
        "The methodology is paired with tools, split into two kits. The software and command-line kit includes ping (reachability and latency), traceroute/tracert (path and where it fails), nslookup/dig (DNS resolution), ipconfig/ifconfig/ip (local IP config), netstat (connections and listening ports), arp (IP-to-MAC), tcpdump and Wireshark (packet capture and protocol analysis), nmap (host/port discovery), and iperf/speed tests (throughput). The hardware kit includes a cable tester and wire-map tester (pinout and continuity), a toner/probe (trace a cable in a bundle), a TDR/OTDR (distance to a cable fault in copper or fiber), a light meter (optical power), and a network tap (inline capture). Picking the right tool for the symptom is half the battle.",
        "Common issues map to predictable causes: name-resolution failures point to DNS; an address conflict or no address points to DHCP or a static-IP mistake (APIPA 169.254.x.x means no DHCP); a half/full duplex mismatch causes slowness and errors; a switching loop with no spanning tree causes broadcast storms; and latency, jitter, and packet loss degrade real-time apps. The disciplined flow — identify, theorize, test, plan, implement, verify, document — combined with the right diagnostic tool, is how professionals (and the exam) resolve network problems, and documentation closes the loop: the fix for today's outage becomes tomorrow's faster diagnosis.",
      ],
      technical: {
  ...
  // ─── 11: Security Operations Reporting, Metrics & Communication ───────────────
  {
    ...
    info: {
      ...
      year: 2018,
      overview: [
        "A huge part of real security operations — and a full domain of the CySA+ exam — is reporting and communication: turning technical findings into action by the right stakeholders. Effective vulnerability reporting identifies stakeholders (system owners, IT, management, compliance) and tailors the message, since executives need risk and business impact while engineers need specific remediation steps; prioritizes by severity/CVSS, asset criticality, and exploitability rather than raw counts; includes metrics and trends such as open vulnerabilities over time, mean time to remediate, and SLA compliance; and defines clear action plans with owners and deadlines while supporting compliance obligations. A report that buries the critical, exploitable, internet-facing vulnerability in a list of 5,000 low findings has failed.",
        "Incident response reporting is about communicating during and after an incident. During the incident, a communication plan defines who is told and when — internal stakeholders such as executives, legal, HR, and PR, and external ones such as affected customers, regulators, law enforcement, and partners — with regulatory breach-notification deadlines (e.g., GDPR's 72 hours) creating hard timelines. After the incident, the team produces a root cause analysis, a lessons-learned report, and an executive summary. Throughout, it tracks KPIs like mean time to detect (MTTD), mean time to respond (MTTR), and dwell time. The goal is to drive improvement and meet legal obligations, not to assign blame.",
        "Communication quality determines outcomes, and good reports share three traits: they are tailored to the audience, with technical depth for engineers and business risk for leadership; timely, since a perfect report delivered too late is useless; and honest and clear, the catastrophic counter-example being the cover-up, which turns a security incident into a legal, financial, and reputational disaster, as Uber learned in 2016. Strong reporting and communication — prioritized vulnerability reports with action plans, a tested incident communication plan, clear stakeholder identification, and meaningful metrics — convert security operations into actual risk reduction and keep an organization on the right side of the law.",
      ],
      technical: {
        title: "Reporting That Drives Action",
        body: [
          "Vulnerability reporting:\n- identify stakeholders and tailor by audience\n- prioritize by risk (CVSS × asset criticality × exploitability/exposure), not count\n- include trends and metrics (open findings over time, MTTR, SLA adherence)\n- give every critical finding an owner, a remediation action, and a deadline\nCompliance reporting maps findings to obligations (PCI, HIPAA, etc.).",
  ...
  // ─── 12: Risk Assessment & Management ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2017,
      overview: [
        "Risk is the combination of a threat exploiting a vulnerability and the resulting impact — commonly modeled as RISK = LIKELIHOOD × IMPACT. The management process runs in four moves — identify risks, assess and analyze them, respond to (treat) them, and monitor and report — and assessments can be ad hoc, one-time, recurring, or continuous. Two analysis styles answer different needs: qualitative analysis rates likelihood and impact on relative scales (low/medium/high) and plots them on a risk matrix or heat map, which is fast and intuitive but subjective, while quantitative analysis puts dollar figures on risk so controls can be cost-justified. Most programs use both: qualitative to triage quickly, quantitative to defend spending.",
        "The quantitative formulas are exam staples. Single Loss Expectancy (SLE) is the loss from one event, calculated as Asset Value (AV) × Exposure Factor (EF, the percentage of the asset lost); Annualized Rate of Occurrence (ARO) is how many times per year the event is expected; Annualized Loss Expectancy (ALE) = SLE × ARO is the expected yearly loss; and a control is worth buying when its annual cost is less than the ALE reduction it provides, turning security spending into a defensible cost-benefit decision. For example, a $200,000 asset with a 50% exposure factor has an SLE of $100,000; if the event is expected once every two years (ARO = 0.5), the ALE is $50,000 — so a control costing under $50,000/year that prevents it pays for itself.",
        "Risks are tracked in a risk register — each with an owner, a key risk indicator (KRI), a threshold, and a status — and two boundaries guide decisions: risk appetite (how much risk the organization will pursue toward its objectives, whether expansionary, neutral, or conservative) and risk tolerance (the acceptable variation around it). For each risk, choose one of four treatment strategies: avoid, stopping the risky activity altogether; mitigate or reduce, applying controls to lower likelihood or impact; transfer, shifting the financial impact via cyber insurance or outsourcing; or accept, acknowledging and living with it under a documented exception. After controls, what remains is residual risk, and the inherent-vs-residual distinction shows a control's effect — accepting a risk you don't understand is how organizations end up as breach headlines.",
      ],
      technical: {
        title: "Putting Numbers on Risk",
        body: [
          "Quantitative workflow:\n- value the asset (AV)\n- estimate the exposure factor (EF) for the threat\n- compute SLE = AV × EF\n- estimate ARO (events/year)\n- compute ALE = SLE × ARO\nCompare control cost to ALE reduction: if a $20,000/year control cuts ALE from $50,000 to $5,000 (a $45,000 reduction), it's worth it. Qualitative analysis complements this where dollar values are hard — use a likelihood × impact matrix to rank and communicate.",
  ...
  // ─── 13: Secure Development Lifecycle (SDLC) ──────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Security activities map to each phase. In requirements, define security and compliance requirements up front. In design, perform threat modeling (e.g., STRIDE) to anticipate attacks and design defenses such as least privilege, input validation, and secure defaults. In development, follow secure coding standards — input validation, output encoding, parameterized queries against injection, and proper authentication and session handling — and do peer code review. In testing, run security tests, and in deployment, sign code and harden configuration. Separation of environments (dev/test/staging/production) and separation of duties prevent unreviewed or malicious changes from reaching production.",
        "Automated security testing is central to a modern DevSecOps pipeline. SAST (Static Application Security Testing) analyzes source code without running it, catching injection, hardcoded secrets, and unsafe patterns early; DAST (Dynamic Application Security Testing) tests the running application from the outside, catching runtime and configuration issues; IAST combines both, while SCA (Software Composition Analysis) inventories third-party and open-source dependencies to flag known-vulnerable components (most code today is borrowed — Log4Shell was a single vulnerable dependency); and secrets scanning catches credentials committed to code while code signing proves integrity and origin. Building these gates into CI/CD — and securing the build pipeline itself — is what the SolarWinds attack proved non-negotiable: attackers compromised the build process to plant a backdoor in trusted, signed software, turning the supply chain into the weapon.",
      ],
      technical: {
...
// factory. Each lab is grounded in the stage's real-world incident. Flags
// mirrored in stage-flags.ts; extraCommands lazy-loaded via LOADERS.
const SECF_CTF: Record<string, CtfConfig> = {
  "sec-foundations-01": mkDeepCtf(
    "An auditor dumps a pile of raw security controls. Classify each by category × type, map them to the NIST CSF, and prove a critical function has no coverage.",
    "OP: CONTROL MAPPING\nTarget: an unmapped pile of security controls.\nGoal: classify, map to NIST CSF, find the gap.\nSequence: audit-controls -> map-csf -> close-gap",
    "FLAG{NIST_CSF_",
    "Mission Brief",
    ["audit-controls", "C0NTR0L_", "Controls Classified", [
      "$ audit-controls controls.csv",
      "Tagged 47 controls by category (technical/managerial/operational/physical) x type (preventive/detective/...).",
      "12 are detective, 9 corrective, only 3 cover Recover.",
      "Next: map-csf",
    ]],
    ["map-csf", "G4P_", "Mapped to CSF", [
      "$ map-csf --framework csf-2.0",
      "Identify ✓  Protect ✓  Detect ✓  Respond ✓  Recover ✗ (0 effective controls)",
      "The RECOVER function is unstaffed — a single ransomware event would be unrecoverable.",
      "Next: close-gap",
    ]],
    ["close-gap", "M4PP3D}", "Gap Closed", [
      "$ close-gap --function recover",
      "Added: immutable backups, tested restore runbook, BC/DR plan. Recover now covered.",
      "Frameworks turn a pile of controls into a measurable map of coverage.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Classify the controls. Run: audit-controls", "Map to NIST CSF. Run: map-csf", "Close the gap. Run: close-gap", "Run 'assemble', then submit the flag"],
    { "controls.csv": "id,control,category,type\n1,firewall,technical,preventive\n2,SIEM,technical,detective\n3,backups,technical,corrective\n... (47 controls; Recover sparse)" },
  ),
  "sec-foundations-02": mkDeepCtf(
    "An air-gapped plant fears a Stuxnet-style intruder. Deploy a honeypot inside the OT segment, watch it get touched, and trace the intruder's pivot.",
    "OP: DECEPTION GRID\nTarget: an OT segment that should never see traffic.\nGoal: deploy a honeypot, catch and trace the intruder.\nSequence: deploy-honeypot -> watch-trap -> trace-intruder",
    "FLAG{H0N3YP0T_",
    "Mission Brief",
    ["deploy-honeypot", "TR4P_", "Honeypot Deployed", [
      "$ deploy-honeypot --emulate plc --segment ot-vlan9",
      "Stood up a decoy PLC with realistic Modbus banners and fake ladder logic.",
      "No legitimate device should ever talk to it — any contact is hostile.",
      "Next: watch-trap",
    ]],
    ["watch-trap", "1NTRUD3R_", "Trap Sprung", [
      "$ watch-trap --follow",
      "10:42 unknown host enumerated the decoy; 10:44 wrote to a fake coil register.",
      "The intruder is already inside the air gap — exactly the Stuxnet pattern.",
      "Next: trace-intruder",
    ]],
    ["trace-intruder", "TR4C3D}", "Intruder Traced", [
      "$ trace-intruder --pivot-path",
      "Source = an infected engineering laptop bridged via USB, then onto the OT VLAN.",
      "Deception caught what perimeter tools missed. Isolate the laptop, kill the bridge.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Deploy the honeypot. Run: deploy-honeypot", "Watch the trap. Run: watch-trap", "Trace the intruder. Run: trace-intruder", "Run 'assemble', then submit the flag"],
    { "honeypot.log": "10:42:11 SYN from 10.9.0.66 -> decoy:502\n10:42:13 modbus read_coils\n10:44:02 modbus write_single_coil  <-- hostile" },
  ),
  "sec-foundations-03": mkDeepCtf(
    "A vendor pushes an untested kernel driver update fleet-wide — the CrowdStrike scenario. Catch it at the change gate, prove the risk, and execute the backout before it grounds the business.",
    "OP: CHANGE CONTROL\nTarget: an emergency change with no test or backout.\nGoal: review, detect the risk, execute backout.\nSequence: review-change -> detect-risk -> execute-backout",
    "FLAG{CH4NG3_",
    "Mission Brief",
    ["review-change", "R1SK_", "Change Reviewed", [
      "$ review-change CR-2024-0719",
      "scope=ALL endpoints  testing=none  staged_rollout=no  backout_plan=missing",
      "An emergency tag was used to skip the CAB. Every red flag is present.",
      "Next: detect-risk",
    ]],
    ["detect-risk", "B4CK0UT_", "Risk Detected", [
      "$ detect-risk --simulate",
      "Driver triggers a boot loop on the target OS version -> 8.5M hosts would BSOD.",
      "Impact analysis would have caught this before deployment.",
      "Next: execute-backout",
    ]],
    ["execute-backout", "R3ST0R3D}", "Backout Executed", [
      "$ execute-backout --to last-known-good",
      "Rolled the channel file back; canary ring healthy; full deploy halted.",
      "Change management exists for exactly this. Approve, test, stage, and always have a backout.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Review the change. Run: review-change", "Detect the risk. Run: detect-risk", "Execute the backout. Run: execute-backout", "Run 'assemble', then submit the flag"],
    { "change-request.txt": "CR-2024-0719\ntype: emergency\ntest: SKIPPED\nstaged: no\nbackout: (none)\nrequested_by: vendor-auto" },
  ),
  "sec-foundations-04": mkDeepCtf(
    "A data center burns down — the OVHcloud fire. Drive recovery off your RTO/RPO: fail over to the recovery site and restore from a 3-2-1 backup, then prove the targets were met.",
    "OP: DISASTER RECOVERY\nTarget: a primary site lost to fire.\nGoal: fail over and restore within RTO/RPO.\nSequence: assess-outage -> failover-site -> restore-backup",
    "FLAG{RT0_RP0_",
    "Mission Brief",
    ["assess-outage", "F41L0V3R_", "Outage Assessed", [
      "$ assess-outage",
      "Primary SBG2 = total loss. RTO target=4h, RPO target=1h.",
      "Some teams kept backups only in the same building — those are gone (3-2-1 violated).",
      "Next: failover-site",
    ]],
    ["failover-site", "B4CKUP_", "Failed Over", [
      "$ failover-site --to dr-region",
      "DNS + traffic shifted to the warm DR site in a different region; services up in 2h11m.",
      "RTO met. Now recover the data to the agreed point.",
      "Next: restore-backup",
    ]],
    ["restore-backup", "R3C0V3R3D}", "Restored", [
      "$ restore-backup --from offsite --point -52m",
      "Restored from the offsite copy (3 copies, 2 media, 1 offsite). Data loss = 52m < RPO.",
      "RTO and RPO both met because the targets drove the design.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Assess the outage. Run: assess-outage", "Fail over. Run: failover-site", "Restore the backup. Run: restore-backup", "Run 'assemble', then submit the flag"],
    { "bcdr-plan.txt": "RTO: 4h\nRPO: 1h\nbackups: 3-2-1 (3 copies / 2 media / 1 offsite)\nDR: warm site, separate region" },
  ),
  "sec-foundations-05": mkDeepCtf(
    "A rogue certificate authority issues a fraudulent *.google.com cert — the DigiNotar breach. Inspect the cert, catch it in Certificate Transparency logs, and revoke it.",
    "OP: ROGUE CERT\nTarget: a fraudulently issued TLS certificate.\nGoal: inspect, confirm via CT logs, revoke.\nSequence: inspect-cert -> check-ct-log -> revoke-cert",
    "FLAG{R0GU3_C4_",
    "Mission Brief",
    ["inspect-cert", "CT_", "Cert Inspected", [
      "$ inspect-cert suspicious.pem",
      "CN=*.google.com  issuer=DigiNotar  notBefore=recent  key=2048",
      "Google's certs don't come from this CA — likely fraudulent issuance.",
      "Next: check-ct-log",
    ]],
    ["check-ct-log", "L0G_", "CT Log Checked", [
      "$ check-ct-log --sct suspicious.pem",
      "Found in public CT logs but absent from Google's known-good set -> mis-issuance confirmed.",
      "Certificate Transparency makes hidden rogue certs impossible to hide.",
      "Next: revoke-cert",
    ]],
    ["revoke-cert", "R3V0K3D}", "Cert Revoked", [
      "$ revoke-cert --ocsp --distrust-ca",
      "Cert revoked via OCSP/CRL; the issuing CA distrusted in the root program.",
      "Trust is a chain — one bad CA breaks it, so PKI hygiene and CT are non-negotiable.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Inspect the certificate. Run: inspect-cert", "Check CT logs. Run: check-ct-log", "Revoke the certificate. Run: revoke-cert", "Run 'assemble', then submit the flag"],
    { "suspicious.pem": "Subject: CN=*.google.com\nIssuer: CN=DigiNotar Public CA\nSerial: 05:e2:e6:...\nSAN: *.google.com" },
  ),
  "sec-foundations-06": mkDeepCtf(
    "One leaked password on a VPN with no MFA took down a pipeline — Colonial Pipeline. Enumerate accounts, find the one missing MFA, and enforce phishing-resistant auth.",
    "OP: IDENTITY HARDENING\nTarget: a remote-access gateway with weak auth.\nGoal: find the unprotected account, enforce MFA.\nSequence: enum-accounts -> find-no-mfa -> enforce-mfa",
    "FLAG{MFA_",
    "Mission Brief",
    ["enum-accounts", "N0_", "Accounts Enumerated", [
      "$ enum-accounts --gateway vpn",
      "318 accounts; 1 is a legacy/disabled-but-still-valid VPN login with a reused password.",
      "That single account is internet-reachable.",
      "Next: find-no-mfa",
    ]],
    ["find-no-mfa", "R3US3_", "Weak Account Found", [
      "$ find-no-mfa",
      "account 'svc-vpn-old' -> MFA: DISABLED, password found in a public breach corpus.",
      "Exactly the Colonial Pipeline entry point: valid creds, no second factor.",
      "Next: enforce-mfa",
    ]],
    ["enforce-mfa", "3NF0RC3D}", "MFA Enforced", [
      "$ enforce-mfa --all --method fido2",
      "Disabled the stale account; enforced FIDO2/passkeys org-wide; blocked breached passwords.",
      "MFA — especially phishing-resistant MFA — stops credential-only intrusions cold.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Enumerate accounts. Run: enum-accounts", "Find the account without MFA. Run: find-no-mfa", "Enforce MFA. Run: enforce-mfa", "Run 'assemble', then submit the flag"],
    { "accounts.txt": "alice  mfa=fido2\nbob    mfa=totp\nsvc-vpn-old  mfa=DISABLED  pw=reused/breached" },
  ),
  "sec-foundations-07": mkDeepCtf(
    "A GDPR audit demands you know your data. Scan the estate, classify the personal data, and prove sensitive records are protected in every state — including at rest.",
    "OP: DATA PROTECTION\nTarget: an estate with unclassified personal data.\nGoal: classify PII and protect it by state.\nSequence: scan-data -> classify-pii -> encrypt-state",
    "FLAG{P11_",
    "Mission Brief",
    ["scan-data", "CL4SS1F13D_", "Data Scanned", [
      "$ scan-data --discover",
      "Found PII across 3 states: in use (app memory), in transit (TLS ✓), at rest (DB + S3).",
      "An unencrypted analytics export of customer records sits in object storage.",
      "Next: classify-pii",
    ]],
    ["classify-pii", "4T_R3ST_", "PII Classified", [
      "$ classify-pii",
      "Tagged: names, emails, national IDs -> 'restricted' (special-category under GDPR).",
      "The at-rest export is the highest-risk asset and currently plaintext.",
      "Next: encrypt-state",
    ]],
    ["encrypt-state", "3NCRYPT3D}", "Protected", [
      "$ encrypt-state --at-rest --kms --tokenize-ids",
      "Enabled envelope encryption + tokenized national IDs; transit already TLS; access logged.",
      "Protect data in every state — at rest is the one teams forget.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Scan the data. Run: scan-data", "Classify the PII. Run: classify-pii", "Encrypt at rest. Run: encrypt-state", "Run 'assemble', then submit the flag"],
    { "data-map.csv": "store,state,contains,encryption\napp-mem,in-use,PII,n/a\napi,in-transit,PII,TLS\ns3-export,at-rest,PII,NONE" },
  ),
  "sec-foundations-08": mkDeepCtf(
    "Attackers vished their way into Twitter's admin tools in 2020. Run a phishing simulation, measure who clicks, and harden the human firewall with targeted training.",
    "OP: HUMAN FIREWALL\nTarget: a workforce vulnerable to social engineering.\nGoal: simulate phishing, measure, then train.\nSequence: launch-phish-sim -> measure-clicks -> harden-humans",
    "FLAG{V1SH1NG_",
    "Mission Brief",
    ["launch-phish-sim", "S1M_", "Simulation Launched", [
      "$ launch-phish-sim --pretext it-helpdesk-call --targets all",
      "Sent a benign lookalike 'reset your VPN' lure + a vishing call script to 2,000 staff.",
      "Authorized, tracked, and safe — this is a drill, not an attack.",
      "Next: measure-clicks",
    ]],
    ["measure-clicks", "HUM4N_", "Behavior Measured", [
      "$ measure-clicks",
      "click_rate=18%  credential_submit=6%  reported_to_soc=11%  (privileged users over-represented)",
      "The 2020 Twitter breach started exactly here — staff with admin tools, social-engineered.",
      "Next: harden-humans",
    ]],
    ["harden-humans", "F1R3W4LL}", "Workforce Hardened", [
      "$ harden-humans --train clickers --phish-resistant-mfa admins",
      "Targeted training for clickers; report-phish button promoted; FIDO2 for privileged tools.",
      "People are a control too — measure and train them like one.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Launch the sim. Run: launch-phish-sim", "Measure clicks. Run: measure-clicks", "Harden the humans. Run: harden-humans", "Run 'assemble', then submit the flag"],
    { "sim-results.txt": "targets: 2000\nclicked: 360 (18%)\nsubmitted_creds: 120 (6%)\nreported: 220 (11%)" },
  ),
  "sec-foundations-09": mkDeepCtf(
    "A flapping link is melting a trading floor. Test the media, map the topology, and fix the physical layer — copper limits, fiber, and a resilient spine-leaf design.",
    "OP: PHYSICAL LAYER\nTarget: an unreliable network backbone.\nGoal: test media, map topology, fix the link.\nSequence: test-media -> map-topology -> fix-link",
    "FLAG{F1B3R_",
    "Mission Brief",
    ["test-media", "SP1N3_", "Media Tested", [
      "$ test-media --certify",
      "A 140m Cat6 copper run exceeds the 100m Ethernet limit -> intermittent errors + CRC.",
      "Long/EMI-heavy runs need fiber, not copper.",
      "Next: map-topology",
    ]],
    ["map-topology", "L34F_", "Topology Mapped", [
      "$ map-topology",
      "Core is a single daisy-chained switch (one cable = whole-floor outage). No redundancy.",
      "A spine-leaf fabric gives every leaf multiple paths.",
      "Next: fix-link",
    ]],
    ["fix-link", "L1NK3D}", "Link Fixed", [
      "$ fix-link --media fiber --topology spine-leaf",
      "Replaced the over-length run with single-mode fiber; rebuilt core as redundant spine-leaf.",
      "Errors gone; any single link can fail without an outage.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Test the media. Run: test-media", "Map the topology. Run: map-topology", "Fix the link. Run: fix-link", "Run 'assemble', then submit the flag"],
    { "link-test.txt": "run A: Cat6 140m -> FAIL (exceeds 100m)\nrun B: OM4 fiber 280m -> PASS\ncore: single switch, daisy-chained" },
  ),
  "sec-foundations-10": mkDeepCtf(
    "Users report 'the internet is down.' Resist the urge to guess: follow the structured troubleshooting methodology, capture the traffic, and isolate the real fault.",
    "OP: METHODICAL TRIAGE\nTarget: an intermittent connectivity fault.\nGoal: follow the 7-step method, capture, isolate.\nSequence: identify-problem -> capture-traffic -> isolate-fault",
    "FLAG{S3V3N_ST3P_",
    "Mission Brief",
    ["identify-problem", "C4PTUR3_", "Problem Identified", [
      "$ identify-problem",
      "Step 1-2: gathered symptoms, established what changed (a new DHCP scope last night).",
      "Theory: address exhaustion or a rogue responder — verify, don't assume.",
      "Next: capture-traffic",
    ]],
    ["capture-traffic", "F4ULT_", "Traffic Captured", [
      "$ capture-traffic --filter 'bootp || dns'",
      "Wireshark shows two DHCP servers answering -> a rogue DHCP server hands out a bad gateway.",
      "Evidence beats guessing.",
      "Next: isolate-fault",
    ]],
    ["isolate-fault", "F0UND}", "Fault Isolated", [
      "$ isolate-fault",
      "Traced the rogue responder to a mis-configured lab switch; enabled DHCP snooping to block it.",
      "Step 6-7: verified full functionality and documented the fix.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Identify the problem. Run: identify-problem", "Capture traffic. Run: capture-traffic", "Isolate the fault. Run: isolate-fault", "Run 'assemble', then submit the flag"],
    { "capture.txt": "DHCP OFFER from 10.0.0.1 (legit)\nDHCP OFFER from 10.0.5.66 (rogue) gw=10.0.5.66" },
  ),
  "sec-foundations-11": mkDeepCtf(
    "A breach must not be buried — the Uber cover-up. Gather the evidence, score severity, and file a report that escalates to the people who must act (and the regulator).",
    "OP: REPORT IT RIGHT\nTarget: a confirmed security incident.\nGoal: evidence -> severity -> proper report.\nSequence: gather-evidence -> score-severity -> report-incident",
    "FLAG{1NC1D3NT_",
    "Mission Brief",
    ["gather-evidence", "S3V3R1TY_", "Evidence Gathered", [
      "$ gather-evidence --timeline",
      "Confirmed data exfil of customer records; preserved logs, scoped affected systems.",
      "Facts first — who, what, when, how much.",
      "Next: score-severity",
    ]],
    ["score-severity", "R3P0RT3D_", "Severity Scored", [
      "$ score-severity",
      "Impact=high (PII exfil) x likelihood=confirmed -> SEV-1; regulatory notification triggered.",
      "Severity drives who is told and how fast.",
      "Next: report-incident",
    ]],
    ["report-incident", "N0_C0V3RUP}", "Incident Reported", [
      "$ report-incident --to execs,legal,regulator",
      "Filed factual report up the chain + regulatory disclosure within the legal window.",
      "Uber's mistake was hiding it. Reporting that drives action is the whole point.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Gather evidence. Run: gather-evidence", "Score severity. Run: score-severity", "Report the incident. Run: report-incident", "Run 'assemble', then submit the flag"],
    { "incident.txt": "type: data exfiltration\nrecords: 57M (PII)\nstatus: confirmed\nseverity: SEV-1\nnotify: legal, regulator" },
  ),
  "sec-foundations-12": mkDeepCtf(
    "Equifax left a known flaw untreated and paid for it. Put numbers on the risk: inventory the asset, compute SLE/ARO/ALE, and choose a treatment that the math justifies.",
    "OP: QUANTIFY RISK\nTarget: a known but untreated vulnerability.\nGoal: inventory, compute ALE, treat the risk.\nSequence: inventory-asset -> compute-ale -> treat-risk",
    "FLAG{SL3_4R0_",
    "Mission Brief",
    ["inventory-asset", "4L3_", "Asset Valued", [
      "$ inventory-asset",
      "Asset = customer PII database. Asset value (AV) = $700M exposure.",
      "An unpatched Apache Struts CVE is exploitable from the internet.",
      "Next: compute-ale",
    ]],
    ["compute-ale", "TR34T_", "ALE Computed", [
      "$ compute-ale",
      "EF=0.5 -> SLE = AV x EF = $350M.  ARO = 0.4/yr.  ALE = SLE x ARO = $140M/yr.",
      "The annualized loss dwarfs the cost of patching.",
      "Next: treat-risk",
    ]],
    ["treat-risk", "M1T1G4T3D}", "Risk Treated", [
      "$ treat-risk --choose mitigate",
      "Of avoid/transfer/accept/mitigate, mitigation (patch + WAF) is cheapest vs $140M ALE.",
      "Equifax 'accepted' it by inaction. Quantified risk makes the right call obvious.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Inventory the asset. Run: inventory-asset", "Compute the ALE. Run: compute-ale", "Treat the risk. Run: treat-risk", "Run 'assemble', then submit the flag"],
    { "risk.txt": "asset_value: 700000000\nexposure_factor: 0.5\nSLE: 350000000\nARO: 0.4\nALE: 140000000" },
  ),
  "sec-foundations-13": mkDeepCtf(
    "SolarWinds attackers compromised the build, not the code. Scan the SDLC pipeline, detect the implant injected at compile time, and lock the pipeline down with signed, verified builds.",
    "OP: SECURE THE BUILD\nTarget: a software build pipeline.\nGoal: scan, detect the implant, sign the build.\nSequence: scan-pipeline -> detect-implant -> sign-build",
    "FLAG{SDLC_",
    "Mission Brief",
    ["scan-pipeline", "1MPL4NT_", "Pipeline Scanned", [
      "$ scan-pipeline",
      "Source is clean, but the CI build step pulls a tampered build tool from an unpinned mirror.",
      "The compromise is in the build, not the repo — the SolarWinds pattern.",
      "Next: detect-implant",
    ]],
    ["detect-implant", "S1GN3D_", "Implant Detected", [
      "$ detect-implant --diff-artifacts",
      "Compiled binary contains code with no source counterpart (SUNBURST-style injected backdoor).",
      "Reproducible builds expose exactly this divergence.",
      "Next: sign-build",
    ]],
    ["sign-build", "S3CUR3}", "Build Secured", [
      "$ sign-build --hermetic --pin-deps --sign sigstore --slsa 3",
      "Hermetic build, pinned deps, signed + attested artifacts (SLSA). Tampered builds now rejected.",
      "Build security into every SDLC phase — especially the pipeline itself.",
      "Run 'assemble', then submit the flag.",
    ]],
    ["Read the briefing. Run: cat briefing.txt", "Scan the pipeline. Run: scan-pipeline", "Detect the implant. Run: detect-implant", "Sign the build. Run: sign-build", "Run 'assemble', then submit the flag"],
    { "pipeline.yaml": "source: clean (signed commits)\nbuild_tool: from unpinned mirror  # <-- risk\nartifact_signing: none\nslsa_level: 0" },
  ),
};

for (const s of secFoundationsStages) {
  const ctf = SECF_CTF[s.id];
