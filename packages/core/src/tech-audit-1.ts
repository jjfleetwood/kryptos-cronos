...
export const techAudit1Stages: StageConfig[] = [
  // ─── audit-01: COBIT 2019 ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "COBIT 2019 is ISACA's flagship framework for IT governance and management, and it underpins almost every IT audit methodology used in regulated industries. Its central move is to draw a precise — and legally significant — line between governance, which sets direction, evaluates options, and monitors outcomes, and management, which plans, builds, runs, and monitors in line with that direction. The distinction isn't academic: regulators and courts have assigned personal liability to board members who abdicated governance and relied entirely on management self-reporting.",
        "Every audit starts by establishing that boundary and then tracing how intent flows through it. The board and executives govern — approving risk appetite, setting strategy, authorizing major IT investments, and ensuring accountability — while IT leadership manages, implementing controls, operating systems, and reporting performance upward within board-approved parameters. The auditor holds an independent third position, verifying both layers and reporting to the audit committee, not management, so that independence is preserved. Intent travels down a governance cascade in which board principles become management policies, those become operational procedures, and those become technical controls; the auditor traces the cascade in reverse, because a control that can't be traced up to policy authority, or a policy that was never implemented as a control, is a governance gap.",
        "The framework organizes forty governance and management objectives across five domains, which it expects the auditor to tailor using a set of design factors. EDM (Evaluate, Direct, Monitor) is the board-governance domain, covering risk appetite, IT investment, and performance monitoring. The four management domains follow the IT lifecycle. APO (Align, Plan, Organise) handles strategy, architecture, risk, and HR planning. BAI (Build, Acquire, Implement) covers projects, change, and requirements. DSS (Deliver, Service, Support) runs operations, incidents, and continuity. MEA (Monitor, Evaluate, Assess) addresses internal controls, compliance, and board reporting. Twelve design factors — enterprise strategy, risk profile, compliance requirements, sourcing model, IT methods, and more — tailor the framework to context, so a regulated bank running its own data center carries a different target profile than a cloud-native startup, and the auditor establishes the applicable factors before assessing capability.",
        "Two practical points dominate real engagements. The first is that COBIT is a governance framework rather than a technical standard — it prescribes no implementations but maps to NIST CSF, ISO 27001, ITIL, and TOGAF, which is why auditors routinely find strong NIST- or ISO-aligned technical controls that are never connected to board-level accountability. The second is that the most common findings cluster in the EDM domain — a missing formal governance charter (EDM01), no approved risk-appetite statement so that risk decisions are made ad hoc without board authority (EDM03), and board reporting that is either absent or reduced to technical metrics the board can't evaluate (EDM05) — together a pervasive governance failure that undermines every other control.",
      ],
      technical: {
...
APO12 Risk Mgmt      2        4      2    HIGH`,
  ...
  // ─── audit-02: Risk Assessment — CRISC ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "CRISC (Certified in Risk and Information Systems Control) is ISACA's risk-focused certification and methodology, built on a four-domain approach that runs from IT Risk Identification through IT Risk Assessment, Risk Response and Mitigation, and finally Risk and Control Monitoring and Reporting. Where compliance frameworks ask whether the required control was implemented, CRISC asks the prior question: what risks are the controls meant to address, and are they actually reducing those risks to acceptable levels? That reframing — from control checklists to risk outcomes — is what makes the methodology distinct.",
        "The first two domains define what an organization must manage and how exposed it is. Risk identification is the most under-resourced domain, because a register populated in a single workshop years ago goes stale; CRISC therefore requires continuous identification as new systems, vendors, regulations, and threats appear, and auditors test it by comparing the register against change logs, vendor onboardings, threat intel, and incident databases for risks that should be present but aren't. Risk assessment then runs a standard method — identify the threats and vulnerabilities, score the inherent risk before controls, evaluate control effectiveness, calculate the residual risk after controls, and compare that residual to the board-approved appetite. The gap between inherent and residual risk is the measure of control effectiveness, and it must be large enough to bring residual risk within appetite.",
        "Two artifacts anchor the rest of the work. The first is the set of four risk responses. To Accept a risk, the board formally documents and absorbs the potential loss, and undocumented acceptance is invalid. To Mitigate is to add or strengthen controls. To Transfer is to shift financial exposure through insurance, indemnification, or outsourcing without eliminating the underlying risk. To Avoid is to discontinue the activity entirely — the most complete response, and often the most impractical. The second artifact is the risk register, which auditors scrutinize most closely. A mature register carries, for each risk, a unique ID, a threat and vulnerability narrative, the affected asset, a named risk owner, an inherent score (likelihood × impact), the controls in place with references, a control-effectiveness rating, a residual score, a response type and treatment plan, the appetite threshold, and a last-review date; any missing field is itself a finding, because without it the organization can't demonstrate it is managing the risk.",
        "Two things determine whether the program is real. Risk appetite must be board-approved and quantitative: a statement like 'we have a low appetite for cybersecurity' gives no usable threshold, whereas 'no residual score above 12 on the 5×5 matrix without a board-approved exception, and scores of 16 or higher escalate immediately to the Audit Committee' can actually be tested. The most consequential finding is residual risk above appetite with no treatment plan, because the organization knows it is over its own threshold and is doing nothing about it; regulators treat that as management negligence, with the FDIC and OCC issuing Matters Requiring Attention and external auditors potentially calling it a significant deficiency or material weakness under SOX 404.",
      ],
      technical: {
...
R-004    Ransomware via RDP        4       5      20       None       20     CRITICAL — NO CONTROLS`,
  ...
  // ─── audit-03: ITGC ────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2002,
      overview: [
        "IT General Controls (ITGCs) are the foundational controls that sit beneath all application-level controls; they are not tied to any one application but apply across the whole IT environment. The useful analogy is a building's foundation and floors — a sound floor resting on a cracked foundation is still unsafe — so if ITGCs fail, every application control built on them becomes suspect, and the auditor can't rely on application output without far more compensating work. ITGCs fall into four domains. Change Management ensures production changes follow an approved, documented process. Logical Access ensures only authorized users reach systems and data at an appropriate privilege level. Computer Operations covers reliable running, backups that are performed and tested, and jobs that complete. Program Development ensures new systems are developed, tested, and approved through a defined SDLC. All four appear in every SOX 404 audit, PCAOB inspection, and financial-services exam.",
        "ITGC effectiveness governs how much an auditor can trust everything else. A clean ITGC environment lets auditors rely on system-generated reports and automated application controls with minimal extra testing, whereas a failure — especially in change management or logical access — forces a substantial expansion of work, because the auditor can no longer assume that system output reflects what the system should produce. Findings are then classified by pervasiveness. An isolated deficiency, such as a single unauthorized payroll change, is serious in itself, but a pervasive one, such as a change process that routinely bypasses approval, casts doubt across systems and periods and typically rises to a significant deficiency or material weakness under SOX that must be disclosed.",
        "Two structural facts shape every engagement. SOX Section 404 requires management to assess and attest to the effectiveness of internal control over financial reporting, including the supporting ITGCs, while PCAOB AS 2201 requires external auditors to verify that assessment — a two-layer accountability in which the CEO and CFO personally sign the Section 302 certification, so ITGC failures create personal legal exposure. Scoping then identifies which systems are 'in scope'. A system is in scope if it processes, stores, or transmits data that flows into the financial statements — the general and subsidiary ledgers, payroll, revenue recognition, and consolidation tools — together with their supporting operating systems, databases, networks, and identity platforms, and, in the cloud, the platform configuration and shared-responsibility controls.",
        "A recurring set of findings appears across industries, each one a breakdown that could allow unauthorized changes or access to financial data. On the logical-access side, auditors find terminated employees with active accounts and shared privileged accounts that carry no individual accountability. On the change-management side, they find developers with production access — a segregation-of-duties failure — and production changes pushed with no approved tickets. And in operations and development, they find backups never tested for restorability and new systems deployed without any security-testing evidence.",
      ],
      technical: {
...
# Output: jsmith terminated 2026-03-15 — still has ACTIVE AD account — FINDING`,
  ...
  // ─── audit-04: Access Control ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "Access control auditing verifies that least privilege holds across the environment, so that every user — whether an employee, a contractor, or a service account — has exactly the permissions their documented job requires and no more. The principle sounds simple but is hard to maintain, because systems steadily accumulate users, people change roles, contractors finish engagements, and business units reorganize. Through all of it, access granted for a former purpose persists unless an active process removes it, and that accumulation — permission creep — is exactly what access audits exist to find and remediate.",
        "The control objectives come from COBIT APO13 and DSS05, aligned with the NIST SP 800-53 Access Control (AC) family, and they translate into four concrete expectations. Provisioning must follow an approved request carrying a documented business justification. Access must be reviewed and recertified at least annually, with managers confirming that each person's access is still appropriate. Terminated employees must be deprovisioned within 24 hours for standard accounts and almost immediately — within roughly four hours — for privileged ones. And privileged access must be minimized, documented with justification, protected with MFA, and subject to enhanced monitoring.",
        "Access findings are the most common in all of IT audit, and the most security-critical of them concern privileged access. They range from the administrative, such as missing recertification evidence, to the critical, such as terminated employees who still hold active privileged accounts, and they reflect a coordination problem spanning HR, IT, and business management — which is why auditors test the process rather than just the current list, since a clean list produced by a broken process will simply fail again. Privileged Access Management (PAM) governs the accounts that bypass most application controls, including domain and database administrators, root, elevated service accounts, and broad cloud IAM roles. Its controls include just-in-time access that expires automatically, dedicated privileged access workstations (PAWs), session recording, and regular review of privileged rights against actual job requirements.",
        "Three concepts complete the picture. Role-based access control (RBAC) attaches rights to roles rather than to individuals, so assigning the 'Accounts Payable Clerk' role configures access automatically and simplifies provisioning, deprovisioning, and recertification; auditors verify that role definitions are approved, that assignments match job descriptions, and that no user holds conflicting roles. Segregation-of-duties (SoD) conflicts arise when one user spans incompatible functions — creating a vendor and approving payments, initiating and approving wire transfers, or posting and approving journal entries — which creates a fraud opportunity, so auditors test access against a conflict matrix. Service accounts, the non-human identities that run applications and scheduled jobs, often hold elevated, shared, embedded credentials that no human reviews daily, so auditors inventory them and confirm a documented owner, scheduled credential rotation, disabled interactive logon where it isn't needed, and tightly limited permissions.",
      ],
      technical: {
...
-- [term]     WRITE    2025-03-01   [TERM]       ← TERMINATED USER - CRITICAL`,
  ...
  // ─── audit-05: Change Management ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1986,
      overview: [
        "Change management ensures that every modification to production passes through a documented, approved, and tested process before it ships. It isn't bureaucracy; it is the control that prevents accidental outages, unauthorized code deployment, and security vulnerabilities from entering production. ITIL 4 sorts changes into three types by risk. Standard changes are pre-approved, low-risk, and routine. Normal changes require Change Advisory Board review. Emergency changes get expedited approval for critical fixes but carry mandatory post-hoc documentation.",
        "The Change Advisory Board (CAB) reviews and approves Normal changes, and auditors test it against what it should enforce. A CAB is cross-functional, drawing in IT management, security, operations, application owners, and business stakeholders to assess risk, confirm that testing is complete, verify an executable rollback plan and an appropriate maintenance window, and formally authorize the change — an effective board challenges assumptions and rejects unacceptable risk rather than rubber-stamping. For each sampled change, auditors verify five elements. The ticket must be approved and opened before the change rather than backdated. The CAB approval must be documented with a name and date. The developer and the approver must be different people, preserving segregation of duties. Test results must be attached from a non-production environment. And a documented, feasible rollback plan must exist.",
        "Two process safeguards are common failure points. The emergency path is a necessary safety valve that is frequently abused — genuine emergencies such as an actively exploited zero-day or a production outage justify it, but an emergency rate above 10 to 15 percent should trigger an investigation into whether developers, or rubber-stamping managers, are routing around CAB review under the guise of urgency. Maintenance windows are the approved low-traffic deployment periods, typically overnight or on weekends, when failure impact is minimized and staff are on hand to watch; a change deployed outside the window without emergency authorization is a finding regardless of its technical content, and auditors flag it by comparing deployment timestamps against the window schedule.",
        "Two modern concerns close the loop. DevOps and CI/CD strain the human-review model, so ITIL 4 introduces the 'change authority,' which can be an automated system for low-risk, well-tested changes; here auditors test whether the pipeline itself is a sufficient control by checking for enforced automated testing, required passing security scans, an immutable deployment audit log, and a halt-on-failure mechanism, because a well-configured pipeline can be more reliable than a manual CAB but only if its own configuration is audited. Post-implementation review (PIR) then documents whether the change met its purpose, along with any side effects, lessons, and follow-ups; auditors verify that PIRs were completed on time, typically within five business days, and that issues were tracked to resolution, and PIRs that consistently surface unexpected side effects reveal a deeper problem — test environments that don't represent production.",
      ],
      technical: {
...
-- CHG-1003  jsmith    jsmith      2026-02-05  SOD VIOLATION  ← FINDING`,
  ...
  // ─── audit-06: BCP/DR ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2005,
      overview: [
        "Business Continuity Planning (BCP) and Disaster Recovery (DR) audits verify that an organization can keep critical operations running and recover its IT systems after a disruption, whether that disruption is a natural disaster, ransomware, a data center fire, or a vendor failure. The CISA body of knowledge dedicates substantial coverage to this area because these plans are systematically neglected — created during a compliance exercise, filed away in a shared drive, and never exercised. An untested recovery plan isn't a control; it is documentation of intent, and intent has never restored a database.",
        "Two foundations define what recovery means and where it applies. The first is the set of metrics. The Recovery Time Objective (RTO) is the maximum tolerable downtime before the consequence becomes unacceptable; the Recovery Point Objective (RPO) is the maximum acceptable data loss, measured in time; and the Maximum Tolerable Downtime (MTD) is the point of irreversible harm. The fundamental rule is that RTO must be less than MTD, or the plan literally can't save the business. The second foundation is the Business Impact Analysis (BIA), which auditors review first, because it maps each critical process, its supporting systems, the impact of downtime, and the resulting RTO and RPO; a BIA that hasn't been updated since a major migration, cloud adoption, or acquisition is stale, so auditors test its currency by comparing its systems against the current production inventory.",
        "Strategy and testing must demonstrably meet the approved targets. Recovery strategies range from hot standby — a fully operational duplicate with near-zero RTO and RPO but a high cost — through warm and cold standby, where cold is cheap but yields RTOs measured in days, to on-demand cloud failover, and auditors verify that the chosen strategy has actually demonstrated the approved RTO and RPO in testing rather than only on paper. Both tabletop and full DR tests are required. A tabletop is a discussion-based walkthrough that tests logical structure, roles, and decisions without disruption, while a full DR test actually fails over to the recovery environment and verifies RTOs and RPOs in practice; testing the document alone misses the technology, and testing the technology alone misses the human coordination.",
        "Two modern requirements reshape the audit. Ransomware broke the old assumption that backups simply survive, because an attack now encrypts production systems, the backups, cloud-connected repositories, and anything else reachable from the compromised network. Audits therefore test for air-gapped or immutable backups, adherence to the 3-2-1 rule of three copies on two media types with one kept offsite, a tested restoration from an isolated backup that was disconnected before the attack, and a ransomware-specific tabletop that includes backup compromise. Geographic separation is equally non-negotiable, as Hurricane Katrina showed. Primary and backup sites must lie outside the same flood zone and far enough apart to survive a regional disaster, with FDIC and OCC guidance generally requiring roughly 100 miles, and auditors check the location documentation for shared dependencies such as a common power-grid segment, ISP backbone, or seismic fault.",
      ],
      technical: {
...
- Customer Portal: not tested in 18 months — CRITICAL FINDING`,
  ...
  // ─── audit-07: Incident Response ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Incident Response (IR) program audits assess whether an organization can detect, contain, eradicate, and recover from security incidents consistently, at scale, and under the time pressure and confusion that real incidents create. ISACA auditors use NIST SP 800-61 as the framework and evaluate all four of its phases — Preparation, Detection and Analysis, Containment/Eradication/Recovery, and Post-Incident Activity — because a program that detects well but can't contain, or contains but learns nothing, isn't mature. Preparation is the most auditable phase and the one most often found deficient, and a sound preparation posture means a current IR plan reviewed within twelve months, named individuals rather than job titles assigned to roles, contact lists that reach external parties such as legal, law enforcement, forensics, the cyber insurer, and PR, a severity matrix that distinguishes a P1 from a P4, notification protocols tied to each severity, and tabletop exercises held within twelve months with action items tracked to closure.",
        "Two forces shape the program beyond preparation. The first is Detection and Analysis, where auditors review the SIEM for log-source coverage of critical systems, for alert rules tuned to the top threats, and for managed false-positive rates — a SIEM firing thousands of alerts a day trains analysts to ignore them and is worse than none — then sample historical incidents to see how each was actually detected, since detection that comes from an external notification by law enforcement or a third party signals that internal detection is insufficient. The second force is the SEC's 2023 disclosure rules, under which material incidents must be disclosed within four business days and the risk-management program described, making IR a financial-reporting matter; auditors at public companies therefore assess whether the materiality determination is documented, whether the escalation from security to legal and finance is clear, and whether the four-day timeline is operationally achievable.",
        "Two operational measures define maturity. The first pair is MTTD, the time from attacker activity to detection, and MTTR, the time from detection to containment; the IBM Cost of a Data Breach report shows that breaches contained within 30 days cost far less, financial-services regulators expect a high-severity MTTD measured in hours rather than days, and an organization with a 78-day MTTD like Equifax has a fundamental failure regardless of its documentation. The second measure is playbooks, the step-by-step procedures for the top incident types — ransomware, data breach or exfiltration, insider threat, phishing with credential compromise, and DDoS — each one specifying detection indicators, triage, containment, eradication, recovery, communication, and evidence-preservation steps; auditors test them against those criteria and against whether IR members can actually execute them under tabletop conditions.",
        "Log retention is the evidentiary foundation of response. On detection, the IR team must answer when the attacker entered, what systems they touched, and what data they accessed or exfiltrated, and answering those questions requires logs reaching back across the full dwell period — typically ninety days online and immediately searchable, plus twelve months archived. Equifax's detection failure was partly a retention problem, because an expired SSL-inspection certificate meant the exfiltration traffic was never logged in a detectable form; and even with adequate retention, unreviewed logs or gaps left by misconfigured log shipping aren't reliable evidence, so auditors verify that all in-scope systems ship logs to the SIEM with no delivery gaps.",
      ],
      technical: {
...
-- Finding: Detection time exceeds benchmark — SIEM tuning required`,
  ...
  // ─── audit-08: Vendor Risk ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Vendor and third-party risk management (TPRM) audits verify that the organization oversees every vendor, supplier, and partner that accesses its systems, processes its data, or runs part of its IT. ISACA places this work under COBIT's APO10 (Managed Vendors), and it has become one of the fastest-growing areas of regulatory scrutiny worldwide. The premise is simple but operationally complex: an organization's security posture is the product of its own controls and its vendors' controls, so a breach of a vendor is a breach of the organization, with the data leaving through the vendor's door. The first lifecycle stage is inventory — knowing every vendor, including the shadow IT bought on a credit card outside procurement and the subcontractors of primary vendors — because organizations consistently undercount, the shadow-IT count alone can exceed the managed count, and the true population must be tested against accounts-payable records, asset inventories, network traffic, and user-reported SaaS.",
        "The next two stages tier the vendors and test their controls. Classification assigns a risk tier based on data sensitivity, system criticality, dependency (how quickly the vendor could be replaced), the vendor's own regulatory status, and incident history. A Tier 1 (Critical) vendor is existential, a Tier 2 (High) vendor processes business data or holds connectivity, and a Tier 3 (Low) vendor has no system access. Assessment then matches rigor to tier. A Tier 1 vendor warrants a questionnaire such as the Standardized Information Gathering (SIG), the vendor's SOC 2 Type II report as an independent attestation of controls over time, penetration-test and vulnerability-scan results, and an on-site review for the highest risk; a questionnaire alone is insufficient for Tier 1 because it rests entirely on vendor self-reporting.",
        "The last two stages bind the relationship and watch it between assessments. Contractual requirements are where auditors verify the security obligations, the data-protection terms covering encryption, access controls, and handling, a breach-notification timeline (GDPR allows 72 hours, though many contracts require 24), the right to audit, data return or destruction at termination, and restrictions on subcontractor approval. Ongoing monitoring then differs by tier. A Tier 1 vendor warrants continuous security-ratings monitoring through services like BitSight, SecurityScorecard, or RiskRecon, tracking of breaches, regulatory actions, and leadership changes, quarterly vendor-access reviews, and vendor security metrics, while Tier 2 gets an annual reassessment plus significant-event tracking and Tier 3 may be limited to contract-renewal review.",
        "Fourth-party risk — the risk from a vendor's own vendors — is the hardest dimension and the one supply-chain attacks most often exploit. SolarWinds succeeded partly because affected organizations had never mapped that their Orion instance held broad network visibility, nor assessed what a compromise of Orion itself would do. Managing this risk requires understanding each Tier 1 vendor's key dependencies — which cloud providers it relies on, which software libraries it ships, and which subcontractors handle the organization's data — and organizations that haven't answered those questions carry a gap that attackers have repeatedly shown they will exploit.",
      ],
      technical: {
...
ORDER BY tier, days_since_assessment DESC;`,
  ...
  // ─── audit-09: Data Privacy ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2018,
      overview: [
        "Data privacy audits assess whether an organization collects, processes, retains, and deletes personal data in line with applicable privacy law — primarily GDPR in Europe and CCPA in California, plus sector rules like HIPAA, GLBA, and COPPA. ISACA's privacy framework maps to GDPR's six lawful bases and eight data-subject rights, giving auditors a methodology that crosses jurisdictions. The stakes are mainstream rather than niche: GDPR's maximum fine is €20 million or 4% of global annual turnover, whichever is higher. A comprehensive audit covers seven domains, and it starts with data mapping — what personal data is held, where it lives, how it flows, who has access, and the documented legal basis for holding it — because without that map no other control can be designed or tested.",
        "The other domains and the findings they generate are tightly linked. Consent mechanisms must be testable; data-subject-rights fulfillment — deletion, access, portability, rectification, and objection — must work reliably at scale within the required timeframes; and retention and deletion schedules must be documented, implemented in the systems, and enforced either automatically or by regular review. The most common findings track the gap between policy and reality. Personal data is retained past its documented period because the schedules exist but automated deletion doesn't; processing runs with no documented Article 6 lawful basis because new analytics, marketing automation, or data-sharing shipped without a privacy assessment; DSARs are missed beyond the 30-day window because the process is manual and untrained; and DPIAs are missing for high-risk processing such as large-scale profiling, systematic monitoring, or sensitive categories.",
        "Two structural elements get tested directly. The Data Protection Officer (DPO) is required for large-scale processing of special categories, large-scale systematic monitoring, or public authorities, and auditors verify that the role exists where required, that it has genuine independence rather than reporting to the people whose processing it oversees, that it has sufficient resources and expertise, and that it is involved in all data-protection matters — flagging the conflict when, for instance, the CISO doubles as the DPO. Data mapping is documented as the Record of Processing Activities (RoPA, GDPR Article 30), which must list the controller and DPO contacts, the processing purposes, the categories of data subjects and personal data, the recipients including any international transfers, the retention periods, and the security measures; it must also be current, so auditors test it against recent deployments, vendor onboardings, and IT data-flow diagrams to find flows that exist but never made it into the RoPA.",
        "Two forward-looking requirements round out the scope. International transfers are the first such requirement. Moving personal data outside the EEA to non-adequacy countries requires Standard Contractual Clauses (SCCs, updated in 2021), Binding Corporate Rules, or an approved certification scheme, and the Schrems II invalidation of Privacy Shield in 2020 made US-EU transfers genuinely complex, so auditors verify that all EEA-to-US flows are identified, that valid mechanisms are in place, and that Transfer Impact Assessments (TIAs) are done where required. Privacy by Design is the second such requirement — privacy controls built in from the start and tested through the SDLC. The auditor probes it by asking whether privacy review is a mandatory pre-production checkpoint, whether minimization, pseudonymization, and encryption are considered at design time, whether data collection is opt-in by default, and whether retention periods are configured at deployment rather than bolted on after data has accumulated. Organizations that keep discovering privacy violations in existing systems rather than preventing them have a Privacy by Design gap that recurs as findings.",
      ],
      technical: {
...
-- Retention schedule: 730 days (2 years)  → EXCEEDS BY 1,632 DAYS — FINDING`,
  ...
  // ─── audit-10: Pen Test Scoping ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2003,
      overview: [
        "Penetration testing is a critical control in any mature security program — a controlled adversarial simulation that validates whether defenses work in practice rather than only on paper. ISACA auditors review pentest programs to confirm that they deliver genuine assurance rather than compliance theater, and that the authorization structure protects both the organization and the testers. The line between a security professional and a criminal is defined entirely by the scope of written authorization, because the same technical actions taken without it are federal crimes under the Computer Fraud and Abuse Act (CFAA) and its equivalents elsewhere.",
        "Two documents make a test legal, and both must come from the right authority. The scope document is the legal foundation. It names exactly which systems are in scope, whether by IP range, domain, application, or environment, and which testing types are permitted, from external and internal to web-app, social-engineering, physical, and wireless, along with the precise testing window, the blackout periods, and the escalation procedure for critical findings; out-of-scope systems must be listed explicitly, since prosecutors apply the most conservative reading of authorization. Authorization itself must flow from the system owner — the business or legal entity responsible — and not from IT management alone when business units own the systems, and cloud workloads need the provider's approval as well, since AWS, Azure, and GCP each publish pentest policies defining what is allowed without notice and what requires advance approval.",
        "The next layer operationalizes the scope and protects the resulting report. Where the scope defines what may be tested, the rules of engagement (ROE) define how the testing is conducted — the authorized tools and methods, the permitted versus prohibited techniques (denial-of-service, for example, may be explicitly barred on production), the handling of discovered credentials, the collection and retention of evidence, the protocol for communicating an urgent finding, and what the tester must do upon discovering an active real intrusion, which happens more often than clients expect. The test report is itself security-sensitive, being a roadmap of working exploits and paths to critical findings, so distribution is restricted to authorized recipients such as the CISO and a few IT leaders, it is transmitted securely over encrypted email or a secure portal, and it is stored in an access-controlled repository rather than on a general shared drive.",
        "Two practices determine whether the test produces lasting value. The first is remediation tracking, which means a kickoff within five business days, a plan with owners and due dates set by severity — critical within 30 days, high within 60, and medium within 90 — and a retest against the specific remediated vulnerabilities rather than a general rescan, since the retest is the evidence that remediation actually worked. The second is recognizing how red-team engagements differ from pentests. A pentest has a defined scope, a find-and-report methodology, and a two-to-four-week duration, whereas a red team simulates an APT pursuing a defined objective such as reaching the most sensitive data or simulating ransomware, using any TTPs including social engineering, physical intrusion, and supply chain, running for months and requiring extra authorization for human targets, facilities, and supplier relationships that auditors verify has been explicitly granted.",
      ],
      technical: {
...
def is_authorized(target_ip):
  ...
  // ─── audit-11: Audit Evidence ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1972,
      overview: [
        "Audit evidence is the information auditors collect and evaluate to support conclusions about whether controls operate effectively. ISACA requires it to carry three attributes. It must be sufficient, meaning there is enough of it through an adequate sample size, period covered, and well-defined population; it must be reliable, coming from a credible source via a sound, unmanipulated method; and it must be relevant, actually testing the control, since evidence that a backup ran does not test whether it is restorable. Failure on any one of the three invalidates the conclusion. Sampling tests a representative subset when testing every item is impractical. For a population of 1,200 change tickets, statistical sampling gives every item a known, non-zero chance of selection and projects results with measurable precision, whereas judgmental, non-statistical sampling relies on auditor expertise but can describe only the sample and not project to the whole population.",
        "Two foundations govern how evidence is gathered and trusted. Attribute sampling, the most common statistical method in IT audit, tests a binary attribute — the control either operated or it didn't — by defining the population, such as all Q1 2026 change tickets, the attribute, such as each ticket carrying CAB approval before deployment, the tolerable deviation rate, the maximum failures that still allow an 'effective' conclusion and typically 5 to 10 percent, the confidence level, where 95 percent is standard, and the expected deviation rate, a prior estimate assumed to be zero if unknown; statistical tables then set the minimum sample size. Chain of custody documents who collected each piece of evidence, when, from what source, by what method, and how it has been stored since, and it matters because workpapers may support legal proceedings, regulatory exams, or litigation where authenticity is challenged, so digital evidence adds hash values such as MD5 or SHA-256 at collection, authoritative timestamps, and access-recording storage.",
        "Two practices scale and standardize the work. Computer-Assisted Audit Techniques (CAATs) automate evidence collection and analysis — data tools like ACL/Galvanize, IDEA, and SQL, log platforms like Splunk and Elastic, and configuration-comparison and reconciliation tools — which lets auditors test whole populations rather than samples, run analyses that would be infeasible by hand such as comparing every payroll disbursement against the current employee roster, and produce reproducible evidence, since the same CAAT run on the same data yields the same result. Workpaper documentation standards require, for each piece of evidence, the test objective, the population and sample definition, the method, the evidence examined with source references, the results including any deviations, and a conclusion, and the test of adequacy is that another auditor with no prior knowledge could understand exactly what was tested, how, and what was found, so a workpaper that requires an oral explanation is not adequately documented.",
        "An evidence-quality hierarchy guides how much weight to place on each type. Physical evidence, where the auditor personally observes the control operating — watching a CAB meeting, observing a restoration, or extracting data directly — sits highest, because it can't be fabricated after the fact. Documentary evidence, including logs, reports, screenshots, signed documents, and system extracts, comes second, and it is reliable when pulled directly from the source system rather than handed over by management. Testimonial evidence, the representations made in interviews, ranks lowest because it relies on the speaker's truthfulness, so auditors must stay skeptical of management claims that physical or documentary evidence does not support, especially for high-risk controls.",
      ],
      technical: {
...
if sample_deviation_rate > 0.05:  # exceeds 5% tolerable rate
  ...
  // ─── audit-12: Audit Reporting ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1978,
      overview: [
        "The audit report is the formal deliverable that turns weeks of testing, evidence, and analysis into actionable information for leadership, the audit committee, and the board. ISACA standards (IS Audit Standards 1401-1405) require it to include the audit scope and objectives, the period covered, the methodology, the findings with their supporting evidence and risk ratings, management's response to each finding, and the agreed remediation timelines. It is not merely a list of problems but a communication document whose quality is measured by whether it produces effective remediation. Each finding must follow the Condition-Criteria-Cause-Effect framework: the Condition is what was found, stated in specific, measurable, objective terms; the Criteria is what the standard or policy requires; the Cause is the root cause rather than the symptom; and the Effect is the business risk if it goes unremediated. Omitting any one element leaves management unable to remediate and the board unable to assess risk.",
        "Two elements determine whether findings actually drive action. Risk rating must be consistent and defensible, assigning Critical, High, Medium, or Low on the basis of likelihood, the magnitude of impact across financial, regulatory, reputational, and operational dimensions, the breadth of systems affected, and any compensating controls; a Critical finding escalates immediately to the audit committee without waiting for the 30-day cycle, a High is remediated in 30 to 60 days, a Medium in 90 days, and a Low may be accepted with documentation. Management responses must substantively engage rather than defend. A bare 'we disagree' that never addresses the condition's factual basis is inadequate, and 'we will remediate by [date]' with no specific actions isn't actionable; a good response acknowledges the finding's accuracy or disputes it specifically, identifies the root cause, lists remediation actions with owners and dates, and notes any interim compensating controls, and auditors review the responses before finalizing and may request revision.",
        "Two operational controls protect and close out the report. Distribution is itself a security requirement, because the report details control weaknesses that are valuable to attackers, so access is restricted to those with a legitimate need — the audit committee, the board for significant or material findings, the CISO, the CIO, and process owners — the report travels by encrypted email or a secure portal rather than standard email, it is stored access-controlled and tracked, and external requests from regulators or external auditors are routed through legal counsel. Remediation tracking then closes the loop that many organizations leave open, using a centralized system that records every open finding with its rating, agreed date, owner, and status, escalates automatically as due dates approach, runs a 30/60/90-day follow-up that verifies progress against evidence, and requires a formal closure with both management's attestation and audit-team verification before a finding is marked closed.",
        "The relationship with the audit committee is the governance structure that gives internal audit its independence and authority. The audit committee, made up of independent board members, is internal audit's primary client rather than management, and the Chief Audit Executive reports to it rather than to the CEO or CFO, so management can't suppress or delay reports about its own weaknesses. Disputed findings are adjudicated by the committee, with the auditor presenting evidence and management presenting its position, and when findings linger unresolved or management repeatedly fails to remediate, the committee can escalate by requiring an independent review, engaging external auditors, or taking disciplinary action.",
      ],
      technical: {
