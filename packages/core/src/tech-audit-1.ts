...
export const techAudit1Stages: StageConfig[] = [
  // ─── audit-01: COBIT 2019 ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "COBIT 2019 is ISACA's flagship framework for IT governance and management, underpinning almost every IT audit methodology in regulated industries. It draws a precise, legally significant line between governance — which sets direction, evaluates options, and monitors outcomes — and management, which plans, builds, runs, and monitors in line with that direction. The distinction isn't academic: regulators and courts have assigned personal liability to board members who abdicated governance and relied entirely on management self-reporting.",
        "Every audit starts by establishing that boundary, then tracing how intent flows through it. The board and executives govern — approving risk appetite, setting strategy, authorizing major IT investments, and ensuring accountability — while IT leadership manages, implementing controls, operating systems, and reporting performance upward within board-approved parameters. Auditors hold an independent third position, verifying both layers and reporting to the audit committee, not management, to preserve their independence. The governance cascade runs from board principles to management policies to operational procedures to technical controls; auditors trace it in reverse, and a control that can't be traced up to policy authority — or a policy never implemented as a control — is a governance gap.",
        "The framework organizes 40 objectives across five domains, tailored by design factors. EDM (Evaluate, Direct, Monitor) is the board-governance domain, covering risk appetite, IT investment, and performance monitoring. The four management domains follow: APO (Align, Plan, Organise) handles strategy, architecture, risk, and HR planning; BAI (Build, Acquire, Implement) handles projects, change, and requirements; DSS (Deliver, Service, Support) handles operations, incidents, and continuity; and MEA (Monitor, Evaluate, Assess) handles internal controls, compliance, and board reporting. The twelve design factors — enterprise strategy, risk profile, compliance requirements, sourcing model, IT methods, and more — tailor the framework to context, so a regulated bank running its own data center has a different target profile than a cloud-native startup, and auditors establish the applicable factors before assessing capability.",
        "Two practical points dominate real engagements. First, COBIT is a governance framework, not a technical standard: it prescribes no implementations but maps to NIST CSF, ISO 27001, ITIL, and TOGAF, so auditors routinely find strong NIST or ISO technical controls that aren't connected to board-level accountability. Second, the most common findings cluster in EDM — EDM01 (no formal governance charter), EDM03 (no approved risk-appetite statement, so risk decisions are made ad hoc without board authority), and EDM05 (IT reporting to the board absent, or buried in technical metrics the board can't evaluate) — which together form a pervasive governance failure that undermines every other control.",
      ],
      technical: {
...
APO12 Risk Mgmt      2        4      2    HIGH`,
  ...
  // ─── audit-02: Risk Assessment — CRISC ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "CRISC (Certified in Risk and Information Systems Control) is ISACA's risk-focused certification and methodology. It defines a four-domain approach — IT Risk Identification, IT Risk Assessment, Risk Response and Mitigation, and Risk and Control Monitoring and Reporting. Where compliance frameworks ask 'did we implement the required control?', CRISC asks the prior question that gives controls their meaning: what risks are the controls meant to address, and are they actually reducing those risks to acceptable levels?",
        "The first two domains define what to manage and how exposed you are. Risk identification is the most under-resourced of them: registers populated in a single workshop years ago go stale, so CRISC requires continuous identification as new systems, vendors, regulations, and threats appear, and auditors test it by comparing the register against change logs, vendor onboardings, threat intel, and incident databases for risks that should be there but aren't. Risk assessment then runs a standard method — identify threats and vulnerabilities, score inherent risk before controls, evaluate control effectiveness, calculate residual risk after controls, and compare residual to board-approved appetite — where the inherent-to-residual gap is the control effectiveness, which must be large enough to bring residual within appetite.",
        "Two artifacts anchor the rest of the work. The first is the set of four risk responses: Accept (the board formally documents and absorbs the loss, since undocumented acceptance is invalid), Mitigate (add or strengthen controls), Transfer (shift financial exposure via insurance, indemnification, or outsourcing without eliminating the risk itself), and Avoid (discontinue the activity — the most complete response, but often impractical). The second is the risk register, which auditors scrutinize most closely: a mature one carries a unique ID, a threat and vulnerability narrative, the affected asset, a named risk owner, an inherent score (likelihood × impact), controls with references, a control-effectiveness rating, a residual score, the response type and treatment plan, the appetite threshold, and a last-review date — and any missing field is itself a finding, because the organization then can't demonstrate it's managing the risk.",
        "Two things determine whether the program is real. Risk appetite must be board-approved and quantitative: 'we have a low appetite for cybersecurity' gives no threshold, whereas 'no residual score above 12 on the 5×5 matrix without a board-approved exception, with scores of 16+ escalating immediately to the Audit Committee' is usable. And the most consequential finding is residual risk above appetite with no treatment plan — the organization knows it's over its own threshold and is doing nothing about it; regulators treat this as management negligence, with the FDIC and OCC issuing Matters Requiring Attention and external auditors potentially calling it a significant deficiency or material weakness under SOX 404.",
      ],
      technical: {
...
R-004    Ransomware via RDP        4       5      20       None       20     CRITICAL — NO CONTROLS`,
  ...
  // ─── audit-03: ITGC ────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2002,
      overview: [
        "IT General Controls (ITGCs) are the foundational controls beneath all application-level controls — not tied to any one app but applying to the whole IT environment. The analogy is a building's foundation and floors: a sound floor on a cracked foundation is still unsafe. If ITGCs fail, every application control built on them is suspect, and auditors can't rely on application output without far more compensating work. The four domains are Change Management (production changes follow an approved, documented process), Logical Access (only authorized users reach systems and data at appropriate privilege), Computer Operations (reliable running, backups performed and tested, jobs completing), and Program Development (new systems developed, tested, and approved via a defined SDLC) — and all four appear in every SOX 404 audit, PCAOB inspection, and financial-services exam.",
        ...
        "Two structural facts shape every engagement. SOX Section 404 makes management assess and attest to ICFR effectiveness, including the ITGCs that support it, while PCAOB AS 2201 makes external auditors verify that assessment — a two-layer accountability in which the CEO and CFO personally sign the Section 302 certification, so ITGC failures create personal legal exposure. Scoping then identifies which systems are 'in scope': those processing, storing, or transmitting data that flows into financial statements (general and subsidiary ledgers, payroll, revenue recognition, consolidation tools) plus their supporting OS, database, network, and identity platforms, and in the cloud the platform configuration and shared-responsibility controls.",
        "A recurring set of findings appears across industries, each a breakdown that could allow unauthorized changes or access to financial data. In logical access, the classics are terminated employees with active accounts and shared privileged accounts with no individual accountability. In change management, they are developers with standing production access (a segregation-of-duties failure) and production changes pushed with no approved tickets. And in operations and development, they are backups never tested for restorability and new systems deployed without any security-testing evidence.",
      ],
      technical: {
...
# Output: jsmith terminated 2026-03-15 — still has ACTIVE AD account — FINDING`,
  ...
  // ─── audit-04: Access Control ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "Access control auditing verifies that least privilege holds across the environment — that every user, whether employee, contractor, or service account, has exactly the permissions their documented job needs and no more. It sounds simple but is hard to maintain, because systems accumulate users, people change roles, contractors finish engagements, and business units reorganize. Through all of it, access granted for a former purpose persists unless an active process removes it, and that accumulation — permission creep — is what access audits exist to find and remediate.",
        "The control objectives come from COBIT APO13 and DSS05, aligned with the NIST SP 800-53 Access Control (AC) family, and they set clear expectations. Provisioning must follow an approved request with documented business justification. Access must be reviewed and recertified at least annually, with managers confirming each person's access is still appropriate. Terminated employees must be deprovisioned within 24 hours for standard accounts and immediately — within roughly four hours — for privileged ones. And privileged access must be minimized, documented with justification, protected with MFA, and subject to enhanced monitoring.",
        "Access findings are the most common in IT audit, and the most security-critical of them concern privileged access. They span the spectrum from administrative (missing recertification evidence) to critical (terminated employees with active privileged accounts), and they reflect a coordination problem across HR, IT, and business management — which is why auditors test the process, not just the current list, since a clean list produced by a broken process will fail again. Privileged Access Management (PAM) governs the accounts that bypass most application controls — domain and database admins, root, elevated service accounts, broad cloud IAM roles — through controls such as just-in-time access that expires automatically, dedicated privileged access workstations (PAWs), session recording, and regular review of privileged rights against job requirements.",
        "Three models and account types complete the picture. RBAC attaches rights to roles rather than individuals, so assigning the 'Accounts Payable Clerk' role configures access automatically and simplifies provisioning, deprovisioning, and recertification; auditors verify that role definitions are approved, that assignments match job descriptions, and that no user holds conflicting roles. Segregation-of-duties (SoD) conflicts arise when one user spans incompatible functions — create vendor and approve payments, initiate and approve wire transfers, post and approve journal entries — creating fraud opportunity, so auditors test access against a conflict matrix. Service accounts, the non-human identities used by apps and jobs, often hold elevated, shared, embedded credentials no human reviews daily, so auditors inventory them and confirm a documented owner, scheduled credential rotation, interactive logon disabled where it isn't needed, and tightly limited permissions.",
      ],
      technical: {
...
-- [term]     WRITE    2025-03-01   [TERM]       ← TERMINATED USER - CRITICAL`,
  ...
  // ─── audit-05: Change Management ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1986,
      overview: [
        "Change management ensures every modification to production goes through a documented, approved, and tested process before it ships. It isn't bureaucracy — it's the control that prevents accidental outages, unauthorized code deployment, and security vulnerabilities from entering production. ITIL 4 defines three change types by risk: Standard (pre-approved, low-risk routine changes), Normal (requiring Change Advisory Board review), and Emergency (expedited approval for critical fixes, with mandatory post-hoc documentation).",
        "The Change Advisory Board (CAB) reviews and approves Normal changes, and auditors test it against what it should enforce. The CAB is cross-functional — IT management, security, operations, application owners, and business stakeholders — and it assesses risk, confirms testing is complete, verifies an executable rollback plan and an appropriate maintenance window, and formally authorizes the change; an effective CAB challenges assumptions and rejects unacceptable risk rather than rubber-stamping. For each sampled change, auditors verify five elements: an approved ticket opened before the change (not backdated), documented CAB approval with name and date, developer and approver as different people (segregation of duties), test results attached from a non-production environment, and a documented, feasible rollback plan.",
        "Two process safeguards are common failure points. The emergency path is a necessary safety valve but frequently abused: genuine emergencies — an actively exploited zero-day, a production outage — justify it, but an emergency rate above 10–15% triggers investigation into whether developers, or rubber-stamping managers, are routing around CAB review under the guise of urgency. Maintenance windows are the approved low-traffic deployment periods (overnight, weekends) when failure impact is minimized and staff are watching; a change deployed outside the window without emergency authorization is a finding regardless of its technical content, and auditors flag it by comparing deployment timestamps to the window schedule.",
        "Two modern concerns close the loop. DevOps and CI/CD strain the human-review model, so ITIL 4 introduces the 'change authority,' which can be an automated system for low-risk, well-tested changes — and auditors test whether the pipeline itself is sufficient control, looking for enforced automated testing, required passing security scans, an immutable deployment audit log, and a halt-on-failure mechanism; a well-configured pipeline can be more reliable than a manual CAB, but only if its configuration is itself audited. Post-implementation review (PIR) then documents whether the change met its purpose, any side effects, the lessons learned, and the follow-ups; auditors verify PIRs were completed on time (typically within five business days) and that issues were tracked to resolution, and PIRs that consistently surface unexpected side effects reveal a deeper problem — test environments that don't represent production.",
      ],
      technical: {
...
-- CHG-1003  jsmith    jsmith      2026-02-05  SOD VIOLATION  ← FINDING`,
  ...
  // ─── audit-06: BCP/DR ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2005,
      overview: [
        "Business Continuity Planning (BCP) and Disaster Recovery (DR) audits verify that an organization can keep critical operations running and recover IT systems after a disruption — a natural disaster, ransomware, a data center fire, or a vendor failure. CISA dedicates substantial coverage to this area precisely because these plans are systematically neglected: created during a compliance exercise, filed in a shared drive, and never exercised. An untested recovery plan isn't a control; it's documentation of intent, and intent has never restored a database.",
        "Two foundations define what recovery means and where it applies. The first is the set of metrics: Recovery Time Objective (RTO), the maximum tolerable downtime before an unacceptable consequence; Recovery Point Objective (RPO), the maximum acceptable data loss measured in time; and Maximum Tolerable Downtime (MTD), the point of irreversible harm — and the fundamental rule is that RTO must be less than MTD, or the plan literally can't save the business. The second is the Business Impact Analysis (BIA), which auditors review first because it maps each critical process, its supporting systems, the impact of downtime, and the resulting RTO and RPO; a BIA not updated since a major migration, cloud adoption, or acquisition is stale, so auditors test its currency by comparing its systems against the current production inventory.",
        "Strategy and testing must demonstrably meet the approved targets. Recovery strategies range from hot standby (a fully operational duplicate with near-zero RTO and RPO, but expensive) through warm and cold standby (cold is cheap but its RTOs run to days) to on-demand cloud failover, and auditors verify the chosen strategy has demonstrated the approved RTO and RPO in testing rather than only on paper. Both tabletop and full DR tests are required: a tabletop is a discussion-based walkthrough that tests logical structure, roles, and decisions without disruption, while a full DR test actually fails over to the recovery environment and verifies RTOs and RPOs in practice — tabletop-only tests the document, and technical-only tests the technology but not the human coordination.",
        "Two modern requirements reshape the audit. Ransomware broke the old assumption that backups survive: attacks encrypt production, backups, cloud-connected repositories, and anything reachable from the compromised network, so audits now test for air-gapped or immutable backups, the 3-2-1 rule (three copies, two media types, one offsite), tested restoration from an isolated backup that was disconnected before the attack, and a ransomware-specific tabletop that includes backup compromise. Geographic separation is equally non-negotiable, as Katrina showed: primary and backup sites must lie outside the same flood zone and far enough apart to survive a regional disaster — FDIC and OCC guidance generally requires roughly 100 miles — and auditors check the location documentation for shared dependencies such as a common power-grid segment, ISP backbone, or seismic fault.",
      ],
      technical: {
...
- Customer Portal: not tested in 18 months — CRITICAL FINDING`,
  ...
  // ─── audit-07: Incident Response ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Incident Response (IR) program audits assess whether an organization can detect, contain, eradicate, and recover from security incidents consistently, at scale, and under the time pressure and confusion real incidents create. ISACA auditors use NIST SP 800-61 as the framework, evaluating all four of its phases — Preparation, Detection and Analysis, Containment/Eradication/Recovery, and Post-Incident Activity — since a program that detects well but can't contain, or contains but learns nothing, isn't mature. Preparation is the most auditable phase and the most often deficient, requiring a current IR plan reviewed within 12 months, named individuals rather than job titles in each role, contact lists covering external parties (legal, law enforcement, forensics, the cyber insurer, PR), a severity matrix defining P1 versus P4, notification protocols per severity, and tabletop exercises within the last 12 months with action items tracked to closure.",
        "Two forces shape the program beyond preparation. The first is Detection and Analysis: auditors review the SIEM for log-source coverage of critical systems, alert rules tuned to the top threats, and managed false-positive rates — a SIEM firing thousands of daily alerts trains analysts to ignore them, which is worse than none — then sample historical incidents to see how each was detected, because external notification by law enforcement or a third party signals that internal detection is insufficient. The second is the SEC's 2023 disclosure rules, which require material incidents to be disclosed within four business days and the risk-management program to be described, making IR a financial-reporting matter; auditors at public companies therefore assess whether the materiality determination is documented, the escalation from security to legal and finance is clear, and the four-day timeline is operationally achievable.",
        "Two operational measures define maturity. The first pair is MTTD (time from attacker activity to detection) and MTTR (time from detection to containment): the IBM Cost of a Data Breach report shows breaches contained within 30 days cost far less, financial-services regulators expect high-severity MTTD in hours not days, and an organization with a 78-day MTTD like Equifax has a fundamental failure regardless of its documentation. The second is playbooks — step-by-step procedures for the top incident types (ransomware, data breach and exfiltration, insider threat, phishing with credential compromise, DDoS), each specifying detection indicators, triage, containment, eradication, recovery, communication, and evidence-preservation steps; auditors test them against those criteria and against whether IR members can actually execute them under tabletop conditions.",
        "Log retention is the evidentiary foundation of response. On detection, the IR team must answer when the attacker entered, what systems they touched, and what data they accessed or exfiltrated — which requires logs reaching back across the full dwell period, typically 90 days online and immediately searchable plus 12 months archived. Equifax's detection failure was partly a retention problem: an expired SSL inspection certificate meant the exfiltration traffic wasn't logged in a detectable form. And even with adequate retention, unreviewed logs or gaps from misconfigured shipping aren't reliable evidence, so auditors verify that all in-scope systems ship logs to the SIEM with no delivery gaps.",
      ],
      technical: {
...
-- Finding: Detection time exceeds benchmark — SIEM tuning required`,
  ...
  // ─── audit-08: Vendor Risk ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Vendor and third-party risk management (TPRM) audits verify that the organization oversees every vendor, supplier, and partner that accesses its systems, processes its data, or runs part of its IT. ISACA places this under COBIT's APO10 (Managed Vendors), and it is one of the fastest-growing areas of regulatory scrutiny worldwide. The premise is simple but operationally complex: your security posture is the product of your controls and your vendors' controls, so a breach of your vendor is a breach of you — your data leaves through their door. The first lifecycle stage is inventory, knowing every vendor including the shadow IT (SaaS bought on a credit card outside procurement) and the subcontractors of primary vendors, since organizations consistently undercount and the shadow-IT count alone can exceed the managed count; auditors test it against accounts-payable records, asset inventories, network traffic, and user-reported SaaS.",
        "The next two stages tier vendors and test their controls. Classification risk-tiers each vendor by data sensitivity, system criticality, dependency (how fast can the vendor be replaced?), the vendor's own regulatory status, and incident history: Tier 1 (Critical) is existential, Tier 2 (High) processes business data or has connectivity, and Tier 3 (Low) has no system access. Assessment then matches rigor to tier — Tier 1 needs a questionnaire such as the SIG (Standardized Information Gathering), the vendor's SOC 2 Type II report (an independent attestation of controls over time), penetration-test and vulnerability-scan results, and on-site review for the highest risk — because questionnaire-only assessment is insufficient for Tier 1, resting entirely on vendor self-reporting.",
        "The last two stages bind the relationship and watch it between assessments. Contractual requirements must be verified: security obligations, data protection (encryption, access controls, handling), a breach-notification timeline (GDPR's 72 hours, though many contracts require 24), the right to audit, data return or destruction at termination, and subcontractor-approval restrictions. Ongoing monitoring then matches intensity to tier — Tier 1 gets continuous security-ratings monitoring (BitSight, SecurityScorecard, RiskRecon), tracking of breaches, regulatory actions, and leadership changes, quarterly vendor-access reviews, and vendor security metrics; Tier 2 gets annual reassessment plus significant-event tracking; and Tier 3 may be limited to contract-renewal review.",
        "Fourth-party risk — the risk from your vendor's vendors — is the hardest dimension and the one supply-chain attacks most often exploit. SolarWinds succeeded partly because affected organizations never mapped that their Orion instance had broad network visibility, nor assessed what compromise of Orion itself would do. Managing it requires understanding each Tier 1 vendor's key dependencies — which cloud providers, which software libraries, which subcontractors handle your data — and organizations that haven't answered these questions carry a gap attackers have repeatedly shown they will exploit.",
      ],
      technical: {
...
-- Retention schedule: 730 days (2 years)  → EXCEEDS BY 1,632 DAYS — FINDING`,
  ...
  // ─── audit-10: Pen Test Scoping ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2003,
      overview: [
        "Penetration testing is a critical control in any mature security program — a controlled adversarial simulation that validates whether defenses work in practice, not just on paper. ISACA auditors review pentest programs to confirm they deliver genuine assurance rather than compliance theater, and that the authorization structure protects both the organization and the testers. The line between a security professional and a criminal is defined entirely by the scope of written authorization: the same technical actions taken without it are federal crimes under the Computer Fraud and Abuse Act (CFAA) and its equivalents elsewhere.",
        "Two documents make a test legal, and they must come from the right authority. The scope document is the legal foundation: it names exactly which systems are in scope (IP ranges, domains, applications, environments), which testing types are permitted (external, internal, web app, social engineering, physical, wireless), the precise testing window and blackout periods, and the escalation procedures for critical findings — and out-of-scope systems must be listed explicitly, since prosecutors apply the most conservative reading of authorization. Authorization itself must flow from the system owner, the business or legal entity responsible, not from IT management alone when business units own the systems; cloud workloads need the provider's approval too, since AWS, Azure, and GCP each publish pentest policies defining what's allowed without notice and what needs advance approval.",
        "The next layer operationalizes scope and protects the resulting report. The rules of engagement (ROE) define how testing is conducted where the scope defines what: authorized tools and methods, permitted versus prohibited techniques (DoS may be explicitly barred on production), data handling for discovered credentials, evidence collection and retention, the urgent-finding communication protocol, and what to do if the tester finds an active real intrusion — which happens more often than clients expect. The test report is itself security-sensitive, a roadmap of working exploits and paths to critical findings, so distribution is restricted to authorized recipients (typically the CISO and a few IT leaders), transmitted securely by encrypted email or a secure portal, and stored in an access-controlled repository, never a general shared drive.",
        "Two practices determine whether the test produces lasting value. The first is remediation tracking: a kickoff within five business days, a plan with owners and due dates by severity (critical within 30 days, high within 60, medium within 90), and a retest against the specific remediated vulnerabilities rather than just a general rescan, since the retest is the evidence of effective remediation. The second is recognizing how red team engagements differ from pentests: a pentest has a defined scope, a find-and-report methodology, and a two-to-four-week duration, whereas a red team simulates an APT toward a defined objective — reach the most sensitive data, simulate ransomware — using any TTPs including social engineering, physical intrusion, and supply chain, running for months and requiring extra authorization for human targets, facilities, and supplier relationships that auditors verify is explicitly granted.",
      ],
      technical: {
...
def is_authorized(target_ip):
  ...
  // ─── audit-11: Audit Evidence ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1972,
      overview: [
        "Audit evidence is the information auditors collect and evaluate to support conclusions about whether controls operate effectively. ISACA requires it to have three attributes: it must be sufficient (enough of it, with an adequate sample size, period covered, and well-defined population), reliable (from a credible source via a sound method, not manipulated), and relevant (it actually tests the control — evidence that a backup ran doesn't test whether it's restorable), and failure on any one of the three invalidates the conclusion. Where testing every item is impractical, sampling tests a representative subset: for 1,200 change tickets, statistical sampling gives every item a known non-zero chance of selection and projects results with measurable precision, whereas judgmental (non-statistical) sampling relies on auditor expertise but can describe only the sample, not project to the population.",
        "Two foundations govern how evidence is gathered and trusted. Attribute sampling, the most common statistical method in IT audit, tests a binary attribute — the control either operated or it didn't — by defining the population (all Q1 2026 change tickets), the attribute (each ticket has CAB approval before deployment), the tolerable deviation rate (the maximum failures still allowing an 'effective' conclusion, typically 5–10%), the confidence level (95% is standard), and the expected deviation rate (a prior estimate, assumed 0% if unknown), with statistical tables then setting the minimum sample size. Chain of custody, in turn, documents who collected each piece of evidence, when, from what source, by what method, and how it's been stored since — essential because workpapers may support legal proceedings, exams, or litigation where authenticity is challenged, so digital evidence adds hash values (MD5, SHA-256) at collection, authoritative timestamps, and access-recording storage.",
        "Two practices scale and standardize the work. Computer-Assisted Audit Techniques (CAATs) automate evidence collection and analysis — data tools (ACL/Galvanize, IDEA, SQL), log platforms (Splunk, Elastic), and configuration-comparison and reconciliation tools — letting auditors test whole populations rather than samples, run analyses infeasible by hand such as comparing every payroll disbursement against current employees, and produce reproducible evidence, since the same CAAT on the same data yields the same result. Workpaper documentation standards then require, for each piece of evidence, the test objective, the population and sample definition, the method, the evidence examined with source references, the results including any deviations, and a conclusion — and the test of adequacy is that another auditor with no prior knowledge could understand exactly what was tested, how, and what was found, so a workpaper that requires oral explanation is not adequately documented.",
        "An evidence-quality hierarchy guides how much weight to place on each type. Physical evidence — the auditor personally observing the control operating, by watching a CAB meeting, observing a restoration, or extracting data directly — is the highest, since it can't be fabricated after the fact. Documentary evidence — logs, reports, screenshots, signed documents, system extracts — is second, reliable when pulled directly from the source system rather than handed over by management. Testimonial evidence — representations made in interviews — is the lowest, relying on the speaker's truthfulness, so auditors must be skeptical of management claims unsupported by physical or documentary evidence, especially for high-risk controls.",
      ],
      technical: {
...
if sample_deviation_rate > 0.05:  # exceeds 5% tolerable rate
  ...
  // ─── audit-12: Audit Reporting ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1978,
      overview: [
        "The audit report is the formal deliverable that turns weeks of testing, evidence, and analysis into actionable information for leadership, the audit committee, and the board. ISACA standards (IS Audit Standards 1401-1405) require it to include the audit scope and objectives, the period covered, the methodology, the findings with their supporting evidence and risk ratings, management's responses to each finding, and the agreed remediation timelines. It is not a list of problems but a communication document whose quality is measured by whether it produces effective remediation, and each finding must use the Condition-Criteria-Cause-Effect framework: Condition (what was found — specific, measurable, objective), Criteria (what the standard or policy requires), Cause (the root cause, not the symptom), and Effect (the business risk if it goes unremediated) — since omitting any element leaves management unable to remediate and the board unable to assess risk.",
        "Two elements determine whether findings drive action. Risk ratings must be consistent and defensible — Critical, High, Medium, or Low, based on likelihood, impact magnitude (financial, regulatory, reputational, operational), the breadth of systems affected, and any compensating controls — with Critical findings escalating immediately to the audit committee rather than waiting for the 30-day cycle, High findings remediated in 30–60 days, Medium in 90 days, and Low potentially accepted with documentation. Management responses must substantively engage rather than defend: 'we disagree' without addressing the condition's factual basis is inadequate, and 'we will remediate by [date]' without specific actions isn't actionable, whereas a good response acknowledges accuracy (or disputes it specifically), identifies the root cause, lists remediation actions with owners and dates, and notes interim compensating controls — and auditors review responses before finalizing, requesting revision where needed.",
        "Two operational controls protect and close out the report. Distribution is a security requirement, since the report details control weaknesses valuable to attackers: restrict it to those with a legitimate need (the audit committee, the board for significant or material findings, the CISO, CIO, and process owners), transmit it via encrypted email or a secure portal rather than standard email, store it access-controlled and tracked, and route external requests from regulators or external auditors through legal counsel. Remediation tracking then closes the loop many organizations leave open — a centralized system recording every open finding with its rating, agreed date, owner, and status, automated escalation as due dates approach, a 30/60/90-day follow-up verifying progress with evidence, and a formal closure that requires both management's attestation and audit-team verification before a finding is marked closed.",
        "The audit committee relationship is the governance structure that gives internal audit its independence and authority. The audit committee — composed of independent board members — is internal audit's primary client rather than management, and the Chief Audit Executive reports to it rather than to the CEO or CFO, so management can't suppress or delay reports about their own weaknesses. Disputed findings are adjudicated by the committee, with the auditor presenting evidence and management presenting its position, and when findings linger unresolved or management repeatedly fails to remediate, the committee can escalate — requiring an independent review, engaging external auditors, or taking disciplinary action.",
      ],
      technical: {
