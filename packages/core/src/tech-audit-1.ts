...
export const techAudit1Stages: StageConfig[] = [
  // ─── audit-01: COBIT 2019 ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "COBIT 2019 is ISACA's flagship framework for IT governance and management, and it underpins almost every IT audit methodology used in regulated industries. Its central move is to draw a precise, legally significant line between governance — which sets direction, evaluates options, and monitors outcomes — and management, which plans, builds, runs, and monitors in line with that direction. The distinction is not academic. Regulators and courts have assigned personal liability to board members who abdicated governance and relied entirely on management self-reporting, so where that line falls, and whether the organization actually honors it, is the first thing an audit establishes.",
        "Every engagement begins by fixing that boundary and then tracing how intent flows across it. The board and executives govern: they approve risk appetite, set strategy, authorize major IT investments, and ensure accountability. IT leadership manages beneath them, implementing controls, operating systems, and reporting performance upward within board-approved parameters. The auditor occupies an independent third position, verifying both layers and reporting to the audit committee rather than to management precisely so that independence is preserved. The governance cascade runs from board principles to management policies to operational procedures to technical controls, and auditors trace it in reverse — a control that cannot be traced up to policy authority, or a policy that was never implemented as a control, is a governance gap.",
        "The framework organizes forty governance and management objectives across five domains, each tailored to context by a set of design factors. EDM — Evaluate, Direct, Monitor — is the board-governance domain, covering risk appetite, IT investment, and performance monitoring, and four management domains sit beneath it: APO (Align, Plan, Organise) handles strategy, architecture, risk, and HR planning; BAI (Build, Acquire, Implement) handles projects, change, and requirements; DSS (Deliver, Service, Support) handles operations, incidents, and continuity; and MEA (Monitor, Evaluate, Assess) handles internal controls, compliance, and board reporting. Twelve design factors — enterprise strategy, risk profile, compliance requirements, sourcing model, IT methods, and more — tune the framework to the organization, so a regulated bank running its own data center carries a different target profile than a cloud-native startup, and auditors establish the applicable factors before they assess capability.",
        "Two practical points dominate real engagements. The first is that COBIT is a governance framework rather than a technical standard: it prescribes no implementations but maps cleanly to NIST CSF, ISO 27001, ITIL, and TOGAF, which is why auditors routinely find strong NIST or ISO technical controls that were never connected upward to board-level accountability. The second is that the most common findings cluster in the EDM domain. EDM01 fails when no formal governance charter exists; EDM03 fails when there is no approved risk-appetite statement and risk decisions are therefore made ad hoc without board authority; and EDM05 fails when IT reporting to the board is absent or consists entirely of technical metrics the board cannot evaluate. Together these amount to a pervasive governance failure that quietly undermines every other control beneath them.",
      ],
      technical: {
...
APO12 Risk Mgmt      2        4      2    HIGH`,
  ...
  // ─── audit-02: Risk Assessment — CRISC ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "CRISC — Certified in Risk and Information Systems Control — is ISACA's risk-focused certification and methodology, built on a four-domain approach: IT Risk Identification, IT Risk Assessment, Risk Response and Mitigation, and Risk and Control Monitoring and Reporting. What sets it apart from compliance frameworks is the question it asks first. Where a compliance checklist asks whether the required control was implemented, CRISC asks the prior question — what risks are the controls meant to address, and are they actually reducing those risks to acceptable levels?",
        "The first two domains define what there is to manage and how exposed the organization really is. Risk identification is the most under-resourced of them: registers populated in a single workshop years ago quietly go stale, so CRISC demands continuous identification as new systems, vendors, regulations, and threats appear, and auditors test it by comparing the register against change logs, vendor onboardings, threat intelligence, and incident databases for risks that should be present but are not. Risk assessment then runs a standard method — identify threats and vulnerabilities, score inherent risk before controls, evaluate control effectiveness, calculate residual risk after controls, and compare that residual to board-approved appetite. The gap between inherent and residual is the control effectiveness, and it must be large enough to bring residual risk within appetite.",
        "Two artifacts anchor the rest of the work. The first is the set of four risk responses. Accept means the board formally documents and absorbs the loss, and acceptance that is not documented is invalid; Mitigate means adding or strengthening controls; Transfer shifts financial exposure through insurance, indemnification, or outsourcing without eliminating the risk itself; and Avoid discontinues the activity altogether, the most complete response and often the most impractical. The second is the risk register, which auditors scrutinize most closely. A mature register carries, for each risk, a unique ID, a threat or vulnerability narrative, the affected asset, a named risk owner, an inherent score of likelihood times impact, the controls with references, a control-effectiveness rating, a residual score, the response type and treatment plan, the appetite threshold, and a last-review date — and any missing field is itself a finding, because without it the organization cannot demonstrate that it is managing the risk at all.",
        "Two things ultimately determine whether the program is real. Risk appetite must be board-approved and quantitative: a statement like \"we have a low appetite for cybersecurity\" gives no usable threshold, whereas \"no residual score above 12 on the 5×5 matrix without a board-approved exception, and scores of 16 or higher escalate immediately to the Audit Committee\" can actually be tested against the register. And the most consequential finding of all is residual risk above appetite with no treatment plan — the organization knows it is over its own threshold and is doing nothing about it. Regulators treat that as management negligence: the FDIC and OCC issue Matters Requiring Attention, and external auditors may classify it as a significant deficiency or material weakness under SOX 404.",
      ],
      technical: {
...
R-004    Ransomware via RDP        4       5      20       None       20     CRITICAL — NO CONTROLS`,
  ...
  // ─── audit-03: ITGC ────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2002,
      overview: [
        "IT General Controls (ITGCs) are the foundational controls that sit beneath all application-level controls — not tied to any single application but applying to the whole IT environment. The useful analogy is a building's foundation and floors: a sound floor laid over a cracked foundation is still unsafe. When ITGCs fail, every application control built on top of them becomes suspect, and an auditor cannot rely on application output without far more compensating work. There are four domains. Change Management ensures production changes follow an approved, documented process; Logical Access ensures only authorized users reach systems and data at the appropriate privilege; Computer Operations ensures reliable running, with backups performed and tested and jobs completing; and Program Development ensures new systems are developed, tested, and approved through a defined SDLC. All four appear in every SOX 404 audit, PCAOB inspection, and financial-services examination.",
        "ITGC effectiveness governs how much the auditor can trust everything else. A clean ITGC environment lets the auditor rely on system-generated reports and automated application controls with minimal extra testing, whereas a failure — especially in change management or logical access — forces substantial expansion, because the team can no longer assume system output reflects what the system was supposed to produce. Findings are classified by pervasiveness. An isolated deficiency, such as a single unauthorized payroll change, is serious in itself, but a pervasive one — a change process that routinely bypasses approval — casts doubt across systems and reporting periods and typically rises to a significant deficiency or material weakness under SOX, which then must be disclosed.",
        "Two structural facts shape every engagement. SOX Section 404 makes management assess and attest to the effectiveness of internal controls over financial reporting, including the supporting ITGCs, while PCAOB AS 2201 makes external auditors independently verify that assessment — a two-layer accountability in which the CEO and CFO personally sign the Section 302 certification, so ITGC failures carry personal legal exposure. The other fact is scoping: the engagement identifies which systems are \"in scope,\" meaning those that process, store, or transmit data flowing into the financial statements — general and subsidiary ledgers, payroll, revenue recognition, and consolidation tools — together with their supporting operating-system, database, network, and identity platforms, and in the cloud the platform configuration and shared-responsibility controls.",
        "A recurring set of findings appears across industries, each one a breakdown that could allow unauthorized changes or access to financial data. In logical access, the classics are terminated employees who retain active accounts and shared privileged accounts with no individual accountability. In change management, they are developers who hold production access — a segregation-of-duties failure — and production changes pushed with no approved tickets. In computer operations, backups are taken but never tested for restorability, and in program development, new systems are deployed with no evidence that security testing was ever performed. Each is mundane on its own, and each is exactly the kind of gap that lets a fraud or an error reach the financial statements unchallenged.",
      ],
      technical: {
...
# Output: jsmith terminated 2026-03-15 — still has ACTIVE AD account — FINDING`,
  ...
  // ─── audit-04: Access Control ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "Access control auditing verifies that least privilege holds across the environment — that every user, whether employee, contractor, or service account, has exactly the permissions their documented job needs and no more. It sounds simple, and it is genuinely hard to maintain, because systems accumulate users, people change roles, contractors finish engagements, and business units reorganize. Through all of that churn, access granted for a former purpose persists unless an active process removes it, and that quiet accumulation — permission creep — is precisely what access audits exist to find and remediate.",
        "The control objectives come from COBIT APO13 and DSS05, aligned with the NIST SP 800-53 Access Control (AC) family, and they set a clear bar. Provisioning must follow an approved request with documented business justification. Access must be reviewed and recertified at least annually, with managers confirming that each person's access is still appropriate. Terminated employees must be deprovisioned within 24 hours for standard accounts and immediately — within roughly four hours — for privileged ones. And privileged access in particular must be minimized, documented with justification, protected with MFA, and subjected to enhanced monitoring.",
        "Access findings are the most common in all of IT audit, and the most security-critical of them concern privileged access. They range from the administrative, such as missing recertification evidence, to the critical, such as a terminated employee with an active privileged account, and they almost always reflect a coordination problem across HR, IT, and business management — which is why auditors test the process and not merely the current list, since a clean list produced by a broken process will fail again next cycle. Privileged Access Management (PAM) covers the accounts that bypass most application controls: domain and database administrators, root, elevated service accounts, and broad cloud IAM roles. The expected controls are just-in-time access that expires automatically, dedicated privileged access workstations (PAWs), session recording, and regular review of privileged rights against actual job requirements.",
        "Three models and account types complete the picture. Role-based access control attaches rights to roles rather than to individuals, so assigning someone the \"Accounts Payable Clerk\" role configures their access automatically, which simplifies provisioning, deprovisioning, and recertification alike; auditors verify that role definitions are approved, that assignments match job descriptions, and that no user holds conflicting roles. Segregation-of-duties conflicts arise when one user spans incompatible functions — creating a vendor and approving payments, initiating and approving wire transfers, posting and approving journal entries — creating an opportunity for fraud, so auditors test access against a conflict matrix. Service accounts, the non-human identities that run applications and scheduled jobs, frequently hold elevated, shared, embedded credentials that no human reviews day to day, so auditors inventory them and confirm a documented owner, scheduled credential rotation, disabled interactive logon where it is not needed, and tightly limited permissions.",
      ],
      technical: {
...
-- [term]     WRITE    2025-03-01   [TERM]       ← TERMINATED USER - CRITICAL`,
  ...
  // ─── audit-05: Change Management ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1986,
      overview: [
        "Change management ensures that every modification to production passes through a documented, approved, and tested process before it ships. This is not bureaucracy for its own sake; it is the control that prevents accidental outages, unauthorized code deployment, and security vulnerabilities from entering production. ITIL 4 sorts changes into three types by risk. Standard changes are pre-approved, low-risk, and routine; Normal changes require Change Advisory Board review; and Emergency changes receive expedited approval for critical fixes but carry mandatory post-hoc documentation so the shortcut is still accountable.",
        "The Change Advisory Board reviews and approves Normal changes, and auditors test the engagement against what an effective CAB should enforce. The board is cross-functional, drawing in IT management, security, operations, application owners, and business stakeholders, and its job is to assess risk, confirm that testing is complete, verify both an executable rollback plan and an appropriate maintenance window, and then formally authorize — an effective CAB challenges assumptions and rejects unacceptable risk rather than rubber-stamping what is put in front of it. For each sampled change, auditors verify five elements: an approved ticket opened before the change rather than backdated, documented CAB approval with a name and date, a developer and approver who are different people so segregation of duties holds, test results attached from a non-production environment, and a documented, feasible rollback plan.",
        "Two process safeguards are the usual failure points. The emergency path is a necessary safety valve but is frequently abused: a genuine emergency such as an actively exploited zero-day or a live production outage justifies it, but an emergency rate above ten to fifteen percent triggers an investigation into whether developers, or rubber-stamping managers, are routing around CAB review under the guise of urgency. Maintenance windows are the approved low-traffic deployment periods — overnight or on weekends — when the impact of a failure is minimized and staff are watching, so a change deployed outside the window without emergency authorization is a finding regardless of its technical content, and auditors flag it simply by comparing deployment timestamps to the window schedule.",
        "Two modern concerns close the loop. DevOps and CI/CD strain the human-review model, so ITIL 4 introduces the concept of a \"change authority\" that can be an automated system for low-risk, well-tested changes, and auditors then test whether the pipeline itself is a sufficient control — looking for enforced automated testing, required passing security scans, an immutable deployment audit log, and a halt-on-failure mechanism — because a well-configured pipeline can be more reliable than a manual CAB, but only when its configuration is itself audited. The post-implementation review documents whether the change met its purpose, surfaces any side effects, and records lessons and follow-ups; auditors verify that PIRs were completed on time, typically within five business days, and that issues were tracked to resolution, and PIRs that consistently surface unexpected side effects reveal a deeper problem — test environments that do not represent production.",
      ],
      technical: {
...
-- CHG-1003  jsmith    jsmith      2026-02-05  SOD VIOLATION  ← FINDING`,
  ...
  // ─── audit-06: BCP/DR ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2005,
      overview: [
        "Business Continuity Planning (BCP) and Disaster Recovery (DR) audits verify that an organization can keep critical operations running and recover its IT systems after a disruption — a natural disaster, ransomware, a data-center fire, or a vendor failure. CISA dedicates substantial coverage to this area precisely because these plans are systematically neglected: they are created during a compliance exercise, filed away in a shared drive, and never exercised again. An untested recovery plan is not a control but documentation of intent, and intent has never once restored a database.",
        "Two foundations define what recovery means and where it applies. The first is the set of metrics. Recovery Time Objective (RTO) is the maximum tolerable downtime before consequences become unacceptable; Recovery Point Objective (RPO) is the maximum acceptable data loss measured in time; and Maximum Tolerable Downtime (MTD) is the point of irreversible harm. The fundamental rule binding them is that RTO must be less than MTD, or the plan literally cannot save the business. The second foundation is the Business Impact Analysis, which auditors review first because it maps each critical process, its supporting systems, the impact of downtime, and the resulting RTO and RPO; a BIA that has not been updated since a major migration, cloud adoption, or acquisition is stale, so auditors test its currency by comparing its systems against the current production inventory.",
        "Strategy and testing must then demonstrably meet the approved targets. Recovery strategies range from hot standby — a fully operational duplicate offering near-zero RTO and RPO at high cost — through warm and cold standby, where cold is cheap but recovery takes days, to on-demand cloud failover, and auditors verify that the chosen strategy has actually demonstrated the approved RTO and RPO in testing rather than only on paper. Both tabletop and full DR tests are required, because they test different things: a tabletop is a discussion-based walkthrough that exercises logical structure, roles, and decisions without disruption, while a full DR test fails over to the recovery environment and verifies RTOs and RPOs in practice. A tabletop alone tests only the document, and a technical test alone tests the technology but not the human coordination around it.",
        "Two modern requirements have reshaped the audit. Ransomware broke the old assumption that backups simply survive: an attack encrypts production, backups, cloud-connected repositories, and anything else reachable from the compromised network, so audits now test for air-gapped or immutable backups, adherence to the 3-2-1 rule of three copies on two media types with one offsite, tested restoration from an isolated backup that was disconnected before the attack, and a ransomware-specific tabletop that explicitly includes backup compromise. Geographic separation is equally non-negotiable, as Hurricane Katrina showed: primary and backup sites must lie outside the same flood zone and far enough apart to survive a regional disaster, with FDIC and OCC guidance generally requiring around a hundred miles, and auditors check the location documentation for shared dependencies such as a common power-grid segment, ISP backbone, or seismic fault.",
      ],
      technical: {
...
- Customer Portal: not tested in 18 months — CRITICAL FINDING`,
  ...
  // ─── audit-07: Incident Response ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Incident Response program audits assess whether an organization can detect, contain, eradicate, and recover from security incidents consistently, at scale, and under the time pressure and confusion that real incidents create. ISACA auditors use NIST SP 800-61 as the framework, evaluating all four of its phases — Preparation, Detection and Analysis, Containment/Eradication/Recovery, and Post-Incident Activity — because a program that detects well but cannot contain, or contains but learns nothing, is not a mature one. Preparation is both the most auditable phase and the most frequently deficient, and it is where the documentary evidence lives: a current IR plan reviewed within the last twelve months, named individuals rather than job titles assigned to roles, contact lists that reach external parties such as legal, law enforcement, forensics, the cyber insurer, and PR, a severity matrix defining P1 through P4, notification protocols per severity, and tabletop exercises within twelve months whose action items were tracked to closure.",
        "Beyond preparation, two forces shape the program. In Detection and Analysis, auditors review the SIEM for log-source coverage of critical systems, for alert rules tuned to the top threats, and for managed false-positive rates — a SIEM firing thousands of alerts a day trains analysts to ignore them and is worse than none — and then sample historical incidents to see how each was actually detected, since detection that came from an external party such as law enforcement signals that internal detection is insufficient. The SEC's 2023 disclosure rules are the second force: a material incident must be disclosed within four business days and the risk-management program described, which makes IR a financial-reporting matter, so at public companies auditors assess whether the materiality determination is documented, whether the escalation from security to legal and finance is clear, and whether the four-day timeline is operationally achievable at all.",
        "Two operational measures define maturity. Mean Time to Detect is the interval from attacker activity to detection, and Mean Time to Respond is the interval from detection to containment; the IBM Cost of a Data Breach report shows that breaches contained within thirty days cost far less, financial-services regulators expect high-severity MTTD in hours rather than days, and an organization with a 78-day MTTD like Equifax has a fundamental failure regardless of how good its documentation looks. Playbooks are the other measure — step-by-step procedures for the top incident types of ransomware, data breach or exfiltration, insider threat, phishing with credential compromise, and DDoS, each specifying detection indicators, triage, containment, eradication, recovery, communication, and evidence-preservation steps — and auditors test them both against those criteria and against whether the IR team members can actually execute them under tabletop conditions.",
        "Log retention is the evidentiary foundation of the whole response. On detection, the IR team must answer when the attacker entered, what systems they touched, and what data they accessed or exfiltrated, and answering those questions requires logs reaching back across the full dwell period — typically ninety days online and immediately searchable, plus twelve months archived. Equifax's detection failure was partly a retention problem: an expired SSL inspection certificate meant the exfiltration traffic was never logged in a detectable form. Even with adequate retention, unreviewed logs or gaps left by misconfigured log shipping are not reliable evidence, so auditors verify that all in-scope systems ship their logs to the SIEM with no delivery gaps.",
      ],
      technical: {
...
-- Finding: Detection time exceeds benchmark — SIEM tuning required`,
  ...
  // ─── audit-08: Vendor Risk ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Vendor and third-party risk management (TPRM) audits verify that the organization oversees every vendor, supplier, and partner that accesses its systems, processes its data, or runs part of its IT. ISACA places this under COBIT's APO10, Managed Vendors, and it has become one of the fastest-growing areas of regulatory scrutiny worldwide. The premise is simple but operationally complex: your security posture is the product of your own controls and your vendors' controls, so a breach of your vendor is a breach of you — your data leaves through their door. The first stage of the lifecycle is therefore inventory, knowing every vendor including the shadow IT bought on a credit card outside procurement and the subcontractors of primary vendors, because organizations consistently undercount and the shadow-IT total alone can exceed the managed count; auditors test the inventory against accounts-payable records, asset inventories, network traffic, and user-reported SaaS.",
        "The next two stages tier the vendors and test their controls. Classification assigns a risk tier by data sensitivity, system criticality, dependency — how quickly the vendor could be replaced — the vendor's own regulatory status, and its incident history: a Tier 1 vendor is existential, a Tier 2 vendor processes business data or holds connectivity, and a Tier 3 vendor has no system access. Assessment then matches rigor to tier. A Tier 1 vendor warrants a questionnaire such as the Standardized Information Gathering (SIG) questionnaire, the vendor's SOC 2 Type II report as an independent attestation of controls operating over time, penetration-test and vulnerability-scan results, and an on-site review for the highest risk. A questionnaire on its own is insufficient for a Tier 1 vendor because it rests entirely on the vendor's self-reporting.",
        "The last two stages bind the relationship and then watch it between assessments. Contractual requirements are verified for security obligations, data protection covering encryption, access controls, and handling, a breach-notification timeline (GDPR's 72 hours, though many contracts require 24), the right to audit, data return or destruction at termination, and restrictions on subcontractor approval. Ongoing monitoring scales to tier: a Tier 1 vendor warrants continuous security-ratings monitoring through services like BitSight, SecurityScorecard, or RiskRecon, tracking of breaches, regulatory actions, and leadership changes, quarterly vendor-access reviews, and vendor security metrics; a Tier 2 vendor gets annual reassessment plus significant-event tracking; and a Tier 3 vendor may warrant only a contract-renewal review.",
        "Fourth-party risk — the risk from your vendor's vendors — is the hardest dimension and the one that supply-chain attacks most often exploit. SolarWinds succeeded partly because affected organizations never mapped that their Orion instance had broad network visibility, and never assessed what a compromise of Orion itself would do to them. Managing this risk requires understanding each Tier 1 vendor's key dependencies — which cloud providers it runs on, which software libraries it relies on, which subcontractors handle your data — and organizations that have not answered those questions carry a gap that attackers have repeatedly shown they will exploit.",
      ],
      technical: {
...
ORDER BY tier, days_since_assessment DESC;`,
  ...
  // ─── audit-09: Data Privacy ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2018,
      overview: [
        "Data privacy audits assess whether an organization collects, processes, retains, and deletes personal data in line with applicable privacy law — primarily GDPR in Europe and CCPA in California, plus sector rules such as HIPAA, GLBA, and COPPA. ISACA's privacy framework maps to GDPR's six lawful bases and eight data-subject rights, giving auditors a methodology that crosses jurisdictions, and the stakes are mainstream rather than niche: GDPR's maximum fine is €20 million or 4% of global annual turnover, whichever is higher. A comprehensive audit covers seven domains, and it starts with data mapping — what personal data is held, where it lives, how it flows, who has access, and the documented legal basis for it — because without that map no other control can be designed or tested.",
        "The remaining domains and the findings they generate are tightly linked. Consent mechanisms must be testable; the fulfillment of data-subject rights such as deletion, access, portability, rectification, and objection must work reliably and at scale within the required timeframes; and retention and deletion schedules must be documented, implemented in the systems themselves, and enforced either automatically or by regular review. The most common findings all track the gap between policy and reality: personal data retained past its documented period because a schedule exists but automated deletion does not; processing with no documented Article 6 lawful basis because new analytics, marketing automation, or data-sharing shipped without a privacy assessment; data-subject access requests missed beyond the 30-day window because the process is manual and untrained; and missing DPIAs for high-risk processing such as large-scale profiling, systematic monitoring, or handling of sensitive categories.",
        "Two structural elements are tested directly. The Data Protection Officer is required for large-scale processing of special categories, large-scale systematic monitoring, or public authorities, so auditors verify that the role exists where required, that it has genuine independence and does not report to those whose processing it oversees, that it has sufficient resources and expertise, and that it is involved in all data-protection matters — flagging a conflict when, for instance, the CISO also serves as DPO. The data map is documented as the Record of Processing Activities under GDPR Article 30, and it must list the controller and DPO contacts, the processing purposes, the categories of data subjects and personal data, the recipients including international transfers, the retention periods, and the security measures; it must also be current, so auditors test it against recent deployments, vendor onboardings, and IT data-flow diagrams to find flows that exist in reality but not in the RoPA.",
        "Two forward-looking requirements round out the scope. International transfers — moving personal data outside the EEA to countries without an adequacy decision — require Standard Contractual Clauses (updated in 2021), Binding Corporate Rules, or an approved certification scheme, and the Schrems II invalidation of Privacy Shield in 2020 made US-EU transfers genuinely complex, so auditors verify that every EEA-to-US flow is identified, that a valid mechanism is in place, and that Transfer Impact Assessments are conducted where required. Privacy by Design is the other requirement, calling for privacy controls built in from the start and tested through the SDLC: is privacy review a mandatory pre-production checkpoint, are minimization, pseudonymization, and encryption considered at the design stage, is data collection opt-in by default, and are retention periods configured at deployment rather than bolted on after data has already accumulated? An organization that keeps discovering privacy violations in existing systems rather than preventing them has a Privacy by Design gap that will keep recurring as findings.",
      ],
      technical: {
...
-- Retention schedule: 730 days (2 years)  → EXCEEDS BY 1,632 DAYS — FINDING`,
  ...
  // ─── audit-10: Pen Test Scoping ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2003,
      overview: [
        "Penetration testing is a critical control in any mature security program — a controlled adversarial simulation that validates whether the defenses actually work in practice rather than only on paper. ISACA auditors review pentest programs to confirm that they deliver genuine assurance rather than compliance theater, and that the authorization structure protects both the organization and the testers. The line between a security professional and a criminal is defined entirely by the scope of written authorization: the very same technical actions taken without it are federal crimes under the Computer Fraud and Abuse Act (CFAA) and its equivalents elsewhere.",
        "Two documents make a test legal, and both must come from the right authority. The scope document is the legal foundation. It names exactly which systems are in scope — IP ranges, domains, applications, environments — which testing types are permitted, from external and internal to web-application, social-engineering, physical, and wireless, the precise testing window and any blackout periods, and the escalation procedures for critical findings; out-of-scope systems must be listed explicitly, because prosecutors apply the most conservative reading of authorization. Authorization itself must flow from the system owner — the business or legal entity responsible — and not from IT management alone when business units own the systems, and cloud workloads require the provider's approval as well, since AWS, Azure, and GCP each publish pentest policies defining what is allowed without notice and what needs advance approval.",
        "The next layer operationalizes the scope and protects the report it produces. The rules of engagement define how testing is conducted where the scope defines what may be tested: the authorized tools and methods, the permitted versus prohibited techniques (denial-of-service may be explicitly barred on production), the handling of any credentials discovered, the collection and retention of evidence, the protocol for communicating urgent findings, and what the tester must do upon finding an active real intrusion, which happens more often than clients expect. The test report is itself security-sensitive — effectively a roadmap of working exploits and paths to critical findings — so its distribution is restricted to authorized recipients, typically the CISO and a few IT leaders, transmitted securely by encrypted email or a secure portal, and stored in an access-controlled repository rather than on a general shared drive.",
        "Two final practices determine whether the test produces lasting value. The first is remediation tracking: a kickoff within five business days, a plan with owners and due dates set by severity — critical within 30 days, high within 60, medium within 90 — and a retest against the specific remediated vulnerabilities rather than a general rescan, because that retest is the actual evidence of effective remediation. The second is understanding how a red team engagement differs from a pentest. A pentest has a defined scope, a find-and-report methodology, and a two-to-four-week duration, whereas a red team simulates an advanced persistent threat toward a defined objective such as reaching the most sensitive data or simulating ransomware, using any tactics, techniques, and procedures including social engineering, physical intrusion, and supply chain, running for months and requiring extra authorization for human targets, facilities, and supplier relationships — authorization that auditors verify was explicitly granted.",
      ],
      technical: {
...
def is_authorized(target_ip):
  ...
  // ─── audit-11: Audit Evidence ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1972,
      overview: [
        "Audit evidence is the information auditors collect and evaluate to support a conclusion about whether controls operate effectively, and ISACA requires it to satisfy three attributes at once. It must be sufficient — enough of it, with an adequate sample size, an appropriate period covered, and a well-defined population. It must be reliable — drawn from a credible source through a sound method and not manipulated. And it must be relevant, actually testing the control in question, because evidence that a backup ran does not test whether that backup is restorable; failure on any one of the three invalidates the conclusion. When testing every item is impractical, sampling tests a representative subset: across 1,200 change tickets, statistical sampling gives every item a known non-zero chance of selection and projects the result with measurable precision, while judgmental, non-statistical sampling leans on auditor expertise but can describe only the sample itself and cannot project to the population.",
        "Two foundations govern how evidence is gathered and trusted. Attribute sampling, the most common statistical method in IT audit, tests a binary attribute — the control either operated or it did not — by defining the population (all Q1 2026 change tickets), the attribute (each ticket has CAB approval before deployment), the tolerable deviation rate (the maximum failures that still allow an \"effective\" conclusion, typically 5–10%), the confidence level (95% is standard), and the expected deviation rate (a prior estimate, assumed to be 0% if unknown), after which statistical tables set the minimum sample size. Chain of custody is the second foundation, documenting who collected each piece of evidence, when, from what source, by what method, and how it has been stored since — essential because workpapers may later support legal proceedings, examinations, or litigation where authenticity is challenged, which is why digital evidence adds hash values such as MD5 or SHA-256 at collection, authoritative timestamps, and storage that records every access.",
        "Two further practices scale and standardize the work. Computer-Assisted Audit Techniques automate evidence collection and analysis — data tools such as ACL/Galvanize, IDEA, and SQL, log platforms such as Splunk and Elastic, and configuration-comparison and reconciliation tools — letting auditors test whole populations rather than samples, run analyses that would be infeasible by hand such as comparing every payroll disbursement against the current employee roster, and produce reproducible evidence, since the same CAAT run on the same data yields the same result. Workpaper documentation standards require, for each piece of evidence, the test objective, the population and sample definition, the method, the evidence examined with source references, the results including any deviations, and a conclusion — and the test of adequacy is that another auditor with no prior knowledge of the engagement could understand exactly what was tested, how, and what was found, so a workpaper that needs an oral explanation is not adequately documented.",
        "An evidence-quality hierarchy guides how much weight to place on each type. Physical evidence — where the auditor personally observes the control operating, by sitting in on a CAB meeting, watching a restoration, or extracting data directly — ranks highest, because it cannot be fabricated after the fact. Documentary evidence — logs, reports, screenshots, signed documents, and system extracts — ranks second, and is reliable when it is pulled directly from the source system rather than handed over by management. Testimonial evidence — representations made in interviews — ranks lowest, because it depends entirely on the speaker's truthfulness, so auditors must remain skeptical of any management claim that physical or documentary evidence does not support, especially for high-risk controls.",
      ],
      technical: {
...
if sample_deviation_rate > 0.05:  # exceeds 5% tolerable rate
  ...
  // ─── audit-12: Audit Reporting ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1978,
      overview: [
        "The audit report is the formal deliverable that turns weeks of testing, evidence, and analysis into actionable information for leadership, the audit committee, and the board. ISACA's IS Audit Standards 1401-1405 require it to include the audit scope and objectives, the period covered, the methodology, the findings with their supporting evidence and risk ratings, management's response to each finding, and the agreed remediation timelines. It is not a list of problems but a communication document, and its quality is measured by whether it produces effective remediation, which is why each finding must use the Condition-Criteria-Cause-Effect framework: the Condition is what was found, stated specifically, measurably, and objectively; the Criteria is what the standard or policy requires; the Cause is the root cause rather than the symptom; and the Effect is the business risk if it goes unremediated. Omit any one element and management cannot remediate while the board cannot assess the risk.",
        "Two elements determine whether findings actually drive action. The risk rating must be consistent and defensible — Critical, High, Medium, or Low based on likelihood, the magnitude of financial, regulatory, reputational, and operational impact, the breadth of systems affected, and any compensating controls — and the rating sets the clock: Critical findings escalate immediately to the audit committee without waiting for the 30-day cycle, High findings remediate within 30 to 60 days, Medium within 90, and Low may be accepted with documentation. Management responses, in turn, must substantively engage rather than defend. A bare \"we disagree\" that never addresses the factual basis of the condition is inadequate, and \"we will remediate by [date]\" with no specific actions is not actionable; a good response acknowledges the finding's accuracy or disputes it specifically, identifies the root cause, lists remediation actions with owners and dates, and notes any interim compensating controls — and auditors review the responses before finalizing, requesting revision where needed.",
        "Two operational controls protect the report and then close it out. Distribution is a security requirement in its own right, because the report details control weaknesses that are valuable to an attacker: it is restricted to those with a legitimate need — the audit committee, the board for significant or material findings, the CISO, the CIO, and process owners — transmitted by encrypted email or a secure portal rather than standard email, stored under access control with tracking, and any external request from regulators or external auditors is routed through legal counsel. Remediation tracking closes the loop that many organizations leave open: a centralized system records every open finding with its rating, agreed date, owner, and status, escalates automatically as due dates approach, runs 30/60/90-day follow-ups that verify progress with evidence, and requires both management attestation and audit-team verification before any finding is formally marked closed.",
        "The relationship with the audit committee is the governance structure that gives internal audit its independence and its authority. The audit committee, made up of independent board members, is internal audit's primary client rather than management, and the Chief Audit Executive reports to it rather than to the CEO or CFO, so management cannot suppress or delay a report about its own weaknesses. Disputed findings are adjudicated there — the auditor presents the evidence and management presents its position — and when findings linger unresolved or management repeatedly fails to remediate, the committee can escalate by requiring an independent review, engaging external auditors, or taking disciplinary action.",
      ],
      technical: {
