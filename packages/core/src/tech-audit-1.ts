...
export const techAudit1Stages: StageConfig[] = [
  // ─── audit-01: COBIT 2019 ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "COBIT 2019 is ISACA's flagship framework for IT governance and management, underpinning almost every IT audit methodology in regulated industries. It draws a precise, legally significant line between governance — which sets direction, evaluates options, and monitors outcomes — and management, which plans, builds, runs, and monitors in line with that direction. The distinction isn't academic, because regulators and courts have assigned personal liability to board members who abdicated governance and relied entirely on management self-reporting.",
        "Every audit starts by establishing that boundary, then tracing how intent flows through it. The board and executives govern by approving risk appetite, setting strategy, authorizing major IT investments, and ensuring accountability, while IT leadership manages by implementing controls, operating systems, and reporting performance upward within board-approved parameters. Auditors hold an independent third position, verifying both layers and reporting to the audit committee, not management, to preserve independence. The governance cascade runs board principles → management policies → operational procedures → technical controls, so auditors trace it in reverse, which means a control that can't be traced up to policy authority (or a policy never implemented as a control) is a governance gap.",
        "The framework organizes 40 objectives across five domains, tailored by design factors. EDM (Evaluate, Direct, Monitor) is board governance covering risk appetite, IT investment, and performance monitoring, while APO (Align, Plan, Organise) handles strategy, architecture, risk, and HR planning, BAI (Build, Acquire, Implement) covers projects, change, and requirements, DSS (Deliver, Service, Support) covers operations, incidents, and continuity, and MEA (Monitor, Evaluate, Assess) covers internal controls, compliance, and board reporting. The twelve design factors — enterprise strategy, risk profile, compliance requirements, sourcing model, IT methods, and more — tailor the framework to context, because a regulated bank running its own data center has a different target profile than a cloud-native startup, so auditors establish the applicable factors before assessing capability.",
        "Two practical points dominate real engagements. COBIT is a governance framework, not a technical standard, so it prescribes no implementations but maps to NIST CSF, ISO 27001, ITIL, and TOGAF, which means auditors routinely find strong NIST/ISO technical controls that aren't connected to board-level accountability. The most common findings are in EDM — EDM01 (no formal governance charter), EDM03 (no approved risk-appetite statement, so risk decisions are made ad hoc without board authority), and EDM05 (IT reporting to the board absent or all technical metrics the board can't evaluate) — which together form a pervasive governance failure that undermines every other control.",
      ],
      technical: {
...
APO12 Risk Mgmt      2        4      2    HIGH`,
  ...
  // ─── audit-02: Risk Assessment — CRISC ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "CRISC (Certified in Risk and Information Systems Control) is ISACA's risk-focused certification and methodology. It defines a four-domain approach spanning IT Risk Identification, IT Risk Assessment, Risk Response and Mitigation, and Risk and Control Monitoring and Reporting. Unlike compliance frameworks asking 'did we implement the required control?', CRISC asks the prior question: 'what risks are the controls meant to address, and are they actually reducing those risks to acceptable levels?'",
        "The first two domains define what to manage and how exposed you are. Risk identification is the most under-resourced domain, because registers populated in one workshop years ago go stale, so CRISC requires continuous identification as new systems, vendors, regulations, and threats appear, and auditors test it by comparing the register against change logs, vendor onboardings, threat intel, and incident databases for risks that should be there but aren't. Risk assessment then runs a standard method: identify threats and vulnerabilities, score inherent risk (before controls), evaluate control effectiveness, calculate residual risk (after controls), and compare residual to board-approved appetite, since the inherent-to-residual gap is the control effectiveness, which must be large enough to bring residual within appetite.",
        "Two artifacts anchor the rest of the work, the first being the four risk responses — Accept (board formally documents and absorbs the loss; undocumented acceptance is invalid), Mitigate (add or strengthen controls), Transfer (shift financial exposure via insurance, indemnification, or outsourcing without eliminating the risk), and Avoid (discontinue the activity — most complete, often impractical). The second is the risk register, which auditors scrutinize most, because a mature one carries a unique ID, threat/vulnerability narrative, affected asset, a named risk owner, inherent score (likelihood × impact), controls with references, control-effectiveness rating, residual score, response type and treatment plan, the appetite threshold, and last-review date, so any missing field is itself a finding because the organization then can't demonstrate it's managing the risk.",
        "Two things determine whether the program is real. Risk appetite must be board-approved and quantitative, because 'we have a low appetite for cybersecurity' gives no threshold, whereas 'no residual score above 12 on the 5×5 matrix without a board-approved exception; scores of 16+ escalate immediately to the Audit Committee' is usable. The most consequential finding is residual risk above appetite with no treatment plan, which means the organization knows it's over its own threshold and is doing nothing, so regulators treat this as management negligence — FDIC/OCC issue Matters Requiring Attention, and external auditors may call it a significant deficiency or material weakness under SOX 404.",
      ],
      technical: {
