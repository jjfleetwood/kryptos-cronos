...
export const spaceRace2Stages: StageConfig[] = [
  // ─── s2-01: Anatomy of a Space Mission & SPARTA (Quiz) ───────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Every space mission has three segments:\n- the space segment (the satellite — its bus, the housekeeping platform, and its payload, the mission instruments)\n- the ground segment (mission control, ground stations, and the operations network)\n- the link segment (the radio between them: the uplink of commands and the downlink of telemetry and data)\nEach is a distinct attack surface, and the link ties them together.",
  ...
  // ─── s2-06: Breaching the Ground Segment (CTF) ───────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "That makes the ground segment the most attractive and, repeatedly, the weakest link. It's a terrestrial network subject to every familiar threat:\n- phishing and credential theft\n- unpatched servers\n- exposed remote access\n- weak segmentation\n- supply-chain compromise of ground software or modems\nThe space-specific defenses (SDLS, encryption) don't help if the attacker is operating from inside mission control.",
      ...
      technical: {
        ...
        body: [
          ...
          "Defense is mostly excellent terrestrial security applied to a high-stakes target, plus space-specifics:\n- strong identity/MFA and least privilege for operators\n- network segmentation isolating the operations network\n- patching and hardening\n- monitoring for anomalous commanding\n- secure software-update practices for ground equipment\n- supply-chain assurance\n- key protection (HSMs) so command-authentication keys can't be stolen\nTreating mission control like critical infrastructure — because it is — is the lesson. In this challenge you'll compromise the ground station, then use its authority to command the satellite.",
        ],
      },
      incident: {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Raising the floor means applying real engineering:\n- authenticated commanding (SDLS) even on small budgets\n- encrypted links\n- secure boot and signed updates on the flight software\n- hardened and segmented ground systems\n- supply-chain scrutiny of buses/radios/software\nInitiatives like the U.S. Air Force/Space Force 'Hack-A-Sat' CTFs and the broader space-ISAC community drive awareness and better defaults. The message: democratized space must not mean insecure space — and at constellation scale, systemic security is everything.",
        ],
      },
      incident: {
  ...
  // ─── s2-08: Inter-Satellite Links & Constellation Pivot (CTF) ────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The same networking lens brings the same defenses:\n- authentication between nodes\n- encryption of ISL traffic\n- segmentation and least privilege so one compromised satellite can't freely command others\n- integrity of routing\n- anomaly detection across the constellation\nAs constellations grow into a true space-based internet, treating them with mature network-security principles — zero trust between satellites — becomes essential. In this challenge you'll hop an inter-satellite link and pivot across the constellation.",
      ],
      technical: {
  ...
  // ─── s2-10: Securing Space Systems (Quiz) ────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Two principles unify it. Build security in despite the constraints: even small, cheap, long-lived spacecraft can have authenticated links and signed updates if designed for it from the start. And resilience over perfection:\n- because you can't physically reach a satellite\n- you plan to survive attacks — redundancy\n- graceful degradation\n- PNT backups\n- constellation diversity\n- tested recovery — so a hack or jam degrades rather than destroys the mission",
        "Frameworks and governance tie it together: SPARTA to model threats and map defenses, the Space-ISAC for threat sharing, government guidance (U.S. Space Policy Directive-5 and related cyber requirements), CCSDS security standards, and international norms on responsible behavior and debris. The conclusion mirrors the OT, vehicle, and robotics arcs:\n- secure the links\n- harden the ground\n- design for resilience\n- verify everything\n- and govern a shared\n- hard-to-reach domain for the long haul",
      ],
      technical: {
