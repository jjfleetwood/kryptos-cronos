...
export const rangeAdStages: StageConfig[] = [
  // ─── Lab 1: BloodHound ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2016,
      overview: [
        "Active Directory is the identity and access backbone of most enterprises, and its complexity is its weakness: thousands of users, groups, nested memberships, access-control entries, sessions, and delegations create an enormous, tangled web of privilege relationships that no admin can reason about by hand. Attackers exploit exactly that. BloodHound, released by SpecterOps in 2016, collects this data with a 'SharpHound' collector and loads it into a Neo4j graph database, then answers the question that matters: what is the shortest path from the low-privilege account I just compromised to Domain Admin? It turns AD privilege into a map you can query.",
        "The paths BloodHound surfaces are the AD attack catalog: nested group memberships that quietly grant admin, dangerous ACLs (GenericAll, WriteDACL, ForceChangePassword) that let one principal control another, sessions where a privileged user is logged into a machine you control (so you can steal their credentials), and Kerberos delegation that can be abused. Its 'pre-built queries' literally include 'Shortest Path to Domain Admins.' Critically, BloodHound is dual-use and arguably more valuable to defenders: blue teams run it to find and remove these attack paths before an attacker walks them. The defensive program is attack-path management — tier your admin model so high-privilege accounts never log into low-trust machines, enforce least privilege, strip dangerous ACLs, and re-run BloodHound to verify the paths are gone.",
      ],
      technical: {
