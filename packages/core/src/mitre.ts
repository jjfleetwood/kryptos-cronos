...
export const mitreStages: StageConfig[] = [
  // ─── mitre-01: Reconnaissance ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "Reconnaissance (TA0043) is the opening tactic of MITRE ATT&CK, and it's the one defenders most often forget is even happening — because it can run for weeks or months before a single exploit fires. Long before the attack, the adversary quietly assembles a dossier: employee names and roles from LinkedIn and company directories, email-address formats recovered from tools like Hunter.io, the organization's technology stack mapped through job postings, Shodan, and BuiltWith, exposed infrastructure surfaced via Censys and FOFA, and already-leaked credentials pulled from HaveIBeenPwned and dark-web forums. By the time anything 'attacks,' the adversary often knows the target better than its own IT team does.",
        "ATT&CK splits reconnaissance into two modes that differ sharply in risk to the attacker. Active scanning (T1595) sends probes directly at the target's infrastructure — thorough and detailed, but capable of tripping IDS alerts and leaving traces. Passive gathering (T1589, T1591, T1592) instead queries public sources — search engines, social media, registries — without ever touching the target's systems, which makes it essentially invisible. A patient adversary does as much as possible passively, only switching to active probing once they know exactly where to look.",
        "Because recon largely happens off your network, defenders counter it by shrinking the attack surface it feeds on. Strip metadata from public documents (T1592.002) so published files stop leaking employee names and internal tooling; watch for the active-scanning that does touch you with honeypots and firewall-log analysis; and use threat-intelligence feeds to spot adversary infrastructure being stood up against you before the rest of the kill chain begins. You can't stop someone from reading LinkedIn — but you can control how much your own footprint hands them for free.",
      ],
      technical: {
...
Fragment-3: APT29}`,
  ...
  // ─── mitre-02: Resource Development ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Resource Development (TA0042) is the attacker's logistics phase — everything they build or buy before striking — and sophisticated groups invest weeks in it the way a military plans a campaign. The arsenal includes command-and-control (C2) infrastructure, exploit tools, and custom malware, but also the quieter assets that make later stages succeed: compromised accounts to launch from and code-signing certificates that make a payload look like trusted, legitimately-signed software. This phase produces nothing that touches the victim yet, which is exactly why it's so often missed.",
        "Four techniques describe how that arsenal gets sourced. T1583 (Acquire Infrastructure) is buying VPS servers, domains, and cloud accounts — often with stolen cards or cryptocurrency to stay anonymous. T1584 (Compromise Infrastructure) is hijacking someone else's legitimate websites or servers to relay C2 traffic, hiding behind a trusted source. T1587 (Develop Capabilities) is building custom malware and zero-days in-house, the hallmark of well-resourced nation-state groups. And T1588 (Obtain Capabilities) is simply purchasing ready-made tools, exploits, and access from dark-web markets — the commoditized path that puts serious capability within reach of lesser actors.",
        "The crucial defensive insight is that this build-out is detectable *before* it's ever used against you. Threat-intelligence feeds flag typosquatted, lookalike domains the moment they're registered; Certificate Transparency logs publicly expose newly issued TLS certificates for those domains; and passive DNS monitoring catches C2 infrastructure resolving and standing up ahead of an attack. An organization that watches for its own name being impersonated can sometimes see the weapon being forged — and brace for, or disrupt, the campaign before the first phish lands.",
      ],
      technical: {
...
Fragment-3: C2}`,
  ...
  // ─── mitre-03: Initial Access ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2016,
      overview: [
        "Initial Access (TA0001) is the moment everything before it pays off — the attacker's first foothold inside the target — and one technique dominates the category so thoroughly it's almost the whole story. Spear phishing (T1566) appears in over 90% of nation-state intrusions according to Verizon DBIR data, and decades of security spending haven't displaced it. The reason is uncomfortable: phishing targets people, not systems, and you can't patch a human. Every firewall and EDR in the world is bypassed the instant a trusted employee chooses to click.",
        "Spear phishing is precision-built from the reconnaissance phase so the message arrives looking entirely trusted — the right sender, the right context, the right ask. It comes in variants: T1566.001 delivers a malicious attachment, T1566.002 a malicious link, and T1566.003 phishes through services like Slack, Teams, or LinkedIn rather than email, sidestepping email security entirely. In each case the payload fires the moment the victim opens or clicks, converting a convincing message into code execution on an internal machine.",
        "A few other footholds round out the tactic for when phishing isn't the route. T1190 (Exploit Public-Facing Application) goes straight at internet-exposed systems with SQL injection or a web shell. T1133 (External Remote Services) walks in through VPN or RDP using stolen or purchased credentials — no exploit required. And T1195 (Supply Chain Compromise) is the SolarWinds route: compromise a trusted vendor and let their software update carry you inside thousands of targets at once. However the attacker arrives, the tactic ends the same way — with execution on a system that's now theirs to build from.",
      ],
      technical: {
...
# Fragment-3: 1N1T14L}`,
  ...
  // ─── mitre-04: Execution ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2017,
      overview: [
        "Execution (TA0002) is how the attacker actually runs code once they're inside, and here too a single tool dominates: T1059 (Command and Scripting Interpreter), above all its PowerShell sub-technique (T1059.001). PowerShell is the perfect weapon precisely because it isn't one — it ships on every Windows machine, is trusted and used constantly by administrators, and so a malicious PowerShell command rarely looks out of place against the background of legitimate ones. The attacker doesn't smuggle in a tool; they turn the defender's own administration language against them.",
        "This is the essence of 'living off the land,' and the LOLBins (living-off-the-land binaries) make it systematic: signed, built-in Microsoft tools bent to run attacker code. PowerShell, WMI, mshta.exe, regsvr32.exe, certutil.exe, and rundll32.exe are the usual suspects — each a legitimate utility with a side use an attacker can exploit (certutil downloads files, mshta runs remote HTA scripts, regsvr32 loads COM objects from URLs). Because every one of them is Microsoft-signed, they slip straight past application whitelisting and many antivirus products that trust the publisher.",
        "Since the binaries themselves are legitimate, detection has to lean on behavior rather than signatures — on *how* the tool is used, not *whether* it's allowed. PowerShell launched with `-EncodedCommand` (a base64 blob) is a classic malware tell, as is PowerShell reaching out to the internet to pull and run code (`IEX (New-Object Net.WebClient).DownloadString`). WMI event-subscription creation and execution-policy-bypass flags round out the indicators. The defensive shift this tactic forces is fundamental: stop asking 'is this program malicious?' and start asking 'is this legitimate program doing something it never should?'",
      ],
      technical: {
...
Fragment-3: L0LB1N}`,
  ...
  // ─── mitre-05: Persistence ────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2014,
      overview: [
        "Persistence (TA0003) answers a problem every intruder faces: a foothold is fragile. A reboot, a password reset, a closed laptop lid, and the attacker's shell is gone — and without persistence, that means starting the whole intrusion over from initial access. With it, the attacker survives reboots, credential changes, and even some remediation attempts, holding their position for the weeks or months a real campaign takes. Persistence is what turns a momentary compromise into a durable presence.",
        "The techniques all share one logic: plant a relaunch trigger somewhere the operating system will faithfully honor on its own. T1547.001 abuses Registry Run keys (`HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run`) that auto-launch programs at logon; T1053.005 creates Scheduled Tasks via schtasks.exe; T1543.003 installs a malicious Windows Service with sc.exe; T1037 hooks boot/logon scripts; and T1505.003 drops a web shell on a web server for a foothold that survives entirely server-side. In every case the attacker borrows a legitimate auto-start mechanism and points it at their own code.",
        "Because persistence means *adding* something to the system's startup state, detection means knowing what that state should look like and watching for the additions. Monitor Registry Run keys for new entries, audit scheduled-task creation (Windows Event ID 4698) and new service installs (Event ID 7045), and — most powerfully — keep a baseline of expected startup items using Autoruns (Sysinternals) or an EDR's persistence inventory, so anything new stands out instantly. The attacker has to leave a hook behind to come back; disciplined baselining is how you find the hook.",
      ],
      technical: {
...
Fragment-3: L4Y3R}`,
  ...
  // ─── mitre-06: Privilege Escalation ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Privilege Escalation (TA0004) is the climb. Attackers rarely land where they want to be — a phish typically gives them a standard user's access — so they push upward: from standard user to local administrator, then to SYSTEM on Windows or root on Linux, and in an Active Directory environment, ultimately to Domain Admin, the keys to the entire kingdom. Each rung unlocks capabilities the one below couldn't reach, and the whole rest of the intrusion depends on winning this climb.",
        "The escalation paths abuse one of three things: credentials, bugs, or tokens. T1078 (Valid Accounts) is the cleanest — just reuse legitimate credentials harvested earlier, no exploit needed. T1068 (Exploitation for Privilege Escalation) weaponizes unpatched kernel or service flaws to jump privilege levels. T1134 (Access Token Manipulation) steals or impersonates the access tokens of higher-privileged processes (like a SYSTEM token). And T1484 (Domain Policy Modification) abuses Group Policy itself to grant the attacker rights across the domain.",
        "Active Directory, the backbone of most enterprises, offers especially sharp escalation routes that turn a modest foothold into total control fast. DCSync impersonates a domain controller to *replicate* the AD database and walk away with every account's password hash. PrintNightmare (CVE-2021-34527) exploited the print spooler to run code as SYSTEM, and AS-REP Roasting targets accounts configured without Kerberos pre-authentication — either of which can carry a standard domain user straight to Domain Admin. These AD-specific paths are why credential hygiene and tiered administration matter so much: in a domain, a single escalation flaw can mean game over.",
      ],
      technical: {
...
Fragment-3: DA}`,
  ...
  // ─── mitre-07: Defense Evasion ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "Defense Evasion (TA0005) is ATT&CK's broadest tactic — 42 techniques and counting — because staying unseen is the work that runs alongside everything else. Its sub-techniques span the full repertoire of hiding: T1027 (Obfuscated Files or Information) conceals payloads through encoding, encryption, and packing; T1055 (Process Injection) runs malicious code inside the memory of a legitimate process so it inherits that process's trust; T1070 (Indicator Removal) clears the logs that would betray the intrusion; T1562 (Impair Defenses) disables AV and EDR outright; and T1218 (Signed Binary Proxy Execution) hides behind trusted system binaries. Evasion is less a step than a constant discipline the attacker applies to every other action.",
        "Those signed system binaries — the LOLBins again — are central to evasion because application whitelisting trusts them by publisher. mshta.exe runs HTA scripts straight from a URL; regsvr32.exe loads COM objects from a remote URL; certutil.exe downloads and decodes files; and rundll32.exe executes DLLs directly. Each is a Microsoft-signed tool doing exactly what it's designed to do, which is precisely why a defense that trusts the signature waves it through.",
        "Modern evasion has escalated to attacking the security tooling itself — going after the watcher. Attackers kill EDR processes and unload its kernel driver to blind it, and the sharpest version is BYOVD (Bring Your Own Vulnerable Driver): the attacker loads a legitimately-signed but vulnerable driver, then exploits it to operate from the kernel and patch or disable the EDR from below the level it can defend. This isn't fringe — Lazarus Group and the ALPHV (BlackCat) ransomware operators have used BYOVD in real campaigns. When the adversary can fight from the kernel, defenders need protections (like driver block-lists and tamper protection) that assume the sensor itself is under attack.",
      ],
      technical: {
...
Fragment-3: BYOVD}`,
  ...
  // ─── mitre-08: Credential Access ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2017,
      overview: [
        "Credential Access (TA0006) is the theft of usernames and passwords, and it may be the single most valuable thing an attacker can obtain — because valid credentials are a skeleton key. One stolen admin credential simultaneously enables lateral movement, privilege escalation, and persistence, and it does all three while generating only legitimate-looking authentication events. The defender sees a valid user logging in normally; the attacker sees a door that every lock in the building will open for. This is why so much of the attack lifecycle bends toward stealing credentials rather than exploiting bugs.",
        "The techniques pull credentials from wherever they live — memory, tickets, files, and keystrokes. T1003 (OS Credential Dumping) reads them out of memory, classically with Mimikatz scraping NTLM hashes and plaintext passwords from the LSASS process. T1558 (Steal or Forge Kerberos Tickets) attacks the Kerberos protocol itself — Pass-the-Ticket, the forged 'Golden Ticket,' and Kerberoasting. T1552 (Unsecured Credentials) simply finds passwords developers left lying around in scripts, config files, and environment variables. And T1056 (Input Capture) keylogs them straight from the user's fingers.",
        "Mimikatz (T1003.001) is the workhorse that made this tactic infamous. It extracts NTLM hashes from LSASS memory, and where the legacy WDigest protocol is enabled, it recovers plaintext passwords directly — no cracking required. Modern Windows disables WDigest by default, closing that plaintext path, but an attacker with admin rights can re-enable it with a single registry tweak and then wait for users to log in. The cat-and-mouse here — Microsoft hardening LSASS, attackers finding new ways in — is a microcosm of the whole credential-access fight, and it's why protections like Credential Guard and LSASS protection exist.",
      ],
      technical: {
...
Fragment-3: DUMP}`,
  ...
  // ─── mitre-09: Lateral Movement ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "Lateral Movement (TA0008) is how an attacker travels from the one machine they first compromised to the machines they actually want — because the phished laptop is rarely the goal. The real targets are domain controllers, file servers, databases, backup systems, and in industrial settings the OT/ICS network. Each hop expands access toward those high-value systems, and a patient adversary may make many quiet hops over days, mapping and pivoting until they're sitting on exactly the system that holds the data or the leverage they came for.",
        "The defining feature of modern lateral movement is that it reuses stolen authentication instead of re-exploiting each new host — which is what makes it so quiet. T1550.002 (Pass-the-Hash) authenticates with a captured NTLM hash directly, and T1550.003 (Pass-the-Ticket) replays a stolen Kerberos ticket; neither needs to crack anything. From there the attacker rides legitimate remote-access channels — T1021.001 (RDP) and T1021.002 (SMB/admin shares) — and can use T1080 (Taint Shared Content) to poison files on a network share so the malware spreads as colleagues open them.",
        "Pass-the-Hash deserves special emphasis because it sidesteps password cracking entirely: an administrator's NTLM hash will authenticate to *any* system where that account has admin rights, exactly as if the attacker knew the password. Tools like `net use`, wmiexec, or Impacket's psexec.py turn that hash into a remote shell on the next machine in seconds. This is why credential hygiene scales catastrophically in flat networks — one reused local-admin password, captured once, becomes a master key to every box that shares it, and the defense (unique credentials, network segmentation, tiered admin) has to break that chain deliberately.",
      ],
      technical: {
...
Fragment-3: PTH}`,
  ...
  // ─── mitre-10: Discovery + Collection ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Discovery (TA0007) is the attacker getting their bearings inside a network they don't yet understand. Having landed somewhere, they map what exists — usually in parallel with collecting whatever they find. T1083 (File and Directory Discovery) and T1082 (System Information Discovery) inventory the local machine; T1016 (System Network Configuration Discovery) and T1018 (Remote System Discovery) scan outward to chart the rest of the network from the inside, where firewalls don't apply. Discovery is reconnaissance run from *within* — and it's how the attacker decides where to move next.",
        "Collection (TA0009) is the harvesting that follows, once the valuable data has been located. T1005 (Data from Local System) and T1039 (Data from Network Shared Drive) pull files off endpoints and shares; T1114 (Email Collection) exports entire mailboxes; and T1119 (Automated Collection) scripts the process at scale, sweeping up everything matching patterns like *.docx, *.xlsx, and *.pst. The attacker is no longer exploring — they're packing the moving truck with whatever they came to take.",
        "Advanced persistent threats invest heavily in this phase before they ever exfiltrate, and the patience is deliberate. They take time to learn the network topology and pinpoint the high-value data repositories, then *stage* what they've collected as small, compressed, and encrypted archives — chunked precisely to stay below the thresholds that data-loss-prevention (DLP) and anomaly detection watch for. By the time data actually leaves the network in the next tactic, it's already been quietly located, gathered, and prepared to slip out unnoticed.",
      ],
      technical: {
...
Fragment-3: APT29}`,
  ...
  // ─── mitre-11: Exfiltration ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2017,
      overview: [
        "Exfiltration (TA0010) is the actual theft — moving the collected data off the target network — and the hard part is not taking the data but smuggling large volumes of it out without being seen. The defender's outbound controls are watching: network DLP inspects what leaves, bandwidth monitoring flags unusual transfer sizes, and anomaly detection notices when a workstation suddenly starts shipping gigabytes to the internet. Exfiltration is therefore an exercise in disguise, not brute force — the attacker has the data; the challenge is getting it past the gate.",
        "So the techniques all work by hiding the outbound flow inside ordinary-looking traffic. T1048 (Exfiltration Over Alternative Protocol) tunnels data through DNS or ICMP, or sends HTTP/S to attacker-controlled domains, abusing protocols that have to be allowed out. T1041 (Exfiltration Over C2 Channel) simply rides the same encrypted channel the attacker already uses for command-and-control, so no new connection raises suspicion. And T1567 (Exfiltration Over Web Service) uploads to Dropbox, Google Drive, or Mega — services so normal in a modern workplace that the traffic blends right in.",
        "Beyond the channel, timing is the other half of staying invisible. Sophisticated actors trickle data out slowly, over days or weeks, deliberately staying below the anomaly thresholds that a fast bulk transfer would trip. DNS tunneling is the archetype of the patient approach: it's painfully slow, but DNS is almost never blocked by enterprise firewalls (the network would break without it), which makes it the preferred covert channel for long-term, low-and-slow theft. The lesson for defenders is that exfiltration won't look like an alarm going off — it'll look like a little extra DNS, every day, for a month.",
      ],
      technical: {
        title: "Exfiltration Detection — Network Layer",
        body: [
          "DNS exfiltration (T1048.003) has tell-tale query patterns:\n- Unusually long subdomain labels (>50 chars), high query frequency from one host, and never-before-seen domains.\n- TXT-record queries from workstations are rare and suspicious; tools encode data in the subdomain (`aGVsbG8gd29ybGQ.c2.attacker.com`).",
          "Volume baselining catches the bulk transfers:\n- Set a per-host daily egress baseline and alert above ~2x the historical average, especially off-hours.\n- With TLS inspection, DLP can spot Office/ZIP magic bytes inside encrypted traffic.",
        ],
        codeExample: {
...
# | stats count by src_ip, query | where count > 100`,
      ...
      incident: {
        ...
        body: [
          "After exploiting Apache Struts (CVE-2017-5638), attackers bled Equifax for 76 days:\n- Exfiltration ran from mid-May to discovery on July 29, 2017 — nearly two and a half months.\n- It stayed invisible because Equifax's SSL inspection certificate had expired 19 months earlier, blinding monitoring to HTTPS traffic.",
          "The operators engineered around volume detection:\n- 9,000 queries over 76 days, staged into small encrypted archives, exfiltrated across 20 IPs in 20 countries.\n- The expired cert turned monitoring into a decorative checkbox while names, SSNs, birth dates, and license numbers for 147.9M Americans walked out.",
          "The fallout made encrypted-traffic visibility mandatory:\n- The $575M FTC settlement — the largest data-breach penalty at the time — required SSL inspection with cert management, scheduled scanning, and annual assessments.\n- DOJ indicted four PLA Unit 54938 members (2020), and SSL inspection entered NIST SP 800-53 rev 5 baselines — monitoring blind to encryption protects nothing.",
        ],
      },
      diagram: {
        nodes: [
...
Fragment-3: 76D4YS}`,
  ...
  // ─── mitre-12: Impact ─────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2021,
      overview: [
        "Impact (TA0040) is the final tactic — the moment the entire intrusion was building toward, where the attacker's actual objective lands. It's the deliberate harm: T1486 (Data Encrypted for Impact), i.e. ransomware, and T1485 (Data Destruction) via wiper malware are the headline techniques, alongside T1491 (Defacement), T1498 (Network Denial of Service), and T1529 (System Shutdown/Reboot) to trigger damage at scale. Everything before this was access and positioning; Impact is where access is finally cashed in for money, disruption, or destruction.",
        "Ransomware (T1486) dominates this tactic because it monetizes access more directly than anything else — it turns a network breach straight into a payment demand. And modern operations are no longer a smash-and-grab: they're multi-stage campaigns that do reconnaissance and *exfiltrate* the data first, enabling 'double extortion' (pay to decrypt, and pay again so we don't leak what we already stole). Only then does the encryptor deploy across every reachable system simultaneously, followed by the ransom note. The encryption is the last five minutes of an intrusion that may have run for weeks.",
        "And supply-chain ransomware (T1195.002) multiplies the blast radius beyond a single victim. By compromising one managed service provider (MSP), an attacker can push ransomware to hundreds of that provider's downstream customers at once — weaponizing the trusted management tooling those customers rely on. The 2021 Kaseya VSA incident proved the model devastatingly: one compromised remote-monitoring-and-management platform cascaded ransomware to roughly 1,500 downstream businesses in a single operation. It's the same logic as the SolarWinds supply-chain compromise, aimed not at espionage but at extortion at scale.",
      ],
      technical: {
        title: "Ransomware Defense — Before and After Encryption",
        body: [
          "The encryption stage announces itself if you're watching:\n- Operators usually exfiltrate first (double extortion), so large outbound transfers are an early tell.\n- Then high-volume file writes (Event ID 4663), extension changes (.docx → .locked), and shadow-copy deletion (vssadmin delete shadows /all) give it away.",
          "Recovery hinges on backups the ransomware can't touch:\n- Immutable backups are the single most effective defense — follow 3-2-1 (3 copies, 2 media, 1 offsite).\n- Air-gapped or cloud-immutable stores (Azure Blob immutability, AWS S3 Object Lock) survive because the compromised network can't reach them.",
        ],
        codeExample: {
...
aws s3api put-bucket-object-lock-configuration \
  ...
  --object-lock-configuration '{"ObjectLockEnabled":"Enabled","Rule":{"DefaultRetention":{"Mode":"COMPLIANCE","Days":30}}}'`,
      ...
      incident: {
        ...
        body: [
          "REvil turned a trusted update channel into a delivery system:\n- They exploited a Kaseya VSA zero-day (the RMM platform MSPs use) to push a malicious 'update' to every connected agent.\n- It reached ~50 MSPs and ~1,500 businesses across 17 countries — timed for July 4th weekend, when IT coverage and change controls were thin.",
          "The whole kill chain ran in under 48 hours:\n- Files encrypted simultaneously across thousands of organizations; Sweden's Coop closed 800 stores when its MSP-run POS went down.\n- From Kaseya exploitation (T1195) to detonation (T1486), with a single $70M demand addressed to Kaseya rather than individual victims.",
          "Kaseya forced ransomware to the top of statecraft and recovery planning:\n- Biden raised it directly with Putin on July 9, 2021; REvil's infrastructure went offline soon after, and the FBI controversially held the universal decryptor for three weeks.\n- It accelerated immutable-backup adoption (S3 Object Lock, Azure Immutable Storage) and made offline immutable backups a primary CISA StopRansomware recommendation.",
        ],
      },
      diagram: {
        nodes: [
