...
export const debate2Stages: StageConfig[] = [
  // ─── debate-2-01: The Toulmin Model ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1958,
      overview: [
        "In 1958, philosopher Stephen Toulmin published a model of argument that captures how people actually reason in everyday and professional life — in law, science, and policy — rather than in pure formal logic. It breaks an argument into six parts: the claim (the conclusion you want accepted), the grounds (the data or evidence supporting it), the warrant (the principle connecting grounds to claim), the backing (support for the warrant itself), the qualifier (how strongly the claim holds — 'certainly', 'probably', 'in most cases'), and the rebuttal (the conditions under which the claim would not hold). The model is the debater's X-ray: it reveals the parts of an argument that the speaker left unstated.",
