...
export const debate2Stages: StageConfig[] = [
  ...
  // ─── debate-2-02: Deductive Reasoning ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: -300,
      overview: [
        "Deductive reasoning moves from general premises to a conclusion that follows necessarily — if the premises are true, the conclusion must be true. The classic form is the syllogism:\n- Major premise — 'All humans are mortal.'\n- Minor premise — 'Socrates is human.'\n- Conclusion — 'Therefore Socrates is mortal.'\nDeduction provides certainty, but only conditional certainty: the conclusion is guaranteed only if the premises are true and the form is valid. Debaters use it to lock in conclusions the opponent can't escape without rejecting a premise.",
        "Two separate properties decide whether a deductive argument is any good:\n- Validity — about form: the conclusion follows logically from the premises, regardless of whether the premises are actually true.\n- Soundness — stronger: the argument is valid AND all its premises are actually true.\nThis is the single most useful idea in the stage: a perfectly valid argument can lead to a false conclusion if a premise is false ('All birds can fly; a penguin is a bird; therefore a penguin can fly' — valid form, false premise, false conclusion).",
  ...
  // ─── debate-2-03: Inductive Reasoning ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The strength of an induction depends on the evidence:\n- A larger sample beats a small one (3 cases prove little; 30,000 prove more).\n- A representative sample beats a biased one (surveying only your friends tells you little about the public).\n- More varied evidence beats narrow evidence (the pattern holding across different conditions, places, and times).\n- The conclusion shouldn't overreach ('this worked in three small towns' ≠ 'this works everywhere').\nThese are the levers you push to strengthen your inductions and the cracks you pry open in your opponent's.",
        "Common forms of induction each have a characteristic weakness:\n- Generalization (sample → population) fails when the sample is too small or unrepresentative — a hasty generalization.\n- Statistical inference fails when the numbers are cherry-picked or the base rate is ignored.\n- Causal inference fails when correlation is mistaken for causation (the next stage).\n- Argument from analogy fails when the cases aren't relevantly similar.\nA debater's empirical edge comes from demanding strong inductions from opponents — 'how big was the sample? was it representative?' — while supplying strong ones of their own.",
      ],
      technical: {
        title: "Sample Size, Representativeness, and Overreach",
        body: [
          "Three questions test any inductive generalization. First is sample size — is the evidence base large enough to support the breadth of the claim? A handful of anecdotes cannot ground a universal conclusion. Second is representativeness — does the sample reflect the population it's generalized to, or is it skewed (self-selected respondents, a convenient but atypical group)? Third is scope — does the conclusion stay within what the evidence supports, or does it overreach? 'Three studies in wealthy nations' does not license a claim about all nations.",
  ...
  // ─── debate-2-04: Causation vs Correlation ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "There are four ways a correlation between A and B can arise without A causing B:\n- Reverse causation — B causes A instead (maybe wealth causes the healthy habit, not the habit causing wealth).\n- Common cause — a third factor C causes both (hot weather causes both ice cream and swimming).\n- Coincidence — with enough variables, some correlate by pure chance (spurious correlations).\n- An indirect or mediated chain that isn't what's claimed.\nBefore accepting 'A causes B', ask which of these alternatives better explains the correlation.",
        "Establishing genuine causation requires more than correlation:\n- Controlled experiments — randomly assigning the cause and watching the effect, which rules out confounders.\n- A plausible mechanism — a reason why A would produce B.\n- Correct temporal order — the cause precedes the effect.\n- Dose-response — more cause, more effect.\n- Consistency across studies.\nSupply as many as you can when you argue causation; when you attack a causal claim, point out which they lack — usually a bare correlation with no mechanism, no experiment, and an obvious confounder.",
      ],
      technical: {
  ...
  // ─── debate-2-05: Formal Fallacies ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1910,
      overview: [
        "A formal fallacy is invalid because of its structure — the conclusion doesn't follow no matter the content — and the two most common counterfeits mimic the valid forms modus ponens and modus tollens:\n- Affirming the consequent — 'If P then Q; Q; therefore P' (If it rains the ground is wet; the ground is wet; therefore it rained — but the sprinkler could have wet it).\n- Denying the antecedent — 'If P then Q; not P; therefore not Q' (If it rains the ground is wet; it didn't rain; therefore the ground is dry — but again, the sprinkler).\nBoth resemble valid forms, which is exactly what makes them dangerous.",
        ...
        "Another formal fallacy worth knowing is the undistributed middle: 'All A are B; all C are B; therefore all C are A' (All dogs are animals; all cats are animals; therefore all cats are dogs) — sharing a middle term doesn't connect A and C. The cleanest refutation of any formal fallacy is two moves:\n- Name the invalid form — 'that's affirming the consequent.'\n- Explain the gap in plain terms — 'Q can be true for reasons other than P, so P doesn't follow.'\nFormal fallacies appear constantly dressed in topical content; spotting the underlying invalid form lets you answer instantly.",
      ],
      technical: {
  ...
  // ─── debate-2-06: Informal Fallacies of Relevance ─────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The relevance fallacies every debater must recognize on sight:\n- Ad hominem — attacks the person instead of the argument ('you can't trust her economics — she's never run a business').\n- Straw man — distorts the opponent's position into a weaker version, then refutes the distortion ('my opponent wants open borders' when they argued for modest visa reform).\n- Red herring — introduces an irrelevant distraction to pull attention off the real issue.\nEach is a relevance failure: even if the diversion is true, it doesn't address the claim on the table.",
        "The irrelevant appeals round out the family:\n- Appeal to emotion — substitutes feeling for reasons ('think of the children!' with no argument).\n- Appeal to popularity (ad populum) — treats wide belief as proof ('most people think X, so X is true').\n- Appeal to inappropriate authority — cites someone outside their expertise (a celebrity on vaccines).\nNote the nuance: a relevant expert is legitimate, a famous non-expert is not; emotion tied to a real argument is fine, emotion as a substitute is the fallacy. The skill is catching the substitution and calmly redirecting: 'that doesn't answer my actual argument, which was…'.",
      ],
      technical: {
  ...
  // ─── debate-2-07: Informal Fallacies of Presumption ───────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The presumption family sneaks in an unjustified assumption:\n- False dilemma (false dichotomy) — presents only two options when more exist ('cut all environmental regulation or the economy collapses').\n- Slippery slope — claims one step inevitably leads to an extreme outcome without justifying each link.\n- Begging the question — assumes the conclusion in the premises ('this policy is best because no better one exists').\n- Hasty generalization — leaps from too few cases to a broad rule.\nEach hides an unproven assumption: that the options are only two, that the chain is inevitable, that the conclusion is already granted, that the sample suffices.",
        "Equivocation, the key ambiguity fallacy, trades on a word changing meaning mid-argument ('Evolution is just a theory, and theories are unproven guesses' — two senses of 'theory'). The fix for all of these is to surface the hidden move:\n- False dilemma — supply the missing third option.\n- Slippery slope — demand justification for each link in the chain.\n- Begging the question — point out the conclusion is assumed, not argued.\n- Equivocation — pin down the word's meaning and show it shifted.\nNaming the fallacy plus exposing the specific buried assumption is, again, the winning combination.",
      ],
      technical: {
  ...
  // ─── debate-2-08: Analogical Reasoning ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Whether an analogy is strong depends on the similarities:\n- A strong analogy rests on relevant, numerous, and significant similarities, with no important differences bearing on the conclusion.\n- A weak analogy relies on superficial similarities while ignoring deep differences.\n- The decisive question is always relevance: do the shared features actually relate to the property being inferred?\nTwo situations can be alike in many ways that don't matter — and alike in few ways that are exactly the point. Counting similarities isn't enough; they must be the right ones.",
        "The standard way to attack an analogy is the disanalogy — identify a relevant difference that undermines the inference:\n- 'Broadcasting used scarce public airwaves, which justified regulation.'\n- 'The internet isn't spectrum-scarce, so the analogy to broadcast regulation breaks at the very feature that justified it.'\nA single relevant disanalogy can collapse an analogy no matter how many surface similarities it had. When you use one, pre-empt this by choosing cases alike in the features that matter; when you face one, hunt for the difference that touches the conclusion.",
      ],
      technical: {
  ...
  // ─── debate-2-09: Statistical Reasoning ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1662,
      overview: [
        "Statistics carry an aura of objectivity that makes misleading ones especially dangerous, so a debater needs enough numerical literacy to use good statistics and expose bad ones. The most common manipulations are simple:\n- Confusing percentages with absolute numbers ('crime up 100%' might mean two incidents became four).\n- Ignoring the base rate (a '99% accurate' test can still be wrong most times it flags something rare).\n- Cherry-picking a favorable time window or subgroup.\n- Citing a scary relative risk while the absolute risk is tiny ('doubles your risk' from 1 in a million to 2 in a million).",
        ...
        "Other red flags to watch for:\n- A statistic with no source or denominator ('10,000 affected' — out of how many? says who?).\n- A graph with a truncated axis exaggerating a trend.\n- An average (mean) hiding a skewed distribution — mean income soars when a few billionaires are added; the median tells the real story.\n- Survivorship bias — studying only the successes.\nThe defense is a set of fast questions: percentage or absolute? what's the base rate? what's the denominator? mean or median? what time window, and why that one? A debater who asks these turns an opponent's impressive-sounding number into a liability — and avoids being fooled themselves.",
      ],
      technical: {
  ...
  // ─── debate-2-10: Burden of Proof and Presumption ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 1769,
      overview: [
        "The burden of proof is the obligation to support a claim, and it falls on the party making the assertion — especially the party seeking to change the status quo ('he who asserts must prove'). It connects to presumption, the default that holds until the burden is met:\n- In a courtroom, the presumption of innocence places the burden on the prosecution.\n- In debate, presumption generally favors the status quo (no change), so the side advocating change carries the burden.\nKnowing where the burden lies tells you who loses if the argument ends in a tie.",
        "A common cheat is the attempt to shift the burden of proof — demanding the other side disprove your claim rather than proving it yourself:\n- 'You can't prove this policy won't work, so we should adopt it' illegitimately flips the burden — the proponent must show it will work.\n- The related fallacy, appeal to ignorance (argumentum ad ignorantiam), treats absence of disproof as proof ('no one has shown ghosts don't exist, so they do').\nRecognizing burden-shifting lets you refuse an obligation that isn't yours: 'It's your claim — the burden is on you to prove it, not on me to disprove it.'",
