  ...
  // ─── cf-01: Electricity ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Three words describe electricity, and a water analogy makes them click. Charge is the 'stuff' that flows, like water itself — electrons carry negative charge. Current is how much charge flows per second, like the rate of water through a pipe, measured in amps. And voltage is the 'push' that drives the flow, like water pressure, measured in volts: no pressure, no flow.",
        "Why start a computing course with physics? Because it never stops being true. A '1' inside your computer is really a higher voltage and a '0' is a lower one, so bits are voltages; everything a CPU does is open and close microscopic gates to route this flow; and because the flow is physical, it leaks — heat, light, radio waves, timing — which is exactly what hardware attacks exploit later in this track.",
      ],
      technical: {
  ...
  // ─── cf-02: Circuits ─────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "A handful of basic components shape the flow, each with a job. A source — a battery or power supply — provides the voltage push; a switch opens or closes the loop and is the ancestor of every bit in your computer; a resistor deliberately limits current to protect parts and set levels; a wire is the low-resistance path the electrons travel; and a load is the thing being powered, whether a bulb, a buzzer, or a billion transistors.",
        "Why obsess over a humble switch? Because computing is switches all the way down. A light switch is a human-operated bit, on or off; the breakthrough of computers was replacing the human finger with an electrical switch — one that another electrical signal can flip, billions of times per second; and everything ahead, from logic gates to memory to processors, is built from switches controlling other switches. Master 'the loop opens or closes', and the rest follows.",
      ],
      technical: {
  ...
  // ─── cf-03: Transistors ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The trick lives in a special material — the semiconductor. Silicon normally conducts only a little, but by deliberately adding impurities ('doping'), engineers create regions that carry charge in controllable ways; arrange these doped regions cleverly and a small gate voltage can switch a much larger current on or off. That controllability is why it is called a semiconductor: not quite a conductor, not quite an insulator — exactly the in-between needed for a switch.",
        "Two superpowers made the transistor civilization-changing. The first is switching: used as an on/off gate, it is the physical bit behind all digital logic. The second is shrinking: transistors can be made astonishingly small and packed by the billions on a fingernail-sized chip, getting faster and cheaper as they shrink — the engine behind Moore's Law. A 1950s computer used room-sized racks of tubes; today an equivalent switch is a few dozen atoms across and one of billions on a single chip.",
      ],
      technical: {
  ...
  // ─── cf-04: Logic Gates ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The three foundational gates are easy to feel intuitively. NOT flips the input — give it 1, get 0; give it 0, get 1 — an inverter. AND outputs 1 only if all inputs are 1 ('both must be true'). OR outputs 1 if at least one input is 1 ('either will do'). Combine them and you also get NAND, NOR, and XOR (true when inputs differ) — the everyday workhorses of real chips.",
        "Why is this such a big deal? Because logic gates are universal. Any decision you can describe with 'and / or / not' can be wired as gates, and in fact a single gate type (NAND or NOR) can build all the others — so an entire computer can be made from billions of copies of one simple gate. This is the bridge from physics to computer science: George Boole's 1850s logic, married to Shannon's switches, becomes a machine that reasons.",
      ],
      technical: {
  ...
  // ─── cf-05: Binary ───────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Counting in binary is just counting with two symbols instead of ten. In our system each place is worth ten times more (1, 10, 100), while in binary each place is worth two times more (1, 2, 4, 8, 16…), so 1011 means 8 + 0 + 2 + 1 = 11 — every number has a binary spelling. Bits are grouped for convenience: 8 bits make a byte, which can hold 256 different values (2^8), enough for one character of text.",
        "Everything in a computer is ultimately bits, encoded by agreed-upon rules. Text gives each character a number (ASCII/Unicode), so 'A' is 65, stored as 01000001; images make each pixel numbers for red, green, and blue brightness; and sound measures the air-pressure wave thousands of times a second, each measurement a number. The profound idea is that a single physical thing — patterns of high and low voltage — can represent anything, as long as everyone agrees on the code.",
      ],
      technical: {
  ...
  // ─── cf-06: Adders / ALU ─────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Binary addition has only four cases per column, and gates handle them perfectly. The cases 0+0 = 0, 0+1 = 1, and 1+0 = 1 are just an XOR that outputs the sum bit, while 1+1 = 10 makes the column 0 and 'carries the 1', which an AND gate detects. Wire an XOR for the sum and an AND for the carry together and you have a 'half adder', a circuit that adds two bits; chain 'full adders' (which also accept a carry coming in) and you can add numbers of any size.",
        "Scale that idea up and you get the heart of the processor. The Arithmetic Logic Unit (ALU) is the part of a CPU that does math and logic — add, subtract, compare, AND, OR — and subtraction, multiplication, and comparison are all built from the same gate-level tricks (subtraction is clever adding). So the chip that runs your spreadsheet, your game, and your encryption is, at its core, vast fields of adders and gates — pure logic doing arithmetic.",
      ],
      technical: {
  ...
  // ─── cf-07: Memory ───────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Memory comes in a few flavors, trading speed for permanence. Registers and cache are tiny, lightning-fast memory inside the CPU, built from logic (flip-flops) for bits in active use; RAM (main memory) is larger and fast, holding your running programs, but 'volatile' — it forgets everything when power is lost; and storage (SSD/flash, hard drives) is slower but 'non-volatile', keeping data without power, which is why your files survive a shutdown.",
        "How a bit is physically held differs by technology — and one of them is secretly quantum. A flip-flop loops a gate's output back to its input so it holds its state, logic remembering itself; DRAM stores each bit as a tiny charge in a capacitor that leaks away, so it must be 'refreshed' thousands of times a second; and flash memory traps electrons on an isolated 'floating gate', where the only way electrons get on and off is quantum tunneling — the spooky ability of particles to cross a barrier they classically shouldn't. Your phone's storage runs on quantum physics.",
      ],
      technical: {
  ...
  // ─── cf-08: The CPU ──────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Every CPU repeats one simple loop, called the fetch-decode-execute cycle. It fetches the next instruction from memory, decodes it to figure out what it means (which operation, which data), and executes it — using the ALU to add, reading or writing memory, or jumping elsewhere — then repeats, forever, with a 'program counter' tracking which instruction is next.",
        "A few parts make this loop possible. The clock is a crystal-driven heartbeat — billions of ticks per second, gigahertz — that paces every step so the parts stay in sync; the registers are the tiny, ultra-fast slots holding the data being worked on right now; and the control unit is the traffic cop that directs data between memory, registers, and the ALU according to each instruction. Put together, a CPU is a spectacularly fast, perfectly obedient machine that does trivial steps so quickly the result feels like thought.",
      ],
      technical: {
  ...
  // ─── cf-09: Code to Electricity ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "From your code down to the metal, the journey passes through several floors. At the top is the high-level language — readable code like Python, JavaScript, or C that humans write; a compiler or interpreter translates that code into the CPU's instruction set; machine code is the binary instructions the CPU actually understands; the microarchitecture is the CPU executing those instructions with gates and the ALU; and at the bottom are transistors and voltages, the physical electrons doing the work, where this track began.",
        "Why build such a tall tower? Because each layer is a superpower. Productivity lets you describe what you want in human terms, not in billions of switch flips, and portability lets the same high-level code run on different chips because only the lower layers change. But abstractions 'leak': performance, bugs, and security flaws from a lower layer can surface in your code unexpectedly — which is why great engineers (and attackers) understand more than one floor of the tower.",
      ],
      technical: {
  ...
  // ─── cf-10: Why This Matters for Security ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The core realization is simple and powerful: computation is physical, and physical things leak and can be disturbed. They leak because running code gives off heat, draws measurable power, emits radio waves, and takes different amounts of time — each of which can carry secrets out through side channels. They linger because bits are physical states that don't vanish instantly, so data persists in RAM after power-off and in 'deleted' flash. And they can be disturbed because chips are sensitive to voltage, timing, and their neighbors, so attackers can glitch power, hammer memory, or inject faults to force misbehavior.",
        "This is exactly where the next track picks up. 'The Physics of Hacking' takes each physical property you met here — power, EM, timing, memory remanence — and turns it into a real attack and a real defense. The lesson of these foundations is that security is not only about clever code; it is about a physical device an adversary can touch, measure, and perturb. An engineer who understands the machine from electrons to code can defend (and test) it at every layer — which is rarer, and more valuable, than knowing software alone.",
      ],
      technical: {
