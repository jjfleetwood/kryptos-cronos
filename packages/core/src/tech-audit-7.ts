  ...
  // ─── rca-06: Forensic Timeline & Evidence ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Two disciplines make the timeline trustworthy. The first is time normalization. Every source must be reconciled to one timezone and clock, and clock skew (a host whose time was wrong) must be identified — out-of-order events produce a false causal story, the cardinal forensic error. The second is evidence integrity. Collect in order of volatility (memory and live network first, then disk, then archives), hash everything on collection, work from copies, and maintain chain of custody so the evidence is defensible if it ends up in an audit finding, a regulatory filing, or court.",
...
rule: answer a root cause as high on this ladder as feasible`,
  ...
  // ─── rca-10: The RCA Report & Blameless Postmortem (capstone) ────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Everything in this epoch converges on a document. The RCA report (in reliability circles, the postmortem) is the deliverable that makes an investigation real:\n- a summary anyone can read in a minute\n- the impact in honest numbers\n- the evidence-backed timeline\n- the causal analysis with the techniques shown (5 Whys chains, the fishbone, the fault tree, the breach matrix)\n- the root causes and contributing factors clearly separated\n- the corrective actions with owners and dates\n- the lessons worth exporting\nIt serves three audiences at once — executives (impact and risk), engineers (the detail), and auditors or regulators (the evidence trail) — and a report that serves only one of them will fail the other two when it matters.",
        ...
        "This stage closes the pipeline the epoch has built:\n- detect, then reconstruct the timeline from evidence (rca-06)\n- ask why with the right technique — 5 Whys for linear chains (rca-03), fishbone for breadth (rca-04), fault tree for combinations (rca-05)\n- walk the human layer without blame (rca-07)\n- for breaches, run the per-step causal matrix (rca-08)\n- convert causes into verified CAPA fixes (rca-09)\n- ship the postmortem so the learning compounds (rca-10)\nOrganizations that run this loop stop having the same incident twice — which, over the years, is the entire difference between the ones that mature and the ones that repeat.",
      ],
      technical: {
