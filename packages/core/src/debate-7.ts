...
export const debate7Stages: StageConfig[] = [
  // ─── debate-7-01: Judge Paradigms ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "Epoch 6 established audience adaptation; this stage formalizes it for the competitive circuit, where judges publish 'paradigms' — written statements of how they decide rounds. A paradigm tells you what the judge values, what arguments they'll vote on, their delivery tolerance, their preferences and pet peeves, and sometimes their 'judging philosophy' (the lens through which they view debate). Reading a judge's paradigm before a round is, at the circuit level, as important as preparing your case — it tells you exactly how to win this particular ballot. On platforms like Tabroom, paradigms are posted for thousands of judges; the skilled competitor reads them and adapts.",
        "Judges fall into recognizable philosophical types, and knowing them lets you debate to each. The tabula rasa ('blank slate') judge claims no predispositions and votes on whatever's argued and won on the flow — debate the flow rigorously. The games-player or 'tech' judge treats debate as a game with rules and rewards technical execution and dropped-argument exploitation. The policymaker judge weighs the round like a real policy decision — does the plan produce net benefits? — rewarding strong cost-benefit weighing. The hypothesis-tester evaluates the resolution like a hypothesis to be tested. The lay judge (epoch 3) decides on clarity, persuasion, and real-world sense. Each type wants a different style of debating, and a paradigm usually signals which the judge is.",
  ...
  // ─── debate-7-02: Theory and Topicality ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1990,
      overview: [
        "Theory and topicality are procedural arguments — arguments about how the debate itself should be conducted, operating on a meta-level above the substantive clash. Topicality (T) argues that the opponent's case isn't within the resolution: the Affirmative must defend the resolution as written, and if their plan falls outside a reasonable interpretation of the topic, the Negative can argue they should lose for being untopical, regardless of the substance. Theory arguments more broadly claim that something about how the opponent is debating is unfair or uneducational and should be a voting issue — e.g., that a particular argument or practice is abusive and warrants a ballot against it.",
        "These arguments are structured around standards and voters. A topicality or theory argument typically presents an interpretation (how debate should be conducted — 'the Aff must defend the whole resolution'), a violation (how the opponent broke it), standards (why the interpretation is good — usually fairness and education: a fair interpretation gives both sides ground, an educational one promotes good debate), and a voter/impact (why this should decide the round — fairness and education as reasons to vote). The opponent answers with a counter-interpretation and their own standards, turning it into a meta-debate about the rules of the game itself.",
  ...
  // ─── debate-7-03: The Kritik ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "A kritik has a recognizable structure borrowed from policy argument. The link: how the opponent's case engages in or relies on the problematic assumption or system the kritik criticizes ('their security-based justification reproduces the very threat-construction that causes violence'). The impact: why that assumption is bad ('this way of thinking leads to endless militarization'). The alternative: what the judge should endorse instead by voting for the kritik (a different orientation, a rejection, a reconceptualization). And often a framework argument about why the judge should evaluate the round at the level of assumptions and discourse rather than just the plan's consequences. The opponent answers by contesting the link, defending their assumptions, attacking the alternative, and arguing the judge should weigh consequences (their plan's real-world benefits) over abstract critique.",
        "Kritiks are intensely debated within the activity. Defenders argue they bring genuine philosophical depth, force debaters to examine the assumptions and ethics beneath policy (a vital critical-thinking skill), and open space for perspectives that pure cost-benefit policy debate excludes. Critics argue they can be obscure, evasive (dodging the actual topic), inaccessible to newcomers and many judges, and sometimes more about jargon than insight. As with theory (epoch 7-02), the kritik is powerful but highly judge-dependent — some judges love them, others won't vote on them. For the advanced competitor, the kritik represents debate at its most intellectually ambitious: arguing not just about what policy is best but about the frameworks and assumptions through which we even think about policy. Understanding the K — its structure, its philosophical roots, how to run and answer it, and when the judge makes it viable — is part of mastering the full range of high-level argument, even for debaters who rarely run them.",
      ],
      technical: {
  ...
  // ─── debate-7-04: Counterplans and Disadvantages ──────────────────────────────
  {
    ...
    info: {
      ...
      year: 1980,
      overview: [
        "Disadvantages (DAs) and counterplans (CPs) are the core advanced offensive arguments of policy debate (introduced in epoch 3), and together they form the most powerful negative strategy. A disadvantage argues that the Affirmative's plan causes a bad consequence that outweighs its benefits: it has a uniqueness (the bad thing isn't happening now), a link (the plan triggers it), an internal link (the chain to the impact), and an impact (the terminal harm). 'The plan increases federal spending (link), which raises interest rates (internal link), which crashes the economy (impact) — and this isn't happening in the status quo (uniqueness).' The DA gives the judge a reason the plan is net harmful.",
        ...
        "These arguments demand precise weighing and answering. The Affirmative responds to a DA by contesting any element (no uniqueness — it's already happening; no link — the plan doesn't trigger it; no impact — it doesn't matter; or turning it, epoch 5-03) and by weighing (the plan's advantages outweigh the DA). They respond to a CP by contesting its competitiveness (it's not a reason to reject the plan — they could do both), its solvency (it doesn't actually solve), or by 'perm'-ing (the permutation: arguing the plan and counterplan could be done together, testing whether the CP is truly competitive). The interplay of CPs, DAs, perms, and weighing is policy debate's deepest strategic layer — a contest of competing policy options weighed against each other, demanding the rigorous cost-benefit and comparative analysis that makes policy debate such formidable training. Mastering this arsenal — running it, answering it, and weighing it — is the height of policy-format skill.",
      ],
      technical: {
  ...
  // ─── debate-7-05: Tournament Strategy ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Several meta-skills matter across a tournament. Stamina and focus: tournaments are long and exhausting (many rounds over one or two days), so managing energy, staying sharp late, and recovering between rounds is real. Consistency: in prelims, you generally can't afford bad rounds, so reliable solid performance matters as much as brilliance. Adaptation across rounds: you face different opponents and judges, and you read and adapt to each (epochs 6-08, 7-01). Scouting and learning: you gather information about likely opponents and their cases (within the rules), and you learn from each round to improve through the tournament. And handling outcomes: bouncing back from a loss without it derailing your next round is a competitive mental skill.",
        "Elimination rounds add their own strategy. They're single-elimination (one loss ends your tournament), often with a panel of multiple judges (you need to win a majority, which complicates adaptation — you may face a tech judge and a lay judge on the same panel and must reach both). They sometimes involve 'side constraints' (a coin flip or seeding determines sides). And the stakes and pressure are higher (epoch 6-09's composure becomes critical). Across the whole campaign, the tournament competitor manages preparation before the event, consistency and energy through prelims, adaptation to each judge and opponent, resilience after losses, and peak performance under elimination pressure. This 'long game' is a distinct skill set layered on top of round-winning — the difference between a debater who can win a round and a competitor who can win a tournament.",
      ],
      technical: {
  ...
  // ─── debate-7-06: NSDA Degrees ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The NSDA degree ladder rises with accumulated points: Degree of Merit, Degree of Honor, Degree of Excellence, Degree of Distinction, Degree of Special Distinction, and the Degree of Premier Distinction at the top — each requiring progressively more points earned across a competitive career. Reaching the higher degrees signals years of serious, successful competition and is a recognized mark of accomplishment. These degrees are real credentials: they appear on records, are recognized by the speech and debate community, and demonstrate sustained commitment and skill in a way colleges and scholarship committees understand.",
        "Beyond the degree ladder, the NSDA confers other notable honors. The Academic All American award recognizes students who combine competitive success with strong academics (a high GPA, a minimum number of NSDA points, and other criteria) — a prestigious distinction signaling excellence in both debate and the classroom. There are All American and special service recognitions, national qualification and placement at the National Tournament (itself a major credential), and various scholarships. For a student, these credentials matter: they provide tangible recognition of years of work, strengthen college applications (admissions officers value demonstrated commitment and the skills debate builds), and can unlock scholarships. The NSDA system answers the question of whether debate offers 'certifications': it does — a structured, recognized ladder of merit points, degrees, and honors that credential a speech and debate career, culminating in the highest degrees and awards like Academic All American.",
      ],
      technical: {
  ...
  // ─── debate-7-07: Toastmasters and Public Speaking Credentials ────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Toastmasters' modern credentialing system is called Pathways, a structured learning experience with multiple specialized 'paths' (focused on different goals like persuasive influence, presentation mastery, or leadership development) divided into five levels of increasing skill. Members complete projects — giving speeches and taking on roles — to advance through the levels, earning recognition at each. Completing a full path is a credential in itself, and the system culminates in the most prestigious Toastmasters distinction: the Distinguished Toastmaster (DTM), the highest award the organization confers, earned by completing extensive education and leadership requirements over time. The DTM is a globally recognized mark of accomplished public speaking and leadership.",
        "The older Toastmasters track (still referenced and held by many) used a different credential ladder — Competent Communicator (CC), then Advanced Communicator levels (Bronze, Silver, Gold), and parallel leadership awards (Competent Leader, Advanced Leader) leading to DTM. Whether through legacy awards or the current Pathways levels, the value is the same: Toastmasters provides structured skill development and recognized credentials for public speaking and leadership that anyone can pursue at any stage of life, in any profession. For a debater, this answers a key question: the speaking skills you build don't have to end when competition does, and they can continue to be developed and credentialed for a lifetime. Toastmasters is the bridge from competitive school debate to lifelong, certified mastery of the spoken word — a recognized path available long after the last tournament.",
      ],
      technical: {
  ...
  // ─── debate-7-08: Collegiate and International ─────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1947,
      overview: [
        "Debate's credentials and competition extend well beyond high school into collegiate and international arenas, each with its own organizations and prestigious honors. At the U.S. collegiate level, several bodies govern different formats: the National Debate Tournament (NDT) and the Cross Examination Debate Association (CEDA) for policy debate; the National Parliamentary Debate Association (NPDA) and the American Parliamentary Debate Association (APDA) for parliamentary; the National Forensic Association (NFA) for Lincoln-Douglas (NFA-LD) and individual events; and the American Forensic Association (AFA), which oversees national individual-events competition. Winning or placing at these national championships is a major credential in the collegiate debate world.",
        "Internationally, the pinnacle formats and championships connect debaters across the globe. The World Universities Debating Championship (WUDC, epoch 3) is the global championship of university debate in the British Parliamentary format — winning or reaching late elimination rounds there is among the highest honors in the activity. The World Schools Debating Championships (WSDC, epoch 3) is the equivalent pinnacle for high school students in the World Schools format. Performing at these world championships marks a debater among the best internationally, and the credential is recognized globally. There are also European, Pan-American, and many regional championships feeding into the international circuit.",
  ...
  // ─── debate-7-09: Coaching and Judging ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Debate's ecosystem depends on coaches and judges, and stepping into these roles is both a way to give back and a path to deeper mastery. Judging is evaluating rounds and rendering decisions — and good judging is a genuine skill: flowing accurately, setting aside personal bias to decide on the arguments (epoch 1), weighing competing arguments fairly, and articulating a clear reason for decision (RFD). The RFD — the explanation of why one side won — is the heart of good judging: it should walk through the key arguments, explain what was won and dropped, and give a clear, argument-based justification, providing the feedback that helps debaters improve. Many experienced debaters judge, and learning to judge well sharpens your understanding of what actually wins.",
        ...
        "Both roles also reveal debate from the other side of the ballot, which improves your competing. Judging teaches you what's persuasive and what's confusing from the decider's chair — you experience firsthand how weighing, signposting, and clarity (epochs 4–6) actually land, which makes you a better competitor. Coaching teaches you the deep structure of every skill in this curriculum by forcing you to explain it. And both are how the debater gives back: the coaches and judges who make the activity possible were once competitors, and stepping into those roles sustains the ecosystem that taught you. The complete arc of a debate life often runs from competitor, to judge and peer coach, to coach and mentor — each role deepening mastery and passing it on. Understanding judging (especially the RFD) and coaching isn't only for after you compete; it makes you better now, and it's how the skills and the community endure.",
      ],
      technical: {
  ...
  // ─── debate-7-10: Careers and Lifelong Value ──────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The specific skills are the through-line of everything you've learned. Research and source evaluation (epochs 1, 4) — finding and judging evidence in an age of misinformation. Logic and the detection of fallacies (epoch 2) — reasoning soundly and spotting bad reasoning. Structured argument and case-building (epoch 4) — constructing a rigorous case for anything. Listening, refutation, and quick thinking (epoch 5) — engaging opposing views and responding under pressure. Persuasion, delivery, and audience adaptation (epoch 6) — communicating effectively to anyone. And the meta-skills: composure under pressure, resilience, and the discipline to prepare. These aren't debate-specific tricks; they are the core competencies of an effective thinker and communicator in any domain.",
