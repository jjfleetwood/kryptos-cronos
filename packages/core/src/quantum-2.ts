...
export const quantum2Stages: StageConfig[] = [
  // ─── quantum-b01: NIST PQC Standardization ──────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The threats epoch left you with a problem: Shor's algorithm will eventually shatter RSA and elliptic-curve cryptography, and Harvest Now Decrypt Later means the clock is already running. This epoch is the answer — but the answer could not be improvised. You don't replace the cryptography that secures the entire internet by having one clever team publish a new algorithm and hoping it holds. So in 2016 NIST did something deliberately slow and deliberately public: it opened a global competition to find the algorithms that would replace RSA and ECC, drawing 69 submissions from cryptographers worldwide, each staking a claim that its math could resist a quantum computer.",
        "What followed was less a contest than a controlled demolition. Over three rounds and eight years, every submission was thrown to the global cryptographic community to attack — and attack they did. The point of the process was not to find algorithms nobody had broken yet; it was to find algorithms that survived years of the world's best cryptanalysts trying their hardest to break them. Several didn't: Rainbow, a multivariate scheme, was cracked over a weekend on a laptop; SIKE, an elegant isogeny-based candidate that had reached the final stretch, fell to a stunning classical attack in 2022. Each break was good news — it meant the process was working, weeding out the fragile before it shipped rather than after.",
        "In August 2024 the survivors were canonized as the first federal PQC standards: FIPS 203 (ML-KEM, from CRYSTALS-Kyber) for key exchange, FIPS 204 (ML-DSA, from CRYSTALS-Dilithium) and FIPS 205 (SLH-DSA, from SPHINCS+) for signatures, with FALCON to follow as FN-DSA. These are now the mandated replacements for RSA and ECC across U.S. government systems, and because the whole process was conducted in the open — every submission, every cryptanalysis paper, every selection criterion public — the world can trust the winners for the right reason: not because an authority declared them safe, but because everyone got to try to break them and the best minds failed. The rest of this epoch takes these standards apart one by one.",
      ],
      technical: {
...
# - SLH-DSA: conservative choice — large sigs, simple security assumptions`,
  ...
  // ─── quantum-b02: Learning With Errors ──────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Why do the new algorithms resist quantum computers when RSA and ECC don't? Because they're built on a completely different kind of hard problem — one with no known quantum shortcut. RSA's security is the difficulty of factoring; ECC's is the discrete logarithm; and Shor's algorithm happens to demolish both because they share a hidden periodic structure a quantum computer can exploit. The leading PQC schemes throw that structure away and stand instead on lattices, and specifically on a problem called Learning With Errors (LWE), introduced by Oded Regev in 2005. The setup is deceptively simple: I give you a pile of linear equations in a secret vector s, but I've added a little random noise to each one. Solve a clean linear system and high-school algebra cracks it instantly; add noise, and finding s becomes staggeringly hard, because the noise hides which equations to trust.",
        "What makes LWE trustworthy rather than merely difficult-looking is Regev's worst-case-to-average-case reduction — one of the most beautiful results in modern cryptography. He proved that solving *random* LWE instances (the kind your keys generate) is at least as hard as solving certain lattice problems, like the Shortest Vector Problem, in their *worst* case. That matters enormously: most cryptography prays that the random instances it actually uses happen to be hard, but LWE comes with a guarantee that breaking a typical key is as hard as breaking the single hardest lattice instance that exists. And high-dimensional lattice problems are believed hard for quantum computers too — the best known quantum algorithms manage only a modest polynomial speedup, nothing like Shor's catastrophic break. There is, so far, no quantum guillotine for lattices.",
        "ML-KEM and ML-DSA — the two workhorse standards — actually use a refined variant called Module-LWE. Plain LWE is secure but clunky: the keys are large matrices and the math is slow. Adding algebraic structure (Ring-LWE) makes it fast but concentrates risk in that structure. Module-LWE is the engineering compromise that won: it organizes the secret and noise into small blocks over polynomial rings, recovering most of the speed and compactness of the ring version while keeping a safety margin closer to the unstructured original. It's the sweet spot the entire lattice family converged on — enough structure to be practical at internet scale, not so much that it hands an attacker a foothold. Every lattice algorithm in the rest of this epoch is a different way of turning this one hard problem into a usable cryptographic tool.",
      ],
      technical: {
...
print("Decrypted:", 1 if abs(decrypted - q//2) < q//4 else 0)  # → 1`,
  ...
  // ─── quantum-b03: ML-KEM (CRYSTALS-Kyber) ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "If you only deploy one post-quantum algorithm, it will be this one. ML-KEM (Module-Lattice Key Encapsulation Mechanism), standardized as FIPS 203 in August 2024, is the direct replacement for RSA and ECDH key exchange — the single most exposed piece of the cryptographic stack, because key exchange is exactly what Harvest Now Decrypt Later attacks. Every HTTPS connection, every VPN tunnel, every moment two parties need to agree on a shared secret over a hostile network currently leans on math Shor's algorithm will break; ML-KEM is the quantum-safe swap-in. A 'KEM' works a little differently from RSA encryption — instead of encrypting a secret you chose, one party generates a fresh random secret, encapsulates it under the other's public key, and ships the ciphertext; both sides end up holding the same secret — but the role it plays in a handshake is identical.",
        "ML-KEM is the standardized, hardened form of CRYSTALS-Kyber, designed by a team spanning IBM, NXP, and academia, resting on the Module-LWE assumption from the previous stage. Its great virtue is that it is *boring* in the best sense: fast enough that turning it on adds no perceptible latency to a TLS handshake, with keys and ciphertexts small enough (around a kilobyte) to fit the existing shape of network protocols. That practicality is why it left the lab faster than almost any cryptographic standard in history — Cloudflare and Google were running it on production traffic before the ink on FIPS 203 was dry, and every major TLS library now speaks it.",
        "It ships in three parameter sets that trade size for security margin: ML-KEM-512 targeting roughly AES-128 strength (NIST level 1), ML-KEM-768 at roughly AES-192 (level 3), and ML-KEM-1024 at roughly AES-256 (level 5). The broad recommendation — and the choice Google and Cloudflare made — is ML-KEM-768: it sits comfortably above any near-term threat with a healthy safety buffer, and the size difference from the weaker set isn't worth the thinner margin. As a rule of thumb for this whole epoch, when in doubt pick the level-3 parameter set; it's the deliberate default the standardizers themselves lean toward.",
      ],
      technical: {
...
with oqs.KeyEncapsulation("Kyber768") as kem:
  ...
  // ─── quantum-b04: ML-DSA (CRYSTALS-Dilithium) ───────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Key exchange protects a conversation's *secrecy*; signatures protect its *authenticity* — the proof that a message, a certificate, or a software update really came from who it claims and wasn't tampered with. That's a different job, and post-quantum needs its own answer for it, because Shor's algorithm breaks RSA and ECDSA signatures just as thoroughly as it breaks key exchange. ML-DSA (Module-Lattice Digital Signature Algorithm), FIPS 204, is the primary replacement: the quantum-safe choice for TLS certificate signing, code signing, authentication tokens — anywhere a system today reaches for RSA-PSS or ECDSA. And signatures carry a special urgency around the 'trust on day one' problem: a certificate authority's signing key compromised by a quantum computer lets an attacker forge identities across the whole web, so the roots of trust are among the first things that must move.",
        "ML-DSA is the standardized form of CRYSTALS-Dilithium, and it leans on two lattice hardness assumptions — Module-LWE (familiar from stage 2) plus Module-SIS, the Short Integer Solution problem — so an attacker has to defeat both to forge a signature. Its most important engineering detail is rejection sampling: a naive lattice signature would leak tiny statistical hints about the private key with every signature, and an attacker collecting enough of them could reconstruct it (this is precisely how careless implementations of older lattice and even some ECDSA systems have been broken). ML-DSA deliberately throws away and re-rolls any candidate signature that would leak, so the published signature is provably independent of the secret. It's slower to sign as a result, but the security is worth it.",
        "Like ML-KEM, it comes in three strengths — ML-DSA-44 (level 2), ML-DSA-65 (level 3, the recommended default), and ML-DSA-87 (level 5) — and again, level 3 is the sensible pick for most systems. The honest tradeoff to plan for is size: ML-DSA signatures run roughly 2,400 to 4,600 bytes, against 64 to 512 bytes for ECDSA and RSA. That's an order of magnitude larger, and in a TLS handshake that carries several signatures it adds up, fattening certificate chains and handshake sizes — a real deployment cost addressed in the TLS stage later. Verification, mercifully, stays fast. You pay in bytes on the wire, not in CPU, for becoming quantum-safe.",
      ],
      technical: {
...
# ML-DSA-65 signature: 3293 bytes — larger but quantum-safe`,
  ...
  // ─── quantum-b05: SLH-DSA (SPHINCS+) ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "There is a quiet worry haunting the lattice algorithms: they're all new. ML-KEM and ML-DSA rest on hardness assumptions that are only about twenty years old, and the SIKE disaster — a finalist broken in a weekend after years of confidence — is a fresh reminder that 'no one has broken it yet' is not the same as 'no one ever will.' SLH-DSA (Stateless Hash-based Digital Signature Algorithm), FIPS 205, is NIST's hedge against that exact nightmare. It is built from SPHINCS+, and its security rests on nothing but the properties of a hash function — the collision and preimage resistance of SHA-256 or SHAKE-256. No lattices, no Module-LWE, no algebraic structure that could harbor an undiscovered weakness.",
        "That makes SLH-DSA the most conservative signature humanity knows how to build. Hash functions are the most battle-tested primitives in all of cryptography — we've been trying and failing to break SHA-256 for two decades — and crucially, they're already believed quantum-resistant (Grover only weakens them, and SLH-DSA's parameters account for that). So the security argument is almost philosophically clean: if SHA-256 still stands, SLH-DSA still stands, full stop. Even in the catastrophic scenario where some future breakthrough makes lattice problems easy and ML-DSA falls, SLH-DSA would be untouched. That's why it's the recommended choice for the crown jewels — root certificate authorities, firmware signing keys, the long-lived roots of trust whose compromise would be unrecoverable.",
        "The conservatism is not free, and the bill is steep: SLH-DSA signatures are enormous — anywhere from about 8,000 to nearly 50,000 bytes depending on parameters — and signing can take milliseconds to seconds, glacial by cryptographic standards. You would never want this in a TLS handshake happening millions of times a second; there, ML-DSA's speed and (relatively) compact signatures win easily. But that's the wrong comparison. SLH-DSA is for the signature you make once and live with for a decade — the firmware that ships in a million devices, the root key that anchors an entire PKI. For those, signing happens rarely and the cost of being wrong is permanent, so paying in bytes and seconds to buy the most paranoid security assumption available is exactly the right trade.",
      ],
      technical: {
...
# - Code signing: SLH-DSA-SHA2-128f (fast, moderate security)`,
  ...
  // ─── quantum-b06: FN-DSA (FALCON) ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "ML-DSA's one real drawback was size — those multi-kilobyte signatures. FALCON, standardized as FN-DSA, is the answer for when that size genuinely hurts. It produces by far the most compact signatures of any NIST post-quantum algorithm: a FALCON-512 signature is just 666 bytes against ML-DSA-65's 3,293, and FALCON-1024 is 1,280 bytes. That's close enough to RSA's footprint that FALCON can slot into bandwidth- and storage-starved places the bulkier lattice signatures simply can't fit. If you're signing inside a constrained IoT device, or stuffing a signature into a protocol field with no room to spare, FALCON is the tool.",
        "It buys that compactness with serious mathematical machinery. FALCON is built on NTRU lattices and generates its signatures using Gaussian sampling over polynomial rings, accelerated by the Fast Fourier Transform. The Gaussian sampling is the secret to the small size — it lets signatures hug the lattice tightly — but it is also FALCON's curse, because it requires high-precision floating-point arithmetic, and floating-point is notoriously hard to make behave identically on every platform and in constant time. Where ML-DSA's operations are integer arithmetic that's relatively straightforward to implement safely, FALCON's sampler is a minefield.",
        "And that minefield is its defining limitation. A signature scheme that leaks information through timing or power consumption can have its private key extracted regardless of how strong the underlying math is — and FALCON's floating-point Gaussian sampler is a textbook side-channel risk, with constant-time implementation widely regarded as one of the harder problems in applied PQC. The practical guidance follows directly: reach for ML-DSA by default, because a slightly larger signature you can implement safely beats a tiny one you can't. Choose FALCON only when its compactness is genuinely required and you have the expertise to implement the sampler without leaking — embedded, IoT, and bandwidth-limited niches where every byte counts and you can afford to get the hard part right.",
      ],
      technical: {
...
# Root CA: SLH-DSA (most conservative)`,
  ...
  // ─── quantum-b07: Hybrid PQC Schemes ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Here is the dilemma every security architect faces in 2024-2025: the lattice algorithms are new and might harbor an undiscovered flaw (remember SIKE), but the classical algorithms are doomed and might already be getting harvested. Trust the new one alone and you're betting on twenty-year-old math; keep the old one alone and you're ignoring the entire threat. Hybrid key exchange refuses the bet. It runs a classical algorithm and a post-quantum one side by side — the canonical pairing is X25519 (proven elliptic-curve) with ML-KEM-768 (post-quantum) — and derives the session's shared secret from *both* outputs combined. To break the session, an attacker now has to break both.",
        "That 'both' is the whole point, and it cuts in two directions at once. If ML-KEM turns out to have a fatal flaw nobody has found yet, X25519 is still standing and your traffic stays secret against any classical adversary. If — when — a cryptographically-relevant quantum computer arrives and snaps X25519 like a twig, ML-KEM is still standing and the quantum attacker gets nothing. You are protected unless *both* fall, and the two rest on entirely unrelated mathematics, so a single breakthrough can't take both. Hybrid also dissolves the deployment chicken-and-egg: you don't have to wait for the entire ecosystem to gain confidence in pure PQC before you start protecting long-lived data today — you get post-quantum protection now with a classical safety net.",
        "This isn't theoretical. Hybrid TLS 1.3 ciphersuites are already carrying real traffic — Cloudflare and Google deployed X25519Kyber768 across their networks back in 2023, and the IETF-standard X25519MLKEM768 followed in 2024; if you've loaded a major website in a recent Chrome or Firefox, you've very likely done a hybrid post-quantum handshake without noticing. NIST's official guidance endorses hybrid as the recommended posture *during* the migration, with pure PQC as the eventual destination once the lattice algorithms have aged enough to earn standalone trust. Hybrid is the bridge: belt-and-suspenders engineering for the years when we must defend against the quantum future without fully letting go of the classical past.",
      ],
      technical: {
...
with oqs.KeyEncapsulation("Kyber768") as kem:
  ...
  // ─── quantum-b08: PQC in TLS ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2025,
      overview: [
        "TLS is where all of this stops being theory, because TLS is the protocol that actually secures the internet — HTTPS, email transport, VPNs, and hundreds of other protocols ride on it. Migrating TLS to post-quantum isn't one swap but three, because a TLS handshake uses cryptography for three distinct jobs. Key exchange (replace RSA/ECDH with ML-KEM, usually hybrid) protects the session secret and is the most urgent because it's what HNDL harvests. Certificate signatures (replace RSA/ECDSA with ML-DSA) are what a CA uses to vouch for a server's identity. And handshake authentication (again ML-DSA) is the live proof, during the handshake, that each side holds the private key it claims. Secrecy and authenticity, the two stages before this, both show up here.",
        "The good news is that TLS 1.3 was, almost by accident, built ready for this. Its clean architectural separation of key exchange, authentication, and bulk encryption means you can swap the key-exchange method without disturbing the rest — exactly the modularity a cryptographic transition needs. The friction isn't in the protocol design; it's in the practical fallout of the previous stages' tradeoffs. ML-DSA's multi-kilobyte signatures inflate certificate chains, and a handshake can carry several of them, so post-quantum handshakes are simply bigger on the wire — occasionally bumping into buffer limits and amplification concerns that never mattered when signatures were 64 bytes. Add the long tail of TLS 1.2 systems that must keep interoperating, and the engineering is less about cryptography than about plumbing larger objects through pipes sized for small ones.",
        "And the ecosystem is already well down this road. BoringSSL ships ML-KEM-768 hybrid, OpenSSL 3.x gets it through the OQS provider, and wolfSSL supports it natively; on the wire, Cloudflare, Chrome, and Firefox have been running hybrid PQC key exchange in production for over a year, and Let's Encrypt — the CA behind a huge fraction of the web's certificates — is actively working through post-quantum certificate issuance. The lesson of this stage is that PQC migration is not a someday research project waiting on better algorithms; the algorithms are standardized, the libraries exist, and the biggest names on the internet have already flipped it on. What remains is the unglamorous work the next two stages cover: finding all your cryptography, and proving your implementation of it is correct.",
      ],
      technical: {
...
# Output: Protocol: TLSv1.3, Key Share: X25519MLKEM768 (hybrid)`,
  ...
  // ─── quantum-b09: Crypto Inventory and Migration ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 2025,
      overview: [
        "You cannot migrate cryptography you don't know you have, and this turns out to be the step that quietly defeats most organizations. Modern enterprise IT has cryptography wired into thousands of places, most of them invisible from any single vantage point: TLS certificates terminating on load balancers, SSH keys baked into CI/CD pipelines, RSA inside VPN concentrators, ECDSA in the code-signing chain, X.509 in API gateways, keys living in HSMs across the infrastructure, and crypto buried inside commercial products whose vendors barely document it. Nobody drew a map as this accreted over twenty years. The first deliverable of a real PQC migration isn't new code — it's that missing map.",
        "Building it is the job of cryptographic discovery, and the artifact it produces has a name borrowed from supply-chain security: the CBOM, or Cryptography Bill of Materials, the cryptographic analogue of the SBOM that lists software dependencies. Discovery tools attack the problem from several angles at once — static scanning of source code and binaries for crypto calls, certificate-inventory crawls, passive network-traffic analysis to see what algorithms are actually negotiated on the wire, and configuration-file inspection. NIST, CISA, and ISO are all standardizing the CBOM format precisely because a one-off spreadsheet doesn't survive contact with a living infrastructure; you need a machine-readable, continuously-updatable inventory you can re-run and diff.",
        "And discovery is genuinely harder than it sounds, which is the warning this stage exists to deliver. Cryptography hides in transitive dependencies three libraries deep, in firmware blobs you can't easily decompile, in vendor-managed appliances that are black boxes by contract, in that one legacy system nobody owns anymore. The failure mode is predictable and expensive: an organization declares discovery 'done,' begins migrating, and then keeps tripping over RSA in places the inventory missed — forcing the whole effort to loop back. Treating discovery as a thorough, ongoing process rather than a checkbox is what separates a migration that finishes from one that thrashes. Find everything first; migrate second.",
      ],
      technical: {
...
for f in findings:
  ...
  // ─── quantum-b10: PQC Implementation and Testing ────────────────────────────
  {
    ...
    info: {
      ...
      year: 2025,
      overview: [
        "A perfect algorithm with a flawed implementation is a broken system — and the post-quantum algorithms are far more treacherous to implement than the RSA and ECDSA they replace. The previous stages hinted at why: ML-DSA's rejection sampling must be done exactly right or signatures leak the private key; FALCON's Gaussian sampler must run in constant time or it bleeds secrets through timing; every scheme has parameter-validation and edge-case handling that, if botched, quietly nullifies the quantum-security guarantee while the code still appears to work. This is not hypothetical hand-wringing — multiple real PQC implementation vulnerabilities have surfaced since the NIST competition began, in serious libraries written by people who knew what they were doing. The math being sound buys you nothing if the code around it isn't.",
        "Because eyeballing crypto code is hopeless, the industry leans on formal validation programs that test implementations against the standard mechanically. NIST's ACVTS (Automated Cryptographic Validation Testing System) drives an implementation through batteries of test vectors, and accredited FIPS 140-3 laboratories certify that a cryptographic module actually conforms. For U.S. federal systems this isn't optional — they're required to use FIPS 140-3 validated modules — and the same discipline is simply good practice for anyone with real security stakes: prefer a validated implementation over a clever home-grown one, every time. Validation is how you transfer trust from 'we wrote it carefully' to 'an independent lab proved it conforms.'",
        "Thorough validation works four distinct seams, because PQC fails in four distinct ways. Known-answer tests (KATs) feed fixed inputs and check for the exact expected output, catching plain correctness bugs. Interoperability testing confirms that your library and someone else's can actually talk — that a ciphertext one produces, the other can decapsulate — which matters enormously in a heterogeneous internet. Side-channel testing verifies the operations run in constant time, so secrets don't leak through timing or power draw (FALCON's whole risk profile lives here). And fault-injection testing checks that the implementation behaves safely when hardware glitches — voltage spikes, induced bit-flips — rather than spilling key material under stress. Pass all four and you have something you can actually deploy: not just quantum-safe mathematics, but a quantum-safe system. That is where this epoch lands — the algorithms exist, they're standardized, and now you know how to choose, combine, find, and verify them.",
      ],
      technical: {
