...
export const aiMlFoundationsStages: StageConfig[] = [
  // ─── 01: Machine Learning Fundamentals ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Artificial intelligence is the broad goal of machines performing tasks that require human-like intelligence; MACHINE LEARNING (ML) is the dominant approach, where a model learns patterns from data rather than following hand-written rules; and DEEP LEARNING is ML using multi-layer neural networks. The core idea: instead of programming the rules, you show the model examples and it infers the rules. ML comes in three main types. SUPERVISED learning trains on LABELED data (inputs paired with correct outputs) to predict labels for new data — split into classification (predict a category, e.g., spam/not-spam) and regression (predict a number, e.g., a house price). UNSUPERVISED learning finds structure in UNLABELED data — clustering (group similar items) and dimensionality reduction. REINFORCEMENT learning trains an agent to maximize a reward through trial-and-error interaction with an environment (how game-playing and robotics AIs learn).",
      ...
      technical: {
        ...
        body: [
          "Pick the learning type by the problem: have labeled examples and want to predict a label? Supervised (classification for categories, regression for numbers). Have unlabeled data and want to discover structure? Unsupervised (clustering, dimensionality reduction). Have an agent that should learn by acting and receiving rewards? Reinforcement learning. Most business ML is supervised, which is why data labeling is so important.",
  ...
  // ─── 02: Data Science & Data Preparation ──────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2014,
      overview: [
        "Machine learning runs on data, and the unglamorous truth is that data preparation is the majority of real ML work. It starts with understanding DATA TYPES: structured data (rows and columns, like a database), unstructured data (text, images, audio, video), and semi-structured (JSON, logs). Features can be numerical (continuous or discrete), categorical (labels like color or country), ordinal (ordered categories), or temporal. For supervised learning you also need LABELS — the correct answers — and labeling is often expensive and a bottleneck (which is why labeled data is so valuable).",
        ...
        "FEATURE ENGINEERING transforms raw data into inputs a model can learn from effectively — and it's often where the biggest accuracy gains come from. Common steps: NORMALIZATION/standardization (scaling numeric features to comparable ranges so one doesn't dominate), ENCODING categorical variables (one-hot encoding turns categories into numeric columns), creating new features from existing ones (e.g., extracting day-of-week from a timestamp), handling text via tokenization/embeddings, and selecting the most useful features. Finally, the prepared dataset is split into train/validation/test sets — being careful to avoid DATA LEAKAGE (letting information from the test set or the future sneak into training, which produces falsely high scores). Mastering data preparation, quality, and feature engineering is foundational across the AI/ML certs, because no algorithm rescues bad data.",
      ],
      technical: {
        ...
        body: [
          ...
          "Watch for the traps: data bias (unrepresentative data → biased, unfair models), class imbalance (use resampling, class weights, or appropriate metrics), and data leakage (compute scaling/feature stats on the training set only, never on the full data including test, and never use future information). Good features and clean, representative data usually beat a fancier algorithm on dirty data.",
        ],
        codeExample: {
  ...
  // ─── 03: Model Training & Evaluation ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "EVALUATION reveals whether a model actually works, and choosing the right METRIC is critical. For classification, raw ACCURACY (fraction correct) is misleading on imbalanced data: a model that always predicts 'not fraud' is 99% accurate on data that's 1% fraud — and useless. So we use the CONFUSION MATRIX (true/false positives and negatives) and derived metrics: PRECISION (of predicted positives, how many were right — minimizes false alarms), RECALL/sensitivity (of actual positives, how many were caught — minimizes misses), and the F1 SCORE (the harmonic mean balancing precision and recall). The ROC curve and AUC summarize performance across thresholds. For regression, metrics like MAE, MSE/RMSE, and R² measure prediction error.",
      ...
      technical: {
        ...
        body: [
          "From the confusion matrix (TP, FP, TN, FN): Precision = TP / (TP + FP) — how trustworthy positive predictions are; Recall = TP / (TP + FN) — how many real positives you catch; F1 = harmonic mean of the two. Accuracy = (TP + TN) / all — fine for balanced data, dangerous for imbalanced. AUC measures ranking quality across thresholds. Choose based on which error is costlier: high recall when misses are dangerous (disease, fraud), high precision when false alarms are costly (spam, blocking legit users).",
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Adapt to your needs in increasing order of effort: prompt engineering (zero/few-shot, chain-of-thought) first; RAG to ground responses in current/private data via embedding retrieval (cuts hallucination, no retraining); fine-tuning when you need consistent task-specific behavior or style. Manage cost and limits via the context window and token usage. Always validate outputs for hallucination, especially for facts, code, and anything high-stakes.",
        ],
        codeExample: {
  ...
  // ─── 06: Cloud AI Platforms (AWS & Google Cloud) ──────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Choosing the right level is a core skill: reach for a pre-trained API when a common task (detect objects, transcribe audio) is needed fast with no ML team; use AutoML/low-code (Vertex AutoML, SageMaker Autopilot, BigQuery ML) when you have your own labeled data but limited ML engineering; and build custom on SageMaker or Vertex when you need full control over architecture and performance. For generative AI, managed model services (Bedrock, Vertex Model Garden) provide foundation models without the cost and complexity of training your own. These platforms also handle the MLOps concerns — scalable serving, pipelines, monitoring — covered earlier. Knowing the platform landscape and the build-vs-buy spectrum is exactly what the AWS AI Practitioner and Google Cloud ML Engineer exams assess.",
      ],
      technical: {
        title: "The Build-vs-Buy Spectrum",
        body: [
          "Decision ladder (least to most effort/control): (1) Pre-trained API — common tasks, no ML expertise, fastest (Rekognition/Comprehend; Vision/Speech APIs). (2) Low-code/AutoML — your data, minimal coding (SageMaker Autopilot, Vertex AutoML, BigQuery ML in SQL). (3) Custom build — full control over architecture/training (SageMaker, Vertex AI custom training). (4) Managed GenAI — foundation models via API without training your own (Amazon Bedrock, Vertex Model Garden/Gemini).",
