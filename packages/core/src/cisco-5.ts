...
export const cisco5Stages: StageConfig[] = [
  ...
  // ─── Stage m39: Cisco Firepower NGIPS — IPS Evasion via Fragmentation ─────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Cisco Firepower is Cisco's Next-Generation Intrusion Prevention System (NGIPS), built into both the Firepower Threat Defense (FTD) software and the dedicated Firepower appliances:\n- It inspects traffic in real time against Snort 3 rule signatures.\n- It performs deep packet inspection (DPI) and enforces application-layer security policies.\n- Firepower Management Center (FMC) centralizes policy management across all sensors.",
        "Despite its capabilities, Firepower — like all signature-based IPS platforms — is vulnerable to evasion that manipulates how packets are fragmented, reordered, or overlapped at the network layer:\n- TCP segmentation evasion splits the payload across segments below the IPS's minimum reassembly threshold.\n- IP fragmentation evasion splits the datagram so the malicious signature never appears in a single fragment.\n- TTL manipulation sends decoy packets that reach the IPS but expire before reaching the endpoint.",
        "Understanding these techniques matters for both red teamers testing IPS effectiveness and defenders tuning Firepower policies — the platform's inline normalization preprocessors counter most classical evasion, but only when correctly enabled and tuned, and default configurations often leave them disabled for performance reasons.",
      ],
      technical: {
        title: "TCP/IP Fragmentation Evasion — Splitting Payloads Past IPS Signature Boundaries",
        body: [
          "Snort-based IPS signatures match byte sequences within reassembled packet payloads, which is exactly what fragmentation evasion targets:\n- The attack deliberately splits the malicious payload across fragment boundaries so no single fragment contains the signature.\n- A SQL injection payload `' OR 1=1--` split as `' OR 1` / `=1--` across two fragments can evade a rule that needs the full sequence.\n- The endpoint's TCP stack then reassembles the fragments into the complete payload and executes the injection.",
          "Cisco Firepower's Stream preprocessor and Frag3 module exist to normalize traffic before signature matching — reassembling fragments and TCP streams before the detection engine sees them:\n- Enabled with `stream_tcp policy windows` (or the right OS policy for the endpoint), Firepower reassembles exactly as the endpoint does, defeating most fragmentation evasion.\n- The key failure mode is misconfigured or disabled stream-reassembly policies.",
          "Modern evasion testing automates fragmented delivery with tools like Fragroute, Scapy, or Nmap's fragmentation flags (`-f`, `--mtu`):\n- Blue-team validation: run Snort/Firepower inline with all preprocessors enabled, then replay known-bad signatures with fragmentation offsets and confirm detection.\n- Any missed detection points straight to a preprocessor gap.",
        ],
        codeExample: {
...
# Accept 10-20% throughput reduction — the trade-off for detection completeness`,
      ...
      incident: {
        ...
        body: [
          "In a documented 2023 red-team engagement at a financial-services firm, the Firepower 4140 appliances ran the default 'balanced' performance profile, which disabled IP fragment reassembly for traffic over 8KB to protect throughput on the high-frequency trading network:\n- The red team used Scapy to deliver fragmented SQL injection payloads against the trading-platform API.\n- None were detected, because Frag3 reassembly wasn't active for that traffic class.\n- The attack was indistinguishable from normal fragmented traffic in FMC's connection events.",
          "The root cause was a documented Cisco deployment trade-off — performance profiles that swap detection completeness for throughput:\n- The 'connectivity over security' inline-normalization policy, chosen to minimize trading-platform latency, disabled fragmentation reassembly.\n- The correct posture for security-critical environments is 'security over connectivity,' which enables all normalizers including Frag3.\n- The engagement drove a full audit of the firm's 47 Firepower sensors — all 47 were in 'connectivity over security' mode, and all were switched over, accepting a 15% throughput reduction on high-bandwidth interfaces.",
  ...
  // ─── Stage m40: FTD/FMC Architecture & IPS Policy — Quiz ──────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Cisco Firepower Threat Defense (FTD) is the unified software image that merges the Cisco ASA firewall engine with the Sourcefire NGIPS engine on one platform:\n- It runs on Firepower appliances (1000/2100/4100/9300), ASA 5500-X hardware via software upgrade, and as a virtual machine (FTDv).\n- Firepower Management Center (FMC) is the centralized management plane.\n- FMC is where all IPS, access-control, SSL-inspection, and network-intelligence policies are authored and deployed to every managed FTD sensor.",
        "FTD runs in three deployment modes with very different security implications, and the mode sets the blast radius of both attacks and misconfigurations:\n- Inline mode places the appliance physically in the traffic path — it can block, reset, and modify connections in real time.\n- Passive (SPAN/tap) mode receives a copy of traffic and can only alert, never block.\n- Inline tap mode is inline but drops all packets rather than blocking — used to shadow-test new IPS policies before production enforcement.",
        "IPS policy tuning is one of the most consequential — and most neglected — tasks in Firepower deployments: the default Snort ruleset has thousands of signatures, many of which throw false positives in specific environments, so an untuned IPS in production either floods the SOC with noise (alert-only) or blocks legitimate traffic (drop). Tuning it well means understanding Snort rule priority tiers, Cisco's recommended tuning workflows, and Firepower's Security Intelligence and Application detectors.",
      ],
      technical: {
        title: "FTD Deployment Modes and IPS Policy Architecture",
        body: [
          "FMC manages FTD sensors through a hierarchical policy stack that every flow is evaluated against:\n- Platform Settings (interface config, HA, licensing) → Access Control Policy (ACP, the master traffic policy) → IPS Policy (Snort rules within ACP rules) → File/Malware Policy (Cisco AMP) → SSL Policy (TLS inspection).\n- The ACP is the entry point — it decides whether traffic is allowed, blocked, or inspected, and which IPS policy applies to each traffic class.",
          "Snort rules in FMC are organized into Cisco Talos-managed rule groups (auto-updated via VRT), community rules, and custom rules:\n- Each rule has a severity (critical/high/medium/low/informational) and a default action (alert, drop, pass, revert).\n- IPS policies run in one of two inspection modes: Detection Only (alert, no blocking) or Prevention (blocks per rule action).\n- New deployments should start in Detection Only to baseline false-positive rates before switching to Prevention.",
          "FMC's Recommendations Engine analyzes the protected network's OS fingerprints (from passive, Nmap-style host discovery) and recommends suppressing rules that can't apply to the detected host OS — Windows SMB rules are irrelevant on a Linux web-server VLAN, for instance. Using it can cut active rule counts by 40–60% while dramatically improving signal-to-noise.",
        ],
        codeExample: {
...
# Add suppression rules for known-good traffic patterns`,
      ...
      incident: {
        ...
        body: [
          "A healthcare IT team deployed Cisco FTD 4150 in inline Prevention mode using the Talos 'Maximum Detection' base policy with no tuning, and it broke clinical operations fast:\n- Within 6 hours, Snort HTTP-anomaly rules flagged HL7 FHIR API calls — which use non-standard verbs like PATCH and PUT with complex JSON bodies — as suspicious and began dropping them.\n- The Electronic Health Records system went inaccessible to clinical staff at 2 PM on a Tuesday, during the hospital's busiest period.",
          "The Cisco-recommended workflow was skipped entirely under schedule pressure from the CISO to have the IPS 'active' before a board audit:\n- That workflow is: baseline in Detection Only for 2+ weeks → run the Recommendations Engine to suppress OS-irrelevant rules → tune on observed false positives → switch to Prevention.\n- The correct emergency fix was immediate reversion to Detection Only followed by proper tuning.\n- Resolution took 4 hours because the network team first suspected the EHR vendor's API, not their newly deployed IPS, as the cause.",
  ...
  // ─── Stage m41: CVE-2022-20927 — Cisco FTD SSL VPN DoS ───────────────────────
  {
    ...
    info: {
      ...
      year: 2022,
      overview: [
        "CVE-2022-20927 is a denial-of-service flaw in the SSL/TLS VPN feature of Cisco Firepower Threat Defense (FTD) software, rooted in mishandling of incomplete TLS handshakes:\n- An unauthenticated remote attacker sends a large volume of TLS ClientHello messages, then abandons each handshake.\n- FTD holds that incomplete handshake state in memory.\n- Once enough accumulates, the appliance stops accepting new VPN connections and may reload.",
        "It's a variant of the well-known 'TLS half-open' or 'SSL exhaustion' attack class — conceptually a TCP SYN flood moved up to the TLS layer — targeting the TLS session table in FTD's VPN subsystem. Unlike a volumetric flood, it needs only enough packets to fill the handshake state table, achievable with modest bandwidth from a single host, and its CVSS 8.6 reflects the unauthenticated, network-exploitable availability impact.",
        "Cisco fixed it in FTD 7.0.4, 7.1.0.3, 7.2.0.1, and later, with stopgaps for unpatched systems:\n- Connection rate limiting on the outside interface.\n- Reducing TLS handshake timeout values.\n- Deploying upstream DDoS mitigation.\nThe vulnerability class captures a core tension in VPN-gateway design — responsiveness to legitimate clients versus resilience against half-open state exhaustion.",
      ],
      technical: {
        title: "TLS Handshake State Exhaustion — Incomplete ClientHello Attack",
        body: [
          "A standard TLS 1.3 handshake runs ClientHello → ServerHello + Certificate + CertificateVerify + Finished → (client) Finished:\n- FTD allocates handshake-state memory the moment a ClientHello arrives and holds it until the handshake completes or times out.\n- CVE-2022-20927 exploits a flaw in FTD's timeout handling for incomplete handshakes — the state isn't reclaimed promptly enough when the client goes silent after ClientHello.",
          "The attack itself is simple resource exhaustion:\n- The attacker sends ClientHello messages at a high rate, each with spoofed source IPs or distinct client-random values to look like unique sessions, and never sends the rest of the handshake.\n- FTD allocates a state-table entry per ClientHello until the table — a bounded resource — fills.\n- New legitimate VPN connection attempts are then rejected; existing established sessions may stay stable, so it's a brownout rather than a full outage.",
          "Detection comes down to spotting incomplete handshakes piling up:\n- A spike in TLS handshake state-table utilization in FTD's `show ssl statistics`, plus a high rate of ClientHello messages in NetFlow/Stealthwatch data.\n- The tell is timing: legitimate TLS clients almost always finish the handshake within 100–500ms, while CVE-2022-20927 exploiters let the 30–60 second timeout elapse before the state is reclaimed.",
        ],
        codeExample: {
...
# Cisco advisory: cisco-sa-ftd-ssl-dos-TGTkWZFz`,
      ...
      incident: {
        ...
        body: [
          "Three weeks after Cisco's advisory, an unpatched FTD 2140 at a manufacturing firm was hit with a TLS handshake exhaustion attack:\n- It began at 6:45 AM on a Monday — peak VPN time as employees started the week — and filled the SSL handshake state table within 22 minutes.\n- All new VPN connection attempts returned 'unable to connect' to 3,200 remote workers.\n- It used diverse source IPs to dodge simple rate-limiting, sending ClientHellos and going silent, each holding a state entry for the full 30-second timeout.",
          "The patch had been staged for the following weekend's maintenance window — a 10-day delay from the advisory — and the attack forced an emergency mid-week deployment:\n- The 4 AM patch carried a 45-minute VPN outage for already-connected users during application.\n- The SOC identified the pattern from FMC connection events showing thousands of SSL 'Unknown' status entries — TLS handshakes that never completed — from diverse source IPs across multiple countries.\n- That spread was the signature of a coordinated exhaustion campaign rather than normal VPN traffic.",
  ...
  // ─── Stage m42: Cisco SecureX/XDR — Threat Hunting CTF ───────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Threat hunting in Cisco XDR leans on a few core tools:\n- The Orbital Query Engine (a Secure Endpoint feature) for live host interrogation.\n- The Investigate portal for IOC pivoting across 100+ Talos threat-intelligence feeds.\n- The Casebook workflow for documenting investigation findings.\nThe XDR Ribbon — the persistent UI bar across Cisco security product screens — enables instant pivots: an IP seen in a Firepower alert can be queried in Umbrella DNS logs, Secure Endpoint telemetry, and Talos reputation without leaving the current screen.",
        "Lateral movement is one of XDR's most important hunt targets precisely because it spans multiple data sources:\n- Network connections visible in Firepower.\n- DNS queries visible in Umbrella.\n- File writes visible in Secure Endpoint.\n- Authentication events visible in Active Directory logs.\nNo single tool sees the whole picture — XDR's value is the correlation, mapping the lateral-movement chain from the initial compromised workstation to the final targeted server.",
      ],
      technical: {
        title: "XDR Cross-Source Pivot — Correlating Endpoint, Network, and DNS Telemetry",
        body: [
          "Cisco XDR's investigation workflow starts with a seed IOC — usually an IP, domain, file hash, or email address that looks suspicious:\n- The Investigate portal submits the seed to all connected modules at once and aggregates results into a single observable graph.\n- Each result carries a disposition (clean, malicious, suspicious, unknown), backed by Talos threat-intel data and first/last-seen timestamps.",
          "For lateral-movement detection, hunters typically pivot on internal IP addresses. When Secure Endpoint flags a suspicious process on workstation WS-01 making an unusual connection to internal server 10.10.50.5, the XDR pivot pulls:\n- Firepower connection logs for that IP pair.\n- Umbrella DNS queries from WS-01 before the connection.\n- Secure Endpoint process events on 10.10.50.5 around that time.\n- Active Directory authentication logs.\nThe resulting timeline exposes the kill chain.",
...
WHERE p.name IN ('chrome.exe', 'firefox.exe', 'msedge.exe')
  AND c.remote_port NOT IN (80, 443, 8080, 8443);`,
      ...
      incident: {
        ...
        body: [
          "A Cisco XDR threat hunt at a national retail chain started from a single Talos alert — a file hash on a store manager's workstation matching a known Cobalt Strike beacon:\n- The Secure Endpoint alert had been triaged as low severity because the host was a retail point-of-sale management station, not classified as high-value infrastructure.\n- An XDR pivot on the beacon's C2 IP revealed that the same C2 had been contacted by 13 other hosts across 8 stores over 18 days.\n- Every one of those contacts sat below the individual product alert thresholds for Secure Endpoint, Umbrella, and Firepower.",
          "The XDR casebook mapped the complete kill chain and enabled one-shot containment:\n- Initial access via phishing email (Secure Email), Cobalt Strike beacon on the store-manager workstation (Secure Endpoint), lateral movement to a store server via SMB (Firepower NetFlow), AD reconnaissance DNS queries (Umbrella), and an attempted reach into the payment-processing VLAN (blocked by Firepower, logged in FMC, but the connection event had gone unreviewed).\n- All 14 compromised hosts were isolated at once via XDR's automated response playbook — quarantine commands delivered to Secure Endpoint across all 8 stores in a single workflow, with no analyst logging into 14 devices.",
  ...
  // ─── Stage m43: Cisco XDR Architecture — Quiz ────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The XDR architecture centers on three capabilities, plus a persistent integration layer:\n- Threat Response — IOC investigation and disposition across all integrated sources.\n- Orbital — a live endpoint query engine for real-time forensics.\n- Automate — SOAR-style playbook orchestration via a drag-and-drop workflow builder.\nThe XDR Ribbon is embedded in every Cisco Security product's UI, enabling instant cross-product pivots without navigating away from the current screen.",
        "XDR's automated response runs pre-built and custom playbooks triggered by detections, and a single playbook can:\n- Quarantine a host via Secure Endpoint and block an IP via Firepower.\n- Add a domain to Umbrella's block list and enrich an alert with Talos threat intelligence.\n- Create a ServiceNow ticket — all within seconds of detection, with no human in the loop.\nFor high-confidence, well-defined attack patterns, that drops mean time to respond (MTTR) from hours to seconds.",
      ],
      technical: {
        title: "XDR Telemetry Sources, Correlation Engine, and Response Orchestration",
        body: [
          "Cisco XDR ingests telemetry through Module integrations — each Cisco product (Secure Endpoint, Umbrella, Firepower, Secure Email) has a registered Module:\n- Modules handle authentication, telemetry streaming, and response-action routing.\n- Third-party modules (Microsoft Defender, CrowdStrike, Splunk, Palo Alto) are available in the module catalog.\n- Each module contributes 'observables' (IPs, domains, hashes, URLs) and 'targets' (hosts, email addresses) to the XDR investigation graph.",
          "The Threat Response engine normalizes observables across all modules into a unified schema and queries each module's threat intelligence for disposition (clean, malicious, suspicious, unknown), aggregating the results into an investigation graph that links observables to targets and to other observables — a visual attack chain — and scores the overall investigation by the severity and confidence of each connected indicator.",
          "Cisco XDR Automate is the SOAR (Security Orchestration, Automation, and Response) component, with workflows built visually from a trigger connected to action blocks:\n- Triggers: an XDR alert, manual, or scheduled.\n- Action blocks: Secure Endpoint quarantine host, Umbrella block domain, Firepower block IP, Threat Intelligence enrich observable, Webex notify SOC.\n- Pre-built workflows from the Cisco Exchange marketplace cover common incident types — ransomware response, phishing triage, lateral-movement containment.",
        ],
        codeExample: {
  ...
  ELSE IF (file_hash = SUSPICIOUS):
    ...
    → XDR: flag for analyst review`,
      ...
      incident: {
        ...
        body: [
          "A LockBit 3.0 ransomware variant gained initial access via a phishing email at a healthcare network, and Cisco Secure Endpoint caught the encryption behavior within 4 minutes of execution:\n- It flagged VSSADMIN shadow-copy deletion, rapid file encryption, and process hollowing as high-confidence malware indicators.\n- The detection fired a pre-configured XDR Automate playbook that executed simultaneously: quarantine commands to Secure Endpoint for 3 infected workstations (off the network within the same second), an Umbrella DNS block for the ransomware's C2 domain across all 12 hospitals, Firepower block rules for the C2 IP at every sensor, and a Webex notification to the SOC with the casebook link.",
          "Total automated response time from detection to containment of all 3 hosts across 12 hospital sites was 47 seconds:\n- The SOC analyst reviewed the casebook 8 minutes later and confirmed the response was correct — no additional hosts had been compromised in that window, because the C2 was already blocked network-wide.\n- Without pre-built playbooks, manually identifying and responding to 3 hosts across 12 distributed sites — logging into each hospital's Secure Endpoint console, coordinating C2 blocks with network teams, and communicating across IT departments — would have taken 30–60 minutes.\n- LockBit's observed lateral-movement cadence is typically 8–15 minutes from execution to reaching file servers.",
  ...
  // ─── Stage m44: Cisco DevNet REST API Security — CTF ─────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "REST API security on a network-management platform carries disproportionate impact — a SQL injection in a web app touches that app's data, but a DNA Center API flaw touches the configuration integrity of the entire managed network. Cisco advisories have documented several real ones:\n- Unauthenticated API access issues in DNA Center (CVE-2021-1257, CVE-2021-1258).\n- Authentication bypass vulnerabilities.\n- Insufficient authorization checks letting a read-only API user invoke privileged operations.",
      ...
      technical: {
        ...
        body: [
          "Cisco DNA Center's REST API uses token-based authentication with two token types:\n- Short-lived service tokens (10 minutes), obtained by exchanging user credentials via POST to `/dna/system/api/v1/auth/token`.\n- Long-lived integration tokens (valid up to 6 months), created by admins for automation systems.\nThe token rides as the `X-Auth-Token` header on every subsequent request, and failing to scope integration tokens to specific operations is a recurring misconfiguration.",
          "The DNA Center API has a documented privilege model — SUPER-ADMIN-ROLE, NETWORK-ADMIN-ROLE, and OBSERVER-ROLE — and the authorization checks between them are critical security boundaries:\n- Verifying that an OBSERVER-ROLE token can't invoke SUPER-ADMIN-ROLE operations is exactly the kind of check Cisco has had to issue advisories for, where OBSERVER tokens could reach restricted operations.\n- The attack pattern: obtain a low-privilege token (say, from an integration account with weak credentials), enumerate the API, and find operations that should require elevated privilege but don't check adequately.",
...
# Rotate all long-lived integration tokens immediately after any exposure`,
      ...
      incident: {
        ...
        body: [
          "A network automation engineer at a research university committed a Python automation script to a public GitHub repo with the SUPER-ADMIN integration token hardcoded as a string:\n- A GitHub Actions workflow in the same repo imported the script, exposing the token in workflow execution logs and the commit history.\n- Within 36 hours of the public commit, an automated credential-scanning service that continuously monitors GitHub for API keys scraped the token.\n- An attacker then used it to start accessing the DNA Center REST API.",
          "With the exposed SUPER-ADMIN token, the attacker turned read access into a quiet persistence play:\n- They called the device-inventory endpoint to enumerate all 600 managed devices and their locations, then identified the switches serving the university's research-computing VLAN.\n- They pushed VLAN-reassignment changes moving 12 research workstations off the monitored, EDR-covered research VLAN onto a general-access VLAN with less monitoring — apparently to create an unmonitored persistence point.\n- The changes were caught 4 hours later during a routine configuration audit, and DNA Center's audit log preserved the full API call history (timestamps + endpoint details) for complete forensic reconstruction.",
  ...
  // ─── Stage m45: NETCONF/YANG/gRPC — Network Programmability Security — Quiz ───
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The security implications of that programmable plane are profound:\n- NETCONF/YANG rides SSH — secure when properly configured — but the authorization model (which YANG operations a given SSH user can invoke) is set per device and is frequently misconfigured.\n- A NETCONF session with SUPER-USER capability can push arbitrary configuration changes to the device.\n- gRPC telemetry streams, if intercepted, reveal real-time operational state — session counts, route tables, interface utilization — valuable reconnaissance for an attacker.",
      ],
      technical: {
        title: "NETCONF Protocol Operations, YANG Capability Negotiation, and gRPC Telemetry Security",
        body: [
          "NETCONF defines four datastores:\n- running (active configuration), candidate (staged changes before commit), startup (persisted config loaded at boot), and operational (live, read-only state).\n- Configuration changes are sent as XML-encoded `<edit-config>` operations targeting the running or candidate datastore, and the YANG model constrains what the XML can contain — invalid structures are rejected.\n- NETCONF over SSH on port 830 is the transport standard, and port 830 should be restricted to trusted management hosts.",
          "YANG capability negotiation happens at session establishment — the device sends a `<hello>` listing every YANG module it supports — so an attacker who intercepts that hello can fingerprint the exact software version and feature set from the capability list and select a targeted exploit; the list of module names, revision dates, and feature flags is far richer than most banner grabs.",
          "gRPC Model-Driven Telemetry uses mutual TLS (mTLS) for transport security when properly configured — and the failure modes are serious:\n- Without mTLS, gRPC streams are either unencrypted or use one-way TLS (server certificate only, no client authentication).\n- An attacker on the management network who can intercept gRPC telemetry gets a continuous stream of operational data.\n- Worse, if the subscription configuration accepts subscriptions without authentication, an attacker can register as a telemetry collector and receive the same data the NOC does.",
        ],
        codeExample: {
...
grpc
  ...
  # Without this: unauthenticated telemetry subscription possible`,
      ...
      incident: {
        ...
        body: [
          "An ISP's automation team provisioned a single NETCONF service account (`netconf-auto`) with full SUPER-USER privilege across all 340 provider-edge routers, justifying it as simpler automation development — fewer accounts, fewer permission errors:\n- When the automation server was compromised via an SQL injection in its web dashboard, the attacker found the NETCONF credentials stored in plaintext in the framework's config directory (`/etc/nso/nso.conf`).\n- Forensics confirmed the attacker had opened SSH connections to all 340 routers and completed NETCONF hello handshakes — enumerating the complete YANG capability set on every device.\n- They hadn't yet pushed any configuration changes when the compromise was caught during an unrelated security scan.",
          "The near-miss forced an emergency, coordinated rollout across all 340 devices:\n- The single SUPER-USER account was replaced with function-specific NETCONF accounts scoped to specific YANG modules — one for interface configuration, one for routing policy, one for telemetry-subscription management.\n- That least-privilege design should have been in place from initial deployment.\n- Because all 340 devices needed simultaneous changes, the rollout took 4 hours of coordinated change management with carrier operations teams across multiple time zones.",
  ...
  // ─── Stage m46: CyberOps — SOC Analyst Kill Chain CTF ────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Cisco CyberOps Associate certification (CBROPS 200-201) defines the core competency framework for SOC analysts, across five domains:\n- Security concepts, security monitoring, host-based analysis, network intrusion analysis, and security policies and procedures.\nThe most critical applied skill is kill chain analysis — given alerts, logs, and network data, reconstruct the complete attacker timeline from initial access through command and control to objectives.",
        "Phishing-to-domain-compromise is the most common enterprise attack pattern SOCs see, and the typical chain is well-worn:\n- Spear-phishing email delivers a macro-enabled document → the user opens it and the macro executes.\n- PowerShell downloads and runs a Cobalt Strike beacon → the beacon establishes a C2 channel.\n- The attacker harvests credentials (Mimikatz) → pass-the-hash lateral movement to a domain controller.\n- Active Directory enumeration and Domain Admin account creation.\nEach step leaves artifacts visible to a SOC analyst with the right data sources.",
      ...
      technical: {
        ...
        body: [
          "Detecting initial access via spearphishing takes correlation across three sources, since none flags it alone:\n- Email gateway analysis (Cisco Secure Email: sender reputation, attachment sandbox verdict, URL analysis).\n- Endpoint telemetry (Cisco Secure Endpoint: a file-write event when the attachment is saved, a process-creation event when the document opens).\n- Network analysis (Firepower: a DNS query to a newly-registered domain, outbound HTTP to a non-categorized URL).\nThe correlation of all three provides high-confidence detection.",
...
| alert SOC HIGH MITRE_ATT&CK=T1550.002`,
      ...
      incident: {
        ...
        body: [
          "A finance employee received a spearphishing email with a macro-laden Word document:\n- Cisco Secure Email flagged the attachment as suspicious — macro-enabled, external sender not on the trusted list — but delivered it, because policy permitted macro documents from external senders for business reasons.\n- Three minutes after delivery, Cisco Secure Endpoint fired: WINWORD.EXE spawning powershell.exe with an `-EncodedCommand` argument — a top-tier ATT&CK T1059.001 indicator matching the Cobalt Strike payload delivery pattern.",
          "The SOC analyst correlated that endpoint alert with an Umbrella DNS alert 45 seconds later — the same workstation querying a newly-registered domain (3 days old) consistent with Cobalt Strike's malleable C2 profile:\n- The two-signal correlation (Office process spawning PowerShell, then a DNS query to a fresh domain) was enough for immediate escalation.\n- The analyst approved the XDR quarantine playbook and the workstation was isolated in 8 seconds.\n- Post-mortem confirmed a Cobalt Strike beacon that had completed the C2 callback handshake but not yet received lateral-movement tasking — total time from phishing delivery to network isolation: 30 minutes.",
  ...
  // ─── Stage m47: CyberOps Associate — Domains & Exam Alignment — Quiz ─────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Cisco CyberOps Associate certification (exam 200-201 CBROPS) is the entry-level credential for SOC analyst roles, organized into five exam domains:\n- Security Concepts (25%), Security Monitoring (22%), Host-Based Analysis (20%), Network Intrusion Analysis (21%), and Security Policies and Procedures (12%).\nIt maps to the NICE Cybersecurity Workforce Framework (SP-OPS-001 — Cyber Defense Analyst work role) and to MITRE ATT&CK.",
        "The Security Monitoring domain covers the data sources a SOC analyst uses to detect threats — and knowing what each can and can't see is a core competency:\n- SIEM correlation rules detect known patterns.\n- Full packet capture (FPC) enables retroactive investigation.\n- NetFlow/IPFIX detects anomalous traffic volumes without payload analysis.\n- Plus security event log management across the stack.",
      ...
      technical: {
        ...
        body: [
          "The SOC data-source hierarchy in a mature Cisco deployment layers detection so each catches what the previous misses:\n- Cisco Secure Email (email threat telemetry) → Cisco Umbrella (DNS-layer security, first detection of C2 domains) → Cisco Secure Endpoint (endpoint behavioral telemetry, file hashes, process chains).\n- → Cisco Firepower (network intrusion signatures, NetFlow, SSL inspection) → Cisco Secure Network Analytics (network behavior analytics, anomaly detection at scale).\n- Email blocks known-malicious attachments, DNS catches C2 callbacks to new domains, endpoint detects payload execution, and network detects lateral-movement patterns.",
          "SIEM correlation rules combine events from multiple sources with temporal and relational logic — a well-designed PowerShell-download-cradle rule fires only when `WINWORD.EXE spawns powershell.exe AND powershell.exe has '-EncodedCommand' in cmdline AND PowerShell makes an outbound HTTP/S connection within 60 seconds`. That three-condition rule has a high true-positive rate and low false-positive rate because each condition alone might be benign, but the combination is almost exclusively malicious.",
          "IDS alert classification in SOC practice rests on four outcomes:\n- True Positive (TP) — alert fires and the attack is real; False Positive (FP) — alert fires but the traffic is benign.\n- True Negative (TN) — no alert and no attack; False Negative (FN) — an attack occurs but no alert fires.\n- SOC ROI depends on minimizing FP (wasted analyst time) and FN (missed attacks): Precision = TP / (TP + FP); Recall = TP / (TP + FN).\nA mature SOC tracks both metrics per rule set and tunes accordingly.",
        ],
        codeExample: {
...
# 4. Document changes in SIEM rule management system`,
      ...
      incident: {
        ...
        body: [
          "A financial-services SOC was generating 2.3 million SIEM alerts a day at a 96% false-positive rate — effectively 2.2 million false alarms a day for 12 analysts on 8-hour shifts:\n- The team had rationally adapted: tickets were opened for all high-severity alerts, but the backlog averaged 9 hours, and analysts had learned that nearly all 'Office spawns PowerShell' alerts came from legitimate IT automation scripts.\n- When LockBit ransomware triggered a real high-severity alert with exactly that rule pattern (Office macro + PowerShell with EncodedCommand + outbound C2 callback), it entered the 9-hour backlog behind 847 other alerts of the same type.",
          "The ransomware alert was actioned 4 hours into the backlog — by which point LockBit's encryption had reached 14 servers, and a $280,000 ransom was paid:\n- The root cause was a known-good IT automation script that had been generating the Office → PowerShell pattern for 11 months without ever being added to a SIEM exclusion list.\n- A 3-month tuning project afterward cut alert volume to 180,000/day (a 92% reduction) while holding true-positive recall at 97%.\n- It did so mainly by suppressing known-good process chains, requiring multiple simultaneous ATT&CK-correlated indicators before firing, and adding ML-assisted scoring that deprioritized single-indicator alerts.",
  ...
  // ─── Stage m48: Cisco Silicon One — Security Architecture — CTF ───────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The security implications of that programmable forwarding pipeline are significant:\n- Silicon One supports P4 (Programming Protocol-Independent Packet Processors), a domain-specific language for defining how the ASIC parses and forwards packets.\n- P4 programs compile to ASIC configuration tables of match-action rules executed at line rate, and the pipeline can be modified at runtime via the control plane.\n- A compromised control plane that can push malicious P4 programs could redirect, drop, or duplicate traffic at wire speed — 25.6 Tbps of traffic-manipulation capability in one ASIC.",
      ...
      technical: {
        ...
        body: [
          "Silicon One's P4-programmable forwarding pipeline works as a match-action table hierarchy:\n- Headers are parsed from the incoming packet, matched against programmed tables, and forwarding actions are applied.\n- The control plane (IOS XR or IOS XE) populates those tables by translating high-level routing and policy config into ASIC table entries.\n- The boundary between the control plane (software) and the forwarding plane (P4 ASIC tables) is critical: unauthorized modification of table entries — via an IOS XR vulnerability, a NETCONF misconfiguration, or a supply-chain compromise in the P4 compiler toolchain — yields hardware-enforced traffic manipulation.",
...
# Monitor for unexpected table entry count changes`,
      ...
      incident: {
        ...
        body: [
          "A cloud service provider running Cisco 8808 routers with Silicon One chips had gRPC Model-Driven Telemetry in plaintext mode — no TLS authentication, no subscriber whitelist — on the out-of-band management network:\n- The config was the lab default, copied to production during a rushed deployment.\n- An attacker from a competing cloud provider reached the management network via a compromised VPN account belonging to a network-monitoring contractor.\n- They registered a gRPC subscription to the Silicon One telemetry stream with a standard gRPC client and began silently receiving per-second traffic-matrix data — the subscription showed up in the subscriber list, but the list wasn't monitored.",
          "The stream handed over a remarkably complete intelligence picture over 11 days:\n- Per-interface utilization on all spine links — the provider's datacenter-interconnect capacity and growth trajectory.\n- Per-flow statistics aggregated by destination Autonomous System — which cloud tenants received traffic from which enterprise customers.\n- Queue-depth statistics at microsecond granularity — which tenants generated bursty traffic and which applications were latency-sensitive.\nThe anomalous subscription was finally caught in a routine telemetry-infrastructure audit, by which point the intercepted matrix gave the competitor detailed intelligence to target the provider's highest-value customers with competitive pricing.",
  ...
  // ─── Stage m49: Silicon One P4 Pipeline Integrity — Quiz ─────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "P4 (Programming Protocol-Independent Packet Processors) is the domain-specific language that defines forwarding behavior in programmable network ASICs, Cisco Silicon One included, and a P4 program has three parts:\n- A packet parser — how headers are extracted from raw bytes.\n- Match-action tables — what to do when a header field matches a rule.\n- A deparser — how to reconstruct the packet for forwarding.\nP4 programs compile to ASIC configuration that runs at wire speed — in Silicon One's case, 25.6 Tbps with no CPU involvement.",
        "The security architecture of a P4 chip like Silicon One separates the data plane from the control plane, and that split cuts both ways:\n- Data plane — the ASIC running P4-compiled tables, processing billions of packets per second with no software involvement.\n- Control plane — IOS XR software on a general-purpose CPU, computing routing tables and pushing entries to the ASIC via the P4Runtime API or vendor ABIs.\n- As a security asset, the data plane runs at hardware speed and can't be directly touched by software — the control plane communicates only through a well-defined interface.\n- As a liability, a compromised control plane can push arbitrary forwarding-table entries to the ASIC.",
      ...
      technical: {
        ...
        body: [
          ...
          "The IOS XR control-plane trust boundary spans several interfaces, and compromise of any of them reaches the ASIC indirectly:\n- NETCONF (port 830) for configuration, gRPC MDT for telemetry, and BGP for routing control.\n- An attacker who compromises IOS XR — via an IOS XR vulnerability (numerous advisories including CVE-2020-3473, CVE-2021-34713) or a management-plane privilege escalation — can program Silicon One indirectly by modifying routing and policy config that IOS XR translates into ASIC table entries.\n- Segment Routing v6 (SRv6) is especially sensitive: an attacker with IOS XR access can insert malicious SRv6 segment IDs that redirect traffic through an eavesdropping node before delivery.",
        ],
        codeExample: {
...
# Cross-reference against Cisco PSIRT signed software manifest`,
      ...
      incident: {
        ...
        body: [
          "A sophisticated threat actor exploited CVE-2021-34713 (authenticated remote code execution in Cisco IOS XR's MPLS ping, CVSS 7.5) to gain root-level access to an IOS XR process on a carrier backbone router:\n- Using that access, the attacker modified the SRv6 segment-routing policy to insert an attacker-controlled segment ID on traffic matching specific destination prefixes — a financial institution's datacenter address range.\n- Packets matching the prefix were steered through the attacker's router before delivery, giving a passive man-in-the-middle position for encrypted-traffic capture and metadata collection.",
          "Detection finally came from an anomalous SRv6 locator entry in `show segment-routing srv6 locator` — pointing to an IP outside the carrier's documented SRv6 domain:\n- It had gone undetected for 6 days because the carrier's monitoring checked BGP route tables, ACL configurations, and interface traffic statistics — none of which showed anomalies, since the malicious SRv6 segment was applied within the legitimate forwarding plane rather than as a BGP route change.\n- Standard monitoring simply had no visibility into SRv6 segment-table contents as a security-relevant artifact.",
  ...
  // ─── Stage m50: Cisco Quantum-Safe Networking — PQC on Silicon One ────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "HARVEST NOW, DECRYPT LATER (HNDL) is the strategic posture nation-state intelligence agencies have adopted against the future threat of large-scale quantum computers:\n- Adversaries are intercepting and archiving encrypted network traffic today — BGP sessions, IPsec VPN tunnels, MACsec-protected inter-datacenter links, management-plane communications.\n- They intend to decrypt that archive once sufficiently powerful quantum computers arrive (estimated 2030–2035 for cryptographically relevant scale).\n- Network infrastructure is a primary HNDL target because it carries the highest-value, highest-volume encrypted traffic in the enterprise.",
        "Cisco's quantum-safe networking roadmap addresses HNDL across the full stack:\n- Transport layer — MACsec on Silicon One will support post-quantum cipher suites (ML-KEM/CRYSTALS-Kyber hybrid with AES-GCM) once NIST and IEEE 802.1AE standardization completes, projected 2025–2026.\n- IKEv2 / IPsec layer — Cisco IOS XR 7.9 added IKEv2 hybrid post-quantum key-exchange groups (RFC 9370) combining classical ECDH-P256 with ML-KEM-768 for HNDL resistance on all tunnel negotiations.\n- BGP control plane — post-quantum authentication of BGP UPDATE messages is being standardized through IETF SIDROPS.",
        "Quantum Key Distribution (QKD) is a complementary approach to PQC: instead of relying on computational hardness assumptions (even quantum-resistant ones), it uses the laws of quantum physics to distribute keys with information-theoretic security. Cisco has partnered with Toshiba and other QKD vendors to feed QKD key material into MKA (MACsec Key Agreement) on Silicon One — refreshing MACsec SAKs with QKD-sourced keys over fiber where QKD infrastructure exists — though QKD needs a dedicated quantum channel (a single-photon optical fiber link) and is currently practical only for high-security point-to-point links up to roughly 100km.",
      ],
      technical: {
        title: "IKEv2 Hybrid PQC, QKD-MACsec Integration, and Post-Quantum BGP Authentication",
        body: [
          "IKEv2 hybrid post-quantum key exchange (RFC 9370) combines classical Elliptic Curve Diffie-Hellman (ECDH-P256) with a NIST PQC KEM (ML-KEM-768, aka CRYSTALS-Kyber-768) in a single IKEv2 exchange:\n- The hybrid keeps the session secure against both classical attacks and quantum attacks (Shor's algorithm).\n- If ML-KEM-768 is later found flawed, ECDH-P256 still provides classical security; if ECDH-P256 is broken by a future quantum computer, ML-KEM-768 still provides quantum security.\n- The two are combined — the final session key is derived from both shared secrets via HKDF.",
...
# Key source: QKD (information-theoretic security)`,
      ...
      incident: {
        ...
        body: [
          "The 2013 Snowden disclosures revealed NSA collection programs that captured internet-backbone traffic including encrypted sessions:\n- MUSCULAR targeted Google and Yahoo datacenter interconnects; UPSTREAM tapped submarine cables and IXP transit links.\n- FISA court documents partially declassified in 2023 confirmed the collection included BGP control-plane session traffic, IKEv2 negotiation handshakes (which reveal the key-exchange algorithm and enable retroactive decryption if it's later broken), and traffic on MACsec-protected inter-datacenter links.\n- The stated rationale for some programs included long-term archiving for retroactive decryption — the Harvest Now, Decrypt Later posture.",
          "Network operators responded by accelerating Perfect Forward Secrecy (PFS) in IKEv2 and MACsec, so that even a later-broken long-term key can't derive individual session keys:\n- But PFS only provides temporal isolation of session keys — it doesn't solve HNDL when the key-exchange algorithm itself (classical ECDH-P256) is vulnerable to Shor's algorithm on a cryptographically relevant quantum computer.\n- The NIST PQC standardization effort, launched in 2016, finalized in 2024 with FIPS 203 (ML-KEM/CRYSTALS-Kyber), FIPS 204 (ML-DSA/CRYSTALS-Dilithium), and FIPS 205 (SLH-DSA/SPHINCS+).\n- Cisco's RFC 9370 IKEv2 hybrid PQC in IOS XR 7.9, combining ML-KEM-768 with ECDH-P256, is the direct operational response to HNDL for network-infrastructure operators.",
