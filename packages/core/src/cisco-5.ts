...
export const cisco5Stages: StageConfig[] = [
  ...
  // ─── Stage m39: Cisco Firepower NGIPS — IPS Evasion via Fragmentation ─────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Cisco Firepower is Cisco's Next-Generation Intrusion Prevention System (NGIPS), built into both the Firepower Threat Defense (FTD) software and the dedicated Firepower appliances. It inspects traffic in real time against Snort 3 rule signatures, performs deep packet inspection (DPI) and enforces application-layer security policies, while Firepower Management Center (FMC) centralizes policy management across all sensors.",
        "Despite its capabilities, Firepower — like all signature-based IPS platforms — is vulnerable to evasion that manipulates how packets are fragmented, reordered, or overlapped at the network layer. TCP segmentation evasion splits the payload across segments below the IPS's minimum reassembly threshold; IP fragmentation evasion splits the datagram so the malicious signature never appears in a single fragment; and TTL manipulation sends decoy packets that reach the IPS but expire before reaching the endpoint.",
...
# Accept 10-20% throughput reduction — the trade-off for detection completeness`,
  ...
  // ─── Stage m40: FTD/FMC Architecture & IPS Policy — Quiz ──────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Cisco Firepower Threat Defense (FTD) is the unified software image that merges the Cisco ASA firewall engine with the Sourcefire NGIPS engine on one platform. It runs on Firepower appliances (1000/2100/4100/9300), on ASA 5500-X hardware via software upgrade, and as a virtual machine (FTDv), while Firepower Management Center (FMC) is the centralized management plane — where all IPS, access-control, SSL-inspection, and network-intelligence policies are authored and deployed to every managed FTD sensor.",
        "FTD runs in three deployment modes with very different security implications, and the mode sets the blast radius of both attacks and misconfigurations. Inline mode places the appliance physically in the traffic path, where it can block, reset, and modify connections in real time; passive (SPAN/tap) mode receives a copy of traffic and can only alert, never block; and inline tap mode is inline but drops all packets rather than blocking, used to shadow-test new IPS policies before production enforcement.",
...
# Add suppression rules for known-good traffic patterns`,
  ...
  // ─── Stage m41: CVE-2022-20927 — Cisco FTD SSL VPN DoS ───────────────────────
  {
    ...
    info: {
      ...
      year: 2022,
      overview: [
        "CVE-2022-20927 is a denial-of-service flaw in the SSL/TLS VPN feature of Cisco Firepower Threat Defense (FTD) software, rooted in mishandling of incomplete TLS handshakes. An unauthenticated remote attacker sends a large volume of TLS ClientHello messages and then abandons each handshake, FTD holds that incomplete handshake state in memory, and once enough accumulates the appliance stops accepting new VPN connections and may reload.",
        ...
        "Cisco fixed it in FTD 7.0.4, 7.1.0.3, 7.2.0.1, and later, with stopgaps for unpatched systems: connection rate limiting on the outside interface, reducing TLS handshake timeout values, and deploying upstream DDoS mitigation. The vulnerability class captures a core tension in VPN-gateway design — responsiveness to legitimate clients versus resilience against half-open state exhaustion.",
      ],
      technical: {
  ...
  // ─── Stage m42: Cisco SecureX/XDR — Threat Hunting CTF ───────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Threat hunting in Cisco XDR leans on a few core tools: the Orbital Query Engine (a Secure Endpoint feature) for live host interrogation, the Investigate portal for IOC pivoting across 100+ Talos threat-intelligence feeds, and the Casebook workflow for documenting investigation findings. The XDR Ribbon — the persistent UI bar across Cisco security product screens — enables instant pivots: an IP seen in a Firepower alert can be queried in Umbrella DNS logs, Secure Endpoint telemetry, and Talos reputation without leaving the current screen.",
        "Lateral movement is one of XDR's most important hunt targets precisely because it spans multiple data sources: network connections visible in Firepower, DNS queries visible in Umbrella, file writes visible in Secure Endpoint, and authentication events visible in Active Directory logs. No single tool sees the whole picture — XDR's value is the correlation, mapping the lateral-movement chain from the initial compromised workstation to the final targeted server.",
      ],
      technical: {
  ...
  // ─── Stage m43: Cisco XDR Architecture — Quiz ────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The XDR architecture centers on three capabilities, plus a persistent integration layer: Threat Response for IOC investigation and disposition across all integrated sources, Orbital as a live endpoint query engine for real-time forensics, and Automate for SOAR-style playbook orchestration via a drag-and-drop workflow builder. The XDR Ribbon is embedded in every Cisco Security product's UI, enabling instant cross-product pivots without navigating away from the current screen.",
        "XDR's automated response runs pre-built and custom playbooks triggered by detections, and a single playbook can quarantine a host via Secure Endpoint and block an IP via Firepower, add a domain to Umbrella's block list and enrich an alert with Talos threat intelligence, and create a ServiceNow ticket — all within seconds of detection, with no human in the loop. For high-confidence, well-defined attack patterns, that drops mean time to respond (MTTR) from hours to seconds.",
      ],
      technical: {
  ...
  // ─── Stage m44: Cisco DevNet REST API Security — CTF ─────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "REST API security on a network-management platform carries disproportionate impact — a SQL injection in a web app touches that app's data, but a DNA Center API flaw touches the configuration integrity of the entire managed network. Cisco advisories have documented several real ones: unauthenticated API access issues in DNA Center (CVE-2021-1257, CVE-2021-1258), authentication bypass vulnerabilities, and insufficient authorization checks letting a read-only API user invoke privileged operations.",
  ...
  // ─── Stage m45: NETCONF/YANG/gRPC — Network Programmability Security — Quiz ───
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The security implications of that programmable plane are profound. NETCONF/YANG rides SSH — secure when properly configured — but the authorization model, which YANG operations a given SSH user can invoke, is set per device and is frequently misconfigured; a NETCONF session with SUPER-USER capability can push arbitrary configuration changes to the device; and gRPC telemetry streams, if intercepted, reveal real-time operational state such as session counts, route tables, and interface utilization — valuable reconnaissance for an attacker.",
      ],
      technical: {
...
grpc
  ...
  // ─── Stage m46: CyberOps — SOC Analyst Kill Chain CTF ────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Cisco CyberOps Associate certification (CBROPS 200-201) defines the core competency framework for SOC analysts across five domains: security concepts, security monitoring, host-based analysis, network intrusion analysis, and security policies and procedures. The most critical applied skill is kill chain analysis — given alerts, logs, and network data, reconstruct the complete attacker timeline from initial access through command and control to objectives.",
        "Phishing-to-domain-compromise is the most common enterprise attack pattern SOCs see, and the typical chain is well-worn: a spear-phishing email delivers a macro-enabled document the user opens, the macro executes PowerShell that downloads and runs a Cobalt Strike beacon establishing a C2 channel, the attacker harvests credentials with Mimikatz and pass-the-hashes laterally to a domain controller, and finally enumerates Active Directory and creates a Domain Admin account. Each step leaves artifacts visible to a SOC analyst with the right data sources.",
...
| alert SOC HIGH MITRE_ATT&CK=T1550.002`,
  ...
  // ─── Stage m47: CyberOps Associate — Domains & Exam Alignment — Quiz ─────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Cisco CyberOps Associate certification (exam 200-201 CBROPS) is the entry-level credential for SOC analyst roles, organized into five exam domains: Security Concepts (25%), Security Monitoring (22%), Host-Based Analysis (20%), Network Intrusion Analysis (21%), and Security Policies and Procedures (12%). It maps to the NICE Cybersecurity Workforce Framework (SP-OPS-001 — Cyber Defense Analyst work role) and to MITRE ATT&CK.",
        "The Security Monitoring domain covers the data sources a SOC analyst uses to detect threats, and knowing what each can and can't see is a core competency: SIEM correlation rules detect known patterns, full packet capture (FPC) enables retroactive investigation, NetFlow/IPFIX detects anomalous traffic volumes without payload analysis, and security event log management ties it together across the stack.",
  ...
  // ─── Stage m48: Cisco Silicon One — Security Architecture — CTF ───────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The security implications of that programmable forwarding pipeline are significant. Silicon One supports P4 (Programming Protocol-Independent Packet Processors), a domain-specific language for defining how the ASIC parses and forwards packets; P4 programs compile to ASIC configuration tables of match-action rules executed at line rate, and the pipeline can be modified at runtime via the control plane; so a compromised control plane that can push malicious P4 programs could redirect, drop, or duplicate traffic at wire speed — 25.6 Tbps of traffic-manipulation capability in one ASIC.",
...
# Monitor for unexpected table entry count changes`,
  ...
  // ─── Stage m49: Silicon One P4 Pipeline Integrity — Quiz ─────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "P4 (Programming Protocol-Independent Packet Processors) is the domain-specific language that defines forwarding behavior in programmable network ASICs, Cisco Silicon One included, and a P4 program has three parts: a packet parser that extracts headers from raw bytes, match-action tables that decide what to do when a header field matches a rule, and a deparser that reconstructs the packet for forwarding. P4 programs compile to ASIC configuration that runs at wire speed — in Silicon One's case, 25.6 Tbps with no CPU involvement.",
        "The security architecture of a P4 chip like Silicon One separates the data plane from the control plane, and that split cuts both ways. The data plane is the ASIC running P4-compiled tables, processing billions of packets per second with no software involvement, while the control plane is IOS XR software on a general-purpose CPU, computing routing tables and pushing entries to the ASIC via the P4Runtime API or vendor ABIs. As a security asset, the data plane runs at hardware speed and can't be directly touched by software, since the control plane communicates only through a well-defined interface — but as a liability, a compromised control plane can push arbitrary forwarding-table entries to the ASIC.",
...
# Cross-reference against Cisco PSIRT signed software manifest`,
  ...
  // ─── Stage m50: Cisco Quantum-Safe Networking — PQC on Silicon One ────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "HARVEST NOW, DECRYPT LATER (HNDL) is the strategic posture nation-state intelligence agencies have adopted against the future threat of large-scale quantum computers. Adversaries are intercepting and archiving encrypted network traffic today — BGP sessions, IPsec VPN tunnels, MACsec-protected inter-datacenter links, and management-plane communications — intending to decrypt that archive once sufficiently powerful quantum computers arrive (estimated 2030–2035 for cryptographically relevant scale). Network infrastructure is a primary HNDL target because it carries the highest-value, highest-volume encrypted traffic in the enterprise.",
        "Cisco's quantum-safe networking roadmap addresses HNDL across the full stack. At the transport layer, MACsec on Silicon One will support post-quantum cipher suites (ML-KEM/CRYSTALS-Kyber hybrid with AES-GCM) once NIST and IEEE 802.1AE standardization completes, projected 2025–2026; at the IKEv2/IPsec layer, Cisco IOS XR 7.9 added IKEv2 hybrid post-quantum key-exchange groups (RFC 9370) combining classical ECDH-P256 with ML-KEM-768 for HNDL resistance on all tunnel negotiations; and at the BGP control plane, post-quantum authentication of BGP UPDATE messages is being standardized through IETF SIDROPS.",
