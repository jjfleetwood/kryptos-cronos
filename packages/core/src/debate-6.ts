...
export const debate6Stages: StageConfig[] = [
  ...
  // ─── debate-6-02: Vocal Delivery ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Each vocal variable does specific work:\n- Rate — brisk enough to fit your content, slow enough to be understood, and varied; slowing on key points signals importance and gives the judge time to write (epoch 1).\n- Pitch variation — prevents monotony and conveys emotion; a question rises, a firm conclusion falls.\n- Pause — the most underused tool; a deliberate silence before or after a key point makes it land, and pausing instead of saying 'um' projects control.\n- Emphasis — stressing the operative words ('this is the ONLY argument that matters') tells the judge what to weight.\n- Volume variation — marks contrast and intensity, unlike constant loudness.",
        "Make vocal delivery serve the argument, not perform for its own sake — and watch for the common faults:\n- Monotone — no cues, so the judge tunes out.\n- Filler words ('um,' 'like,' 'you know') — replace them with a deliberate pause.\n- Trailing off at the ends of sentences — losing the punchline.\n- Uncontrolled speed that sacrifices clarity (epoch 1).\nSlow down and emphasize on your most important claims, pause to let a key impact land, and vary pitch to stay engaging — the same words, delivered with vocal command, persuade far more.",
      ],
      technical: {
  ...
  // ─── debate-6-03: Nonverbal Communication ─────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The key nonverbal elements are learnable:\n- Eye contact — look at the judge, not your notes or the floor; it builds credibility, projects confidence, and lets you read whether they're following (epoch 1).\n- Posture/stance — stand stable and upright, weight balanced, not swaying; a grounded stance communicates command.\n- Gesture — purposeful hand movements to emphasize and illustrate, reinforcing the words, not nervous fidgeting.\n- Facial expression — engaged and appropriate to the content, not blank or anxious.\n- Overall presence — the composure that says 'I belong here and I know my material.'",
        "The faults are equally learnable to fix:\n- Reading face-down from notes — breaks eye contact, muffles voice, signals unpreparedness.\n- Swaying or pacing nervously — distracts and projects anxiety.\n- Fidgeting with hands, notes, or clothing — signals nervousness.\n- A closed or anxious posture — undermines ethos.\nThe fixes: look up and at the judge, plant your feet, give your hands a purpose, and breathe into a composed posture. Nonverbal command matters most in lay-judge formats (epoch 3), but in every format the judge's impression of your credibility affects how they weigh your arguments — composed, grounded, engaged presence is a learnable skill that makes everything you say land with more authority.",
      ],
      technical: {
        title: "Building Command Presence",
        body: [
          "Train the elements deliberately. For eye contact, practice delivering with eyes up, looking at the judge (or, when practicing, a fixed point), referring to notes only with quick glances — this builds credibility and lets you read the judge. For stance, plant your feet shoulder-width and balanced, and resist the urge to sway, rock, or pace; a stable base reads as command and also steadies your voice. For gesture, give your hands a job — purposeful gestures that emphasize and illustrate — so they don't fidget; when not gesturing, a neutral resting position beats nervous movement. For expression, stay engaged and appropriate, not blank or anxious.",
  ...
  // ─── debate-6-04: Spreading vs Persuasion ─────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1970,
      overview: [
        "One of debate's longest-running controversies is 'spreading' — speaking extremely fast (often 250–350+ words per minute) to fit the maximum arguments into a speech, on the theory that a dropped argument is conceded (epoch 5-10):\n- Defenders argue it rewards information density, rigorous coverage, and the ability to process arguments fast.\n- Critics argue it sacrifices persuasion, accessibility, and debate's communicative purpose, turning rounds into unintelligible blurs.\nIt's common in technical policy and circuit Lincoln-Douglas, where judges are experienced and flow every word.",
        "The crucial point is that the right delivery speed depends entirely on the judge and format:\n- An experienced 'flow judge' on the national circuit can follow fast delivery and may reward the coverage it allows.\n- A lay judge (epoch 3) — a parent volunteer or community member — cannot follow spreading and will be lost, crediting only what they understand (epoch 1).\nSpreading to a lay judge is a guaranteed loss; speaking slowly to a fast flow judge who expects density may leave you out-covered. The skill is not 'fast is good' or 'slow is good' — it's adaptation.",
  ...
  // ─── debate-6-05: Word Economy ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Debate speeches are strictly timed, which makes words a scarce resource and concision a competitive necessity:\n- Word economy — expressing an idea in the fewest words that fully convey it — lets you fit more substance, responses, and weighing into the same time.\n- A debater who rambles, repeats, and pads wastes seconds an efficient opponent spends making or developing arguments.\nConcision isn't about speaking fast (epoch 6-04); it's about eliminating waste, so each second carries content rather than filler.",
        "The enemies of concision are identifiable:\n- Filler phrases ('I think that what we're basically trying to say here is…' = 'we argue…').\n- Redundancy — saying the same thing two or three ways.\n- Throat-clearing preambles ('Before I get into my argument, I just want to note that…').\n- Hedging ('it kind of seems like maybe…').\n- Over-explaining a point the judge already grasped.\nEach can be cut without losing meaning, freeing time for substance. The skill is to say exactly what you mean, once, clearly, and move on.",
  ...
  // ─── debate-6-06: Narrative and Framing ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Humans understand the world through stories and frames, and two related skills exploit that:\n- Narrative — presenting your case as a coherent story rather than a disconnected list, with stakes, a logic, and a clear arc from problem to resolution.\n- Framing — controlling how the round (and the facts in it) are understood, defining what the debate is 'really about,' which automatically makes some arguments central and others peripheral.\nBoth work because the human mind organizes information through narrative and frames, not through raw data.",
        "Framing is especially powerful because it operates before the arguments are weighed:\n- Frame the round as 'fundamentally about individual freedom' and every argument is now evaluated through that lens — freedom arguments become central, others shrink.\n- It connects to the framework from epoch 4, but is broader: the story you tell about what matters and the lens you install on the whole round.\n'This round isn't about X (their ground); it's about Y (our ground)' is a framing move that can decide a round before the line-by-line — making your strongest ground the ground the round is decided on, while an opponent who lets the framing go fights on your terms.",
  ...
  // ─── debate-6-07: Rhetorical Devices ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Rhetorical devices are learnable language techniques that make phrases memorable, emphatic, and persuasive — and a few are especially useful:\n- Anaphora — repeating a phrase at the start of successive clauses ('We shall fight on the beaches, we shall fight on the landing grounds…') for rhythm and emphasis.\n- Tricolon — grouping in threes ('government of the people, by the people, for the people'), satisfying and memorable.\n- Antithesis — contrasting ideas in parallel structure ('ask not what your country can do for you — ask what you can do for your country').\nThe most quoted lines in history use them.",
        "Other high-value devices include:\n- The rhetorical question — posing a question for effect, to make the judge reach your conclusion themselves ('how can we call this justice?').\n- Parallelism — matching grammatical structure across phrases for rhythm and clarity.\n- A well-placed metaphor or analogy — making an abstract point concrete and vivid (though analogies must survive the disanalogy test, epoch 2).\nThese work because the ear and memory respond to rhythm, repetition, contrast, and imagery — making a thought not just understood but felt and retained. A crystallizing one-liner can become the line the judge remembers when writing the ballot.",
  ...
  // ─── debate-6-08: Audience Adaptation ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Every persuasion skill in this epoch converges on one meta-skill — audience adaptation, tailoring arguments, evidence, delivery, and style to the specific judge. Different judges decide differently:\n- A former policy debater rewards technical coverage and tolerates speed.\n- A parent volunteer rewards clarity, persuasion, and real-world impact.\n- A coach might prize sound logic; a community member might respond most to ethos and stakes.\nThe same speech that wins before one judge loses before another, so reading the judge and adapting is an enormous advantage over running the same approach regardless.",
        "Adaptation spans everything:\n- Delivery (epoch 6-04) — fast and dense for a flow judge, clear and measured for a lay judge.\n- Arguments — technical and jargon-heavy for an experienced judge; accessible and intuitive for a lay judge.\n- Evidence vs. intuition — a flow judge wants cards and warrants; a lay judge may be moved more by clear reasoning and vivid stakes.\n- Weighing — a lay judge especially needs explicit, plain-language weighing (epoch 4).\n- Style — more pathos and presence for a lay judge, more pure logos for a technical one.\nYou read the judge before the round (their paradigm, experience, role) and adjust all of these to fit.",
        "How do you read the judge? Many judges provide a 'paradigm' (a written or verbal statement of how they decide, what they want, how fast they'll follow — a practice formalized on the national circuit and covered further in epoch 7); absent that, you infer from their role and experience (coach, former debater, parent, community member) and the format's norms. Then you adapt — not your honesty or substance, but your presentation: which arguments to lead with, how fast to talk, how much to explain, how much to weigh explicitly, how much pathos to use. This is not pandering; it's communication. The same truth must be delivered differently to be received by different audiences — a doctor explains a diagnosis differently to a colleague than to a patient. Audience adaptation is the culminating delivery skill: it takes everything else in this epoch and aims it precisely at the one person whose judgment decides the round.",
      ],
      technical: {
  ...
  // ─── debate-6-09: Managing Nerves ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Nervousness is universal in debate — even the best feel it, especially in high-stakes rounds. The goal isn't to eliminate nerves but to manage them so they don't sabotage you, and ideally channel the adrenaline into focused energy. Unmanaged, they show up as:\n- Racing speech and filler words.\n- A shaky voice and fidgeting.\n- A lost train of thought or going blank.\n- Poor decisions under pressure.\nManaging them is a learnable skill, built on preparation, physical techniques, and a healthy framing of the anxiety itself.",
        "The deepest source of composure is preparation, backed by physical techniques:\n- Preparation — topic analysis, strong cases and blocks, red-teaming, practiced delivery (epoch 4) — shrinks the unknown that drives most anxiety ('what if they say something I can't answer?').\n- Slow, deep breathing before and during a round lowers arousal.\n- A grounded stance (epoch 6-03) steadies you.\n- Deliberate pauses (epoch 6-02) both calm you and project control.\nMuch debate anxiety is fear of the unknown, and thorough preparation is what shrinks it.",
      ...
      technical: {
        ...
        body: [
          "Build composure on three pillars. The first is preparation — the more thoroughly you've prepared (cases, blocks, red-teaming, practice rounds, anticipated arguments — epoch 4), the less the unknown there is to fear and the more justified your confidence. Most performance anxiety is fear of surprise; preparation shrinks the surprises. The second is physiology — manage the body's stress response with slow, deep breathing before and during the round (this directly lowers physiological arousal), a grounded stance, and deliberate pauses that calm you while projecting control. These are concrete, practicable techniques, not vague advice to 'relax.'",
  ...
  // ─── debate-6-10: Authentic Voice ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "This epoch's final skill is integration — weaving the techniques (the three appeals, vocal and physical delivery, speed and adaptation, concision, narrative, devices, composure) into a persuasive style that is genuinely your own. There is no one correct debating persona:\n- Some great debaters are intense and rapid-fire.\n- Others are calm and conversational.\n- Some are warm and humorous; others precise and surgical.\nWhat they share is not a style but mastery of the underlying skills, deployed in a way that fits who they authentically are — and authenticity itself is persuasive, because audiences trust what reads as genuine and distrust forced imitation.",
        "Finding your voice means matching the skills to your natural strengths and temperament:\n- If you're naturally warm and humorous, lean into ethos and pathos and a conversational delivery.\n- If you're naturally precise and analytical, lean into logos and surgical clarity.\nYou still develop all the skills (a warm debater still needs sound logos; an analytical one still needs presence and stakes), but integrate them in proportions that fit you. The failure mode is forcing a borrowed persona — a naturally calm debater trying to be artificially intense — which reads as inauthentic and undermines the very ethos delivery is meant to build.",
