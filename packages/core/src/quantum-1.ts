...
export const quantum1Stages: StageConfig[] = [
  // ─── quantum-01: Qubit Superposition ────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "A classical bit is a coin lying flat — heads (1) or tails (0). A qubit is a coin spinning in the air: not secretly already-heads-or-tails, but genuinely in a blend of both at once, called superposition. That is not a metaphor or a measurement limitation — it is the physical state of the system, and it is what makes a quantum computer categorically different from a fast classical one:\n- A single qubit holds a weighted combination of 0 and 1; n qubits hold a combination of all 2^n possibilities simultaneously.\n- 300 qubits encode 2^300 amplitudes at once — more numbers than there are atoms in the observable universe, which no classical machine can even store.",
        "There is a catch that trips up every newcomer, and it's the whole game: you cannot simply read all 2^n answers out. The moment you measure, the superposition collapses to one classical value, at random, weighted by the amplitudes:\n- So a quantum computer is not a magic parallel computer that returns every answer for free.\n- The art of every quantum algorithm is to arrange the amplitudes — using interference — so the answer you want becomes overwhelmingly likely to be the one you measure, and the wrong answers cancel out.",
        "That single idea is what threatens modern cryptography, because today's encryption rests on math problems that are easy to state and astronomically hard to undo on a classical computer:\n- RSA's security is the difficulty of factoring a 2048-bit number; ECC and Diffie-Hellman rest on the discrete-logarithm problem.\n- A classical attacker needs on the order of 2^1024 operations — effectively forever. Shor's algorithm (next stage) uses superposition + interference to fold that into polynomial time, turning 'forever' into hours.",
        "Why a learner in 2024 should already care, before any large quantum computer exists, comes down to two words — Harvest Now, Decrypt Later:\n- An adversary can record your RSA/ECC-protected traffic today and simply store it until a Cryptographically Relevant Quantum Computer (CRQC) arrives (consensus window ~2030–2035).\n- Any secret with a multi-year shelf life — health records, state secrets, long-lived keys — is therefore already exposed. The qubit isn't a future curiosity; it's the reason post-quantum migration has to start now.",
      ],
      technical: {
        title: "Superposition and Quantum States",
        body: [
          "Mathematically a qubit's state is a unit vector in a two-dimensional complex space — a weighted blend of the basis states |0⟩ and |1⟩:\n- |ψ⟩ = α|0⟩ + β|1⟩, where α and β are complex amplitudes (they carry a phase, not just a magnitude) constrained by |α|² + |β|² = 1.\n- Measuring collapses |ψ⟩ to 0 with probability |α|² or 1 with probability |β|². The phases of α and β never show up in a single measurement — but they are exactly what algorithms manipulate to make interference work.",
          "Geometrically the state lives on the surface of the Bloch sphere — every pure single-qubit state is a point on a globe, with |0⟩ at the north pole and |1⟩ at the south. Quantum gates are rotations of that sphere:\n- The Hadamard gate (H) maps |0⟩ → (|0⟩ + |1⟩)/√2 — a pole to the equator, i.e. equal superposition.\n- Applying H to each of n qubits superposes all 2^n computational basis states in one step. That exponential breadth is the raw fuel; Shor's and Grover's are the engines that turn it into a usable answer.",
        ],
        codeExample: {
...
# Qubit was in superposition — 50/50 on measurement`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Build Shor's period-finding circuit (QFT) before estimating qubit cost. Run: shor-circuit RSA-2048",
      ...
      fragments: [
        ...
        { trigger: "qubit-sim superposition", value: "POS1TION_", label: "Superposition Observed — Quantum Parallelism Confirmed" },
        { trigger: "shor-circuit RSA-2048", value: "SHOR_", label: "Period-Finding Circuit Built — QFT Compiled" },
      ...
      files: {
        "/threat-brief.txt": [
          ...
          "Sequence: qubit-sim → shor-circuit → shor-estimate → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "shor-circuit": (_args: string[]) => ({
          lines: [
            "Building Shor's period-finding circuit for the RSA-2048 modulus...",
            "  register 1: ~4096 qubits in superposition over all x",
            "  register 2: modular exponentiation a^x mod N (reversible arithmetic)",
            "  apply the inverse QFT to register 1 → amplitude peaks at k/r",
            "  measure → continued fractions → period r of a mod N",
            "Circuit compiled. Period r is recoverable on a CRQC.",
            "Next: shor-estimate RSA-2048",
            "",
            ">> LEARN: Shor's power is the QFT, not brute force",
            "   Modular exponentiation in superposition + an inverse QFT extracts the",
            "   period r; from r the factors fall out with classical arithmetic.",
            "   That is why RSA/ECC fall to a CRQC while symmetric crypto only weakens.",
          ],
        }),
  ...
  // ─── quantum-02: Shor's Algorithm ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1994,
      overview: [
        "In 1994, working at Bell Labs, the mathematician Peter Shor answered a question nobody knew was urgent: could a quantum computer do something genuinely, exponentially better than a classical one on a problem people cared about? His answer factored large integers in polynomial time, and it landed like a bomb in the cryptography world because the entire edifice of public-key security is built on that one problem being hard:\n- Shor's needs about O((log N)³) quantum operations; the best classical factoring algorithm (the general number field sieve) runs in sub-exponential O(e^(1.9·(log N)^(1/3)·(log log N)^(2/3))) — fast enough that RSA-2048 is safe for billions of years classically.\n- The same machinery breaks not just RSA but Diffie-Hellman and elliptic-curve cryptography too. One algorithm, three dead cryptosystems.",
        "To see why factoring secures RSA, picture the trapdoor: multiplying two 1024-bit primes p and q to get N is instant, but handed only N, recovering p and q is the wall classical computers cannot climb. Shor's doesn't attack that wall head-on — it cleverly sidesteps it:\n- A classical reduction (number theory going back to the 1700s) turns 'factor N' into 'find the period of the function f(x) = a^x mod N' — how often it repeats as x grows.\n- Finding that period is the part a classical computer can't do efficiently but a quantum one can, using the Quantum Fourier Transform to read periodicity straight out of a superposition. Once you have the period, p and q drop out with a bit of arithmetic.",
        "The blast radius is essentially the whole internet's trust fabric, because Shor's also solves the discrete-logarithm problem that Diffie-Hellman and ECDH rest on:\n- Every TLS handshake (https), SSH login, IPsec/VPN tunnel, signed software update, and certificate-authority signature derives its security from factoring or discrete logs.\n- So 'Shor's breaks RSA' is shorthand for 'on Q-Day, the confidentiality and authenticity of nearly all current network traffic and code-signing collapses at once' — which is exactly why migration to post-quantum algorithms (FIPS 203/204) is a now problem, not a later one.",
      ],
      technical: {
...
# Shor's on CRQC: hours`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Run the classical post-processing (period → gcd) to recover the prime factors. Run: factor-verify 3127",
      ...
      fragments: [
        ...
        { trigger: "quantum-factor 3127", value: "ALG0_", label: "Quantum Period Found — Shor's Order-Finding" },
        { trigger: "factor-verify 3127", value: "RSA_", label: "RSA Semiprime Factored — p=53, q=59" },
      ...
      files: {
        "/shor-brief.txt": [
          ...
          "Sequence: quantum-factor → factor-verify → crqc-timeline → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "factor-verify": (_args: string[]) => ({
          lines: [
            "Classical post-processing of the quantum period for N=3127...",
            "  chosen base a = 7,  recovered period r = 14  (7^14 ≡ 1 mod 3127)",
            "  r is even and 7^(r/2) ≢ -1 mod N → usable result",
            "  gcd(7^7 - 1, 3127) = 53",
            "  gcd(7^7 + 1, 3127) = 59",
            "Factors verified: 3127 = 53 × 59.",
            "Next: crqc-timeline",
            "",
            ">> LEARN: The quantum step only finds the period",
            "   Turning period r into factors is plain gcd arithmetic on a classical CPU.",
            "   The quantum computer does exactly one hard thing — order-finding.",
            "   Real RSA uses a 2048-bit N; the math is identical, just far larger.",
          ],
        }),
  ...
  // ─── quantum-03: Grover's Algorithm ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1996,
      overview: [
        "Where Shor's algorithm is a guillotine for public-key crypto — it doesn't weaken RSA, it abolishes it — Grover's algorithm is something subtler and, in a way, more instructive. Published by Lov Grover at Bell Labs in 1996, it answers the most generic question in computing: you have an unsorted pile of N possibilities and a way to recognize the right one when you see it; how fast can you find it? Classically the answer is brute force — check them one by one, expecting to look at about N/2 before you get lucky, O(N) in the worst case. Grover proved a quantum computer can do it in roughly √N steps. That square-root is the whole story, and for cryptographers it lands on exactly the thing brute force attacks: the key.",
        "Think about what 'brute-forcing a cipher' actually is. AES-128 has 2^128 possible keys; breaking it means searching that keyspace for the one key that decrypts your ciphertext into something sensible — an unsorted search over 2^128 items with a clear recognizer. Grover turns that 2^128 search into a 2^64 one. Not because it's clever about AES (it knows nothing about AES's internals — Grover treats the cipher as an opaque oracle), but because square-rooting an exponent halves it: √(2^128) = 2^64. The effect is mechanical and universal — any symmetric cipher, any hash preimage, any keyed construction sees its effective security level cut in half against a large enough quantum computer.",
        "But halving is not breaking, and that gap is the entire post-quantum symmetric story. 2^64 operations is uncomfortable — within reach of a future cryptographically-relevant quantum computer running at scale — so AES-128 should be treated as quantum-weakened. 2^128 operations, the post-Grover cost of AES-256, is not within anyone's reach and won't be: it's the same astronomical wall that protects AES-128 classically today. So the fix is almost insultingly simple — double your key sizes. NIST's guidance is exactly this: use AES-256 and SHA-384, whose post-Grover floors (2^128 and 2^192) stay comfortably infeasible. Symmetric cryptography survives the quantum era essentially intact, as long as you stop using the small-margin variants.",
        "This is why the post-quantum migration is so lopsided. The terrifying half — Shor — forces us to rip out RSA and elliptic-curve entirely and replace them with new, unfamiliar lattice-based algorithms. The reassuring half — Grover — asks only that we turn a dial we already had, from 128-bit keys to 256-bit. One is a transplant; the other is a tune-up. Knowing which threat is which is the difference between panicking about the wrong thing and migrating the right thing.",
      ],
      technical: {
...
# for quantum-resistant symmetric security`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Count the Grover iterations to quantify the effective post-quantum strength. Run: grover-iterate AES-128",
      ...
      fragments: [
        ...
        { trigger: "grover-analyze AES-128", value: "AES128_", label: "Grover's Analysis — Quadratic Speedup Confirmed" },
        { trigger: "grover-iterate AES-128", value: "WEAK3N3D_", label: "Iterations Counted — 64-bit Effective Security" },
      ...
      files: {
        "/crypto-audit.txt": [
          ...
          "Sequence: grover-analyze → grover-iterate → crypto-audit-run → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "grover-iterate": (_args: string[]) => ({
          lines: [
            "Counting Grover iterations to brute-force a 128-bit AES key...",
            "  search space N = 2^128",
            "  optimal iterations ≈ (π/4)·√N = (π/4)·2^64 ≈ 2^64",
            "  each iteration = 1 oracle call + 1 diffusion (the Grover operator)",
            "  effective security: 128-bit → ~64-bit against a quantum attacker",
            "Quadratic speedup quantified — AES-128 margin halved, not broken.",
            "Next: crypto-audit-run",
            "",
            ">> LEARN: Grover halves the bits; Shor destroys the scheme",
            "   √N means symmetric keys only need to DOUBLE (AES-256 keeps a 128-bit margin).",
            "   Asymmetric crypto has no such luck — Shor is exponential, not quadratic.",
            "   That asymmetry is why PQC migration targets RSA/ECC first.",
          ],
        }),
  ...
  // ─── quantum-04: Harvest Now Decrypt Later ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2023,
      overview: [
        "Here is the uncomfortable thing about the quantum threat: for some of your data, it has already happened. Not the decryption — that waits on a machine nobody has built yet — but the theft. Harvest Now, Decrypt Later (HNDL, also called Store Now Decrypt Later) is the attack of simply recording encrypted traffic today and filing it away, encrypted, to crack open the day a cryptographically-relevant quantum computer exists. The adversary doesn't need to break anything now. They need a hard drive and patience. Every HTTPS session, VPN tunnel, encrypted email, and diplomatic cable they can intercept is a sealed envelope they're keeping until they have the machine that opens it.",
        "This is what makes HNDL so strategically nasty: it collapses the comfort of the quantum timeline. People hear 'a CRQC is probably 10-15 years away' and relax — but that estimate is the deadline for *decryption*, not for *protection*. If your data must stay secret for longer than the time until Q-Day, it is already exposed. A medical record, a state secret, a 50-year sealed-court file, the genome of a living person, the master keys to long-lived infrastructure — capture it today under RSA or ECC, and the adversary decrypts it the moment the hardware matures. The quantity that matters is Mosca's inequality: if (how long your data must stay secret) plus (how long migration takes you) is greater than (time until a CRQC), you are already too late. Most organizations have never even computed the left-hand side.",
        "And the harvesting is not hypothetical. Bulk passive collection of internet backbone traffic is exactly the capability the Snowden documents revealed a decade ago, and the storage economics have only improved since. CISA, the NSA, and the Five Eyes intelligence alliance now warn about HNDL by name, and the U.S. has written it into law — the Quantum Computing Cybersecurity Preparedness Act directs agencies to inventory and migrate vulnerable cryptography precisely because long-lived data captured today is the part of the quantum threat with no later remedy. You cannot un-harvest what's already been recorded. The only defense is to make the captured envelopes uncrackable *before* they're sealed — which means moving long-lifetime secrets to post-quantum algorithms now, not on Q-Day.",
      ],
      technical: {
...
# MIGRATION: Replace RSA/ECC with CRYSTALS-Kyber for key exchange`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Reconstruct the full RSA private key (d = e⁻¹ mod φ(n)) from the factors. Run: rsa-reconstruct",
      ...
      fragments: [
        ...
        { trigger: "shor-factor-cert", value: "HARVEST_", label: "RSA Modulus Factored — p, q Recovered" },
        { trigger: "rsa-reconstruct", value: "NOW_", label: "Private Key Reconstructed — d Derived" },
      ...
      files: {
        "/intercept-log.txt": [
          ...
          "Sequence: shor-factor-cert → rsa-reconstruct → decrypt-session → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "rsa-reconstruct": (_args: string[]) => ({
          lines: [
            "Reconstructing the RSA private key from the recovered factors...",
            "  n = p × q  (2048-bit modulus from the captured certificate)",
            "  φ(n) = (p-1)(q-1)",
            "  public exponent e = 65537 (from the cert)",
            "  d = e⁻¹ mod φ(n)  ← extended Euclidean algorithm",
            "  assembling PKCS#1 private key (n, e, d, p, q, dP, dQ, qInv)",
            "Private key rebuilt — every session under this cert is now readable.",
            "Next: decrypt-session",
            "",
            ">> LEARN: Factoring n is the whole game for RSA",
            "   Once p and q are known, d follows from simple modular inversion;",
            "   the 'private' key was only ever protected by the hardness of factoring.",
            "   Forward secrecy (ephemeral ECDHE) limits this — but ECDHE also falls to Shor.",
          ],
        }),
  ...
  // ─── quantum-05: Quantum Entanglement ───────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Entanglement is the part of quantum mechanics that bothered Einstein for the rest of his life. Take two qubits and entangle them, and they stop being two separate things with their own states — they become one joint system that the universe insists on describing as a whole. Measure one and find it 'up,' and you instantly know its partner is 'down,' even if you've since carried that partner to the far side of the planet. There was no signal, no hidden note slipped into each particle at the start (Bell's theorem proved that 'local hidden variable' escape hatch is closed by experiment). The correlation is real, instantaneous, and distance-independent. Einstein called it 'spukhafte Fernwirkung' — spooky action at a distance — and meant it as an insult. Nature took it as a design spec.",
        "The first thing to get straight is what entanglement *can't* do, because the science fiction always gets it wrong: it cannot send a message faster than light. The outcome of your measurement is genuinely random — you can't choose to make your qubit come up 'up' to flash a 1 across the galaxy — so the distant partner sees only random noise until they compare notes with you over an ordinary, light-speed channel. No information travels faster than light; the no-communication theorem is safe. What entanglement *does* give you is a shared, perfectly-correlated source of randomness that two distant parties provably hold together and that no third party can have been part of — and that, it turns out, is the raw material of quantum-secure communication.",
        "That's why entanglement is the foundation under quantum networking rather than a mere curiosity. It powers quantum teleportation (transferring an unknown quantum state from here to there by consuming an entangled pair plus classical bits), the quantum repeaters that will one day stitch short fragile links into a continental quantum internet, and entanglement-based key distribution like the E91 protocol. The security payoff is profound: because any eavesdropper who touches the channel necessarily disturbs the entangled correlations — and that disturbance is measurable — the two legitimate parties can *detect* surveillance as a violation of physics, not merely assume an attacker lacks the compute to break their math. Where classical crypto rests on 'this problem is hard,' entanglement-based QKD rests on 'the laws of quantum mechanics are true.' That's the leap from computational security to information-theoretic security, and it's the subject of the QKD epoch that follows.",
      ],
      technical: {
...
# Alice and Bob always agree — foundation of E91 QKD`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Run the CHSH inequality test to certify the channel is eavesdropper-free. Run: chsh-test",
      ...
      fragments: [
        ...
        { trigger: "bell-sim 100", value: "B3LL_", label: "Bell State Verified — Cisco Entanglement Chip Online" },
        { trigger: "chsh-test", value: "E91_", label: "CHSH S=2.7 — No Eavesdropper, Channel Certified" },
      ...
      files: {
        "/e91-brief.txt": [
          ...
          "Sequence: bell-sim → chsh-test → e91-keygen → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "chsh-test": (_args: string[]) => ({
          lines: [
            "Running the CHSH Bell-inequality test on the 100 entangled pairs...",
            "  measuring correlations at angles a/a' (Alice) × b/b' (Bob)",
            "  E(a,b)=0.71  E(a,b')=0.69  E(a',b)=0.70  E(a',b')=-0.71",
            "  S = |E(a,b) - E(a,b') + E(a',b) + E(a',b')| = 2.70",
            "  classical (local hidden-variable) bound: S ≤ 2",
            "  S = 2.70 > 2 → genuine entanglement, no measurement leak",
            "Channel certified eavesdropper-free. Safe to extract a key.",
            "Next: e91-keygen",
            "",
            ">> LEARN: E91 security comes from violating Bell's inequality",
            "   An eavesdropper measuring the pairs collapses them and pushes S toward 2.",
            "   S > 2 proves no one intercepted — security from physics, not math.",
            "   This is what makes QKD resistant even to a CRQC adversary.",
          ],
        }),
  ...
  // ─── quantum-06: NISQ Era and Current Limits ────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The name of today's quantum era is also its central limitation — Noisy Intermediate-Scale Quantum (NISQ), a term John Preskill coined in 2018:\n- 'Intermediate-Scale' means 50–1000 qubits — enough to do things no classical computer can simulate, but far short of what cryptography-breaking needs.\n- 'Noisy' means there is no error correction: every qubit is a fragile analog object that drifts, leaks, and decoheres in microseconds, so the longer a computation runs the more likely it is to be wrong.\n- NISQ machines genuinely beat classical hardware on a few narrow, carefully chosen benchmarks (random-circuit sampling, some quantum-chemistry simulations), but they cannot run Shor's algorithm at RSA-relevant scale.",
        "Two hard limits define the NISQ ceiling — and they multiply against each other:\n- Error rate: each two-qubit gate fails ~0.1–1% of the time. Errors don't just add, they compound across a circuit, so a long computation is overwhelmingly likely to finish in garbage.\n- Scale via error correction: the fix is to encode one reliable 'logical' qubit out of many noisy physical ones — roughly ~1000 physical qubits per logical qubit at today's error rates. Shor's on RSA-2048 needs thousands of logical qubits and ~10^12 error-corrected gate operations, which is why a real attack is millions of physical qubits away, not a firmware update away.",
        "Those limits are exactly what makes the threat manageable — if you act on them:\n- The cryptographic threat is real and dated, but not immediate: a Cryptographically Relevant Quantum Computer (CRQC) is an engineering problem of scale and time, not an unsolved physics mystery.\n- The danger is Harvest Now, Decrypt Later (HNDL): an adversary can record your RSA/ECC-protected traffic today and decrypt it the day a CRQC exists. Any secret with a 10-year sensitivity window is therefore already exposed.\n- Migration to post-quantum cryptography takes years across a real enterprise, so the correct posture in the NISQ era is to start the inventory-and-migration now, well ahead of Q-Day.",
      ],
      technical: {
        title: "NISQ Limitations — Error Rates and the Road to CRQC",
        body: [
          "A CRQC for RSA-2048 is a long way from today's hardware, and the gap is dominated by error correction, not raw qubit count:\n- Shor's algorithm with modern optimizations needs roughly 4096 logical qubits to factor a 2048-bit modulus.\n- At ~0.1% gate error, the surface code needs on the order of ~1000 physical qubits to protect each logical one — about 4 million physical qubits total, plus the control wiring and cryogenics to run them.\n- That is why headlines about '1000-qubit' chips are misleading: those are physical, not logical, qubits. The number to watch is logical qubits with a low, stable error rate.",
          "The roadmaps show steady engineering progress, not a physics wall — which is precisely why the threat has a date:\n- IBM's published roadmap targets 100,000+ qubits by 2033 and is shipping the modular hardware (Heron, Flamingo) to get there.\n- Google's Willow chip (2024) demonstrated below-threshold error correction: for the first time, adding more physical qubits to the code made the logical error rate go down instead of up. That crossover is the hinge the whole field was waiting for.\n- Below threshold means logical-qubit quality now compounds with scale, so the relevant clock — time-to-CRQC — genuinely starts ticking. The Five Eyes consensus window for a CRQC is roughly 2030–2035.",
        ],
        codeExample: {
...
# ** Trapped ion gate fidelity, two-qubit operations`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Extrapolate the hardware roadmap to project the CRQC arrival window. Run: crqc-project",
      ...
      fragments: [
        ...
        { trigger: "nisq-status", value: "CR0C_", label: "Hardware Status — Willow Milestone Confirmed" },
        { trigger: "crqc-project", value: "TIMELINE_", label: "Roadmap Extrapolated — CRQC Window Projected" },
      ...
      files: {
        "/nisq-brief.txt": [
          ...
          "Sequence: nisq-status → crqc-project → crqc-risk → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "crqc-project": (_args: string[]) => ({
          lines: [
            "Extrapolating logical-qubit growth toward CRQC scale (~4096 logical)...",
            "  2019: 53 (Sycamore)   2023: 433   2024: 1121 physical, ~Willow QEC",
            "  below-threshold QEC reached → logical qubits now compound",
            "  fitting an exponential: roughly 2× logical qubits / ~2 years",
            "  crossover to ~4096 logical qubits → projected 2030–2035",
            "Projection matches Five Eyes consensus window.",
            "Next: crqc-risk 10",
            "",
            ">> LEARN: Watch logical qubits and error rate, not raw qubit count",
            "   Raw physical qubits mislead; the inflection is below-threshold QEC,",
            "   when adding physical qubits finally REDUCES the logical error rate.",
            "   That is the moment the CRQC clock genuinely starts ticking.",
          ],
        }),
  ...
  // ─── quantum-07: Quantum Error Correction ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "If breaking RSA were only a matter of building more qubits, we'd already be doing it — IBM and Google have machines with hundreds of them. The reason Q-Day is still years out is this stage's subject: qubits are catastrophically fragile, and the harder you push a quantum computer, the faster its answer dissolves into noise. A qubit holds its delicate superposition only as long as the universe leaves it alone, and the universe never does — a stray photon, a thermal vibration, a flicker in the control electronics, and the quantum information 'decoheres' into ordinary classical mush. Worse, the two obvious classical fixes are both forbidden by physics: you can't copy a qubit to make backups (the no-cloning theorem), and you can't just peek to check whether it's gone wrong, because measuring a qubit collapses the very superposition you were trying to protect. Classical error correction's entire toolkit is illegal here.",
        "Quantum Error Correction (QEC) is the brilliant escape from that trap, and it's genuinely subtle: it spreads one pristine 'logical' qubit across many noisy 'physical' qubits, then measures *the relationships between them* — parity checks called syndromes — rather than the qubits themselves. Those syndrome measurements reveal that an error occurred and which correction to apply, without ever revealing (and thus collapsing) the protected logical state. It's like determining that two cards don't match without being allowed to look at either card's face. The leading scheme, the Surface Code, arranges physical qubits in a 2D grid and is forgiving enough to be practical — but the overhead is brutal: at today's error rates it takes roughly 1,000 physical qubits to maintain a single reliable logical qubit, which is why breaking RSA-2048 is estimated to need millions of physical qubits, not thousands.",
        "Everything hinges on one number: the error-correction threshold. Add more qubits to a code and you're in a race — each new physical qubit helps catch errors but also brings its own noise. Above the threshold the noise wins and adding qubits makes things *worse*; below it, the correction outpaces the noise and scaling up genuinely drives the logical error rate down, exponentially. For decades operating below threshold was theory. In December 2024, Google's Willow processor crossed it — each time they grew the code (a 3×3 to 5×5 to 7×7 grid) the logical error rate fell, the first convincing demonstration of error correction that gets better with scale. That is the milestone that reframes the whole field: a cryptographically-relevant quantum computer stopped being a question of physics and became a question of engineering scale-up. Hard, expensive, years of work — but no longer waiting on a discovery.",
      ],
      technical: {
...
# Willow proves below-threshold is achievable — now scale it`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Compute the logical error rate and physical-qubit overhead per logical qubit. Run: logical-error-calc",
      ...
      fragments: [
        ...
        { trigger: "surface-code-calc 0.001", value: "SURF4CE_", label: "Code Distance Calculated — d Determined" },
        { trigger: "logical-error-calc", value: "CODE_", label: "Logical Error Rate + Overhead Computed" },
      ...
      files: {
        "/qec-brief.txt": [
          ...
          "Sequence: surface-code-calc → logical-error-calc → crqc-scale → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "logical-error-calc": (_args: string[]) => ({
          lines: [
            "Computing logical error rate from the chosen surface-code distance d...",
            "  physical error p = 0.001, threshold p_th ≈ 0.01  (p/p_th = 0.1)",
            "  logical error p_L ≈ (p/p_th)^((d+1)/2)",
            "  d=21 → p_L ≈ 0.1^11 ≈ 1e-11 (deep enough for Shor's long circuit)",
            "  qubit overhead per logical qubit ≈ 2d² ≈ 2·441 ≈ 882 physical",
            "Per-logical overhead locked: ~882 physical qubits at d=21.",
            "Next: crqc-scale RSA-2048",
            "",
            ">> LEARN: Error correction is a brutal multiplier",
            "   Driving the logical error low enough for Shor's millions of gates needs",
            "   a high code distance — and overhead scales as ~2d² per logical qubit.",
            "   That ~1000× tax is why CRQC needs millions of physical qubits.",
          ],
        }),
  ...
  // ─── quantum-08: Quantum Threat Assessment ──────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "It's tempting to treat the quantum threat as a single switch that flips on Q-Day, but a competent security program treats it the way it treats any risk: as a register of assets, each with its own exposure and its own deadline. The calibration uses two clocks running toward each other. One counts *down* to when a cryptographically-relevant quantum computer arrives — Q-Day, with Five Eyes planning horizons clustering around 2030–2035. The other counts the *shelf life* of each secret: how many years this particular data must remain confidential. Where the shelf life overshoots Q-Day, the asset is already in jeopardy from Harvest Now Decrypt Later, regardless of how distant Q-Day feels. A 20-year secret encrypted with RSA today is, for risk-register purposes, a breach that simply hasn't finished happening yet.",
        "CISA's Quantum Readiness Roadmap (2023) turns that intuition into a repeatable process, and the un-glamorous first step is the one most organizations skip: a cryptographic inventory. You cannot migrate what you cannot see, and most enterprises genuinely do not know where RSA and ECC live — they're buried in TLS endpoints, VPN concentrators, code-signing pipelines, firmware, hardware security modules, database encryption, and third-party products whose internals are opaque. The roadmap's discipline is to enumerate every cryptographic dependency, tag each with the sensitivity lifetime of the data it protects, flag the HNDL-exposed long-lived assets, and rank the whole register by risk. The output is not a feeling of dread; it's a prioritized worklist.",
        "And that worklist is the entire point, because the alternative — 'migrate everything at once' — is a recipe for paralysis on a cryptographic estate that can run to thousands of systems. The register lets you triage. Long-lived, high-sensitivity data (government classified material, medical and genomic records, trade secrets, root keys and certificate authorities) goes to the front of the line and moves to post-quantum algorithms now. Ephemeral data (a streaming session key that's worthless in an hour, a short-lived auth token) can ride the normal refresh cycle and wait for tooling to mature. Migrating by risk rather than by panic is what turns an impossible-sounding project into a multi-year program that actually finishes — and finishing before Q-Day is the only success criterion that matters.",
      ],
      technical: {
...
print(quantum_risk("session_tokens", 0.001, False)) # MEDIUM`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Apply Mosca's inequality (X+Y>Z) to classify each asset's HNDL exposure. Run: hndl-classify",
      ...
      fragments: [
        ...
        { trigger: "crypto-inventory", value: "RISK_", label: "Crypto Inventory — 47 RSA/ECC Assets Found" },
        { trigger: "hndl-classify", value: "CISA_", label: "Mosca's Inequality Applied — Exposure Classified" },
      ...
      files: {
        "/cisa-framework.txt": [
          ...
          "",
          "Sequence: crypto-inventory → hndl-classify → quantum-risk-report → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "hndl-classify": (_args: string[]) => ({
          lines: [
            "Applying Mosca's inequality to each inventoried asset...",
            "  Mosca: if X (secrecy years) + Y (migration years) > Z (years to CRQC) → act now",
            "  assume Y = 5 (migration), Z = 8 (CRQC ~2032)",
            "  patient records   X=25 → 25+5=30 > 8  → P1 (exposed for decades)",
            "  TLS to partners   X=3  → 3+5=8  = 8   → P2 (borderline)",
            "  ephemeral tokens  X=0  → 0+5=5  < 8   → P3 (low HNDL risk)",
            "Exposure windows classified by data lifetime, not by system age.",
            "Next: quantum-risk-report",
            "",
            ">> LEARN: HNDL risk is about data lifetime, not system age",
            "   Mosca's X+Y>Z says long-secrecy data must migrate FIRST — it is already",
            "   being harvested today to be decrypted after Q-Day.",
            "   This is the prioritization CISA's PQC roadmap is built around.",
          ],
        }),
  ...
  // ─── quantum-09: Quantum Random Number Generation ───────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Cryptography has a dirty secret: the algorithms are usually fine, and the randomness is where things actually break. Every key you generate, every nonce, every initialization vector is only as unguessable as the random numbers underneath it — and a computer, by its nature, is a deterministic machine that cannot produce randomness at all. What we call a PRNG (pseudo-random number generator) is a formula: feed it the same seed and it spits out the exact same 'random' stream every time, forever. That's not a flaw to be patched; it's the definition. The whole game is hiding the seed and the internal state well enough that an attacker can't reproduce the sequence.",
        "Cryptographically-secure PRNGs (CSPRNGs) paper over the determinism by mixing in entropy harvested from the messy physical world — the jitter between keystrokes, thermal noise in a circuit, the timing of disk and network interrupts. It works, but it rests on an uncomfortable assumption: that you genuinely gathered as much unpredictability as you think you did. Underestimate the entropy, seed the generator from a too-predictable source, and the output merely *looks* random while being brute-forceable. History is littered with exactly this failure — the Debian OpenSSL bug that shrank the key space to 32,767 possibilities, embedded devices that generate the same 'random' keys because they booted with no entropy. The randomness was the weakest link, not the cipher.",
        "Quantum Random Number Generators (QRNGs) end the argument by going to the one place in nature where randomness is not ignorance but law. Fire a single photon at a 50/50 beam splitter: quantum mechanics says it will be transmitted or reflected with exactly even odds, and — this is the crucial part — the outcome is not determined by any hidden variable you failed to measure. It is irreducibly, provably random, the same indeterminacy at the heart of superposition. Detect which path the photon took and you've harvested one perfect bit of entropy that no adversary, however powerful, could have predicted, because the universe itself hadn't decided it until the measurement. This is why QRNGs are becoming the gold standard for high-assurance key material: commercial hardware (ID Quantique), public beacons (the ANU quantum RNG), and cloud QRNG services now make physics-grade entropy something you can actually build on — closing the gap that algorithms alone never could.",
      ],
      technical: {
...
# secrets.token_bytes uses OS CSPRNG (/dev/urandom on Linux)`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Recover the generator's internal state and predict the next block to confirm. Run: state-recover",
      ...
      fragments: [
        ...
        { trigger: "dual-ec-predict", value: "BACKD00R_", label: "Backdoor Relationship Identified" },
        { trigger: "state-recover", value: "QRNG_", label: "Internal State Recovered — Next Block Predicted" },
      ...
      files: {
        "/dual-ec-analysis.txt": [
          ...
          "Sequence: dual-ec-predict → state-recover → qrng-keygen → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "state-recover": (_args: string[]) => ({
          lines: [
            "Recovering the Dual_EC_DRBG internal state from observed output...",
            "  the backdoor: Q = d·P, so knowing d links consecutive outputs",
            "  observed 30 output bytes → recover the truncated curve point",
            "  brute-force the 16 missing high bits → candidate state s_i",
            "  predict next block with s_i → MATCHES the real next 32 bytes",
            "Internal state locked — all future output is now predictable.",
            "Next: qrng-keygen 256",
            "",
            ">> LEARN: A backdoored RNG breaks every key it ever produced",
            "   With d, one observed output reveals the state and all future bytes —",
            "   the 2013 Dual_EC scandal showed a standardized PRNG can be the backdoor.",
            "   Fix is a trustworthy entropy source (here, QRNG), not a bigger key.",
          ],
        }),
  ...
  // ─── quantum-10: Quantum Computing Timeline and Migration ───────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "When will Q-Day come — the day a quantum computer can break RSA-2048? Ask the experts and you'll get answers from five years to thirty, which sounds like ignorance but is really honesty: nobody can timestamp an engineering breakthrough that hasn't happened. Intelligence agencies, quantum hardware companies (whose incentives run optimistic), and academic skeptics genuinely disagree. But the people who have to *plan* — the Five Eyes governments protecting classified systems with multi-decade secrecy requirements — have stopped waiting for certainty and converged on a 2030–2035 horizon. Not because they know, but because planning requires a number, and the cost of being late is unbounded while the cost of being early is merely some wasted diligence.",
        "And here is the liberating insight that this whole epoch has been building toward: the exact date of Q-Day doesn't actually matter to your decision. What matters is a subtraction. Enterprise cryptographic migrations are not flips of a switch — discovering every place RSA hides, testing post-quantum replacements, coordinating across vendors and protocols and embedded devices, takes a realistic organization three to seven years. NIST published the finished PQC standards (ML-KEM, ML-DSA) in August 2024, so the tools exist now. Do the arithmetic: begin in 2024 and a seven-year migration lands you safely by 2031, ahead of even pessimistic Q-Day estimates. Begin in 2027 and the same migration finishes in 2034 — a coin flip against the threat. The deadline isn't Q-Day; it's Q-Day minus your migration time, and that earlier deadline is already inside most planning windows.",
        "This capstone pulls the Foundations threat model together into a single decision: you don't need to predict the future, you need to start the clock. The qubit's superposition (stage 1) and Shor's algorithm (stage 2) tell you public-key crypto will fall; Grover (stage 3) tells you symmetric crypto only needs bigger keys; Harvest Now Decrypt Later (stage 4) tells you the long-lived data is already exposed; error correction crossing threshold (stage 7) tells you the hardware is now an engineering problem, not a physics one; and the risk register (stage 8) tells you where to begin. From here the journey continues — the PQC epoch covers the specific quantum-resistant algorithms and how they work, and the QKD and migration-engineering epochs cover physics-based key distribution and how to actually execute the transition across a real enterprise. The threat is understood. The only remaining variable is when you start.",
      ],
      technical: {
...
# FIPS 205: SLH-DSA (SPHINCS+) — stateless hash-based signatures`,
    ...
    ctf: {
      ...
      hints: [
        ...
        "Cross-map the intel to the asset inventory to build the prioritized backlog. Run: migration-prioritize",
      ...
      fragments: [
        ...
        { trigger: "quantum-intel-brief", value: "C0UNTDOWN_", label: "Intel Brief — Five Eyes Timeline Confirmed" },
        { trigger: "migration-prioritize", value: "MIGRATE_", label: "Intel × Inventory — Prioritized Backlog Built" },
      ...
      files: {
        "/ciso-template.txt": [
          ...
          "Sequence: quantum-intel-brief → migration-prioritize → ciso-quantum-report → assemble",
        ].join("\n"),
      },
      ...
      extraCommands: {
        ...
        "migration-prioritize": (_args: string[]) => ({
          lines: [
            "Cross-mapping the threat intel against the crypto asset inventory...",
            "  threat window Z ≈ 2030–2035 (Five Eyes); standards: FIPS 203/204 ready",
            "  joining each asset's HNDL class with vendor PQC support status:",
            "  P1: 12 internet-facing RSA/ECDH → ML-KEM hybrid TLS now (vendors ready)",
            "  P2: 23 internal PKI/VPN → ML-KEM + ML-DSA over the next 18 months",
            "  P3: 12 short-lived/symmetric → AES-256, schedule opportunistically",
            "Backlog ordered by exposure × feasibility — board-ready.",
            "Next: ciso-quantum-report",
            "",
            ">> LEARN: A roadmap is intel JOINED to your own inventory",
            "   Generic timelines don't drive budget; mapping the threat window onto",
            "   YOUR assets and vendors' PQC readiness produces an actionable backlog.",
            "   That join is exactly what a CISO quantum-readiness report must show.",
          ],
        }),
