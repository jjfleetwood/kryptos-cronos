  ...
  // ─── debate-3-01: Policy Debate ───────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The Affirmative case is judged on the stock issues — the standard burdens any policy must meet: Significance (is the harm in the status quo big enough to matter?), Inherency (is there a barrier in the status quo preventing the solution now?), Solvency (does the plan actually solve the harm?), and Topicality (does the plan fall within the resolution?). Plus the implicit weighing of advantages against disadvantages. The Affirmative must win all the stock issues; the Negative wins by defeating any one of them — proving the harm is trivial, the status quo already solves it, the plan won't work, or the plan isn't topical.",
        "The Negative's arsenal is large: disadvantages (the plan causes bad consequences that outweigh its benefits), counterplans (a different, often non-topical plan that solves better, making the Affirmative plan unnecessary), topicality (the plan doesn't meet the resolution's wording), and kritiks (challenges to the assumptions underlying the case — covered in epoch 7). Policy's defining features are speed ('spreading' — speaking 300+ words per minute to fit maximum arguments), evidence density, and technical line-by-line. It builds extraordinary research and information-processing skills, and it's the format most associated with the Tournament of Champions and intercollegiate national circuits.",
      ],
      technical: {
