  ...
  // ─── si-01: Sand to Silicon ──────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Getting from sand to chip-grade silicon takes a ladder of refining steps. Smelting heats quartz with carbon in an electric arc furnace, stripping away the oxygen to leave 'metallurgical-grade' silicon that is about 99% pure. The Siemens process then turns that silicon into a gas, distills it, and re-deposits it as 'electronic-grade' polysilicon at 99.9999999% purity — called '9N', nine nines — or better. Finally, crystal growing melts the pure polysilicon and pulls it into a single, flawless crystal so the atoms line up in one perfect lattice.",
        "That last step is the famous Czochralski process, and it is mesmerizing to picture. A tiny 'seed' crystal is dipped into molten silicon and slowly pulled upward while rotating, coaxing the melt to freeze onto it atom-by-atom in the same orientation; hours later you have a 'boule' (ingot), a shiny cylinder of monocrystalline silicon, today about 300 mm across and weighing over 100 kg. Purity and crystal perfection matter for trust as well as performance: contamination or hidden defects are exactly the kind of flaw that can make a chip behave unpredictably, which is why supply-chain integrity starts here at the raw material.",
      ],
      technical: {
  ...
  // ─── si-02: Wafers & the Cleanroom ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Turning a rough-cut slice into a flawless surface takes several finishing steps. Slicing uses a diamond wire saw to cut the boule into wafers, with a small notch ground in to mark the crystal orientation; polishing by lapping and chemical-mechanical planarization (CMP) makes one face atomically flat and mirror-smooth, because lithography later needs perfect focus; and epitaxy sometimes grows a thin, ultra-clean crystalline layer on top to give transistors a pristine surface to live in.",
        "Then the wafers move into the cleanroom, and the obsession with cleanliness is hard to overstate. The most advanced cleanrooms are 'ISO Class 1', thousands of times cleaner than a hospital operating room, with the air constantly filtered and flowing downward; workers wear full 'bunny suits' and the lighting is yellow, because the light-sensitive coatings used in patterning must not be exposed too early. A single particle smaller than a bacterium, landing in the wrong place, can kill a chip — so dust control is not housekeeping, it is the difference between profit and scrap, and a core part of manufacturing integrity.",
      ],
      technical: {
  ...
  // ─── si-03: Photolithography ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "One lithography cycle has a clear rhythm, and a chip needs dozens of them. First the wafer is spin-coated with 'photoresist', a chemical that changes when hit by light; then light is projected through a patterned 'mask' (reticle), shrinking the image about 4× as it lands on the wafer; and finally the exposed (or unexposed) resist washes away during develop, leaving a stencil that protects some areas and bares others for the next step of etching or doping.",
        "The whole game is making the printed features smaller, and the wavelength of the light sets the limit. Older tools used near-ultraviolet light (365 nm), and the industry then moved to 'deep ultraviolet' (DUV) at 248 nm and 193 nm to print finer lines; a clever trick called 'immersion' puts water between the lens and wafer to sharpen 193 nm light further (193i), squeezing years more progress out of it. The machines that do this — made chiefly by ASML, with Nikon and Canon — are among the most precise instruments ever built, aligning each layer to the ones beneath within nanometers.",
      ],
      technical: {
  ...
  // ─── si-04: EUV Lithography ──────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "EUV is so extreme that almost nothing about it is ordinary. The light comes from tiny tin droplets hit by a powerful laser to form a plasma that glows at 13.5 nm, repeated about 50,000 times per second; there are no lenses, because EUV is absorbed by glass and even by air, so the whole machine runs in a vacuum and uses ultra-smooth mirrors (coated with dozens of nanometer-thin layers) instead; and the scale is staggering — a single EUV scanner can cost on the order of $200 million, weigh as much as a bus, and is assembled from hundreds of thousands of parts.",
        "EUV matters because it replaced an increasingly painful workaround. Before EUV, fabs printed tiny features by 'multipatterning' — splitting one layer across several DUV masks and exposures, which is slow, costly, and error-prone; EUV prints many of those features in a single shot, simplifying the most advanced layers and enabling the 7 nm, 5 nm, and 3 nm generations. The newest 'High-NA' EUV pushes further still — and because only one company can build these tools, who is allowed to buy them has become a defining issue of national security and the global chip rivalry.",
      ],
      technical: {
  ...
  // ─── si-05: Doping & Ion Implantation ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Doping creates two complementary kinds of silicon, and transistors need both. N-type silicon, made by adding elements like phosphorus or arsenic, gains extra free electrons as negative carriers; p-type silicon, made by adding boron, gains 'holes' — places where an electron is missing, which act like positive carriers. Put n-type and p-type regions next to each other and you get a 'junction', the fundamental building block that lets current flow one way and not the other, and the basis of every transistor.",
        "The modern way to dope is ion implantation, which is exactly as dramatic as it sounds. Atoms of the dopant are ionized, accelerated to high speed, and fired like a beam into the wafer, embedding at a precise depth and amount controlled by the beam's energy and dose; the bombardment damages the crystal, so the wafer is 'annealed' (heated) to heal the lattice and lock the dopants into place. Precision is everything: the location and concentration of dopants define the transistor's behavior, so a doping process is closely guarded — and subtle, malicious changes to it are one theoretical way to hide a 'hardware trojan' in a chip.",
      ],
      technical: {
  ...
  // ─── si-06: Deposition & Etching ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Deposition is how every film on the chip is laid down, and there are several methods for different jobs. CVD (chemical vapor deposition) lets gases react on the wafer to grow a solid film, used for many insulating and structural layers; PVD or sputtering knocks atoms off a target so they land on the wafer, used mostly for metals; and ALD (atomic layer deposition) builds a film literally one atomic layer at a time — the only way to coat the tiniest features evenly and to lay down special 'high-k' insulators.",
        "Etching removes material with surgical control, and the wiring ties it all together. Dry (plasma) etching uses reactive gases in a plasma to carve straight, vertical walls, essential for tiny, dense features, while older 'wet' etching uses liquid chemicals; connections between layers use the 'damascene' process, in which trenches are etched, filled with copper, then polished flat, repeated to build 10–15+ metal wiring levels. Between layers, planarization (CMP) keeps each level perfectly flat so the next can be printed — and because these hidden internal layers are nearly impossible to inspect once buried, they are exactly where a malicious modification would be hardest to find.",
      ],
      technical: {
  ...
  // ─── si-07: FinFET to Gate-All-Around ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The transistor's shape has gone through a clear evolution. The planar MOSFET was the original flat design, which ran out of room as the channel got too short to control; the FinFET, introduced around 2011, stands the channel up as a thin 'fin' so the gate wraps around three sides and grips the current far better; and gate-all-around (GAA/nanosheet), the newest design (Samsung in 2022, then TSMC and Intel), has the gate fully surround stacked horizontal channels on all sides for the tightest control yet.",
        "This is also where the famous 'nanometer' node names stop meaning what you think. A '3 nm' or '2 nm' process does not have any part that is literally 3 or 2 nanometers — the numbers are now marketing labels for a generation, not a measurement — and what actually advances each generation is a mix of transistor shape, materials, and density improvements. These leading designs are why a modern AI GPU can pack tens of billions of transistors into a thumbnail of silicon, and why only a few fabs in the world can make them, a concentration with real economic and security weight.",
      ],
      technical: {
  ...
  // ─── si-08: Building a GPU ───────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "These chips have grown so large that a single die is no longer enough. The reticle limit means lithography can only print one die up to roughly 800 mm² in a single shot, a hard ceiling on how big one piece of silicon can be, so designers split the processor into multiple smaller dies — 'chiplets' or 'tiles' — and connect them tightly, with the newest flagship GPUs joining two large dies to act as one. This 'disaggregation' also improves yield and lets different parts use the best-suited process.",
        "The real magic of a modern GPU is in the packaging that binds it all together. HBM (high-bandwidth memory) stacks towers of DRAM chips connected by vertical 'through-silicon vias', then places them right beside the GPU for enormous memory bandwidth; 2.5D packaging such as TSMC's CoWoS sits the GPU dies and HBM stacks on a silicon 'interposer', a tiny circuit board of silicon that wires them together with thousands of connections; and 3D stacking goes further, bonding chips directly face-to-face ('hybrid bonding') for even denser, shorter links. Because these multi-die packages mix silicon from different sources, verifying that every component is genuine and untampered is a growing hardware-security challenge.",
      ],
      technical: {
  ...
  // ─── si-09: Yield, Testing & Cost ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Turning a finished wafer into sellable products takes several steps. Wafer test touches tiny probes to each chip on the wafer to check if it works before it is even cut out; binning sorts working chips by quality, so a part with one bad core or a lower top speed is sold as a cheaper model and almost nothing is wasted; and dicing and packaging saws the wafer into individual dies, which are packaged, connected to pins or balls, and tested again — sometimes 'burned in' under stress — before shipping.",
        "All of this explains why advanced chips are so expensive, and why so few can make them. Bigger dies like AI GPUs catch more defects, so their yield is lower and each good one is far more costly; a leading-edge fab costs $20–40 billion to build, and a single EUV machine runs around $200 million — capital only a handful of companies can muster. That concentration also creates security stakes: counterfeit, recycled, or subtly altered chips entering the supply chain are a real threat, so traceability and authentication of genuine parts matter as much as raw performance.",
      ],
      technical: {
  ...
  // ─── si-10: Fabricating Quantum Chips ────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The leading qubit technologies are built in strikingly different ways. Superconducting qubits (IBM, Google) are circuits of aluminum or niobium on a silicon or sapphire chip, using 'Josephson junctions', that must be cooled to about 10–15 millikelvin — colder than outer space — in a dilution refrigerator; trapped ions (IonQ, Quantinuum) are individual charged atoms held in a vacuum by electromagnetic fields from a micro-fabricated trap chip and manipulated with lasers; and silicon spin qubits (Intel) are single electron spins held in quantum dots, deliberately designed to reuse standard CMOS transistor fabrication.",
        "What makes quantum fabrication so hard is that the qubits are exquisitely fragile. Tiny imperfections, stray materials, or vibrations cause 'decoherence', collapsing the quantum state, so material purity and interface quality matter even more than in classical chips; some silicon spin qubits even use isotopically purified silicon-28 to remove magnetic 'noise' from other isotopes, an extra purification step beyond anything classical chips need. Because errors are so common, today's machines need heavy 'error correction', and scaling up qubit counts while keeping them stable is the central manufacturing challenge — quantum chipmaking is roughly where classical chipmaking was in its earliest, hand-built decades, and a field where post-quantum security planning is already underway.",
      ],
      technical: {
