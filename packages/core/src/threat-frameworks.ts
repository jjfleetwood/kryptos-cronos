...
export const threatFrameworksStages: StageConfig[] = [
  ...
  // ─── tf-02: The Cyber Kill Chain (CTF) ───────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Cyber Kill Chain (Lockheed Martin, 2011) models an intrusion as seven sequential stages: Reconnaissance (research the target), Weaponization (build the malware/lure), Delivery (send it — phish, USB, web), Exploitation (trigger the vulnerability), Installation (establish persistence), Command & Control (C2, the attacker's remote control), and Actions on Objectives (the actual goal — steal, encrypt, destroy).",
  ...
  // ─── tf-03: The Diamond Model (CTF) ──────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The Diamond Model of Intrusion Analysis (2013) describes a single intrusion event as a diamond with four connected vertices: Adversary (who), Capability (the tool/technique — malware, exploit), Infrastructure (the adversary's IPs/domains/servers), and Victim (the target). Edges connect them: an adversary develops capabilities and uses infrastructure to reach a victim.",
  ...
  // ─── tf-04: MITRE ATT&CK (Quiz) ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "ATT&CK is behavior-focused and evidence-based: every technique is documented from observed, in-the-wild activity, with descriptions, examples of which groups/software use it, detection guidance, and mitigations. There are matrices for Enterprise, Mobile, and ICS, plus Groups (tracked actors) and Software (malware/tools) mapped to the techniques they use.",
  ...
  // ─── tf-10: Threat-Informed Defense (Quiz) ───────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Threat-informed defense (a discipline championed by MITRE's Center for Threat-Informed Defense) means letting actual adversary behavior drive your security decisions, rather than generic checklists. It combines every lens in this epoch into one cycle: understand the threat, map it, prioritize, defend, share, and repeat.",
