  ...
  // ─── r2-02: Exploiting a DDS Domain (CTF) ────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Without SROS2, that discovery is a map for an attacker. You join the domain, enumerate every topic (camera feeds, joint states, velocity commands, e-stop), then subscribe to read data or publish to command. There's no authentication to stop you and no access control to limit you — you are a peer like any node.",
  ...
  // ─── r2-07: Legged & Humanoid Robots (Quiz) ──────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Their attack surface is the union of everything in both robotics epochs:\n- the comms stack (often ROS 2/DDS, sometimes app/cloud control)\n- wireless control (Wi-Fi/Bluetooth/cellular)\n- onboard perception ML\n- firmware\n- a rich sensor payload (cameras, mics) that's a privacy and surveillance concern\nSeveral of these platforms have had documented vulnerabilities in their apps, network services, or firmware.",
        "What raises the stakes is capability and proximity — these robots are strong, fast, mobile, and operate near or among people, sometimes carrying tools or payloads. A compromise can mean:\n- physical danger\n- covert surveillance through the robot's sensors\n- use of the robot as a mobile attack platform inside a facility\nFunctional-safety and security must be designed together — the theme that closes both robotics epochs.",
      ],
      technical: {
        ...
        body: [
          ...
          "Defenses combine everything taught so far with safety engineering:\n- secure the comms (enable SROS2, authenticate app/cloud)\n- protect and verify firmware (secure boot, signed updates)\n- minimize and protect sensor data\n- segment and monitor the robots' network\n- design hard safety limits (e-stops, speed/force limits, geofencing) that hold even if higher-level control is compromised\nSecurity and functional safety are one problem here.",
        ],
      },
      incident: {
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Robust localization is the answer:\n- fuse GNSS with IMU, odometry, and SLAM\n- run consistency checks (the position can't jump faster than physics allows; sources must agree)\n- use authenticated GNSS where available\n- add spoofing/jamming detection (signal strength, multiple antennas, anomaly detection)\n- design fail-safe behavior on a detected attack (hold, slow, or hand to a human)\nThe lesson generalizes the perception work: never trust a single localization input. In this challenge you'll spoof GPS to inject a false position, then redirect the robot.",
        ],
      },
      incident: {
  ...
  // ─── r2-09: Poisoning a Robot's Learned Policy (CTF) ─────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The danger is stealth and scale: a backdoored policy passes normal testing and ships, then the trigger activates malicious physical behavior in the field — and a single popular poisoned model can affect many robots. Defenses are ML-security practices with robotics teeth:\n- trusted/verified data and model provenance\n- dataset and model scanning\n- robust training\n- behavioral testing including adversarial/trigger search\n- runtime safety limits that bound what any policy can do\nIn this challenge you'll poison a dataset and deploy the backdoored policy.",
      ],
      technical: {
  ...
  // ─── r2-10: Securing Robot Fleets at Scale (Quiz) ────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Two principles bind it together. The first is zero trust — never trust a peer, a sensor, a command, a model, or a cloud caller without verification, because the recurring failure in every stage was misplaced trust. The second is an independent safety envelope: hard, lower-level limits (e-stop, speed/force, geofencing) that physically bound the robot and hold even if every higher layer is compromised — so a hack can degrade but not endanger.",
        "Finally, robots are long-lived, updatable, physical assets, so security is a lifecycle:\n- secure boot and signed OTA to patch the fleet\n- SBOMs and supply-chain management for components and models\n- monitoring/telemetry to a SOC for the robot fleet\n- incident response that accounts for physical consequences\n- standards (IEC 62443 for industrial, ISO 10218/15066 for robot safety, the Robot Vulnerability Database, and emerging robot-security guidance)\nBuild it in, verify everything, bound the physical risk, and operate it for life.",
      ],
      technical: {
