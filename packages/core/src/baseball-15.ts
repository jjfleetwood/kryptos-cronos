...
export const baseball15Stages: StageConfig[] = [
  ...
  // ─── baseball-15-02: Reading the Ball and the Jump ────────────────────────────
  {
    ...
    info: {
      ...
      year: 1990,
      overview: [
        "Like every outfielder, the right fielder's most important skill is reading the ball off the bat and getting a good first step — the jump. An outfielder who reads the ball instantly and breaks in the right direction covers far more ground than one who's faster but reacts late or takes a poor route. The read begins before contact with anticipation — studying the pitch, count, and hitter's tendencies — and continues at contact, reading the ball's trajectory from the sound and initial flight, then taking the proper first step and an efficient route.",
        "The right fielder's distinctive read is the slice on balls from left-handed pull hitters, which hook toward the right-field line as they travel. He must account for this curve in his route, rather than taking a straight line to where the ball started. On balls over his head, he drop-steps and turns to run (never backpedaling); on balls in the gap, he breaks hard; on balls in front, he charges under control. Reading the slice and running efficient routes are what let the right fielder cover his corner.",
  ...
  // ─── baseball-15-03: Catching Fly Balls and Line Drives ───────────────────────
  {
    ...
    info: {
      ...
      year: 1985,
      overview: [
        "Catching fly balls is the right fielder's most fundamental skill, and for the strong-arm corner, catching in a way that sets up the throw is especially important. The technique: get to the ball's landing spot quickly with a good route, get under control, and catch with two hands at or above the throwing-shoulder side of the head — positioned so the catch flows directly into a throw. Because the right fielder's arm is his weapon, catching above the throwing shoulder (rather than basket-style at the belt) lets him transfer and unleash the cannon immediately on plays at the bases.",
        "Different balls require different catches. A routine fly ball is caught with two hands above the throwing shoulder while moving toward the target if possible. A line drive is the hardest read — the right fielder must judge whether it's sinking (charge and possibly catch low or short-hop it) or carrying (drift back) and commit. The over-the-shoulder catch is used on balls hit well over the head — after a drop step and full sprint, the right fielder catches the ball over his shoulder on the run. Each catch is built through repetition.",
  ...
  // ─── baseball-15-05: Building the Right Fielder's Body ────────────────────────
  {
    ...
    info: {
      ...
      year: 2015,
      overview: [
        "The right fielder's body combines the speed of an outfielder with the strongest corner arm. The key physical priorities are first-step quickness and acceleration (for the jump and range), straight-line and change-of-direction speed (to cover the corner), endurance, and — most distinctively — elite arm strength for the longest outfield throws to third and home. While the right fielder doesn't need quite the range of a center fielder, his arm is his defining weapon, so building and protecting a powerful, durable throwing arm is a top priority.",
        "Speed and quickness come from the same training as any outfielder: first-step quickness and acceleration for the jump, straight-line speed for deep balls, and change-of-direction agility for slicing balls and the gaps, built through sprint training, acceleration drills, and reaction work. Strong legs and a powerful core drive the sprint and, crucially, the throws. The right fielder's lower body and core are the foundation of both his range and his cannon — the legs and core generate much of the power on the long throw.",
  ...
  // ─── baseball-15-06: Playing the Line and the Sun ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 1990,
      overview: [
        "Like the left fielder, the right fielder must play the line, the wall, and the corner — and he often deals with the sun more than any outfielder. A ball hit down the right-field line that gets past the right fielder rolls into the corner for an extra-base hit, so he must cut off balls down the line and play the corner caroms cleanly to hold the hitter to as few bases as possible. Playing the wall — judging whether a ball will be caught, clear the fence, or carom back, and knowing the specific wall's angles — turns would-be doubles into singles and outs.",
        ...
        "Playing the corner well combines all of these. The right fielder reads the ball, plays the line to prevent extra-base hits, knows the wall's caroms to hold runners, and manages the sun to make catches he'd otherwise lose. Positioning matters too: in no-doubles situations and against pull hitters (especially lefties), the right fielder guards the line. Reading and playing the line, the wall, and the sun — turning extra-base hits into singles and saving catches — is a skill that separates a complete right fielder from one who only catches routine fly balls in open space.",
      ],
      technical: {
  ...
  // ─── baseball-15-07: Backing Up First Base ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "The right fielder also backs up second base on certain plays — on throws to second from the left side, and on balls hit up the middle when the center fielder is occupied — and backs up the center fielder on balls in right-center. He backs up bunt plays and pickoffs at first. The principle, as for all outfielders, is that an overthrow or a misplayed ball in his area should never go unguarded — the right fielder positions behind the play so a misplay doesn't cost extra bases. Backing up first base, in particular, is a constant, essential responsibility.",
        "Knowing his backup responsibilities and reacting instantly is part of the right fielder's job on every pitch. Backing up first base on pickoffs and throws, backing up second and the center fielder, and backing up bunt plays — these are constant, unglamorous responsibilities that prevent extra bases on misplays. Combined with his catching, his cannon, and his corner play, the right fielder's backup work makes him a contributor on every play, even when the ball isn't hit to him. Like every position, he's never standing still.",
      ],
      technical: {
  ...
  // ─── baseball-15-08: Cutoffs, Relays, and Where to Throw ──────────────────────
  {
    ...
    info: {
      ...
      year: 1995,
      overview: [
        "For the right fielder, whose arm is his weapon, knowing where to throw — and deciding before fielding the ball — is essential to making the cannon effective. As the ball comes to him, the right fielder must already know the situation (outs, runners, score) and where the play is: hold the ball, throw to second to keep a hitter to a single, throw to third (his signature long throw) to retire or hold an advancing runner, or throw home on a play at the plate. Deciding in advance lets him field the ball with momentum toward the target and unleash the cannon decisively.",
  ...
  // ─── baseball-15-09: Situational IQ ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2000,
      overview: [
        "A complete right fielder plays the game mentally before each pitch. He processes the situation — score, inning, outs, runners and their speed, the count, and the hitter's tendencies — and sets his depth and position accordingly (normal, shallow, deep no-doubles, shaded to the line or gap). He also pre-decides what he'll do if the ball is hit to him: where the play is and where he'll throw, so his cannon fires decisively. This pre-pitch thinking lets him react instantly and correctly when the ball is hit.",
        "The elements — the sun, the wind, and the field — are part of the right fielder's situational awareness, and the sun especially. The right fielder must know where the sun is and use sunglasses or his glove to shield it on fly balls, account for the wind's effect on fly balls, and know the field's quirks (the line, the corner, the wall, the warning track). Checking the sun and wind before each at-bat, and knowing the warning track by feel, are part of playing right field well and safely — the sun in particular is often a bigger factor in right field than elsewhere.",
