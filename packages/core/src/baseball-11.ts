...
export const baseball11Stages: StageConfig[] = [
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Depth by situation: play normal depth in most situations; guard the line (close to the foul line) late in close games to take away the double; play in when a bunt is likely; adjust deeper and toward the line against hard pull hitters, or in against slap and bunt threats. Each is a pre-pitch decision weighing range, reaction time, and the cost of an extra-base hit down the line. Guarding the line concedes the hole to prevent the double.",
        ],
        codeExample: {
  ...
  // ─── baseball-11-03: Reactions and Soft Hands ─────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1970,
      overview: [
        "The defining skill of a third baseman is handling the hardest-hit balls with quick, soft hands and lightning reactions. Many balls at third can't be fielded cleanly — they're hit too hard, take a bad hop, or arrive in a blink. The great third basemen knock these balls down, keep them close, and still make the play. The combination of fast reflexes (to get the glove or body to the ball) and soft hands (to absorb and control it) is what turns rockets into outs. There's often time to recover after a knockdown because the runner is usually a step slower than the ball.",
        ...
        "Reactions are built through repetition and a ready, anticipating mindset. A third baseman who expects the ball on every pitch — who is low, loaded, and mentally ready for a smash — reacts far faster than one hoping the ball goes elsewhere. Reaction drills (rapid-fire short hops, balls off a wall, reaction balls with unpredictable bounces) train the hands and reflexes. Fearlessness matters too: a third baseman who flinches or turns away from hard-hit balls can't field them. Quick hands, soft hands, and courage are the hot corner's core.",
      ],
      technical: {
  ...
  // ─── baseball-11-04: The Long Throw ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "The throw from third base to first is the longest routine throw an infielder makes, so a strong, accurate arm is one of the two defining requirements of the position (along with quick reactions). But arm strength alone isn't enough — the third baseman must pair it with footwork that creates momentum toward first and a quick, clean transfer. Many third-base throws come from awkward positions: deep behind the bag, charging in on a slow roller, or after knocking down a smash, so the ability to throw accurately from any angle is essential.",
        "Footwork creates both velocity and accuracy on the long throw. On a ball fielded with time, the third baseman gathers, steps toward first with the front shoulder aligned to the target, and throws through the target — using the legs and a crow-hop or shuffle to generate momentum across the diamond. On a ball deep in the hole, they may need to plant the back foot and throw across the body. The four-seam grip is essential for a straight, true ball over that long distance — a poor grip causes the throw to tail or sink and pull the first baseman off the bag.",
  ...
  // ─── baseball-11-05: Building the Third Baseman's Body ────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "The third baseman's body is built around two physical priorities: reaction quickness (fast-twitch reflexes and explosive first-step movement to handle the hardest-hit balls) and arm strength (to make the longest infield throw). Unlike the second baseman's emphasis on lateral range or the shortstop's all-around athleticism, third base concentrates on quick hands and reactions over short distances, plus the power and durability to throw across the diamond from any position. Lateral agility and quickness still matter, but the signature traits are reflexes and arm.",
        "Reaction and quickness training is central. Fast-twitch reflexes are developed through reaction drills (rapid short hops, reaction balls, hand-eye and reflex training) and first-step explosiveness work. Strong legs and a powerful core support the explosive first step, the throws from awkward positions, and the ability to charge and field on the run. Single-leg strength and balance help the third baseman throw accurately from off-balance positions — deep in the hole or charging a slow roller.",
  ...
  // ─── baseball-11-07: Backhands and Guarding the Line ──────────────────────────
  {
    ...
    info: {
      ...
      year: 2004,
      overview: [
        "While third base prioritizes reactions over range, the third baseman must still cover ground to two areas: down the third-base line (where a ball that gets past is an extra-base hit) and into the hole toward shortstop. Both often require a backhand play — fielding the ball on the glove side with the glove turned over — and sometimes a dive. The backhand into the hole, followed by a strong throw across the body from deep, is one of the most demanding and impressive plays a third baseman makes.",
        "The backhand technique: as the ball is hit to the glove side, take a crossover step and get the glove out and turned over (palm facing the ball, fingers pointing down or to the side), field the ball out in front on the backhand, and then plant the back foot to throw. On a ball deep in the hole, the third baseman fields on the backhand, plants hard on the right foot, and throws across the body all the way to first — a play that demands a strong arm and excellent footwork. On balls just out of reach, a dive to the backhand keeps the ball from going down the line for extra bases.",
  ...
  // ─── baseball-11-08: Cutoffs, Bunts, and Backups ──────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Bunt coverage is a major third-base responsibility. On a sacrifice bunt, the third baseman often charges to field bunts on the left side, and the defense must rotate coverage — typically the shortstop covers third base when the third baseman charges. On the aggressive wheel play (going for the lead runner at third), the corners charge hard and the shortstop covers third. The third baseman must read the bunt situation, charge or hold based on the play call, and communicate with the shortstop and pitcher about coverage.",
        "The third baseman also covers third base on steals and tag plays and backs up bases. He covers third when a runner steals third (taking the catcher's throw for the tag), backs up throws to third from the outfield when he's not the cutoff or coverer, and is part of the bunt and relay coverage system. As at every position, the third baseman must know before each pitch whether he'll be charging a bunt, covering third, serving as a cutoff, or backing up — there's a job on every ball in play.",
      ],
      technical: {
  ...
  // ─── baseball-11-09: Situational IQ ───────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2000,
      overview: [
        "Because the ball arrives so fast at third base, situational thinking must be done entirely before the pitch. The third baseman processes the situation — score, inning, outs, runners and their speed, the count, and the hitter's tendencies — and decides positioning (normal, guard the line, infield in, in for a bunt) and what to do with the ball if it's hit to him. With a runner on third and the infield in, he charges and throws home; with a runner on first and a double play in order, he may start a 5-4-3; on a tag play at third, he applies the tag. These decisions can't wait until the ball is hit.",
        "Tag plays and force decisions are key reads. On a ground ball, the third baseman must know instantly whether he has a force at third (a runner forced from second with the bases loaded or a runner on first and second) or whether he needs to tag a runner. He must know whether to take the sure out at first, start a double play, look the runner back, or make a play at the plate. With the infield in, the priority shifts to cutting down the run at home. Each scenario requires a pre-made plan executed instantly.",
