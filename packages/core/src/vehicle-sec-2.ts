...
export const vehicleSec2Stages: StageConfig[] = [
    ...
    info: {
      ...
      technical: {
        ...
        body: [
          ...
          "Robustness comes from redundancy and cross-checks:\n- multi-sensor fusion so a single compromised modality is outvoted\n- temporal consistency (objects shouldn't teleport)\n- adversarial training of the perception models\n- anomaly detection on sensor inputs\n- conservative fail-safe behavior (slow/stop on uncertainty)\nDefense-in-depth treats perception as untrusted input to be validated, exactly like the rest of this epoch treats radios and buses. The fusion and ML-robustness themes tie AD security back to both the robotics and AI tracks.",
        ],
      },
      incident: {
  ...
  // ─── v2-08: Connected-Car & Fleet APIs (CTF) ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Connected cars are managed through automaker cloud platforms — the mobile app, the telematics backend, and fleet-management portals. All of them expose APIs to locate the car, unlock it, start it, read diagnostics, and manage owners. This is enormously convenient — and it moves a big part of the car's attack surface into ordinary web/API security.",
      ...
      technical: {
        ...
        body: [
          ...
          "Telematics and fleet platforms amplify it: a flaw in a dealer or fleet portal, or in the SSO between systems, can expose bulk command capability across many vehicles. Defenses are standard but must be airtight:\n- enforce per-object authorization server-side on every endpoint\n- scope tokens tightly\n- apply least privilege to dealer/fleet roles\n- rate-limit and monitor command APIs\n- pen-test them continuously\nThe lesson bridges this epoch to the whole web-security curriculum: a two-ton car is now also a web object. In this challenge you'll probe a fleet API and change a VIN to take over a vehicle.",
        ],
      },
      incident: {
  ...
  // ─── v2-10: Securing the Software-Defined Vehicle (Quiz) ─────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Underneath it all is software supply-chain security: a modern car integrates code from hundreds of suppliers, so SBOMs (software bills of materials), known-vulnerability tracking, and supplier security requirements (driven by ISO/SAE 21434) are essential. The whole-vehicle model unites everything:\n- build security in (21434/R155)\n- segment and authenticate inside\n- harden every external interface\n- secure the supply chain and backend\n- monitor and update the fleet for life",
      ],
      technical: {
