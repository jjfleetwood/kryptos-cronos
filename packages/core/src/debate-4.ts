...
export const debate4Stages: StageConfig[] = [
  ...
  // ─── debate-4-02: Research and Cutting Cards ──────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2004,
      overview: [
        "Once topic analysis tells you what to look for, research finds the evidence to support it. The skill has two halves: finding strong sources and organizing them so you can deploy them instantly in a round. Strong research means going beyond the first search result to authoritative, recent, relevant sources — peer-reviewed studies, government and institutional data, expert analysis, quality journalism — and evaluating each with the CRAAP/quality tests from epoch 1. The goal is not the most evidence but the best evidence for the specific arguments your map identified.",
        "'Cutting a card' is debate's term for turning a source into usable evidence. A card has three parts: a tag (a short label stating what the card proves, in your own words), the citation (author, qualification, date, source), and the quoted text (the relevant excerpt, accurately reproduced, often with the key sentences emphasized). The tag is what you read first in a round to tell the judge what's coming; the citation establishes credibility; the quote is the proof. Cutting cards honestly — never distorting the source's meaning (epoch 1's evidence ethics) — and tagging them clearly is the craft that turns raw reading into a usable arsenal.",
  ...
  // ─── debate-4-03: Building the Constructive Case ──────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The constructive case is the structured presentation of your side, delivered in your first speech. Its job is to give the judge a clear, complete, self-standing reason to vote for you before any clash begins. A strong case has a recognizable architecture: an introduction and (where the format calls for it) a framework, followed by a small number of contentions — typically two or three — each of which is a complete argument with claim, warrant, and impact (epoch 1), arranged in a logical order and signposted so the judge can flow them.",
        ...
        "Architecture matters as much as content. Order contentions purposefully — often strongest first, or in a logic that builds. Make the framework (if any) come before the contentions, since it tells the judge how to weigh them. Signpost relentlessly so the case is flowable. And keep the case within time, which forces ruthless prioritization: every second spent on a weak contention is a second not spent strengthening a winning one. A well-built constructive is like a sound structure — a clear thesis (the keystone), a few load-bearing contentions, each complete, arranged so the whole stands even under attack. Build it well and the rest of the round is defending a strong position rather than scrambling to assemble one.",
      ],
      technical: {
  ...
  // ─── debate-4-04: The Framework ───────────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Frameworks take different forms by format. In Lincoln-Douglas, it's the explicit value and criterion (epoch 3): the core value (justice, autonomy) and the standard for measuring it (maximizing well-being, protecting rights). In policy debate, frameworks appear as the weighing of advantages against disadvantages and as 'role of the ballot' arguments about what the judge should be deciding. In Public Forum, framing often takes the form of a weighing mechanism or a stated standard ('prioritize the most probable and largest impact'). Across all of them, the framework answers: by what standard should the judge decide who won?",
        "Because the framework is so decisive, sophisticated debaters fight for it directly. You argue why your framework should be preferred (it's more fundamental, more inclusive, better grounded in the resolution, or avoids a flaw in the opponent's), and you also argue under the opponent's framework as a backup ('even if you adopt their standard, I still win because…') — the layered reasoning from epoch 3. You then explicitly weigh your contentions under the winning framework, showing they best satisfy the standard while the opponent's fail it. A debater who controls the framework controls how every other argument is counted, which is why mastering framework construction and the framework debate is the core skill of high-level case construction.",
      ],
      technical: {
  ...
  // ─── debate-4-05: Policy Case Construction ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1970,
      overview: [
        "A policy case (the Affirmative case in policy debate, and the structure behind any 'should the government do X' resolution) is built to satisfy the stock issues from epoch 3. It has a recognizable skeleton: a harm (a significant problem in the status quo), inherency (a barrier preventing the status quo from solving it), a plan (the specific action the Affirmative proposes), solvency (proof the plan actually solves the harm), and advantages (the net benefits, which must outweigh any disadvantages). Each element is a distinct burden, and a missing or weak one is a hole the Negative will exploit.",
        ...
        "Advantages are how a policy case wins on weighing. An advantage is a chain: the plan leads to a good outcome (the internal link), and that outcome matters (the impact). 'Plan funds public transit → reduces emissions → mitigates climate harm' is an advantage with a link and an impact. The Affirmative builds one or more advantages and must show they outweigh the disadvantages the Negative will run. Strong policy case construction means each stock issue is proven (significant harm, real barrier, specific plan, demonstrated solvency) and the advantages are large, probable, and clearly weighed — so that even granting some Negative offense, the judge sees the plan as net beneficial.",
      ],
      technical: {
  ...
  // ─── debate-4-06: Public Forum Case and Links ─────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2002,
      overview: [
        "A Public Forum case is built for the reasonable-person standard from epoch 3: clear, accessible contentions that a non-specialist judge can follow and that connect to tangible real-world impacts. PF cases typically have two contentions, each a complete argument, framed for persuasion rather than technical density. Because topics change monthly and judges are often lay, the premium is on clarity and a clean link chain — the logical steps from your contention's claim to an impact that obviously matters — rather than on stacks of cards or jargon.",
        ...
        "PF case-building also demands ruthless accessibility and weighing. Each contention should be explainable to an intelligent non-expert in plain language, with evidence cited cleanly (epoch 1) but not buried under technicality. And because lay judges need to be told how to compare impacts, PF cases are built with weighing in mind from the start — choosing impacts that are large, probable, and clearly significant, and preparing to explain why they outweigh the opponent's. A strong PF case is two clear contentions, each with a solid link chain to a weighable real-world impact, delivered so a reasonable person not only follows it but is persuaded by it.",
      ],
      technical: {
  ...
  // ─── debate-4-07: Weighing Mechanisms ─────────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "There are standard weighing mechanisms — dimensions along which impacts are compared. Magnitude: how big is the impact (how many affected, how severe)? Probability: how likely is it to actually happen? Timeframe: how soon does it occur (and does a sooner impact preempt a later one)? Scope: how many people or how wide an area? Reversibility: can the harm be undone, or is it permanent? Each is a lever for arguing your impact matters more: 'our impact is larger (magnitude) and far more likely (probability) than theirs, which is speculative and distant.' Knowing the mechanisms lets you make precise comparative arguments rather than vague claims of importance.",
  ...
  // ─── debate-4-08: Briefs, Blocks, and Frontlines ──────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "Because topic analysis (epoch 4-01) reveals the arguments both sides will make, you can pre-write your responses to the predictable ones. These pre-written response sets are called blocks, briefs, or frontlines, and they are one of the highest-return investments in preparation. A block is a ready-made set of responses to a specific anticipated argument — written calmly in advance with your best evidence and reasoning, so that when that argument appears in a round, you deliver a polished answer instead of improvising under pressure. The difference between a prepared block and an in-round improvisation is usually the difference between a clean win on that argument and a scramble.",
        "Blocks come in two flavors. Offensive blocks (or 'extensions') prepare how you'll defend and extend your own contentions against the responses you expect — your second-line answers to the opponent's predictable attacks on your case. Defensive blocks (or 'answers') prepare your responses to the opponent's predictable arguments — your refutations, ready to go, when they run the disadvantage or contention you knew was coming. A well-prepared debater walks into a round with a folder of blocks covering the main clashes their topic analysis identified, organized so they can pull the right one instantly (epoch 4-02's filing).",
  ...
  // ─── debate-4-09: Roadmaps and Signposting ────────────────────────────────────
  {
    ...
    info: {
      ...
      overview: [
        ...
        "Signposting (introduced in epoch 1) is the in-speech companion to the roadmap: continuous verbal labels marking where you are. 'Now to their first contention…', 'My second response here is…', 'Moving back to my own case…'. Where the roadmap previews the structure, signposts keep the judge oriented within it as you move. Together they make a speech navigable — the judge always knows which argument you're on and how it fits the whole. This is not cosmetic: a judge who can follow your organization flows your arguments accurately and credits them; a judge lost in a disorganized speech misses arguments you actually made.",
  ...
  // ─── debate-4-10: Preparing Against Your Own Case ─────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "The final step of case construction is to turn against your own case and attack it as hard as the toughest opponent would — a practice sometimes called 'red-teaming' or 'prepping out' your own case. You've built your contentions, framework, and blocks; now you deliberately look for every weakness: the shaky link, the vulnerable premise, the evidence that overstates, the framework an opponent could flip, the response you don't have a good answer to. Finding these in your own preparation, when you have time to fix them, is infinitely better than discovering them when an opponent exploits them in a round.",
        "This works best as adversarial practice. Have a teammate or coach run the strongest possible case against yours, or argue the opposite side yourself and genuinely try to beat your own case. The goal is to surface the arguments you'll actually face and test whether your blocks (epoch 4-08) hold. Where they don't, you have a choice: strengthen the weak part (better evidence, a tighter link, a defended framework), build a block for the response you lacked, or — if a contention can't be defended — cut it and replace it with something stronger. A case that has survived a serious internal attack is far more robust than one that has never been tested.",
