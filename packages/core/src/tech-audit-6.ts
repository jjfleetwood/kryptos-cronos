...
re-vet:      tools re-checked on every version bump (stops rug pulls)`,
      ...
      technical: {
        ...
        body: [
          ...
          "NHI governance test — require a non-human identity inventory whose entries each capture:\n- ownership\n- scope\n- credential type\n- rotation policy\n- last-used time\nThen flag any credential that is orphaned, stale, over-scoped, shared, or non-rotating. Artifact: NHI inventory, IAM policies, rotation/expiry config, attribution logs. The single best advanced-integration pull.",
...
note:     every single-agent test was green; only the SYSTEM failed`,
      ...
      overview: [
        ...
        "Containment at this level is system-wide and tested:\n- blast-radius limits per agent\n- validated hand-offs\n- loop/depth caps\n- circuit breakers that trip on system-level anomalies (not just single-agent errors)\n- a kill switch that can halt the whole system, not one agent\nThe defining advanced-amplification evidence is a record of the system surviving an injected fault — 'we caused a cascade in staging and the circuit breaker contained it in N seconds' is worth more than any architecture review.",
      ],
      technical: {
...
=> reconstructable AND tamper-evident -> admissible evidence`,
      ...
      overview: [
        ...
        "The reconstruction standard is the test: for any production action, can the team replay it well enough to explain and, ideally, reproduce it? That replay must include:\n- what the agent saw\n- every model and tool call\n- every decision made\n- the resulting action\nThis is the question from the baseline epoch, now held to a forensic standard. 'We log the outputs' fails it; 'here is the full span tree for request X with inputs, tool calls, and the model version live at the time' passes it.",
        ...
        "Tracing also carries its own risks the auditor must weigh. Traces contain prompts, retrieved data, and outputs that may include PII or secrets, so the trace store needs its own controls:\n- access control\n- retention policy\n- redaction of sensitive fields\nObservability can become a data-leak surface if logged carelessly. And retention must match obligation: the EU AI Act and many regulators require logs be kept for defined periods to enable traceability. The auditor balances 'capture enough to reconstruct' against 'don't turn the trace store into a breach.'",
      ],
      technical: {
        title: "Auditing Traces and Their Integrity",
        body: [
          "Depth test: sample production traces and verify each captures:\n- inputs/context\n- every model call (with version + params)\n- every tool call (args/result/errors)\n- decisions and hand-offs\n- the final action\nArtifact: trace samples, telemetry schema (OpenTelemetry GenAI conventions). Gap: traces that record only prompts and final outputs can't support forensics.",
...
caveat:          the monitor-agent is itself validated (kappa 0.82)`,
      ...
      technical: {
        ...
        body: [
          ...
          "Auditor-of-the-auditor test: if monitoring agents are used, audit them on four axes:\n- coverage (what failure modes can they miss?)\n- accuracy (validated like any eval/judge?)\n- independence (can the monitored system influence them?)\n- gaming resistance\nArtifact: monitoring-agent validation report. Never accept 'green dashboard' as evidence the green is real.",
        ],
        codeExample: {
...
fix:         pin exact hash; require signed weights;
      ...
      year: 2025,
      overview: [
        "A modern agent is a supply chain:\n- a base model (usually third-party)\n- fine-tuning data\n- prompts\n- tools and MCP servers\n- libraries\n- the orchestration code\nAdvanced integration/amplification assurance has to answer where each component came from, whether it's authentic and unmodified, and whether you can trust the parties that produced it. This is the AI supply-chain audit, and its central artifact is the AIBOM — the AI Bill of Materials.",
      ...
      technical: {
        ...
        body: [
          "AIBOM completeness test: require a per-release AIBOM, generated by the pipeline, covering:\n- models and datasets\n- tools/MCP and dependencies\n- versions and licenses\n- provenance\nArtifact: AIBOM (SPDX/CycloneDX-derived). Gap: missing components (especially tools and data) or a hand-maintained, stale AIBOM.",
...
remediation: add context to approval UI; cap reviewer load; test override`,
      ...
      technical: {
        ...
        body: [
          "EU AI Act mapping test — for a high-risk/agentic system, map each obligation to its evidencing control and artifact, then flag unmet obligations as compliance findings. The obligations to map:\n- risk management\n- data governance\n- Article 12 logging\n- Article 14 human oversight\n- accuracy, robustness, and cybersecurity\n- transparency\nArtifact: obligation-to-control matrix, evidence references.",
...
=> resilience is built BEFORE the incident, in the artifacts`,
      ...
      technical: {
        ...
        body: [
          ...
          "Learn + close the loop:\n- convert the incident into new eval cases\n- tightened guardrails/scopes\n- added monitoring\n- updated runbooks\n- an assessment of whether the evidence trail was sufficient\nArtifact: post-incident review, updated eval set, control changes, tracing-gap findings. Incidents are the highest-value input to control improvement.",
        ],
        codeExample: {
...
trend:        findings closing > opening; mean-time-to-detect -60%`,
      ...
      overview: [
        ...
        "A mature function is risk-based and tiered. You can't deeply audit every agent equally, so you maintain an agent inventory, classify each by autonomy and blast radius, and allocate assurance accordingly:\n- High-autonomy, high-impact agents (money, safety, customers, regulated decisions) — deep periodic audits plus intensive continuous monitoring.\n- Low-risk assistive agents — lightweight baseline checks and automated monitoring.\nThe inventory itself is a deliverable — an organization that can't list its agents can't assure them, and shadow agents are a finding.",
        ...
        "Finally, a mature function reports a clear, board-level picture and improves continuously. The deliverable to the audit committee is the organization's agentic risk posture, answered with evidence, not adjectives:\n- how many agents, at what autonomy/blast radius\n- what G-I-A coverage\n- what open findings, by severity\n- what regulatory exposure\nAnd the program feeds itself: incidents, monitoring signals, and audit findings continuously sharpen the control catalog, the eval sets, and the risk model. That is the end state this entire two-epoch track builds toward — not the ability to audit an agent, but the capability to give an organization durable, evidenced assurance over autonomous AI as it scales.",
      ],
      technical: {
        ...
        body: [
          ...
          "Board reporting + continuous improvement: report the agentic risk posture with evidence:\n- agent count × tier\n- G-I-A coverage\n- findings by severity\n- regulatory exposure\nThen feed incidents, signals, and findings back into the control catalog, eval sets, and risk model. Artifact: board risk report, improvement backlog.",
        ],
        codeExample: {
