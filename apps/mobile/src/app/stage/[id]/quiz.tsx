...
const PER_ATTEMPT = 5;
const HERO_COLORS = ["#0e2230", "#141a3a", "#1a1430"] as const;
...
export default function StageQuizScreen() {
  ...
  const back = () => (router.canGoBack() ? router.back() : router.replace(`/stage/${id}`));
  ...
  if (questions.length === 0) {
    return (
      <View style={[s.center, { paddingTop: insets.top }]}>
        <Stack.Screen options={{ headerShown: false }} />
        <Text style={s.muted}>No quiz available for this stage yet.</Text>
      </View>
    );
  }
  ...
  if (done) {
    return (
      <View style={s.root}>
        <Stack.Screen options={{ headerShown: false }} />
        <LinearGradient colors={HERO_COLORS} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
        <View style={[s.glow, s.glowCyan]} />
        <View style={[s.glow, s.glowViolet]} />
        <ScrollView contentContainerStyle={[s.doneWrap, { paddingTop: insets.top + 40 }]}>
          ...
          {isHalfClear ? (
            <View style={s.halfCard}>
              ...
              <Text style={s.muted}>Capture the flag in the CTF to fully clear this stage and earn its 🪙.</Text>
            </View>
          ) : (
            <View style={s.xpCard}>
          ...
          <Pressable onPress={() => router.replace("/(tabs)")} style={{ alignSelf: "stretch" }}>
            <LinearGradient colors={["#22d3ee", "#818cf8"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.primaryBtn}>
              <Text style={s.primaryText}>Back to Stages →</Text>
            </LinearGradient>
          </Pressable>
        </ScrollView>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        >
          ...
          <View style={s.heroTopRow}>
            <Pressable onPress={back} hitSlop={10}><Text style={s.back}>‹ Briefing</Text></Pressable>
          ...
          <Text style={s.heroSub}>Stage {stage.order} · Quiz</Text>
...
const s = StyleSheet.create({
  ...
  hero: { paddingHorizontal: 16, paddingBottom: 18, overflow: "hidden", borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.06)" },
  glow: { position: "absolute", borderRadius: 999 },
  glowCyan: { top: -90, right: -50, width: 240, height: 240, backgroundColor: "rgba(34,211,238,0.10)" },
  glowViolet: { bottom: -100, left: -40, width: 220, height: 220, backgroundColor: "rgba(167,139,250,0.10)" },
  ...
  bar: { backgroundColor: "rgba(255,255,255,0.1)", height: 5, borderRadius: 999 },
  ...
  secondaryBtn: { borderColor: "#39424f", borderWidth: 1, borderRadius: 12, paddingVertical: 14, alignItems: "center", marginTop: 12, alignSelf: "stretch" },
