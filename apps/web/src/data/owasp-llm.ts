...
class RateLimitedLLMClient:
  ...
  // ─── llm-11: LLM Red Teaming ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2024,
      overview: [
        "LLM red teaming is systematic adversarial testing to find failures before deployment:\n- It hunts safety failures, jailbreaks, harmful output modes, and security vulnerabilities.\n- Major labs (Anthropic, OpenAI, Google DeepMind, Meta) run dedicated teams pairing security expertise with AI-safety research.",
        "It spans two distinct goals:\n- Safety red teaming — jailbreaks, prompt injection, harmful-content elicitation, and bias.\n- Security red teaming — the OWASP LLM Top 10 vulnerabilities in the deployed application stack.",
        "Automated AI-against-AI red teaming is scaling fast:\n- Garak (an LLM vulnerability scanner) and PAIR (Prompt Automatic Iterative Refinement) use LLMs to discover jailbreaks against other LLMs.\n- That explores the attack surface far faster than manual red teaming.",
      ],
      technical: {
...
class CustomInjectionProbe(Probe):
  ...
  // ─── llm-12: LLM Security Program ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2025,
      overview: [
        "A complete LLM security program is defense-in-depth across every layer of the AI stack:\n- Model selection and supply-chain security at the bottom.\n- Application controls (input/output validation, rate limiting, least-privilege agents), then runtime monitoring and incident response.",
        "NIST AI RMF supplies the governance structure:\n- Four functions — GOVERN (policies, roles, accountability), MAP (risk identification), MEASURE (risk metrics), MANAGE (controls, response, recovery).\n- The OWASP LLM Top 10 maps onto the MEASURE and MANAGE functions.",
        "Production monitoring must watch two threat classes at once:\n- Traditional security incidents — prompt-injection attempts, abnormal API usage, context-based exfiltration.\n- AI-specific failures — rising hallucination rates, jailbreak attempts, and model drift — needing tooling beyond classic SIEM and WAF.",
      ],
      technical: {
