...
export const techAudit1Stages: StageConfig[] = [
  // ─── audit-01: COBIT 2019 ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2019,
      overview: [
        "COBIT 2019 is ISACA's flagship framework for IT governance and management, underpinning almost every IT audit methodology in regulated industries:\n- It draws a precise, legally significant line between governance — which sets direction, evaluates options, and monitors outcomes — and management, which plans, builds, runs, and monitors in line with that direction.\n- The distinction isn't academic: regulators and courts have assigned personal liability to board members who abdicated governance and relied entirely on management self-reporting.",
        "Every audit starts by establishing that boundary, then tracing how intent flows through it:\n- The board and executives govern — approving risk appetite, setting strategy, authorizing major IT investments, and ensuring accountability.\n- IT leadership manages — implementing controls, operating systems, and reporting performance upward within board-approved parameters.\n- Auditors hold an independent third position, verifying both layers and reporting to the audit committee, not management, to preserve independence.\n- The governance cascade runs board principles → management policies → operational procedures → technical controls; auditors trace it in reverse, and a control that can't be traced up to policy authority (or a policy never implemented as a control) is a governance gap.",
        "The framework organizes 40 objectives across five domains, tailored by design factors:\n- EDM (Evaluate, Direct, Monitor) — board governance: risk appetite, IT investment, performance monitoring.\n- APO (Align, Plan, Organise) — strategy, architecture, risk, and HR planning; BAI (Build, Acquire, Implement) — projects, change, requirements; DSS (Deliver, Service, Support) — operations, incidents, continuity; MEA (Monitor, Evaluate, Assess) — internal controls, compliance, board reporting.\n- The twelve design factors (enterprise strategy, risk profile, compliance requirements, sourcing model, IT methods, and more) tailor the framework to context — a regulated bank running its own data center has a different target profile than a cloud-native startup, so auditors establish the applicable factors before assessing capability.",
        "Two practical points dominate real engagements:\n- COBIT is a governance framework, not a technical standard — it prescribes no implementations but maps to NIST CSF, ISO 27001, ITIL, and TOGAF, so auditors routinely find strong NIST/ISO technical controls that aren't connected to board-level accountability.\n- The most common findings are in EDM: EDM01 (no formal governance charter), EDM03 (no approved risk-appetite statement, so risk decisions are made ad hoc without board authority), and EDM05 (IT reporting to the board absent or all technical metrics the board can't evaluate) — together a pervasive governance failure that undermines every other control.",
      ],
      technical: {
...
APO12 Risk Mgmt      2        4      2    HIGH`,
  ...
  // ─── audit-02: Risk Assessment — CRISC ────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2010,
      overview: [
        "CRISC (Certified in Risk and Information Systems Control) is ISACA's risk-focused certification and methodology:\n- It defines a four-domain approach: IT Risk Identification, IT Risk Assessment, Risk Response and Mitigation, and Risk and Control Monitoring and Reporting.\n- Unlike compliance frameworks asking 'did we implement the required control?', CRISC asks the prior question: 'what risks are the controls meant to address, and are they actually reducing those risks to acceptable levels?'",
        "The first two domains define what to manage and how exposed you are:\n- Risk identification is the most under-resourced domain — registers populated in one workshop years ago go stale, so CRISC requires continuous identification as new systems, vendors, regulations, and threats appear; auditors test it by comparing the register against change logs, vendor onboardings, threat intel, and incident databases for risks that should be there but aren't.\n- Risk assessment runs a standard method: identify threats and vulnerabilities, score inherent risk (before controls), evaluate control effectiveness, calculate residual risk (after controls), and compare residual to board-approved appetite — and the inherent-to-residual gap is the control effectiveness, which must be large enough to bring residual within appetite.",
        "Two artifacts anchor the rest of the work:\n- The four risk responses — Accept (board formally documents and absorbs the loss; undocumented acceptance is invalid), Mitigate (add or strengthen controls), Transfer (shift financial exposure via insurance, indemnification, or outsourcing without eliminating the risk), and Avoid (discontinue the activity — most complete, often impractical).\n- The risk register, which auditors scrutinize most: a mature one carries a unique ID, threat/vulnerability narrative, affected asset, a named risk owner, inherent score (likelihood × impact), controls with references, control-effectiveness rating, residual score, response type and treatment plan, the appetite threshold, and last-review date — any missing field is itself a finding, because the organization then can't demonstrate it's managing the risk.",
        "Two things determine whether the program is real:\n- Risk appetite must be board-approved and quantitative — 'we have a low appetite for cybersecurity' gives no threshold, whereas 'no residual score above 12 on the 5×5 matrix without a board-approved exception; scores of 16+ escalate immediately to the Audit Committee' is usable.\n- The most consequential finding is residual risk above appetite with no treatment plan — the organization knows it's over its own threshold and is doing nothing; regulators treat this as management negligence (FDIC/OCC issue Matters Requiring Attention, and external auditors may call it a significant deficiency or material weakness under SOX 404).",
      ],
      technical: {
...
R-004    Ransomware via RDP        4       5      20       None       20     CRITICAL — NO CONTROLS`,
  ...
  // ─── audit-03: ITGC ────────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2002,
      overview: [
        "IT General Controls (ITGCs) are the foundational controls beneath all application-level controls — not tied to any one app but applying to the whole IT environment:\n- The analogy is a building's foundation and floors: a sound floor on a cracked foundation is still unsafe.\n- If ITGCs fail, every application control built on them is suspect, and auditors can't rely on application output without far more compensating work.\n- The four domains are Change Management (production changes follow an approved, documented process), Logical Access (only authorized users reach systems and data at appropriate privilege), Computer Operations (reliable running, backups performed and tested, jobs completing), and Program Development (new systems developed, tested, and approved via a defined SDLC) — and all four appear in every SOX 404 audit, PCAOB inspection, and financial-services exam.",
        "ITGC effectiveness governs how much an auditor can trust everything else:\n- A clean ITGC environment lets auditors rely on system-generated reports and automated application controls with minimal extra testing; a failure — especially in change management or logical access — forces substantial expansion, because they can no longer assume system output reflects what the system should produce.\n- Findings are classified by pervasiveness: an isolated deficiency (a single unauthorized payroll change) is serious, but a pervasive one (a change process that routinely bypasses approval) casts doubt across systems and periods and typically rises to a significant deficiency or material weakness under SOX, requiring disclosure.",
        "Two structural facts shape every engagement:\n- SOX Section 404 makes management assess and attest to ICFR effectiveness (including supporting ITGCs) while PCAOB AS 2201 makes external auditors verify that assessment — a two-layer accountability where the CEO and CFO personally sign the Section 302 certification, so ITGC failures create personal legal exposure.\n- Scoping identifies which systems are 'in scope' — those processing, storing, or transmitting data that flows into financial statements (general and subsidiary ledgers, payroll, revenue recognition, consolidation tools) plus their supporting OS, database, network, and identity platforms, and in the cloud the platform configuration and shared-responsibility controls.",
        "A recurring set of findings appears across industries, each a breakdown that could allow unauthorized changes or access to financial data:\n- Terminated employees with active accounts and shared privileged accounts with no individual accountability (logical access).\n- Developers with production access (change-management segregation of duties) and production changes with no approved tickets (change management).\n- Backups not tested for restorability (computer operations) and new systems deployed without security-testing evidence (program development).",
      ],
      technical: {
...
# Output: jsmith terminated 2026-03-15 — still has ACTIVE AD account — FINDING`,
  ...
  // ─── audit-04: Access Control ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2013,
      overview: [
        "Access control auditing verifies that least privilege holds across the environment — every user (employee, contractor, or service account) has exactly the permissions their documented job needs, and no more:\n- It sounds simple but is hard to maintain: systems accumulate users, people change roles, contractors finish engagements, and business units reorganize.\n- Through all of it, access granted for a former purpose persists unless an active process removes it — that accumulation, permission creep, is what access audits exist to find and remediate.",
        "The control objectives come from COBIT APO13 and DSS05, aligned with the NIST SP 800-53 Access Control (AC) family:\n- Provisioning must follow an approved request with documented business justification.\n- Access must be reviewed and recertified at least annually, with managers confirming each person's access is still appropriate.\n- Terminated employees must be deprovisioned within 24 hours for standard accounts and immediately (within ~4 hours) for privileged ones.\n- Privileged access must be minimized, documented with justification, protected with MFA, and subject to enhanced monitoring.",
        "Access findings are the most common in IT audit, and the most security-critical concern privileged access:\n- They span administrative (missing recertification evidence) to critical (terminated employees with active privileged accounts), reflecting a coordination problem across HR, IT, and business management — so auditors test the process, not just the current list, since a clean list from a broken process will fail again.\n- Privileged Access Management (PAM) covers the accounts that bypass most application controls (domain and database admins, root, elevated service accounts, broad cloud IAM roles): controls include just-in-time access that expires automatically, dedicated privileged access workstations (PAWs), session recording, and regular review of privileged rights against job requirements.",
        "Three models and account types complete the picture:\n- RBAC attaches rights to roles, not individuals, so assigning 'Accounts Payable Clerk' configures access automatically — simplifying provisioning, deprovisioning, and recertification; auditors verify role definitions are approved, assignments match job descriptions, and no user holds conflicting roles.\n- Segregation of duties (SoD) conflicts arise when one user spans incompatible functions (create vendor + approve payments, initiate + approve wire transfers, post + approve journal entries), creating fraud opportunity, so auditors test access against a conflict matrix.\n- Service accounts — non-human identities for apps and jobs — often hold elevated, shared, embedded credentials no human reviews daily, so auditors inventory them and confirm a documented owner, scheduled credential rotation, disabled interactive logon where unneeded, and tightly limited permissions.",
      ],
      technical: {
...
-- [term]     WRITE    2025-03-01   [TERM]       ← TERMINATED USER - CRITICAL`,
  ...
  // ─── audit-05: Change Management ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1986,
      overview: [
        "Change management ensures every modification to production goes through a documented, approved, and tested process before it ships:\n- It isn't bureaucracy — it's the control that prevents accidental outages, unauthorized code deployment, and security vulnerabilities entering production.\n- ITIL 4 defines three change types by risk: Standard (pre-approved low-risk routine changes), Normal (requiring Change Advisory Board review), and Emergency (expedited approval for critical fixes, with mandatory post-hoc documentation).",
        "The Change Advisory Board (CAB) reviews and approves Normal changes, and auditors test against what it should enforce:\n- The CAB is cross-functional (IT management, security, operations, application owners, business stakeholders), assessing risk, confirming testing is complete, verifying an executable rollback plan and an appropriate maintenance window, and formally authorizing — an effective CAB challenges assumptions and rejects unacceptable risk rather than rubber-stamping.\n- For each sampled change, auditors verify five elements: an approved ticket opened before the change (not backdated), documented CAB approval with name and date, developer and approver are different people (segregation of duties), test results attached from a non-production environment, and a documented, feasible rollback plan.",
        "Two process safeguards are common failure points:\n- The emergency path is a necessary safety valve but frequently abused — genuine emergencies (an actively exploited zero-day, a production outage) justify it, but an emergency rate above 10–15% triggers investigation into whether developers, or rubber-stamping managers, are routing around CAB review under the guise of urgency.\n- Maintenance windows are the approved low-traffic deployment periods (overnight, weekends) when failure impact is minimized and staff are watching; a change deployed outside the window without emergency authorization is a finding regardless of its technical content, and auditors flag it by comparing deployment timestamps to the window schedule.",
        "Two modern concerns close the loop:\n- DevOps and CI/CD strain the human-review model, so ITIL 4 introduces the 'change authority,' which can be an automated system for low-risk, well-tested changes — and auditors test whether the pipeline itself is sufficient control (enforced automated testing, required passing security scans, an immutable deployment audit log, and a halt-on-failure mechanism); a well-configured pipeline can be more reliable than a manual CAB, but only if its configuration is itself audited.\n- Post-implementation review (PIR) documents whether the change met its purpose, any side effects, lessons, and follow-ups — auditors verify PIRs were completed on time (typically 5 business days) and issues tracked to resolution, and PIRs that consistently surface unexpected side effects reveal a deeper problem: test environments that don't represent production.",
      ],
      technical: {
...
-- CHG-1003  jsmith    jsmith      2026-02-05  SOD VIOLATION  ← FINDING`,
  ...
  // ─── audit-06: BCP/DR ──────────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2005,
      overview: [
        "Business Continuity Planning (BCP) and Disaster Recovery (DR) audits verify an organization can keep critical operations running and recover IT systems after a disruption — natural disaster, ransomware, data center fire, or vendor failure:\n- CISA dedicates substantial coverage because these plans are systematically neglected — created during compliance exercises, filed in shared drives, and never exercised.\n- An untested recovery plan isn't a control; it's documentation of intent, and intent has never restored a database.",
        "Two foundations define what recovery means and where it applies:\n- The metrics — Recovery Time Objective (RTO, max tolerable downtime before unacceptable consequence), Recovery Point Objective (RPO, max acceptable data loss measured in time), and Maximum Tolerable Downtime (MTD, the point of irreversible harm); the fundamental rule is RTO must be less than MTD, or the plan literally can't save the business.\n- The Business Impact Analysis (BIA), which auditors review first — it maps each critical process, its supporting systems, the impact of downtime, and the resulting RTO/RPO; a BIA not updated since a major migration, cloud adoption, or acquisition is stale, so auditors test currency by comparing its systems against the current production inventory.",
        "Strategy and testing must demonstrably meet the approved targets:\n- Recovery strategies range from hot standby (a fully operational duplicate, near-zero RTO/RPO but expensive) through warm and cold standby (cold is cheap but RTOs in days) to on-demand cloud failover — and auditors verify the chosen strategy has demonstrated the approved RTO/RPO in testing, not just theoretically.\n- Both tabletop and full DR tests are required: a tabletop is a discussion-based walkthrough that tests logical structure, roles, and decisions without disruption, while a full DR test actually fails over to the recovery environment and verifies RTOs/RPOs in practice — tabletop-only tests the document, technical-only tests the technology but not the human coordination.",
        "Two modern requirements reshape the audit:\n- Ransomware broke the old assumption that backups survive: attacks encrypt production, backups, cloud-connected repositories, and anything reachable from the compromised network, so audits now test for air-gapped or immutable backups, the 3-2-1 rule (three copies, two media types, one offsite), tested restoration from an isolated backup disconnected before the attack, and a ransomware-specific tabletop that includes backup compromise.\n- Geographic separation is non-negotiable, as Katrina showed: primary and backup sites must be outside the same flood zone and far enough apart to survive a regional disaster (FDIC/OCC guidance generally requires ~100 miles), and auditors check location documentation for shared dependencies (power grid segment, ISP backbone, seismic fault).",
      ],
      technical: {
...
- Customer Portal: not tested in 18 months — CRITICAL FINDING`,
  ...
  // ─── audit-07: Incident Response ──────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2012,
      overview: [
        "Incident Response (IR) program audits assess whether an organization can detect, contain, eradicate, and recover from security incidents — consistently, at scale, under the time pressure and confusion real incidents create:\n- ISACA auditors use NIST SP 800-61 as the framework, with four phases — Preparation, Detection and Analysis, Containment/Eradication/Recovery, and Post-Incident Activity — all evaluated, since a program that detects well but can't contain, or contains but learns nothing, isn't mature.\n- Preparation is the most auditable and most often deficient: a current IR plan (reviewed within 12 months), named individuals (not job titles) in roles, contact lists covering external parties (legal, law enforcement, forensics, cyber insurer, PR), a severity matrix defining P1 versus P4, notification protocols per severity, and tabletop exercises within 12 months with action items tracked to closure.",
        "Two forces shape the program beyond preparation:\n- Detection and Analysis — auditors review the SIEM for log-source coverage of critical systems, alert rules tuned to the top threats, and managed false-positive rates (a SIEM firing thousands of daily alerts trains analysts to ignore them, worse than none), then sample historical incidents to see how each was detected, since external notification (law enforcement, a third party) signals internal detection is insufficient.\n- The SEC's 2023 disclosure rules — material incidents must be disclosed within four business days and the risk-management program described, making IR a financial-reporting matter, so auditors at public companies assess whether the materiality determination is documented, the escalation from security to legal and finance is clear, and the four-day timeline is operationally achievable.",
        "Two operational measures define maturity:\n- MTTD (time from attacker activity to detection) and MTTR (time from detection to containment) — the IBM Cost of a Data Breach report shows breaches contained within 30 days cost far less, financial-services regulators expect high-severity MTTD in hours not days, and an organization with a 78-day MTTD like Equifax has a fundamental failure regardless of its documentation.\n- Playbooks — step-by-step procedures for the top incident types (ransomware, data breach/exfiltration, insider threat, phishing with credential compromise, DDoS), each specifying detection indicators, triage, containment, eradication, recovery, communication, and evidence-preservation steps; auditors test them against those criteria and against whether IR members can actually execute them under tabletop conditions.",
        "Log retention is the evidentiary foundation of response:\n- On detection, the IR team must answer when the attacker entered, what systems they touched, and what data they accessed or exfiltrated — which requires logs reaching back across the full dwell period, typically 90 days online (immediately searchable) and 12 months archived.\n- Equifax's detection failure was partly a retention problem: an expired SSL inspection certificate meant the exfiltration traffic wasn't logged in a detectable form — and even with adequate retention, unreviewed logs or gaps from misconfigured shipping aren't reliable evidence, so auditors verify all in-scope systems ship logs to the SIEM with no delivery gaps.",
      ],
      technical: {
...
-- Finding: Detection time exceeds benchmark — SIEM tuning required`,
  ...
  // ─── audit-08: Vendor Risk ─────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2020,
      overview: [
        "Vendor and third-party risk management (TPRM) audits verify the organization oversees every vendor, supplier, and partner that accesses its systems, processes its data, or runs part of its IT:\n- ISACA places this under COBIT's APO10 (Managed Vendors), and it is one of the fastest-growing areas of regulatory scrutiny worldwide.\n- The premise is simple but operationally complex: your security posture is the product of your controls and your vendors' controls, so a breach of your vendor is a breach of you — your data leaves through their door.\n- The first lifecycle stage is inventory: knowing every vendor including shadow IT (SaaS bought on a credit card outside procurement) and subcontractors of primary vendors, since organizations consistently undercount and the shadow IT count alone can exceed the managed count — tested against accounts-payable records, asset inventories, network traffic, and user-reported SaaS.",
        "The next two stages tier vendors and test their controls:\n- Classification — risk-tier by data sensitivity, system criticality, dependency (how fast can the vendor be replaced?), the vendor's own regulatory status, and incident history: Tier 1 (Critical) is existential, Tier 2 (High) processes business data or has connectivity, Tier 3 (Low) has no system access.\n- Assessment — match rigor to tier: Tier 1 needs a questionnaire (such as the SIG, Standardized Information Gathering), the vendor's SOC 2 Type II report (independent attestation of controls over time), penetration-test and vulnerability-scan results, and on-site review for the highest risk; questionnaire-only is insufficient for Tier 1 because it rests entirely on vendor self-reporting.",
        "The last two stages bind the relationship and watch it between assessments:\n- Contractual requirements — verify security obligations, data protection (encryption, access controls, handling), a breach-notification timeline (GDPR's 72 hours; many require 24), the right to audit, data return/destruction at termination, and subcontractor-approval restrictions.\n- Ongoing monitoring — for Tier 1, continuous security-ratings monitoring (BitSight, SecurityScorecard, RiskRecon), tracking of breaches/regulatory actions/leadership changes, quarterly vendor-access reviews, and vendor security metrics; Tier 2 gets annual reassessment plus significant-event tracking, and Tier 3 may be limited to contract-renewal review.",
        "Fourth-party risk — the risk from your vendor's vendors — is the hardest dimension and the one supply-chain attacks most often exploit:\n- SolarWinds succeeded partly because affected organizations never mapped that their Orion instance had broad network visibility, nor assessed what compromise of Orion itself would do.\n- Managing it requires understanding each Tier 1 vendor's key dependencies — which cloud providers, which software libraries, which subcontractors handle your data — and organizations that haven't answered these questions have a gap attackers have shown they will exploit.",
      ],
      technical: {
...
ORDER BY tier, days_since_assessment DESC;`,
  ...
  // ─── audit-09: Data Privacy ────────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2018,
      overview: [
        "Data privacy audits assess whether an organization collects, processes, retains, and deletes personal data in line with applicable privacy law — primarily GDPR (Europe) and CCPA (California), plus sector rules like HIPAA, GLBA, and COPPA:\n- ISACA's privacy framework maps to GDPR's six lawful bases and eight data-subject rights, giving auditors a methodology that crosses jurisdictions.\n- The stakes are mainstream, not niche: GDPR's maximum fine is €20 million or 4% of global annual turnover, whichever is higher.\n- A comprehensive audit covers seven domains, starting with data mapping (what personal data is held, where, how it flows, who has access, and the documented legal basis) — without it no other control can be designed or tested.",
        "The other domains and the findings they generate are tightly linked:\n- Consent mechanisms must be testable; data-subject-rights fulfillment (deletion, access, portability, rectification, objection) must work reliably at scale within the required timeframes; and retention/deletion schedules must be documented, implemented in systems, and enforced automatically or by regular review.\n- The most common findings track the gap between policy and reality: personal data retained past its documented period (schedules exist but automated deletion doesn't), processing with no documented Article 6 lawful basis (new analytics, marketing automation, or data-sharing shipped without a privacy assessment), DSARs missed beyond the 30-day window (manual, untrained processes), and missing DPIAs for high-risk processing (large-scale profiling, systematic monitoring, sensitive categories).",
        "Two structural elements get tested directly:\n- The Data Protection Officer (DPO) is required for large-scale processing of special categories, large-scale systematic monitoring, or public authorities — auditors verify the role exists where required, has genuine independence (not reporting to those whose processing it oversees), has sufficient resources and expertise, and is involved in all data-protection matters, flagging conflicts when, say, the CISO is also DPO.\n- Data mapping, documented as the Record of Processing Activities (RoPA, GDPR Article 30), must list the controller and DPO contacts, processing purposes, categories of data subjects and personal data, recipients (including international transfers), retention periods, and security measures — and must be current, so auditors test it against recent deployments, vendor onboardings, and IT data-flow diagrams to find flows that exist but aren't in the RoPA.",
        "Two forward-looking requirements round out the scope:\n- International transfers — moving personal data outside the EEA to non-adequacy countries requires Standard Contractual Clauses (SCCs, updated 2021), Binding Corporate Rules, or an approved certification scheme; the Schrems II invalidation of Privacy Shield (2020) made US-EU transfers complex, so auditors verify all EEA-to-US flows are identified, valid mechanisms are in place, and Transfer Impact Assessments (TIAs) are done where required.\n- Privacy by Design — privacy controls built in from the start, tested through the SDLC: is privacy review a mandatory pre-production checkpoint, are minimization/pseudonymization/encryption considered at design, is data collection opt-in by default, and are retention periods configured at deployment rather than applied after data accumulates? Organizations that keep discovering privacy violations in existing systems rather than preventing them have a Privacy by Design gap that recurs as findings.",
      ],
      technical: {
...
-- Retention schedule: 730 days (2 years)  → EXCEEDS BY 1,632 DAYS — FINDING`,
  ...
  // ─── audit-10: Pen Test Scoping ────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 2003,
      overview: [
        "Penetration testing is a critical control in any mature security program — a controlled adversarial simulation that validates whether defenses work in practice, not just on paper:\n- ISACA auditors review pentest programs to confirm they deliver genuine assurance rather than compliance theater, and that the authorization structure protects both the organization and the testers.\n- The line between a security professional and a criminal is defined entirely by the scope of written authorization — the same technical actions taken without it are federal crimes under the Computer Fraud and Abuse Act (CFAA) and equivalents elsewhere.",
        "Two documents make a test legal, and they must come from the right authority:\n- The scope document is the legal foundation — it names exactly which systems are in scope (IP ranges, domains, applications, environments), which testing types are permitted (external, internal, web app, social engineering, physical, wireless), the precise testing window and blackout periods, and escalation procedures for critical findings; out-of-scope systems must be listed explicitly, since prosecutors apply the most conservative reading of authorization.\n- Authorization must flow from the system owner (the business or legal entity responsible), not IT management alone when business units own the systems, and cloud workloads need the provider's approval too — AWS, Azure, and GCP each publish pentest policies defining what's allowed without notice and what needs advance approval.",
        "The next layer operationalizes scope and protects the resulting report:\n- The rules of engagement (ROE) define how testing is conducted where the scope defines what — authorized tools and methods, permitted versus prohibited techniques (DoS may be explicitly barred on production), data handling for discovered credentials, evidence collection and retention, the urgent-finding communication protocol, and what to do if the tester finds an active real intrusion (which happens more often than clients expect).\n- The test report is itself security-sensitive — a roadmap of working exploits and paths to critical findings — so distribution is restricted to authorized recipients (typically the CISO and a few IT leaders), transmitted securely (encrypted email or a secure portal), and stored in an access-controlled repository, never a general shared drive.",
        "Two practices determine whether the test produces lasting value:\n- Remediation tracking — a kickoff within five business days, a plan with owners and due dates by severity (critical within 30 days, high within 60, medium within 90), and a retest against the specific remediated vulnerabilities (not just a general rescan), since the retest is the evidence of effective remediation.\n- Red team engagements differ from pentests — a pentest has defined scope, methodology (find and report), and a two-to-four-week duration, while a red team simulates an APT toward a defined objective (reach the most sensitive data, simulate ransomware) using any TTPs including social engineering, physical intrusion, and supply chain, running for months and requiring extra authorization for human targets, facilities, and supplier relationships that auditors verify is explicitly granted.",
      ],
      technical: {
...
def is_authorized(target_ip):
  ...
  // ─── audit-11: Audit Evidence ──────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1972,
      overview: [
        "Audit evidence is the information auditors collect and evaluate to support conclusions about whether controls operate effectively:\n- ISACA requires three attributes: sufficient (enough of it — adequate sample size, period covered, population well-defined), reliable (from a credible source via a sound method, not manipulated), and relevant (it actually tests the control — evidence a backup ran doesn't test whether it's restorable); failure on any one invalidates the conclusion.\n- Sampling tests a representative subset when testing every item is impractical — for 1,200 change tickets, statistical sampling gives every item a known non-zero chance of selection and projects results with measurable precision, while judgmental (non-statistical) sampling relies on auditor expertise but can describe only the sample, not project to the population.",
        "Two foundations govern how evidence is gathered and trusted:\n- Attribute sampling, the most common statistical method in IT audit, tests a binary attribute (the control operated or it didn't) by defining the population (all Q1 2026 change tickets), the attribute (each ticket has CAB approval before deployment), the tolerable deviation rate (the max failures still allowing an 'effective' conclusion, typically 5–10%), the confidence level (95% is standard), and the expected deviation rate (prior estimate; assume 0% if unknown) — and statistical tables set the minimum sample size.\n- Chain of custody documents who collected each piece of evidence, when, from what source, by what method, and how it's been stored since — essential because workpapers may support legal proceedings, exams, or litigation where authenticity is challenged, so digital evidence adds hash values (MD5, SHA-256) at collection, authoritative timestamps, and access-recording storage.",
        "Two practices scale and standardize the work:\n- Computer-Assisted Audit Techniques (CAATs) automate evidence collection and analysis — data tools (ACL/Galvanize, IDEA, SQL), log platforms (Splunk, Elastic), configuration-comparison and reconciliation tools — letting auditors test whole populations rather than samples, run analyses infeasible by hand (comparing every payroll disbursement against current employees), and produce reproducible evidence (the same CAAT on the same data yields the same result).\n- Workpaper documentation standards require, per piece of evidence, the test objective, population and sample definition, method, evidence examined with source references, results including deviations, and a conclusion — and the test of adequacy is that another auditor with no prior knowledge could understand exactly what was tested, how, and what was found, so a workpaper requiring oral explanation is not adequately documented.",
        "An evidence-quality hierarchy guides how much weight to place on each type:\n- Physical evidence — the auditor personally observes the control operating (watching a CAB meeting, observing a restoration, extracting data directly) — is highest, since it can't be fabricated after the fact.\n- Documentary evidence — logs, reports, screenshots, signed documents, system extracts — is second, reliable when pulled directly from the source system rather than from management.\n- Testimonial evidence — representations made in interviews — is lowest, relying on the speaker's truthfulness, so auditors must be skeptical of management claims unsupported by physical or documentary evidence, especially for high-risk controls.",
      ],
      technical: {
...
if sample_deviation_rate > 0.05:  # exceeds 5% tolerable rate
  ...
  // ─── audit-12: Audit Reporting ─────────────────────────────────────────────
  {
    ...
    info: {
      ...
      year: 1978,
      overview: [
        "The audit report is the formal deliverable that turns weeks of testing, evidence, and analysis into actionable information for leadership, the audit committee, and the board:\n- ISACA standards (IS Audit Standards 1401-1405) require it to include the audit scope and objectives, the period covered, the methodology, findings with supporting evidence and risk ratings, management responses to each finding, and agreed remediation timelines.\n- It's not a list of problems but a communication document whose quality is measured by whether it produces effective remediation — and each finding must use the Condition-Criteria-Cause-Effect framework: Condition (what was found: specific, measurable, objective), Criteria (what the standard or policy requires), Cause (the root cause, not the symptom), and Effect (the business risk if unremediated), since omitting any element leaves management unable to remediate and the board unable to assess risk.",
        "Two elements determine whether findings drive action:\n- Risk rating must be consistent and defensible — Critical/High/Medium/Low based on likelihood, impact magnitude (financial, regulatory, reputational, operational), breadth of systems affected, and any compensating controls; Critical findings escalate immediately to the audit committee (no waiting for the 30-day cycle), High remediate in 30–60 days, Medium in 90 days, and Low may be accepted with documentation.\n- Management responses must substantively engage, not defend: 'we disagree' without addressing the condition's factual basis is inadequate, and 'we will remediate by [date]' without specific actions isn't actionable — a good response acknowledges accuracy (or disputes it specifically), identifies the root cause, lists remediation actions with owners and dates, and notes interim compensating controls; auditors review responses before finalizing and may request revision.",
        "Two operational controls protect and close out the report:\n- Distribution is a security requirement, since the report details control weaknesses valuable to attackers — restrict it to those with a legitimate need (audit committee, board for significant/material findings, CISO, CIO, process owners), transmit via encrypted email or a secure portal rather than standard email, store it access-controlled and tracked, and route external requests (regulators, external auditors) through legal counsel.\n- Remediation tracking closes the loop many organizations leave open — a centralized system recording every open finding with rating, agreed date, owner, and status, automated escalation as due dates approach, a 30/60/90-day follow-up verifying progress with evidence, and a formal closure requiring both management's attestation and audit-team verification before a finding is marked closed.",
        "The audit committee relationship is the governance structure that gives internal audit its independence and authority:\n- The audit committee — independent board members — is internal audit's primary client, not management, and the Chief Audit Executive reports to it rather than to the CEO or CFO, so management can't suppress or delay reports about their own weaknesses.\n- Disputed findings are adjudicated by the audit committee (the auditor presents evidence, management presents its position), and when findings linger unresolved or management repeatedly fails to remediate, the committee can escalate — requiring independent review, engaging external auditors, or taking disciplinary action.",
      ],
      technical: {
