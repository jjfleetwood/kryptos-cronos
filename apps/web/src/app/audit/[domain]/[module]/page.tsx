...
export default async function AuditModulePage({
  ...
  // Strip secrets before handing the stage to the client container.
  ...
  const backHref = `/audit/${domain}`;

  return (
    <div style={{ background: "linear-gradient(135deg, #0d1117 0%, #160f24 50%, #1a1a2e 100%)" }}>
      <div className="max-w-4xl mx-auto px-4 pt-12">
        <Link href={backHref} className="text-violet-300 hover:text-violet-200 text-sm transition-colors">
          ← {epoch.name}
        </Link>
        ...
        {/* Scores + the smaller scoring-rationale box */}
        ...
        {downloads.length > 0 && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <p className="mt-1.5 text-xs text-gray-500">
              Real, runnable read-only MCP tooling for this sub-process. Reviewed below in the CTF + quiz.
            </p>
        ...
        <div className="mt-8 mb-2 text-center text-xs font-semibold uppercase tracking-widest text-violet-300/70">
          ↓ Briefing · workflow diagram · CTF + 10-question quiz ↓
        </div>
      </div>
