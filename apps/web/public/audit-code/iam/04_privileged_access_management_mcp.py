#!/usr/bin/env python3
"""Read-only MCP server — Identity & Access Management (IAM): "Privileged access management" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_privileged_access_management_mcp.py                 # expose to an agent
  python 04_privileged_access_management_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Privileged access management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IAM Privileged access management Audit (read-only)")
