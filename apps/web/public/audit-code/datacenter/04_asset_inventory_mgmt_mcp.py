#!/usr/bin/env python3
"""Read-only MCP server — Datacenter / Lab / CoLo: "Asset inventory mgmt" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_asset_inventory_mgmt_mcp.py                 # expose to an agent
  python 04_asset_inventory_mgmt_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Asset inventory mgmt" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DCR Asset inventory mgmt Audit (read-only)")
