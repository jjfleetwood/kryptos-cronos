#!/usr/bin/env python3
"""Read-only MCP server — Datacenter / Lab / CoLo: "Safety controls" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_safety_controls_mcp.py                 # expose to an agent
  python 03_safety_controls_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Safety controls" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DCR Safety controls Audit (read-only)")
