#!/usr/bin/env python3
"""Read-only MCP server — Datacenter / Lab / Colocation (CoLo): "Asset inventory management" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_asset_inventory_management_mcp.py                 # expose to an agent
  python 04_asset_inventory_management_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Asset inventory management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DCR Asset inventory management Audit (read-only)")
