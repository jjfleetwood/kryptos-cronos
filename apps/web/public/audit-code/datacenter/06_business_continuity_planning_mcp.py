#!/usr/bin/env python3
"""Read-only MCP server — Datacenter / Lab / CoLo: "Business continuity planning" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_business_continuity_planning_mcp.py                 # expose to an agent
  python 06_business_continuity_planning_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Business continuity planning" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DCR Business continuity planning Audit (read-only)")
