#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Container image security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_container_image_security_mcp.py                 # expose to an agent
  python 03_container_image_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Container image security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Container image security Audit (read-only)")
