#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "SBOM image provenance" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_sbom_image_provenance_mcp.py                 # expose to an agent
  python 13_sbom_image_provenance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "SBOM image provenance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD SBOM image provenance Audit (read-only)")
