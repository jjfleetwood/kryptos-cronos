#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Workload security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_workload_security_mcp.py                 # expose to an agent
  python 06_workload_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Workload security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Workload security Audit (read-only)")
