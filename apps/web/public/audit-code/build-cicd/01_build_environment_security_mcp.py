#!/usr/bin/env python3
"""Read-only MCP server — Build Environment & CI/CD (Continuous Integration / Continuous Delivery): "Build environment security" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    inventory runners: ephemeral vs persistent, self-hosted vs hosted, who can target them
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_build_environment_security_mcp.py                 # expose to an agent
  python 01_build_environment_security_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Build environment security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Build environment security Audit (read-only)")
