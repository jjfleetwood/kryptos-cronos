#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "License scanning" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 15_license_scanning_mcp.py                 # expose to an agent
  python 15_license_scanning_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "License scanning" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD License scanning Audit (read-only)")
