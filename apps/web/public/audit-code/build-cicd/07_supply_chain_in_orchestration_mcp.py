#!/usr/bin/env python3
"""Read-only MCP server — Build Environment & CI/CD (Continuous Integration / Continuous Delivery): "Supply chain in orchestration" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    Kyverno/Gatekeeper policy: enforce signed + approved-registry + provenance
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_supply_chain_in_orchestration_mcp.py                 # expose to an agent
  python 07_supply_chain_in_orchestration_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Supply chain in orchestration" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Supply chain in orchestration Audit (read-only)")
