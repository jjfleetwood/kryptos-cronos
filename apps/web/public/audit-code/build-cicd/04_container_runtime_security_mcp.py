#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Container runtime security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_container_runtime_security_mcp.py                 # expose to an agent
  python 04_container_runtime_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Container runtime security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Container runtime security Audit (read-only)")
