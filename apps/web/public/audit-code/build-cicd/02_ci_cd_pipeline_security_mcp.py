#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "CI/CD pipeline security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_ci_cd_pipeline_security_mcp.py                 # expose to an agent
  python 02_ci_cd_pipeline_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "CI/CD pipeline security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD CI/CD pipeline security Audit (read-only)")
