#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Orchestration cluster security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_orchestration_cluster_security_mcp.py                 # expose to an agent
  python 05_orchestration_cluster_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Orchestration cluster security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Orchestration cluster security Audit (read-only)")
