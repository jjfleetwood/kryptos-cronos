#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Audit logging" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_audit_logging_mcp.py                 # expose to an agent
  python 10_audit_logging_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Audit logging" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Audit logging Audit (read-only)")
