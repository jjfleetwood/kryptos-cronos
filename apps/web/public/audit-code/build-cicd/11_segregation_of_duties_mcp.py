#!/usr/bin/env python3
"""Read-only MCP server — Build Env / CI/CD: "Segregation of duties" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_segregation_of_duties_mcp.py                 # expose to an agent
  python 11_segregation_of_duties_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Segregation of duties" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD Segregation of duties Audit (read-only)")
