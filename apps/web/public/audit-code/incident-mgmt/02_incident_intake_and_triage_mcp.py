#!/usr/bin/env python3
"""Read-only MCP server — Incident Management: "Incident intake and triage" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    map intake channels → the monitored triage queue (alerts, user reports, vendor notices, threat intel)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_incident_intake_and_triage_mcp.py                 # expose to an agent
  python 02_incident_intake_and_triage_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident intake and triage" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC Incident intake and triage Audit (read-only)")
