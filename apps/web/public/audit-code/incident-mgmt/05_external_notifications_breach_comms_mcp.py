...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_external_notifications_breach_comms_mcp.py                 # expose to an agent
  python 05_external_notifications_breach_comms_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "External notifications / breach comms" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC External notifications / breach comms Audit (read-only)")
