...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_incident_escalation_mcp.py                 # expose to an agent
  python 07_incident_escalation_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident escalation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC Incident escalation Audit (read-only)")
