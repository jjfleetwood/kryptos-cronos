...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_incident_logging_and_monitoring_mcp.py                 # expose to an agent
  python 06_incident_logging_and_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident logging and monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC Incident logging and monitoring Audit (read-only)")
