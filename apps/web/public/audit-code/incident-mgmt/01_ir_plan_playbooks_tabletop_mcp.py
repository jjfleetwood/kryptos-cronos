...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_ir_plan_playbooks_tabletop_mcp.py                 # expose to an agent
  python 01_ir_plan_playbooks_tabletop_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "IR plan, playbooks, tabletop" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC IR plan, playbooks, tabletop Audit (read-only)")
