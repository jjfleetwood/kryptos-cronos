...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_evidence_handling_and_forensics_mcp.py                 # expose to an agent
  python 03_evidence_handling_and_forensics_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Evidence handling and forensics" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("INC Evidence handling and forensics Audit (read-only)")
