...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_dr_site_strategy_mcp.py                 # expose to an agent
  python 04_dr_site_strategy_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "DR site strategy" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES DR site strategy Audit (read-only)")
