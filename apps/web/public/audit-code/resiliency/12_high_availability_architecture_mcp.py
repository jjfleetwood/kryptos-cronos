#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "High-availability architecture" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_high_availability_architecture_mcp.py                 # expose to an agent
  python 12_high_availability_architecture_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "High-availability architecture" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES High-availability architecture Audit (read-only)")
