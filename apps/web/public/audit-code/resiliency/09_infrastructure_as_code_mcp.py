#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "Infrastructure as code" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_infrastructure_as_code_mcp.py                 # expose to an agent
  python 09_infrastructure_as_code_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Infrastructure as code" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Infrastructure as code Audit (read-only)")
