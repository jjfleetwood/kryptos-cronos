#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "Secret and certificate mgmt" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_secret_and_certificate_mgmt_mcp.py                 # expose to an agent
  python 11_secret_and_certificate_mgmt_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secret and certificate mgmt" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Secret and certificate mgmt Audit (read-only)")
