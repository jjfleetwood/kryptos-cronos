...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_data_and_service_replication_mcp.py                 # expose to an agent
  python 06_data_and_service_replication_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data and service replication" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Data and service replication Audit (read-only)")
