...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_vendor_lock_in_risk_mcp.py                 # expose to an agent
  python 10_vendor_lock_in_risk_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Vendor lock-in risk" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Vendor lock-in risk Audit (read-only)")
