...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_multi_region_az_deployment_mcp.py                 # expose to an agent
  python 08_multi_region_az_deployment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Multi-region / AZ deployment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Multi-region / AZ deployment Audit (read-only)")
