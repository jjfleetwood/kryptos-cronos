...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_tabletop_and_bcp_dr_testing_mcp.py                 # expose to an agent
  python 03_tabletop_and_bcp_dr_testing_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Tabletop and BCP/DR testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Tabletop and BCP/DR testing Audit (read-only)")
