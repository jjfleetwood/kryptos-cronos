#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "BCP, BIA, ITDR" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_bcp_bia_itdr_mcp.py                 # expose to an agent
  python 01_bcp_bia_itdr_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "BCP, BIA, ITDR" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES BCP, BIA, ITDR Audit (read-only)")
