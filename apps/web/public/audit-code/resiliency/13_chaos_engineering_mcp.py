...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_chaos_engineering_mcp.py                 # expose to an agent
  python 13_chaos_engineering_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Chaos engineering" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Chaos engineering Audit (read-only)")
