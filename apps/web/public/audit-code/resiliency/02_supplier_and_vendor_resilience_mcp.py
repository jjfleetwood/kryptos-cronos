...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_supplier_and_vendor_resilience_mcp.py                 # expose to an agent
  python 02_supplier_and_vendor_resilience_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Supplier and vendor resilience" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Supplier and vendor resilience Audit (read-only)")
