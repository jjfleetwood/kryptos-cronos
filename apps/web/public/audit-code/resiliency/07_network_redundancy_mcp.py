...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_network_redundancy_mcp.py                 # expose to an agent
  python 07_network_redundancy_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Network redundancy" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Network redundancy Audit (read-only)")
