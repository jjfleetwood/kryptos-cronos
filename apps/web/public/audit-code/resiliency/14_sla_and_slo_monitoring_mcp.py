#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "SLA and SLO monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 14_sla_and_slo_monitoring_mcp.py                 # expose to an agent
  python 14_sla_and_slo_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "SLA and SLO monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES SLA and SLO monitoring Audit (read-only)")
