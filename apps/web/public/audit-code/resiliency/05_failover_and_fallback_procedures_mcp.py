#!/usr/bin/env python3
"""Read-only MCP server — Resiliency & Redundancy: "Failover and fallback procedures" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_failover_and_fallback_procedures_mcp.py                 # expose to an agent
  python 05_failover_and_fallback_procedures_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Backup + replication platform, DR orchestration / runbooks, Multi-AZ/region infrastructure, BCP / BIA documentation.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Failover and fallback procedures" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RES Failover and fallback procedures Audit (read-only)")
