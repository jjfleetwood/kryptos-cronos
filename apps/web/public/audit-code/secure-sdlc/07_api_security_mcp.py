#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "API security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_api_security_mcp.py                 # expose to an agent
  python 07_api_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "API security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD API security Audit (read-only)")
