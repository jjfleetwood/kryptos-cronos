#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Post-quantum protection support" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_post_quantum_protection_support_mcp.py                 # expose to an agent
  python 12_post_quantum_protection_support_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Post-quantum protection support" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Post-quantum protection support Audit (read-only)")
