#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Container and cloud-native security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_container_and_cloud_native_security_mcp.py                 # expose to an agent
  python 08_container_and_cloud_native_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Container and cloud-native security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Container and cloud-native security Audit (read-only)")
