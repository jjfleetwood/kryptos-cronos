#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Infrastructure as code (IaC) security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_infrastructure_as_code_iac_security_mcp.py                 # expose to an agent
  python 06_infrastructure_as_code_iac_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Infrastructure as code (IaC) security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Infrastructure as code (IaC) security Audit (read-only)")
