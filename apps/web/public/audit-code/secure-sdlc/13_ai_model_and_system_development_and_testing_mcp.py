#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "AI model and system development and testing" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_ai_model_and_system_development_and_testing_mcp.py                 # expose to an agent
  python 13_ai_model_and_system_development_and_testing_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
SAST / DAST / SCA tooling, Issue tracker (Jira), CI security gates, Threat-model + design records.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI model and system development and testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD AI model and system development and testing Audit (read-only)")
