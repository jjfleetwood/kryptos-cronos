#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Software composition analysis" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_software_composition_analysis_mcp.py                 # expose to an agent
  python 04_software_composition_analysis_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Software composition analysis" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Software composition analysis Audit (read-only)")
