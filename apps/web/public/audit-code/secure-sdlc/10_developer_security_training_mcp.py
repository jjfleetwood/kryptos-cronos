#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Developer security training" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_developer_security_training_mcp.py                 # expose to an agent
  python 10_developer_security_training_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Developer security training" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Developer security training Audit (read-only)")
