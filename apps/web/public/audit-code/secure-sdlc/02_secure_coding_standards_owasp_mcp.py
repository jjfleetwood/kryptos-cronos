#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Secure coding standards (OWASP)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_secure_coding_standards_owasp_mcp.py                 # expose to an agent
  python 02_secure_coding_standards_owasp_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secure coding standards (OWASP)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Secure coding standards (OWASP) Audit (read-only)")
