#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Open source security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_open_source_security_mcp.py                 # expose to an agent
  python 05_open_source_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Open source security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Open source security Audit (read-only)")
