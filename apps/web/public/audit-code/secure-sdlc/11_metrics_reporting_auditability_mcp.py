#!/usr/bin/env python3
"""Read-only MCP server — Secure Software Dev: "Metrics, reporting, auditability" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_metrics_reporting_auditability_mcp.py                 # expose to an agent
  python 11_metrics_reporting_auditability_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Metrics, reporting, auditability" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SSD Metrics, reporting, auditability Audit (read-only)")
