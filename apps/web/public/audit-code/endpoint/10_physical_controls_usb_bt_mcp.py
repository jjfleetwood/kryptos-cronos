...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_physical_controls_usb_bt_mcp.py                 # expose to an agent
  python 10_physical_controls_usb_bt_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Physical controls (USB, BT)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Physical controls (USB, BT) Audit (read-only)")
