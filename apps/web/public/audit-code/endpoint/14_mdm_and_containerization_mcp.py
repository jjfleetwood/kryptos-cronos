...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 14_mdm_and_containerization_mcp.py                 # expose to an agent
  python 14_mdm_and_containerization_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "MDM and containerization" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT MDM and containerization Audit (read-only)")
