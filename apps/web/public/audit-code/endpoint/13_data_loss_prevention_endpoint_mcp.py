...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_data_loss_prevention_endpoint_mcp.py                 # expose to an agent
  python 13_data_loss_prevention_endpoint_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data loss prevention (endpoint)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Data loss prevention (endpoint) Audit (read-only)")
