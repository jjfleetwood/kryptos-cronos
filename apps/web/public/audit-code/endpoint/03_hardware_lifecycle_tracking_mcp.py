...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_hardware_lifecycle_tracking_mcp.py                 # expose to an agent
  python 03_hardware_lifecycle_tracking_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Hardware lifecycle tracking" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Hardware lifecycle tracking Audit (read-only)")
