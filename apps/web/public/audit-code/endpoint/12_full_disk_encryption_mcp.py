...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_full_disk_encryption_mcp.py                 # expose to an agent
  python 12_full_disk_encryption_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Full disk encryption" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Full disk encryption Audit (read-only)")
