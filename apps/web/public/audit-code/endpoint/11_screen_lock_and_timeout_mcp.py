...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_screen_lock_and_timeout_mcp.py                 # expose to an agent
  python 11_screen_lock_and_timeout_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Screen lock and timeout" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Screen lock and timeout Audit (read-only)")
