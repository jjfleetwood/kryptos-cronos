...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_end_point_inventory_and_unauthorized_devices_mcp.py                 # expose to an agent
  python 02_end_point_inventory_and_unauthorized_devices_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
MDM / UEM (Intune / Jamf), EDR (CrowdStrike / Defender / SentinelOne), Disk-encryption manager (BitLocker/FileVault), Endpoint inventory / NAC.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "End point inventory and unauthorized devices" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT End point inventory and unauthorized devices Audit (read-only)")
