...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_device_image_version_mgmt_mcp.py                 # expose to an agent
  python 04_device_image_version_mgmt_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Device image version mgmt" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Device image version mgmt Audit (read-only)")
