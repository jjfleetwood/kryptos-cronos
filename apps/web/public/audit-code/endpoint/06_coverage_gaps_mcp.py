#!/usr/bin/env python3
"""Read-only MCP server — Endpoint Devices: "Coverage gaps" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_coverage_gaps_mcp.py                 # expose to an agent
  python 06_coverage_gaps_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Coverage gaps" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Coverage gaps Audit (read-only)")
