...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_device_management_platform_mcp.py                 # expose to an agent
  python 01_device_management_platform_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Device management platform" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Device management platform Audit (read-only)")
