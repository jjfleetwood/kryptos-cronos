...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_os_patching_cadence_mcp.py                 # expose to an agent
  python 07_os_patching_cadence_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "OS patching cadence" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT OS patching cadence Audit (read-only)")
