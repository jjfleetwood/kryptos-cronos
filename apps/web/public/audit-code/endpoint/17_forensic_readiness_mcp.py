...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 17_forensic_readiness_mcp.py                 # expose to an agent
  python 17_forensic_readiness_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Forensic readiness" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Forensic readiness Audit (read-only)")
