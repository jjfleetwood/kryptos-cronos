...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 16_gps_log_collection_coverage_mcp.py                 # expose to an agent
  python 16_gps_log_collection_coverage_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "GPS, log collection coverage" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT GPS, log collection coverage Audit (read-only)")
