#!/usr/bin/env python3
"""Read-only MCP server — Endpoint Devices: "Device image version management" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_device_image_version_management_mcp.py                 # expose to an agent
  python 04_device_image_version_management_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Device image version management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("EPT Device image version management Audit (read-only)")
