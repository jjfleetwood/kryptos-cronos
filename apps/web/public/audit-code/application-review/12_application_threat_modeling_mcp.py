...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_application_threat_modeling_mcp.py                 # expose to an agent
  python 12_application_threat_modeling_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Application threat modeling" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AAR Application threat modeling Audit (read-only)")
