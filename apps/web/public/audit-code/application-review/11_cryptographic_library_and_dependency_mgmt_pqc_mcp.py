...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_cryptographic_library_and_dependency_mgmt_pqc_mcp.py                 # expose to an agent
  python 11_cryptographic_library_and_dependency_mgmt_pqc_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cryptographic library and dependency mgmt (+PQC)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AAR Cryptographic library and dependency mgmt (+PQC) Audit (read-only)")
