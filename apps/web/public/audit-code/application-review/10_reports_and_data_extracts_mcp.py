...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_reports_and_data_extracts_mcp.py                 # expose to an agent
  python 10_reports_and_data_extracts_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Reports and data extracts" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AAR Reports and data extracts Audit (read-only)")
