...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_application_logging_and_monitoring_mcp.py                 # expose to an agent
  python 05_application_logging_and_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Application logging and monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AAR Application logging and monitoring Audit (read-only)")
