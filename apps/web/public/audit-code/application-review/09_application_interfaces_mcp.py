#!/usr/bin/env python3
"""Read-only MCP server — Application Review: "Application interfaces" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_application_interfaces_mcp.py                 # expose to an agent
  python 09_application_interfaces_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Application interfaces" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AAR Application interfaces Audit (read-only)")
