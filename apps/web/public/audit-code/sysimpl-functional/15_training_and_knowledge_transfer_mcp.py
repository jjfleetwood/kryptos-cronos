#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Functional: "Training and knowledge transfer" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 15_training_and_knowledge_transfer_mcp.py                 # expose to an agent
  python 15_training_and_knowledge_transfer_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Requirements / design records, Test management, Deployment pipeline, Vendor / SLA documentation.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Training and knowledge transfer" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Training and knowledge transfer Audit (read-only)")
