...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_design_and_requirements_mcp.py                 # expose to an agent
  python 02_design_and_requirements_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Design and requirements" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Design and requirements Audit (read-only)")
