...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_testing_mcp.py                 # expose to an agent
  python 04_testing_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Testing Audit (read-only)")
