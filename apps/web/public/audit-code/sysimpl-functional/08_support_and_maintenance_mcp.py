...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_support_and_maintenance_mcp.py                 # expose to an agent
  python 08_support_and_maintenance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Support and maintenance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Support and maintenance Audit (read-only)")
