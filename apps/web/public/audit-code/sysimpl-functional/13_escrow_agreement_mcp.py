...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_escrow_agreement_mcp.py                 # expose to an agent
  python 13_escrow_agreement_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Escrow agreement" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Escrow agreement Audit (read-only)")
