#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Functional: "Contract and SLA review" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_contract_and_sla_review_mcp.py                 # expose to an agent
  python 12_contract_and_sla_review_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Contract and SLA review" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Contract and SLA review Audit (read-only)")
