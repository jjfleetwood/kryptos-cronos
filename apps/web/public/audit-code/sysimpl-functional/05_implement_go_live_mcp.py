...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_implement_go_live_mcp.py                 # expose to an agent
  python 05_implement_go_live_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Implement (go-live)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Implement (go-live) Audit (read-only)")
