...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_steering_committee_mcp.py                 # expose to an agent
  python 09_steering_committee_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Steering committee" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Steering committee Audit (read-only)")
