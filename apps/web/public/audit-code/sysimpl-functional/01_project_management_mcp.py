#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Functional: "Project management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_project_management_mcp.py                 # expose to an agent
  python 01_project_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Project management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Project management Audit (read-only)")
