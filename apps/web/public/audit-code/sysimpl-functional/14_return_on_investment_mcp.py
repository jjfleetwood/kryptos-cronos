...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 14_return_on_investment_mcp.py                 # expose to an agent
  python 14_return_on_investment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Return on investment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Return on investment Audit (read-only)")
