#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Functional: "Data conversion and migration" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_data_conversion_and_migration_mcp.py                 # expose to an agent
  python 07_data_conversion_and_migration_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data conversion and migration" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIF Data conversion and migration Audit (read-only)")
