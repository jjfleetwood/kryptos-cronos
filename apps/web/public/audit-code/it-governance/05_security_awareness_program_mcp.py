#!/usr/bin/env python3
"""Read-only MCP server — IT Governance: "Security awareness program" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_security_awareness_program_mcp.py                 # expose to an agent
  python 05_security_awareness_program_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Security awareness program" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("GOV Security awareness program Audit (read-only)")
