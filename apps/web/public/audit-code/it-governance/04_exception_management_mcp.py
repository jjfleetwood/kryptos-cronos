#!/usr/bin/env python3
"""Read-only MCP server — IT Governance: "Exception management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_exception_management_mcp.py                 # expose to an agent
  python 04_exception_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Exception management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("GOV Exception management Audit (read-only)")
