#!/usr/bin/env python3
"""Read-only MCP server — IT Governance: "Policy and standard lifecycle" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_policy_and_standard_lifecycle_mcp.py                 # expose to an agent
  python 01_policy_and_standard_lifecycle_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Policy and standard lifecycle" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("GOV Policy and standard lifecycle Audit (read-only)")
