#!/usr/bin/env python3
"""Read-only MCP server — IT Governance: "Security metrics and reporting" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_security_metrics_and_reporting_mcp.py                 # expose to an agent
  python 03_security_metrics_and_reporting_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
GRC platform, Policy + standard repository, Risk register, Metrics / KRI dashboard.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Security metrics and reporting" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("GOV Security metrics and reporting Audit (read-only)")
