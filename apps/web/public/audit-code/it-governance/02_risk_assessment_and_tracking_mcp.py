#!/usr/bin/env python3
"""Read-only MCP server — IT Governance: "Risk assessment and tracking" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_risk_assessment_and_tracking_mcp.py                 # expose to an agent
  python 02_risk_assessment_and_tracking_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Risk assessment and tracking" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("GOV Risk assessment and tracking Audit (read-only)")
