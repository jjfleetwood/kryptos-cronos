#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Dev, test, deploy cycle" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_dev_test_deploy_cycle_mcp.py                 # expose to an agent
  python 03_dev_test_deploy_cycle_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Dev, test, deploy cycle" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Dev, test, deploy cycle Audit (read-only)")
