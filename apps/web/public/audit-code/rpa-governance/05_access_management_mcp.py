#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Access management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_access_management_mcp.py                 # expose to an agent
  python 05_access_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Access management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Access management Audit (read-only)")
