#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Legal and regulatory compliance" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_legal_and_regulatory_compliance_mcp.py                 # expose to an agent
  python 02_legal_and_regulatory_compliance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Legal and regulatory compliance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Legal and regulatory compliance Audit (read-only)")
