#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "RPA strategy and governance" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_rpa_strategy_and_governance_mcp.py                 # expose to an agent
  python 01_rpa_strategy_and_governance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "RPA strategy and governance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA RPA strategy and governance Audit (read-only)")
