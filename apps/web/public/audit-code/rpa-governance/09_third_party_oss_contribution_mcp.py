#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Third-party / OSS contribution" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_third_party_oss_contribution_mcp.py                 # expose to an agent
  python 09_third_party_oss_contribution_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Third-party / OSS contribution" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Third-party / OSS contribution Audit (read-only)")
