#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Data handling and encryption" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_data_handling_and_encryption_mcp.py                 # expose to an agent
  python 06_data_handling_and_encryption_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data handling and encryption" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Data handling and encryption Audit (read-only)")
