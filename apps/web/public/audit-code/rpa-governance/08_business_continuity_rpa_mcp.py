#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Business Continuity (RPA)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_business_continuity_rpa_mcp.py                 # expose to an agent
  python 08_business_continuity_rpa_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Business Continuity (RPA)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Business Continuity (RPA) Audit (read-only)")
