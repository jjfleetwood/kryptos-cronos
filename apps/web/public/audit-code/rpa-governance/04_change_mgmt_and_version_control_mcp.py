#!/usr/bin/env python3
"""Read-only MCP server — RPA Governance: "Change mgmt and version control" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_change_mgmt_and_version_control_mcp.py                 # expose to an agent
  python 04_change_mgmt_and_version_control_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
RPA platform (UiPath/Automation Anywhere/Power Automate), Bot credential vault, Bot orchestration + logs, Version control for bots.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Change mgmt and version control" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Change mgmt and version control Audit (read-only)")
