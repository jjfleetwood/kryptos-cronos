#!/usr/bin/env python3
"""Read-only MCP server — Post-Quantum Readiness: "Historical encryption assessment" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_historical_encryption_assessment_mcp.py                 # expose to an agent
  python 05_historical_encryption_assessment_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Cryptographic inventory / CBOM tooling, TLS + certificate estate, KMS / HSM + PKI, Vendor PQC roadmaps.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Historical encryption assessment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Historical encryption assessment Audit (read-only)")
