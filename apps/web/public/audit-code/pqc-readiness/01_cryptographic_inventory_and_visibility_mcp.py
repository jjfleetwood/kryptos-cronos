...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_cryptographic_inventory_and_visibility_mcp.py                 # expose to an agent
  python 01_cryptographic_inventory_and_visibility_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cryptographic inventory and visibility" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Cryptographic inventory and visibility Audit (read-only)")
