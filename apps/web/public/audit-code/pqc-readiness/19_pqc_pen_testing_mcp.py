...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 19_pqc_pen_testing_mcp.py                 # expose to an agent
  python 19_pqc_pen_testing_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "PQC pen testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC PQC pen testing Audit (read-only)")
