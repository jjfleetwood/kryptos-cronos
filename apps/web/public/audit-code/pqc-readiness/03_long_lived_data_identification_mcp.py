...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_long_lived_data_identification_mcp.py                 # expose to an agent
  python 03_long_lived_data_identification_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Long-lived data identification" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Long-lived data identification Audit (read-only)")
