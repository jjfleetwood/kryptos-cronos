...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_crypto_bill_of_materials_cbom_mcp.py                 # expose to an agent
  python 02_crypto_bill_of_materials_cbom_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Crypto Bill of Materials (CBOM)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Crypto Bill of Materials (CBOM) Audit (read-only)")
