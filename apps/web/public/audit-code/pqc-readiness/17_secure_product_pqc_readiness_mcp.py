...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 17_secure_product_pqc_readiness_mcp.py                 # expose to an agent
  python 17_secure_product_pqc_readiness_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secure product PQC readiness" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Secure product PQC readiness Audit (read-only)")
