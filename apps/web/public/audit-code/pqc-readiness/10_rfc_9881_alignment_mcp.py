...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_rfc_9881_alignment_mcp.py                 # expose to an agent
  python 10_rfc_9881_alignment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "RFC 9881 alignment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC RFC 9881 alignment Audit (read-only)")
