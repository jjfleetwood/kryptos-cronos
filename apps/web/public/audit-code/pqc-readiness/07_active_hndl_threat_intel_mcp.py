...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_active_hndl_threat_intel_mcp.py                 # expose to an agent
  python 07_active_hndl_threat_intel_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Active HNDL threat intel" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Active HNDL threat intel Audit (read-only)")
