...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_secure_communications_pqc_migration_mcp.py                 # expose to an agent
  python 09_secure_communications_pqc_migration_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secure communications PQC migration" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Secure communications PQC migration Audit (read-only)")
