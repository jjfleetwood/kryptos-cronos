...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_cnsa_2_0_compliance_planning_mcp.py                 # expose to an agent
  python 11_cnsa_2_0_compliance_planning_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "CNSA 2.0 compliance planning" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC CNSA 2.0 compliance planning Audit (read-only)")
