...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 15_vendor_pqc_roadmap_mcp.py                 # expose to an agent
  python 15_vendor_pqc_roadmap_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Vendor PQC roadmap" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Vendor PQC roadmap Audit (read-only)")
