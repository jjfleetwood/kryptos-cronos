...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_crypto_protocol_hardware_agility_mcp.py                 # expose to an agent
  python 08_crypto_protocol_hardware_agility_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Crypto, protocol, hardware agility" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Crypto, protocol, hardware agility Audit (read-only)")
