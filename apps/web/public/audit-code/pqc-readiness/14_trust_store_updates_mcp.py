...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 14_trust_store_updates_mcp.py                 # expose to an agent
  python 14_trust_store_updates_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Trust store updates" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Trust store updates Audit (read-only)")
