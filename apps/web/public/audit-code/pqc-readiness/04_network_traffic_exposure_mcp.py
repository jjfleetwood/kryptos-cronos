...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_network_traffic_exposure_mcp.py                 # expose to an agent
  python 04_network_traffic_exposure_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Network traffic exposure" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Network traffic exposure Audit (read-only)")
