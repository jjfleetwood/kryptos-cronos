...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_standards_monitoring_ietf_nist_mcp.py                 # expose to an agent
  python 12_standards_monitoring_ietf_nist_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Standards monitoring (IETF, NIST)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Standards monitoring (IETF, NIST) Audit (read-only)")
