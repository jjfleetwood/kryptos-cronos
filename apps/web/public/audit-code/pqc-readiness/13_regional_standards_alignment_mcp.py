#!/usr/bin/env python3
"""Read-only MCP server — Post-Quantum Readiness: "Regional standards alignment" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_regional_standards_alignment_mcp.py                 # expose to an agent
  python 13_regional_standards_alignment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Regional standards alignment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("PQC Regional standards alignment Audit (read-only)")
