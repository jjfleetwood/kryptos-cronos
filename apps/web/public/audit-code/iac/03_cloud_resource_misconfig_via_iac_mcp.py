#!/usr/bin/env python3
"""Read-only MCP server — IaC: "Cloud resource misconfig via IaC" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_cloud_resource_misconfig_via_iac_mcp.py                 # expose to an agent
  python 03_cloud_resource_misconfig_via_iac_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Terraform / CloudFormation / Bicep, Policy-as-code (OPA / Sentinel), IaC scanners (tfsec/Checkov), GitOps controller (Argo/Flux).
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cloud resource misconfig via IaC" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IAC Cloud resource misconfig via IaC Audit (read-only)")
