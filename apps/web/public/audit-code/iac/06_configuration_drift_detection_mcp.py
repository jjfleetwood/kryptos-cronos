#!/usr/bin/env python3
"""Read-only MCP server — IaC: "Configuration drift detection" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_configuration_drift_detection_mcp.py                 # expose to an agent
  python 06_configuration_drift_detection_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Configuration drift detection" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IAC Configuration drift detection Audit (read-only)")
