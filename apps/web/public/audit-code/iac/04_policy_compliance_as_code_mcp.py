#!/usr/bin/env python3
"""Read-only MCP server — IaC: "Policy/compliance as code" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_policy_compliance_as_code_mcp.py                 # expose to an agent
  python 04_policy_compliance_as_code_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Policy/compliance as code" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IAC Policy/compliance as code Audit (read-only)")
