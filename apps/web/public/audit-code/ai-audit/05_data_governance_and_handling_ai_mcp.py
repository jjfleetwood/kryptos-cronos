#!/usr/bin/env python3
"""Read-only MCP server — AI: "Data governance and handling (AI)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_data_governance_and_handling_ai_mcp.py                 # expose to an agent
  python 05_data_governance_and_handling_ai_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data governance and handling (AI)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG Data governance and handling (AI) Audit (read-only)")
