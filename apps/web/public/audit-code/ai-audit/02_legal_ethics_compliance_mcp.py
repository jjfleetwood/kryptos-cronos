#!/usr/bin/env python3
"""Read-only MCP server — AI: "Legal, ethics, compliance" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_legal_ethics_compliance_mcp.py                 # expose to an agent
  python 02_legal_ethics_compliance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Legal, ethics, compliance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG Legal, ethics, compliance Audit (read-only)")
