#!/usr/bin/env python3
"""Read-only MCP server — AI: "AI system architecture and dev" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_ai_system_architecture_and_dev_mcp.py                 # expose to an agent
  python 03_ai_system_architecture_and_dev_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI system architecture and dev" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG AI system architecture and dev Audit (read-only)")
