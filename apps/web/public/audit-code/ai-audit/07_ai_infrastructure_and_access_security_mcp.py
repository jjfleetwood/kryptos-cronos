#!/usr/bin/env python3
"""Read-only MCP server — Artificial Intelligence (AI): "AI infrastructure and access security" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_ai_infrastructure_and_access_security_mcp.py                 # expose to an agent
  python 07_ai_infrastructure_and_access_security_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI infrastructure and access security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG AI infrastructure and access security Audit (read-only)")
