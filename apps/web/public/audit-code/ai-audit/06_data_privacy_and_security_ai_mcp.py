#!/usr/bin/env python3
"""Read-only MCP server — AI: "Data privacy and security (AI)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_data_privacy_and_security_ai_mcp.py                 # expose to an agent
  python 06_data_privacy_and_security_ai_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data privacy and security (AI)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG Data privacy and security (AI) Audit (read-only)")
