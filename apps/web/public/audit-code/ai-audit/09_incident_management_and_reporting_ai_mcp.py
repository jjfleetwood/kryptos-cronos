#!/usr/bin/env python3
"""Read-only MCP server — AI: "Incident management and reporting (AI)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_incident_management_and_reporting_ai_mcp.py                 # expose to an agent
  python 09_incident_management_and_reporting_ai_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident management and reporting (AI)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG Incident management and reporting (AI) Audit (read-only)")
