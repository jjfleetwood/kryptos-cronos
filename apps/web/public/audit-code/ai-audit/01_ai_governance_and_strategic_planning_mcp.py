#!/usr/bin/env python3
"""Read-only MCP server — AI: "AI governance and strategic planning" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_ai_governance_and_strategic_planning_mcp.py                 # expose to an agent
  python 01_ai_governance_and_strategic_planning_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Model registry + lineage, Eval / red-team harness, AI gateway + guardrails, Model + prompt monitoring.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI governance and strategic planning" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG AI governance and strategic planning Audit (read-only)")
