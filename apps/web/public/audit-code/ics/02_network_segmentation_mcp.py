#!/usr/bin/env python3
"""Read-only MCP server — ICS: "Network segmentation" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_network_segmentation_mcp.py                 # expose to an agent
  python 02_network_segmentation_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Network segmentation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS Network segmentation Audit (read-only)")
