#!/usr/bin/env python3
"""Read-only MCP server — ICS: "Supply chain integrity" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_supply_chain_integrity_mcp.py                 # expose to an agent
  python 10_supply_chain_integrity_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Supply chain integrity" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS Supply chain integrity Audit (read-only)")
