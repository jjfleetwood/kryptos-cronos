#!/usr/bin/env python3
"""Read-only MCP server — ICS: "IAM (ICS)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_iam_ics_mcp.py                 # expose to an agent
  python 04_iam_ics_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "IAM (ICS)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS IAM (ICS) Audit (read-only)")
