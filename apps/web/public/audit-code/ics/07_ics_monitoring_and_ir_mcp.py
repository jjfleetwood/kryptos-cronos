#!/usr/bin/env python3
"""Read-only MCP server — ICS: "ICS monitoring and IR" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_ics_monitoring_and_ir_mcp.py                 # expose to an agent
  python 07_ics_monitoring_and_ir_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "ICS monitoring and IR" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS ICS monitoring and IR Audit (read-only)")
