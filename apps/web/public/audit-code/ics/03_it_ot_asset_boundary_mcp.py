#!/usr/bin/env python3
"""Read-only MCP server — ICS: "IT/OT asset boundary" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_it_ot_asset_boundary_mcp.py                 # expose to an agent
  python 03_it_ot_asset_boundary_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "IT/OT asset boundary" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS IT/OT asset boundary Audit (read-only)")
