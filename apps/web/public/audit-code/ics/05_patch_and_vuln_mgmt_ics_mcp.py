#!/usr/bin/env python3
"""Read-only MCP server — ICS: "Patch and vuln mgmt (ICS)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_patch_and_vuln_mgmt_ics_mcp.py                 # expose to an agent
  python 05_patch_and_vuln_mgmt_ics_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
ICS/SCADA + PLC/RTU/HMI, OT network monitoring (Dragos/Nozomi), IT/OT boundary firewalls (DMZ), OT asset inventory.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Patch and vuln mgmt (ICS)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS Patch and vuln mgmt (ICS) Audit (read-only)")
