#!/usr/bin/env python3
"""Read-only MCP server — ICS: "ICS asset inventory" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_ics_asset_inventory_mcp.py                 # expose to an agent
  python 01_ics_asset_inventory_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "ICS asset inventory" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS ICS asset inventory Audit (read-only)")
