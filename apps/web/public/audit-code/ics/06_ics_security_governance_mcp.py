#!/usr/bin/env python3
"""Read-only MCP server — ICS: "ICS security governance" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_ics_security_governance_mcp.py                 # expose to an agent
  python 06_ics_security_governance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "ICS security governance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("ICS ICS security governance Audit (read-only)")
