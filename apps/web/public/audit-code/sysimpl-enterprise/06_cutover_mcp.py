#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Enterprise: "Cutover" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    detailed cutover runbook (sequenced tasks, owners, timings, freeze period)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_cutover_mcp.py                 # expose to an agent
  python 06_cutover_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cutover" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Cutover Audit (read-only)")
