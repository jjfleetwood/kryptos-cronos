...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_vendor_selection_mcp.py                 # expose to an agent
  python 11_vendor_selection_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Vendor selection" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Vendor selection Audit (read-only)")
