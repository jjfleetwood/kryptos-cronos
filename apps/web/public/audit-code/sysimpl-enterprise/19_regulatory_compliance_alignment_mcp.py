...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 19_regulatory_compliance_alignment_mcp.py                 # expose to an agent
  python 19_regulatory_compliance_alignment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Regulatory / compliance alignment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Regulatory / compliance alignment Audit (read-only)")
