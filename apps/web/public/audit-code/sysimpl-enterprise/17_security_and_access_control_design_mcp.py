#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Enterprise: "Security and access control design" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    security architecture: auth/SSO + role-based authz + encryption + segmentation + logging (designed in)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 17_security_and_access_control_design_mcp.py                 # expose to an agent
  python 17_security_and_access_control_design_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Security and access control design" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Security and access control design Audit (read-only)")
