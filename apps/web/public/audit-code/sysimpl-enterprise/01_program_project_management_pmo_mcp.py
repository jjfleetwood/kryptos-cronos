#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Enterprise: "Program & project management (PMO)" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_program_project_management_pmo_mcp.py                 # expose to an agent
  python 01_program_project_management_pmo_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Program & project management (PMO)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Program & project management (PMO) Audit (read-only)")
