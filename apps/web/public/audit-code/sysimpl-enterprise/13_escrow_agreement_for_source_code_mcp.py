...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_escrow_agreement_for_source_code_mcp.py                 # expose to an agent
  python 13_escrow_agreement_for_source_code_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Escrow agreement for source code" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Escrow agreement for source code Audit (read-only)")
