...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_contract_sla_commercial_risk_mcp.py                 # expose to an agent
  python 12_contract_sla_commercial_risk_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Contract, SLA, commercial risk" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Contract, SLA, commercial risk Audit (read-only)")
