#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Security by design" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_security_by_design_mcp.py                 # expose to an agent
  python 01_security_by_design_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Security by design" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Security by design Audit (read-only)")
