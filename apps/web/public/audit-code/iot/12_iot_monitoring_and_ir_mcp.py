#!/usr/bin/env python3
"""Read-only MCP server — IoT: "IoT monitoring and IR" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_iot_monitoring_and_ir_mcp.py                 # expose to an agent
  python 12_iot_monitoring_and_ir_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "IoT monitoring and IR" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT IoT monitoring and IR Audit (read-only)")
