#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Threat modeling" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_threat_modeling_mcp.py                 # expose to an agent
  python 04_threat_modeling_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Threat modeling" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Threat modeling Audit (read-only)")
