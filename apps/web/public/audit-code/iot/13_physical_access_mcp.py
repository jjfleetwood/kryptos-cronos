#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Physical access" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_physical_access_mcp.py                 # expose to an agent
  python 13_physical_access_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Physical access" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Physical access Audit (read-only)")
