#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Lightweight cryptography" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_lightweight_cryptography_mcp.py                 # expose to an agent
  python 06_lightweight_cryptography_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Lightweight cryptography" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Lightweight cryptography Audit (read-only)")
