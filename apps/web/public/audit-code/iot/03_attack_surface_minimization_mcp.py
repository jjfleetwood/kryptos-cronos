#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Attack surface minimization" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_attack_surface_minimization_mcp.py                 # expose to an agent
  python 03_attack_surface_minimization_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
IoT device fleet + firmware, IoT gateway / broker, Device-identity / certificate service, IoT monitoring (NDR/asset).
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Attack surface minimization" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Attack surface minimization Audit (read-only)")
