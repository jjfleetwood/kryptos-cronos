#!/usr/bin/env python3
"""Read-only MCP server — IoT: "NAC" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_nac_mcp.py                 # expose to an agent
  python 08_nac_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "NAC" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT NAC Audit (read-only)")
