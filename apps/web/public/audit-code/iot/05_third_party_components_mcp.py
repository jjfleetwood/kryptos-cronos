#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Third-party components" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_third_party_components_mcp.py                 # expose to an agent
  python 05_third_party_components_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Third-party components" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Third-party components Audit (read-only)")
