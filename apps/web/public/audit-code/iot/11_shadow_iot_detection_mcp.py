#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Shadow IoT detection" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_shadow_iot_detection_mcp.py                 # expose to an agent
  python 11_shadow_iot_detection_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Shadow IoT detection" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Shadow IoT detection Audit (read-only)")
