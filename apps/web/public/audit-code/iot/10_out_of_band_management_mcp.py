#!/usr/bin/env python3
"""Read-only MCP server — IoT: "Out-of-band management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_out_of_band_management_mcp.py                 # expose to an agent
  python 10_out_of_band_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Out-of-band management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("IOT Out-of-band management Audit (read-only)")
