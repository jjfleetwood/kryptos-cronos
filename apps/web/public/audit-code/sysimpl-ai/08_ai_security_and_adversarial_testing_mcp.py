#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI: "AI security and adversarial testing" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_ai_security_and_adversarial_testing_mcp.py                 # expose to an agent
  python 08_ai_security_and_adversarial_testing_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI security and adversarial testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA AI security and adversarial testing Audit (read-only)")
