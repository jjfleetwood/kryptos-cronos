#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI: "Model development and validation" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_model_development_and_validation_mcp.py                 # expose to an agent
  python 03_model_development_and_validation_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Model development and validation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA Model development and validation Audit (read-only)")
