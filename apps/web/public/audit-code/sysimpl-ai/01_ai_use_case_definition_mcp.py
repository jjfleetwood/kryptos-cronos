#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI (Artificial Intelligence): "AI use case definition" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_ai_use_case_definition_mcp.py                 # expose to an agent
  python 01_ai_use_case_definition_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI use case definition" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA AI use case definition Audit (read-only)")
