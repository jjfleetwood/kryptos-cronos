#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI: "AI ethics, fairness, responsible AI" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_ai_ethics_fairness_responsible_ai_mcp.py                 # expose to an agent
  python 04_ai_ethics_fairness_responsible_ai_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI ethics, fairness, responsible AI" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA AI ethics, fairness, responsible AI Audit (read-only)")
