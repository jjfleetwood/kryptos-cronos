#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI: "Data governance and quality for AI" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_data_governance_and_quality_for_ai_mcp.py                 # expose to an agent
  python 02_data_governance_and_quality_for_ai_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data governance and quality for AI" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA Data governance and quality for AI Audit (read-only)")
