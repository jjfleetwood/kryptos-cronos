#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — AI: "AI performance / post-deployment" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_ai_performance_post_deployment_mcp.py                 # expose to an agent
  python 12_ai_performance_post_deployment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI performance / post-deployment" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIA AI performance / post-deployment Audit (read-only)")
