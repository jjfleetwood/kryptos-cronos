...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_data_loss_prevention_mcp.py                 # expose to an agent
  python 05_data_loss_prevention_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data loss prevention" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Data loss prevention Audit (read-only)")
