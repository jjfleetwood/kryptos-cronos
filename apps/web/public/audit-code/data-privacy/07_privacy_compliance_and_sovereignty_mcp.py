#!/usr/bin/env python3
"""Read-only MCP server — Data Protection & Privacy: "Privacy compliance and sovereignty" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_privacy_compliance_and_sovereignty_mcp.py                 # expose to an agent
  python 07_privacy_compliance_and_sovereignty_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
DLP (Purview / Symantec), Data classification + catalog, KMS / encryption services, Backup + retention platform.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Privacy compliance and sovereignty" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Privacy compliance and sovereignty Audit (read-only)")
