...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_data_inventorying_lineage_provenance_mcp.py                 # expose to an agent
  python 08_data_inventorying_lineage_provenance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data inventorying, lineage, provenance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Data inventorying, lineage, provenance Audit (read-only)")
