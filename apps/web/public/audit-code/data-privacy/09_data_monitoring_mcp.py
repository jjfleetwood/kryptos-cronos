#!/usr/bin/env python3
"""Read-only MCP server — Data Protection & Privacy: "Data monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_data_monitoring_mcp.py                 # expose to an agent
  python 09_data_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Data monitoring Audit (read-only)")
