...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_backup_governance_and_restore_testing_mcp.py                 # expose to an agent
  python 04_backup_governance_and_restore_testing_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Backup governance and restore testing" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Backup governance and restore testing Audit (read-only)")
