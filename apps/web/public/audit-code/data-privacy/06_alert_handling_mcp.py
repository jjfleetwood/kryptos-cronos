...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_alert_handling_mcp.py                 # expose to an agent
  python 06_alert_handling_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Alert handling" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Alert handling Audit (read-only)")
