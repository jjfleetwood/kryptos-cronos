...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_encryption_in_transit_at_rest_in_use_mcp.py                 # expose to an agent
  python 02_encryption_in_transit_at_rest_in_use_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Encryption in transit/at rest/in use" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Encryption in transit/at rest/in use Audit (read-only)")
