...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_data_retention_and_disposal_mcp.py                 # expose to an agent
  python 03_data_retention_and_disposal_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data retention and disposal" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DPP Data retention and disposal Audit (read-only)")
