#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Baseline enforcement" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_baseline_enforcement_mcp.py                 # expose to an agent
  python 05_baseline_enforcement_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Baseline enforcement" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Baseline enforcement Audit (read-only)")
