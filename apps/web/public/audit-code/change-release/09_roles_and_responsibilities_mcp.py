#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Roles and responsibilities" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_roles_and_responsibilities_mcp.py                 # expose to an agent
  python 09_roles_and_responsibilities_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Roles and responsibilities" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Roles and responsibilities Audit (read-only)")
