#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Emergency change process" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_emergency_change_process_mcp.py                 # expose to an agent
  python 02_emergency_change_process_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Emergency change process" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Emergency change process Audit (read-only)")
