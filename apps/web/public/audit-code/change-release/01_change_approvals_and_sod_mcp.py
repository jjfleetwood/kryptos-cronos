#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Change approvals and SoD" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_change_approvals_and_sod_mcp.py                 # expose to an agent
  python 01_change_approvals_and_sod_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Change approvals and SoD" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Change approvals and SoD Audit (read-only)")
