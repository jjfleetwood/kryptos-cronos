#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Post-implementation review" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_post_implementation_review_mcp.py                 # expose to an agent
  python 06_post_implementation_review_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
ITSM change tooling (ServiceNow), Release/deploy pipeline, Configuration baseline (CMDB), Change audit log.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Post-implementation review" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Post-implementation review Audit (read-only)")
