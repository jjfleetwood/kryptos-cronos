#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Version control" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_version_control_mcp.py                 # expose to an agent
  python 08_version_control_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Version control" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Version control Audit (read-only)")
