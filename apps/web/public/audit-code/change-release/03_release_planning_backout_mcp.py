#!/usr/bin/env python3
"""Read-only MCP server — Change, Release & Config Mgmt: "Release planning & backout" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_release_planning_backout_mcp.py                 # expose to an agent
  python 03_release_planning_backout_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Release planning & backout" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CRC Release planning & backout Audit (read-only)")
