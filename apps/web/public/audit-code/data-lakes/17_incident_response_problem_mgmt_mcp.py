#!/usr/bin/env python3
"""Read-only MCP server — Data Lakes & Warehouses: "Incident response / problem mgmt" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 17_incident_response_problem_mgmt_mcp.py                 # expose to an agent
  python 17_incident_response_problem_mgmt_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Lakehouse / warehouse (Snowflake/Databricks/BigQuery), Ingestion + ETL/ELT pipelines, Data catalog + lineage, Fine-grained access + masking.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident response / problem mgmt" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Incident response / problem mgmt Audit (read-only)")
