...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_source_onboarding_and_ingestion_mcp.py                 # expose to an agent
  python 02_source_onboarding_and_ingestion_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Source onboarding and ingestion" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Source onboarding and ingestion Audit (read-only)")
