...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 19_regulatory_contractual_data_req_mcp.py                 # expose to an agent
  python 19_regulatory_contractual_data_req_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Regulatory / contractual data req" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Regulatory / contractual data req Audit (read-only)")
