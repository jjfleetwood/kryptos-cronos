...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_etl_elt_transformation_controls_mcp.py                 # expose to an agent
  python 03_etl_elt_transformation_controls_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "ETL/ELT transformation controls" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW ETL/ELT transformation controls Audit (read-only)")
