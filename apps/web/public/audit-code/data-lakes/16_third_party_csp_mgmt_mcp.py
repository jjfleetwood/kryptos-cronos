...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 16_third_party_csp_mgmt_mcp.py                 # expose to an agent
  python 16_third_party_csp_mgmt_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Third-party / CSP mgmt" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Third-party / CSP mgmt Audit (read-only)")
