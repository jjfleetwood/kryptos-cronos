#!/usr/bin/env python3
"""Read-only MCP server — Data Lakes & Warehouses: "Environment segregation" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_environment_segregation_mcp.py                 # expose to an agent
  python 11_environment_segregation_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Environment segregation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Environment segregation Audit (read-only)")
