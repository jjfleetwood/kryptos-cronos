#!/usr/bin/env python3
"""Read-only MCP server — Data Lakes & Warehouses: "Sensitive data protection" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_sensitive_data_protection_mcp.py                 # expose to an agent
  python 08_sensitive_data_protection_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Sensitive data protection" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Sensitive data protection Audit (read-only)")
