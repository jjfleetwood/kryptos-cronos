...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_logging_monitoring_alerting_mcp.py                 # expose to an agent
  python 09_logging_monitoring_alerting_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Logging, monitoring, alerting" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Logging, monitoring, alerting Audit (read-only)")
