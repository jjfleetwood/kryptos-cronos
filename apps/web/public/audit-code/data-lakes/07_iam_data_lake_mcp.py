...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_iam_data_lake_mcp.py                 # expose to an agent
  python 07_iam_data_lake_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "IAM (data lake)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW IAM (data lake) Audit (read-only)")
