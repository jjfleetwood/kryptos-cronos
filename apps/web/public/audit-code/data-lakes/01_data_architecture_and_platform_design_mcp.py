...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_data_architecture_and_platform_design_mcp.py                 # expose to an agent
  python 01_data_architecture_and_platform_design_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data architecture and platform design" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Data architecture and platform design Audit (read-only)")
