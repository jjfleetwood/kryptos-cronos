...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 18_capacity_performance_cost_mcp.py                 # expose to an agent
  python 18_capacity_performance_cost_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Capacity, performance, cost" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Capacity, performance, cost Audit (read-only)")
