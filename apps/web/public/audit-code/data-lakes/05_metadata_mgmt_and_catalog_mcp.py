...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_metadata_mgmt_and_catalog_mcp.py                 # expose to an agent
  python 05_metadata_mgmt_and_catalog_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Metadata mgmt and catalog" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Metadata mgmt and catalog Audit (read-only)")
