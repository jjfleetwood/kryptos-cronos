#!/usr/bin/env python3
"""Read-only MCP server — Data Lakes & Warehouses: "Incident response / problem management" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    data-platform incident playbooks (exfil / corruption / ransomware / mass-delete)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 17_incident_response_problem_management_mcp.py                 # expose to an agent
  python 17_incident_response_problem_management_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident response / problem management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW Incident response / problem management Audit (read-only)")
