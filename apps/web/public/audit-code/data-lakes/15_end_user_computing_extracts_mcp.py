...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 15_end_user_computing_extracts_mcp.py                 # expose to an agent
  python 15_end_user_computing_extracts_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "End-user computing / extracts" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DLW End-user computing / extracts Audit (read-only)")
