#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Audit logging and monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_audit_logging_and_monitoring_mcp.py                 # expose to an agent
  python 08_audit_logging_and_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Audit logging and monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Audit logging and monitoring Audit (read-only)")
