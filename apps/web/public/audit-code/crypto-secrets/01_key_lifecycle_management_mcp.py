#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Key Lifecycle Management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_key_lifecycle_management_mcp.py                 # expose to an agent
  python 01_key_lifecycle_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Key Lifecycle Management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Key Lifecycle Management Audit (read-only)")
