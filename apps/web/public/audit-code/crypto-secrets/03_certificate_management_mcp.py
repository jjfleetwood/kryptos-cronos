#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Certificate Management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_certificate_management_mcp.py                 # expose to an agent
  python 03_certificate_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Certificate Management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Certificate Management Audit (read-only)")
