#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Secrets Management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_secrets_management_mcp.py                 # expose to an agent
  python 02_secrets_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secrets Management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Secrets Management Audit (read-only)")
