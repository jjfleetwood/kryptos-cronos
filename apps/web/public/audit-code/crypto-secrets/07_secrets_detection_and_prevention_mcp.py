#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Secrets detection and prevention" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_secrets_detection_and_prevention_mcp.py                 # expose to an agent
  python 07_secrets_detection_and_prevention_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secrets detection and prevention" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Secrets detection and prevention Audit (read-only)")
