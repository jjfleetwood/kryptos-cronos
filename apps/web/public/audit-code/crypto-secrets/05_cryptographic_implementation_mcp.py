#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "Cryptographic implementation" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_cryptographic_implementation_mcp.py                 # expose to an agent
  python 05_cryptographic_implementation_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cryptographic implementation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS Cryptographic implementation Audit (read-only)")
