#!/usr/bin/env python3
"""Read-only MCP server — Crypto Key & Secrets: "HSM controls" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_hsm_controls_mcp.py                 # expose to an agent
  python 04_hsm_controls_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "HSM controls" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CKS HSM controls Audit (read-only)")
