#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Configuration drift monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_configuration_drift_monitoring_mcp.py                 # expose to an agent
  python 03_configuration_drift_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Configuration drift monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Configuration drift monitoring Audit (read-only)")
