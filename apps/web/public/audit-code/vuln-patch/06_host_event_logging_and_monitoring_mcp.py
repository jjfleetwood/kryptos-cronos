#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Host event logging and monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_host_event_logging_and_monitoring_mcp.py                 # expose to an agent
  python 06_host_event_logging_and_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Host event logging and monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Host event logging and monitoring Audit (read-only)")
