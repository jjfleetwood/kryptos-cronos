#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Server build standards and hardening" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_server_build_standards_and_hardening_mcp.py                 # expose to an agent
  python 01_server_build_standards_and_hardening_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Vuln scanner (Tenable/Qualys/Rapid7), Patch management (SCCM/Intune/Ansible), CMDB / asset inventory, CISA KEV feed.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Server build standards and hardening" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Server build standards and hardening Audit (read-only)")
