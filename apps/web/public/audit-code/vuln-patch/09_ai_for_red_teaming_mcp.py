#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "AI for red teaming" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_ai_for_red_teaming_mcp.py                 # expose to an agent
  python 09_ai_for_red_teaming_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI for red teaming" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM AI for red teaming Audit (read-only)")
