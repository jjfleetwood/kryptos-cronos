#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Administrative access controls" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_administrative_access_controls_mcp.py                 # expose to an agent
  python 04_administrative_access_controls_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Administrative access controls" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Administrative access controls Audit (read-only)")
