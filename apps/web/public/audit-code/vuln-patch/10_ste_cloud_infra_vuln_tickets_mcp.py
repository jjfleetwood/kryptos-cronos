#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "STE — cloud infra vuln tickets" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_ste_cloud_infra_vuln_tickets_mcp.py                 # expose to an agent
  python 10_ste_cloud_infra_vuln_tickets_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "STE — cloud infra vuln tickets" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM STE — cloud infra vuln tickets Audit (read-only)")
