#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Patch mgmt and exception handling" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_patch_mgmt_and_exception_handling_mcp.py                 # expose to an agent
  python 02_patch_mgmt_and_exception_handling_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Patch mgmt and exception handling" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Patch mgmt and exception handling Audit (read-only)")
