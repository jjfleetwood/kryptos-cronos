#!/usr/bin/env python3
"""Read-only MCP server — Vuln & Patch Mgmt: "Red and blue teaming" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_red_and_blue_teaming_mcp.py                 # expose to an agent
  python 07_red_and_blue_teaming_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Red and blue teaming" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("VPM Red and blue teaming Audit (read-only)")
