...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_integration_and_interface_security_mcp.py                 # expose to an agent
  python 02_integration_and_interface_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Integration and interface security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Integration and interface security Audit (read-only)")
