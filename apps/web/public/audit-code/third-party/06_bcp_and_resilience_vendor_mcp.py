#!/usr/bin/env python3
"""Read-only MCP server — Third Party Systems: "BCP and resilience (vendor)" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_bcp_and_resilience_vendor_mcp.py                 # expose to an agent
  python 06_bcp_and_resilience_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
TPRM / GRC platform (Archer/OneTrust), Vendor inventory + contracts, SOC 2 / attestation repository, Integration / API gateway.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "BCP and resilience (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS BCP and resilience (vendor) Audit (read-only)")
