...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_compliance_and_regulatory_alignment_mcp.py                 # expose to an agent
  python 05_compliance_and_regulatory_alignment_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def _register():
    ...
    mcp = FastMCP("TPS Compliance and regulatory alignment Audit (read-only)")
