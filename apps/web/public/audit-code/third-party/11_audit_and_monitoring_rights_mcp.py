...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_audit_and_monitoring_rights_mcp.py                 # expose to an agent
  python 11_audit_and_monitoring_rights_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Audit and monitoring rights" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Audit and monitoring rights Audit (read-only)")
