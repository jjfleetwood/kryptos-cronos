...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_data_backup_and_recovery_vendor_mcp.py                 # expose to an agent
  python 09_data_backup_and_recovery_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data backup and recovery (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Data backup and recovery (vendor) Audit (read-only)")
