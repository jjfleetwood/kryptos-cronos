#!/usr/bin/env python3
"""Read-only MCP server — Third Party Systems: "Change and release management (vendor)" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_change_and_release_management_vendor_mcp.py                 # expose to an agent
  python 03_change_and_release_management_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Change and release management (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Change and release management (vendor) Audit (read-only)")
