...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 13_identity_federation_and_authentication_mcp.py                 # expose to an agent
  python 13_identity_federation_and_authentication_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Identity federation and authentication" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Identity federation and authentication Audit (read-only)")
