...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_data_security_and_privacy_mcp.py                 # expose to an agent
  python 01_data_security_and_privacy_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data security and privacy" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Data security and privacy Audit (read-only)")
