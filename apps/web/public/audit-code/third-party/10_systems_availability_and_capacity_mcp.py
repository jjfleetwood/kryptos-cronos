...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_systems_availability_and_capacity_mcp.py                 # expose to an agent
  python 10_systems_availability_and_capacity_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Systems availability and capacity" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Systems availability and capacity Audit (read-only)")
