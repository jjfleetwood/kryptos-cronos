...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_subcontractor_nth_party_risk_mcp.py                 # expose to an agent
  python 12_subcontractor_nth_party_risk_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Subcontractor / nth party risk" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Subcontractor / nth party risk Audit (read-only)")
