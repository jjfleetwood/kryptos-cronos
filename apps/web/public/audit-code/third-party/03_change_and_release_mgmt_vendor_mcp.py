...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_change_and_release_mgmt_vendor_mcp.py                 # expose to an agent
  python 03_change_and_release_mgmt_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Change and release mgmt (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Change and release mgmt (vendor) Audit (read-only)")
