...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_incident_and_problem_mgmt_vendor_mcp.py                 # expose to an agent
  python 04_incident_and_problem_mgmt_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident and problem mgmt (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Incident and problem mgmt (vendor) Audit (read-only)")
