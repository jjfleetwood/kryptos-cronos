...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 14_vendor_ai_supply_chain_mcp.py                 # expose to an agent
  python 14_vendor_ai_supply_chain_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Vendor AI supply chain" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Vendor AI supply chain Audit (read-only)")
