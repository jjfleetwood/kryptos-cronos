...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_secure_network_architecture_mcp.py                 # expose to an agent
  python 05_secure_network_architecture_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Secure network architecture" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Secure network architecture Audit (read-only)")
