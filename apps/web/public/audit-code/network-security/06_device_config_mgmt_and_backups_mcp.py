...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_device_config_mgmt_and_backups_mcp.py                 # expose to an agent
  python 06_device_config_mgmt_and_backups_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Device config mgmt and backups" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Device config mgmt and backups Audit (read-only)")
