...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_network_segmentation_and_trust_zones_mcp.py                 # expose to an agent
  python 01_network_segmentation_and_trust_zones_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
Next-gen firewalls (Palo Alto/Fortinet), Network segmentation / microsegmentation, ZTNA / VPN gateways, NDR + flow logs.
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Network segmentation and trust zones" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Network segmentation and trust zones Audit (read-only)")
