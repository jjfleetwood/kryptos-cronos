...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_remote_access_vpn_ztna_mcp.py                 # expose to an agent
  python 03_remote_access_vpn_ztna_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Remote access (VPN, ZTNA)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Remote access (VPN, ZTNA) Audit (read-only)")
