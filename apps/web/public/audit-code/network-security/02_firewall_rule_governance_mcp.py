...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_firewall_rule_governance_mcp.py                 # expose to an agent
  python 02_firewall_rule_governance_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Firewall rule governance" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Firewall rule governance Audit (read-only)")
