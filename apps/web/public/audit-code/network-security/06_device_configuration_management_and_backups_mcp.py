#!/usr/bin/env python3
"""Read-only MCP server — Network Security: "Device configuration management and backups" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_device_configuration_management_and_backups_mcp.py                 # expose to an agent
  python 06_device_configuration_management_and_backups_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Device configuration management and backups" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("NET Device configuration management and backups Audit (read-only)")
