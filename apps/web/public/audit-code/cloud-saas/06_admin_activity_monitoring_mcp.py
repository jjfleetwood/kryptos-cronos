#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Admin activity monitoring" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_admin_activity_monitoring_mcp.py                 # expose to an agent
  python 06_admin_activity_monitoring_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Admin activity monitoring" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Admin activity monitoring Audit (read-only)")
