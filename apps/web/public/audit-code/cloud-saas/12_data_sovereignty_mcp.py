#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Data sovereignty" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_data_sovereignty_mcp.py                 # expose to an agent
  python 12_data_sovereignty_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Data sovereignty" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Data sovereignty Audit (read-only)")
