#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Cloud partner" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 09_cloud_partner_mcp.py                 # expose to an agent
  python 09_cloud_partner_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cloud partner" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Cloud partner Audit (read-only)")
