#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "SaaS configuration" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_saas_configuration_mcp.py                 # expose to an agent
  python 05_saas_configuration_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "SaaS configuration" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD SaaS configuration Audit (read-only)")
