#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Security assessments" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_security_assessments_mcp.py                 # expose to an agent
  python 10_security_assessments_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Security assessments" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Security assessments Audit (read-only)")
