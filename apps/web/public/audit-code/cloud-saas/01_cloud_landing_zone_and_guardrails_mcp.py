#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Cloud landing zone and guardrails" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_cloud_landing_zone_and_guardrails_mcp.py                 # expose to an agent
  python 01_cloud_landing_zone_and_guardrails_mcp.py --selftest       # reproduce findings against fixtures, offline

Wire real sources by replacing the _gather() fixtures with read-only API calls to
AWS / Azure / GCP control plane, CSPM (Wiz / Prisma / Defender), SaaS admin consoles (M365/Salesforce), Cloud audit logs (CloudTrail).
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cloud landing zone and guardrails" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Cloud landing zone and guardrails Audit (read-only)")
