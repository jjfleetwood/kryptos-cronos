#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Cloud supply chain" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_cloud_supply_chain_mcp.py                 # expose to an agent
  python 08_cloud_supply_chain_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Cloud supply chain" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Cloud supply chain Audit (read-only)")
