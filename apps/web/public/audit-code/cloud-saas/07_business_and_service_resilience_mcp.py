#!/usr/bin/env python3
"""Read-only MCP server — Cloud Platform & SaaS: "Business and service resilience" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 07_business_and_service_resilience_mcp.py                 # expose to an agent
  python 07_business_and_service_resilience_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Business and service resilience" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("CLD Business and service resilience Audit (read-only)")
