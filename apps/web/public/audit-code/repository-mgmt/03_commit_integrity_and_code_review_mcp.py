#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Commit integrity and code review" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 03_commit_integrity_and_code_review_mcp.py                 # expose to an agent
  python 03_commit_integrity_and_code_review_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Commit integrity and code review" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Commit integrity and code review Audit (read-only)")
