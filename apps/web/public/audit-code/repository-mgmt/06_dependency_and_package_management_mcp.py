#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Dependency and package management" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 06_dependency_and_package_management_mcp.py                 # expose to an agent
  python 06_dependency_and_package_management_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Dependency and package management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Dependency and package management Audit (read-only)")
