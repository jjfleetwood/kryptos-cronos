#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Branch management and protection" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 02_branch_management_and_protection_mcp.py                 # expose to an agent
  python 02_branch_management_and_protection_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Branch management and protection" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Branch management and protection Audit (read-only)")
