#!/usr/bin/env python3
"""Read-only MCP server — Repository Management: "Access control and authentication" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    GitHub: org members + roles (gh api orgs/{org}/members; repo collaborators + permission level)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 01_access_control_and_authentication_mcp.py                 # expose to an agent
  python 01_access_control_and_authentication_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Access control and authentication" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Access control and authentication Audit (read-only)")
