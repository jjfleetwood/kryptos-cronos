#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Third-party and open source contribution" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 11_third_party_and_open_source_contribution_mcp.py                 # expose to an agent
  python 11_third_party_and_open_source_contribution_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Third-party and open source contribution" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Third-party and open source contribution Audit (read-only)")
