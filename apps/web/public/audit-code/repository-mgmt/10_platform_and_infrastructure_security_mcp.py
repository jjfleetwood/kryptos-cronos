#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Platform and infrastructure security" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 10_platform_and_infrastructure_security_mcp.py                 # expose to an agent
  python 10_platform_and_infrastructure_security_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Platform and infrastructure security" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Platform and infrastructure security Audit (read-only)")
