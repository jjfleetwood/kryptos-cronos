#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Repository governance and lifecycle" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 08_repository_governance_and_lifecycle_mcp.py                 # expose to an agent
  python 08_repository_governance_and_lifecycle_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Repository governance and lifecycle" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Repository governance and lifecycle Audit (read-only)")
