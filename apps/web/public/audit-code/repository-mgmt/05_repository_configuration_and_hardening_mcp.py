#!/usr/bin/env python3
"""Read-only MCP server — Repository Mgmt: "Repository configuration and hardening" audit evidence.
...
never changes state — the hard requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_repository_configuration_and_hardening_mcp.py                 # expose to an agent
  python 05_repository_configuration_and_hardening_mcp.py --selftest       # reproduce findings against fixtures, offline
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Repository configuration and hardening" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("REPO Repository configuration and hardening Audit (read-only)")
