...
**Version:** 3.8  
**Date:** 2026-06-08  
**Total stages:** 871 across 85 epochs (delivered on **web + native mobile** from the shared `@kryptos/core` content package)
...
## Epoch 63: Robotics — Robot & Autonomous-System Security
...
## Epoch 68: Wired & Autonomous II — The Software-Defined Vehicle, Deeper
**Stages:** 10 (v2-01 through v2-10) | **Color:** Lime | **Track:** Wired & Autonomous (2nd epoch in section) | **6 CTF + 4 quiz**

Goes beyond CAN/OBD basics into where cars are heading. The software-defined vehicle — zonal architecture, Automotive Ethernet, DoIP/SOME/IP, the Jeep-hack lesson (01, quiz); SOME/IP service-discovery spoofing (02, CTF); V2X/V2V — forging Basic Safety Messages, SCMS PKI, Sybil/misbehavior, the "phantom car" (03, CTF); TPMS & sensor spoofing — unauthenticated 315/433 MHz radios, tracking + false warnings (04, CTF); relay attacks & digital keys — PKES, CCC Digital Key, UWB distance-bounding (05, CTF); ISO 15118 Plug & Charge & V2G — charging PKI, OCPP, forge a contract (06, CTF); fooling the self-driving stack — adversarial patches, LiDAR/GPS spoofing, sensor fusion (07, quiz); connected-car & fleet APIs — BOLA/IDOR on VINs, fleet-wide takeover, Sam Curry research (08, CTF); OTA + UNECE R155/R156 + ISO/SAE 21434 — CSMS/SUMS/TARA, Uptane (09, quiz); securing the SDV — HSM/secure boot, SecOC, zonal segmentation, VSOC, SBOMs, defense in depth (10, quiz).

## Epoch 69: Robotics II — Fleets, Farms, Swarms & Learned Behavior
**Stages:** 10 (r2-01 through r2-10) | **Color:** Orange | **Track:** Robotics (2nd epoch in section) | **6 CTF + 4 quiz**

Scales the robotics arc out and specializes it. ROS 2 & SROS2 — the DDS security model and why it's often disabled (01, quiz); exploiting a DDS domain — auto-discovery, join the bus, publish commands (02, CTF); commandeering a warehouse AMR fleet — VDA5050/MQTT fleet manager, one server → hundreds of robots (03, CTF); hijacking an autonomous tractor — RTK auto-steer, ISOBUS, telematics, food-security stakes (04, CTF); swarm robotics & trust — Byzantine/Sybil, BFT consensus (05, quiz); cloud-robotics backends — API authz, tenant isolation, teleop hijack (06, CTF); legged & humanoid robots — capability + proximity, surveillance, supply chain (07, quiz); navigation & GPS spoofing — GNSS jam vs spoof, drone/ship hijacks, sensor fusion (08, CTF); poisoning a learned policy — data poisoning/backdoors in imitation/RL/foundation models (09, CTF); securing robot fleets at scale — zero trust, independent safety envelope, lifecycle ops (10, quiz).

## Epoch 70: Race Through Space II — Links, Ground Segments & Constellations
**Stages:** 10 (s2-01 through s2-10) | **Color:** Violet | **Track:** Race Through Space (2nd epoch in section) | **6 CTF + 4 quiz**
