...
- [x] **Removed the inert admin downloads-access panel — DONE (2026-06-22):** deleted `DownloadsAccessPanel` + `type DownloadsMode` from `admin/_panels.tsx`, its import + nav anchor + render block from `admin/page.tsx`, and the `/api/admin/downloads-access` route. Web tsc + build green.
- [x] **Moved `guides/agent-risk-audit` → apps/audit — DONE (2026-06-22):** the field guide is owner-only now.
  - apps/audit gets: `content/AGENT_RISK_AUDIT_GUIDE.md` (`git mv` from web secured-docs), `/guide` route (react-markdown render + a Generate-deck button), `/api/guide-deck` route (owner-gated by the edge proxy), self-contained `lib/guide-deck.ts` (buildGuideDeck + helpers, copied off pptx-lens), `pptxgenjs` dep, `outputFileTracingIncludes` for the markdown, and a guide link on the landing.
  - apps/web fully cleaned: deleted the `/guides/agent-risk-audit` route + `GuideDeckButton`; stripped `buildGuideDeck`/`parseGuide` from `pptx-lens.ts`; reverted `export/pptx` to epoch-only (dropped the `{guide}` branch + `GUIDE_DECKS`); removed the `next.config` tracing entry, the `DocsViewer` tab, the `/api/docs` allowlist entry, and the `/stages` link. Zero `AGENT_RISK_AUDIT`/`buildGuideDeck` refs remain in apps/web. (`/guides/agent-fleet` stays — it's the dev-fleet overview, unrelated. `docs/AGENT_RISK_AUDIT_GUIDE.md` stays as the internal canonical copy referenced by `.claude/agents/audit-agent.md`.)
  - Both apps tsc clean + `npm run build` = 2 successful.
