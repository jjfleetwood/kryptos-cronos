#!/usr/bin/env python3
"""Read-only MCP server — Robotic Process Automation (RPA) Governance: "Change management and version control" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_change_management_and_version_control_mcp.py                 # expose to an agent
  python 04_change_management_and_version_control_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Change management and version control" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("RPA Change management and version control Audit (read-only)")
