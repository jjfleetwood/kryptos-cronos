#!/usr/bin/env python3
"""Read-only MCP server — Datacenter / Lab / Colocation (CoLo): "Maintenance and vendor management" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 05_maintenance_and_vendor_management_mcp.py                 # expose to an agent
  python 05_maintenance_and_vendor_management_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Maintenance and vendor management" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("DCR Maintenance and vendor management Audit (read-only)")
