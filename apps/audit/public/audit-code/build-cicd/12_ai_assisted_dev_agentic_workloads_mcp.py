#!/usr/bin/env python3
"""Read-only MCP server — Build Environment & CI/CD (Continuous Integration / Continuous Delivery): "AI-assisted dev / agentic workloads" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    inventory AI dev tools + agentic CI agents and their repo/deploy permissions
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 12_ai_assisted_dev_agentic_workloads_mcp.py                 # expose to an agent
  python 12_ai_assisted_dev_agentic_workloads_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "AI-assisted dev / agentic workloads" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("BCD AI-assisted dev / agentic workloads Audit (read-only)")
