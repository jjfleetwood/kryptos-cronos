#!/usr/bin/env python3
"""Read-only MCP server — Artificial Intelligence (AI): "Model testing and validation" audit evidence.
...
REAL SOURCES / COMMANDS to wire in place of the fixtures (read-only):
    pre-deployment validation: accuracy/performance + robustness + bias/fairness (the eval suite)
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_model_testing_and_validation_mcp.py                 # expose to an agent
  python 04_model_testing_and_validation_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Model testing and validation" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("AIG Model testing and validation Audit (read-only)")
