#!/usr/bin/env python3
"""Read-only MCP server — Third Party Systems: "Incident and problem management (vendor)" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 04_incident_and_problem_management_vendor_mcp.py                 # expose to an agent
  python 04_incident_and_problem_management_vendor_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Incident and problem management (vendor)" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("TPS Incident and problem management (vendor) Audit (read-only)")
