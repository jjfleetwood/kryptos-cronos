#!/usr/bin/env python3
"""Read-only MCP server — System Implementation — Enterprise: "Training, change management, knowledge transfer" audit evidence.
...
requirement for audit tooling.
  ...
  pip install "mcp[cli]"
  mcp run 15_training_change_management_knowledge_transfer_mcp.py                 # expose to an agent
  python 15_training_change_management_knowledge_transfer_mcp.py --selftest       # reproduce findings against fixtures, offline
"""
...
def build_inventory() -> list[dict]:
    """List every in-scope item the "Training, change management, knowledge transfer" control must cover."""
...
def _register():
    ...
    mcp = FastMCP("SIE Training, change management, knowledge transfer Audit (read-only)")
